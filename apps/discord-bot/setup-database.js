...
async function setupDatabase() {
  ...
  try {
    ...
    const onboardingSchema = fs.readFileSync(
      path.join(__dirname, 'database', 'onboarding_schema.sql'),
      'utf8'
    );
    ...
    const migrationSchema = fs.readFileSync(
      path.join(__dirname, 'migrations', '001_fortune_100_enhancements.sql'),
      'utf8'
    );
    ...
    for (const statement of migrationStatements) {
      ...
      if (trimmedStatement) {
        ...
        if (error) {
          console.log(
            `⚠️  Migration statement may have failed (this might be normal): ${error.message}`
          );
        }
      }
    }
    ...
    for (const statement of additionalStatements) {
      ...
      if (trimmedStatement) {
        ...
        if (error) {
          console.log(
            `⚠️  Additional table creation may have failed (this might be normal): ${error.message}`
          );
        }
      }
    }
    ...
    const { error: configError } = await supabase.from('onboarding_config').upsert({
      id: 'main',
      config: {
    ...
    const { error: flowError } = await supabase.from('onboarding_flows').upsert({
      id: 'default',
