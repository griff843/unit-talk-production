...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand(subcommand =>
    subcommand.setName('coaching').setDescription('Get AI-powered betting coaching and insights')
  )
  .addSubcommand(subcommand =>
    subcommand
      .setName('parlay')
...
async function handleAnalytics(interaction: ChatInputCommandInteraction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Performance analysis for ${timeframe === 'all' ? 'all time' : `last ${timeframe}`}`
    )
    .setColor(0x3498db)
    .addFields([
      {
    ...
    .setFooter({
      text: `${userTier.toUpperCase()} Analytics • Updated ${new Date().toISOString().toLocaleString()}`,
    })
    .setTimestamp();
