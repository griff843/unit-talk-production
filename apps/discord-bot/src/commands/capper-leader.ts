...
export async function execute(interaction: ChatInputCommandInteraction) {
  try {
        ...
        .addFields(
          {
            name: '💎 VIP+ Benefits',
            value:
              '• Full capper leaderboard access\n• Advanced performance metrics\n• Historical data & trends\n• Capper comparison tools\n• ROI tracking by sport',
            inline: false,
          },
          {
            name: '⭐ Current VIP Benefits',
            value:
              'As a VIP member, you have access to all picks and basic stats. Upgrade to VIP+ for advanced analytics!',
            inline: false,
          }
        );
    ...
    leaderboardData.forEach((capper, index) => {
      const medal =
        index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `${capper.rank}.`;
      embed.addFields({
    ...
    logger.info(
      `Capper leaderboard viewed by ${interaction.user.tag} (${userTier}) - ${timeframe}/${sport}`
    );
  } catch (error) {
