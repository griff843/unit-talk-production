...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand(subcommand =>
    subcommand
      ...
      .addStringOption(option =>
        option.setName('pick_id').setDescription('Specific pick ID for detailed coaching')
      )
  );
...
async function handlePickSubmission(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const pickSubmission: UserPickSubmission = {
    ...
    created_at: toISOString(new Date()), // Add required created_at
    pick_data: {
      // Add required pick_data
      id: `pick-${Date.now()}`,
      user_id: interaction.user.id,
      sport,
      bet_type: 'standard',
    ...
    submitted_at: toISOString(new Date()), // Add required submitted_at
  ...
  try {
    ...
    // Save to database (unified_picks)
    ...
    // Pick-only policy: schedule opening the thread 15 minutes before game start
    try {
      // Build minimal pick context
      const teams = pickSubmission?.pick?.team1 && pickSubmission?.pick?.team2
        ? `${pickSubmission.pick.team1} vs ${pickSubmission.pick.team2}`
        : description;

      const { GameDayThreadScheduler } = await import('../services/gameDayThreadScheduler');
      const scheduler = new GameDayThreadScheduler(interaction.client, supabaseService);
      await scheduler.scheduleForPick({
        id: savedPick.id,
      ...
      logger.warn('Failed to schedule game-day thread (pick-only)', e);
    }
    ...
    const resultEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: '🔍 Analysis',
          value: gradingResult.feedback || gradingResult.notes || 'Analysis pending...',
        }
      )
      .setTimestamp()
    ...
    logger.info(`Pick submitted and grading_status for user ${interaction.user.username}`, {
      pickId: savedPick.id,
      grade: gradingResult.grade,
      sport,
    });
...
async function handlePickHistory(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try {
    ...
    const recentPicks = picks
      .slice(0, 5)
      .map(pick => `${getGradeEmoji(pick.grade)} **${pick.description}** (${pick.grade})`)
      .join('\n');
  ...
  try {
    ...
    const coachingEmbed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: '💡 Key Insights',
          value: coachingResult.insights?.join('\n') || 'No insights available',
        },
        {
          name: '📈 Improvement Areas',
          value: coachingResult.improvements?.join('\n') || 'No improvements identified',
        },
        {
          name: '🎯 Next Steps',
          value: coachingResult.recommendations?.join('\n') || 'No recommendations available',
        }
      )
      .setTimestamp()
