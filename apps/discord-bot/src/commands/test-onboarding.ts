...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  try {
    ...
    console.log(`[TEST-ONBOARDING] Testing journey: ${journey}, sendDM: ${sendDM}, testMode: ${testMode}`);
    ...
    if (!sendDM) {
      ...
      await interaction.editReply({
        content: `✅ **${getTierDisplay(tier)} onboarding started!**\n\n` +
    ...
    await interaction.editReply({
      content: '❌ An error occurred while testing the onboarding sequence. Check logs for details.',
    });
  }
}
...
async function previewWhyJourney(
  ...
  if (sendDM) {
    ...
    await interaction.editReply({
      content: `✅ **${info.theme} Journey Started!**\n\n` +
...
async function showJourneyPreview(
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `Unit Talk ${tier.toUpperCase()} Onboarding Preview | Matches Production Logic` })
    .setTimestamp();

  await interaction.editReply({
    content: `**🎯 EXACT PREVIEW MODE** - This shows what ${tier} users actually receive\n` +
