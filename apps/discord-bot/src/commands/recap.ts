...
export async function execute(interaction: ChatInputCommandInteraction) {
  try {
    ...
    if (userTier === 'member') {
      embed.setDescription('🔒 **Limited Preview** - Upgrade for full recap access!').addFields(
        {
      ...
      embed.setDescription(`Complete performance breakdown for ${targetDate}`).addFields(
        {
          name: '📈 Overall Performance',
      ...
      embed.setDescription('Basic daily performance summary').addFields(
        {
          name: '📈 Performance',
          value: `**Win Rate:** ${recapData.winRate}%\n**Total Picks:** ${recapData.totalPicks}`,
          inline: false,
