...
export class AdminCommands {
    ...
    return [
      ...
      new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
          subcommand.setName('dashboard').setDescription('Show the main admin dashboard')
        )
        .addSubcommand(subcommand =>
          subcommand.setName('onboarding').setDescription('Manage onboarding flows and settings')
        )
        .addSubcommand(subcommand =>
          subcommand.setName('analytics').setDescription('View detailed onboarding analytics')
        )
        .addSubcommand(subcommand =>
          subcommand
            .setName('test')
      ...
      new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
          subcommand
            ...
            .addUserOption(option =>
              option.setName('user').setDescription('User to check status for').setRequired(true)
            )
        )
        .addSubcommand(subcommand =>
          subcommand
            ...
            .addUserOption(option =>
              option.setName('user').setDescription('User to retry DM for').setRequired(true)
            )
        ),
      ...
      new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
          subcommand.setName('overview').setDescription('Get analytics overview')
        )
        .addSubcommand(subcommand =>
          subcommand
            .setName('user')
            ...
            .addUserOption(option =>
              option.setName('user').setDescription('User to get analytics for').setRequired(true)
            )
        )
        .addSubcommand(subcommand =>
          subcommand
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields([
          ...
          {
            name: 'Started',
            value: status.startedAt ? new Date(status.startedAt).toLocaleString() : 'Unknown',
            inline: true,
          },
          {
            name: 'Completed',
   ...
   */
  private async handleOnboardingCommand(
    interaction: ChatInputCommandInteraction,
    subcommand: string
  ): Promise<void> {
    switch (subcommand) {
   ...
   */
  private async handleAnalyticsCommand(
    interaction: ChatInputCommandInteraction,
    subcommand: string
  ): Promise<void> {
    switch (subcommand) {
        ...
        .addFields([
          ...
          },
          {
            ...
            value: status.startedAt
              ? `<t:${Math.floor(new Date(status.startedAt).getTime() / 1000)}:R>`
              : 'Not started',
            inline: true,
          },
          {
            name: '🏁 Completed At',
            value: status.completedAt
              ? `<t:${Math.floor(new Date(status.completedAt).getTime() / 1000)}:R>`
              : 'Not completed',
            inline: true,
          },
        ])
  ...
  private async handleRetryDM(interaction: ChatInputCommandInteraction): Promise<void> {
    try {
      ...
      const success = await this.onboardingService.sendDM(
        userId,
  ...
  private async handleUserAnalytics(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      if (journey.steps.length > 0) {
        const stepDetails = journey.steps
          .map(step => `📝 **${step.action}** - ${new Date(step.timestamp).toLocaleString()}`)
          .join('\n');
  ...
  private async handleExportAnalytics(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      await interaction.editReply(
        `✅ Analytics export in ${format.toUpperCase()} format has been generated. Check your DMs for the download link.`
      );
    } catch (error) {
  ...
  private hasAdminPermissions(interaction: ChatInputCommandInteraction): boolean {
    ...
    return (
      this.permissionsService.hasRole(interaction.member as GuildMember, 'admin') ||
