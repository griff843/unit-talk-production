...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option
        .setName(`sports`)
        .setDescription(`Enter the sports`)
        .setRequired(true)
        .addChoices(
          { name: `NBA`, value: `nba` },
          { name: `CBB`, value: `cbb` },
          ...
          { name: `otherVIP`, value: `othervip` },
          { name: `FreePick`, value: `freepick` }
        )
    )
    .addStringOption(option =>
      option.setName(`date`).setDescription(`Enter the date in YYYY-MM-DD format`).setRequired(true)
    )
    .addStringOption(option =>
      option.setName(`teams`).setDescription(`Enter the teams/matchup`).setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName(`bet_type`)
        .setDescription(`Enter the bet type`)
        .setRequired(true)
        .addChoices(
    ...
    .addStringOption(option =>
      option
        .setName(`units`)
        .setDescription(`Enter the units`)
        .setRequired(true)
        .addChoices(
          { name: `0.5`, value: `0.5` },
          { name: `1`, value: `1` },
    ...
    .addStringOption(option =>
      option.setName(`book`).setDescription(`Enter the book`).setRequired(true)
    )
    .addStringOption(option =>
      option.setName(`odds`).setDescription(`Enter the odds`).setRequired(true)
    )
    .addStringOption(option =>
      option.setName(`pick`).setDescription(`Enter the picknode `).setRequired(true)
    )
    .addStringOption(option =>
      option.setName(`notes`).setDescription(`Enter notes you wish to add`).setRequired(false)
    ),
  ...
  async execute(interaction, client) {
    ...
    const data = await mysql.queryExecute(
      `INSERT INTO SportsPick (User_id, Sport, Teams, Pick, Book, Notes, Units, Odds, Bet_type, Date, Access_tier) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        interaction.user.id,
        sports,
        teams,
    ...
    const pickEmbed = new EmbedBuilder()
      .setDescription(
        `**New pick Incoming from ${interaction.user}** \n\nSports: ${sports} \nGame date: ${date} \nMatchup: ${teams} \nPick: ${pick} \nUnits: ${units} \nBook: ${book} \nOdds: ${odds} \nBet type: ${bet_type}\nNotes: ${notes} \nAccess tier: ${accessTier} \n\nSubmitted: ${formattedTimestamp}`
      )
      .setColor(`Blue`);
