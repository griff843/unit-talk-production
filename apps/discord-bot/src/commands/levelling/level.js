...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const mysqlGuildData = await mysql.queryExecute(
      `SELECT * FROM Level WHERE Guild_id = ${guild.id}`
    );

    if (mysqlData.length === 0)
      return interaction.reply({
        content: `${member} has not earned any XP in this server and does not have any level data in this server.`,
      });
    ...
    const profileEmbed = new EmbedBuilder()
      .setColor(`Blue`)
      .setAuthor({
        name: `${tag}'s level data on ${interaction.guild.name}`,
