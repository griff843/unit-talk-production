...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption(option =>
      option.setName('level').setDescription(`Enter a level`).setRequired(false)
    ),
  ...
  async execute(interaction, client) {
    ...
    if (operation === 'add') {
      ...
      const data = await mysql.queryExecute(
        `SELECT * FROM LevelRoles WHERE Guild_id = ? AND Level = ?`,
        [guild.id, level]
      );
      if (data.length)
        return interaction.reply({ content: `There is already a role setup for level ${level}` });
      await mysql.queryExecute(`INSERT INTO LevelRoles VALUES (?, ?, ?)`, [
        guild.id,
        level,
        role.id,
      ]);
      ...
      if (!data.length)
        return interaction.reply({ content: `There is no role setup for level ${level}` });
      await mysql.queryExecute(`DELETE FROM LevelRoles WHERE Guild_id = ? AND Level = ?`, [
        guild.id,
        level,
      ]);
      return interaction.reply({ content: `Role has been removed for level ${level}` });
    } else if (operation === 'list') {
      const data = await mysql.queryExecute(`SELECT * FROM LevelRoles WHERE Guild_id = ?`, [
        guild.id,
      ]);
      if (!data.length)
        return interaction.reply({
          content: `There are no roles setup for any level in this server`,
        });
