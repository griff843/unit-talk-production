...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option
        .setName('reason')
        .setDescription('Enter the reason to timeout the user')
        .setRequired(true)
    )
    .addStringOption(option =>
      option
        .setName('duration')
        .setDescription(`Enter the timeout duration (30m, 1h, 1 day)`)
        .setRequired(true)
    ),
  async execute(interaction, client) {
    ...
    const nopermEmbed = new EmbedBuilder()
      .setDescription(
        `You don't have permissions to run this command. You need \`Moderate Members\` permission to run this command!`
      )
      .setColor(`Red`);
      ...
      if (timeoutUser.isCommunicationDisabled()) {
        await mysql.queryExecute(
          ...
          [
            ...
            mentionable.id,
            interaction.user.id,
            `Timeout [${prettyMs(msDuration, { verbose: true })}]`,
            reason,
        ...
        await interaction.reply(
          `${timeoutUser}'s timeout has been updated to ${prettyMs(msDuration, { verbose: true })}\nReason: ${reason}`
        );
