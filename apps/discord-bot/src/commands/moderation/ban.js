...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option =>
      option.setName('user').setDescription(`Select the user to ban`).setRequired(true)
    )
    .addStringOption(option =>
      option.setName('reason').setDescription('Enter the reason to ban the user').setRequired(true)
    ),
  ...
  async execute(interaction, client) {
    ...
    const nopermEmbed = new EmbedBuilder()
      .setDescription(
        `You don't have permissions to run this command. You need \`Ban Members\` permission to run this command!`
      )
      .setColor(`Red`);
    ...
    const errEmbed = new EmbedBuilder()
      .setDescription(
        `You can't take action on ${backtick}${target.username}${backtick} since they have a higher role!`
      )
      .setColor('Red');
    ...
    if (!banhammer.bannable) {
      return interaction.reply({
        content: `I cannot ban this user! I do not have permissions or there might be a error`,
      });
    }
    // Ban hammer
    try {
      ...
      await mysql.queryExecute(
        `INSERT INTO Modlogs(Guild_id, User_id,Moderator_id,Punishment_type,Reason, Timestamp) VALUES (?,?,?,?,?,?)`,
        [
