...
export async function execute(interaction: ChatInputCommandInteraction) {
  try {
      ...
      today: [
        {
          ...
          units: 5,
          analysis:
            'Elite matchup advantage with key injury factor. Historical dominance in this scenario.',
    ...
    const timeframeText =
      timeframe === 'today' ? 'Today' : timeframe === 'week' ? 'This Week' : 'Weekend Special';

    const embed = new EmbedBuilder()
      .setTitle(`🖤 Black Label Top Plays - ${timeframeText}`)
      .setDescription(
        '**EXCLUSIVE ACCESS** - Highest confidence plays for Black Label members only'
      )
      .setColor(0x000000)
      .setTimestamp();
    ...
    if (plays.length === 0) {
      ...
      embed.addFields({
        ...
        value:
          'These are maximum confidence plays with higher unit recommendations. Please bet responsibly and within your bankroll limits.',
        inline: false,
      });
    }
    ...
    logger.info(
      `Black Label top plays accessed by ${interaction.user.tag} (${userTier}) - ${timeframe}`
    );
  } catch (error) {
