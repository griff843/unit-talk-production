...
export class AdminOverrideService {
  ...
  private async toggleMaintenanceMode(adminId: string, params: any): Promise<any> {
    try {
      ...
      await this.supabaseService.client.from('system_config').upsert({
  ...
  private async overrideCooldowns(adminId: string, params: any): Promise<any> {
    try {
      ...
      if (action === 'reset') {
        ...
        await this.supabaseService.client.from('user_cooldowns').delete().eq('user_id', userId);
  ...
  private async performSystemHealthCheck(adminId: string): Promise<any> {
    try {
      ...
      await this.supabaseService.client.from('system_health_checks').insert(healthCheck);
  ...
  private async logAdminAction(override: AdminOverride): Promise<void> {
    try {
      await this.supabaseService.client.from('admin_actions').insert({
        id: override.id,
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: 'Completed At',
            value: override.completedAt ? new Date(override.completedAt).toISOString() : 'N/A',
            inline: true,
          }
        )
        .setColor('#00FF00')
      ...
      if (override.result) {
        embed.addFields({
          name: 'Result',
          value: JSON.stringify(override.result).substring(0, 1000),
          inline: false,
        });
      }
  ...
  private async sendPickCorrectionNotification(
    pickId: string,
    newResult: string,
    reason: string
  ): Promise<void> {
  ...
  private generateHealthRecommendations(healthCheck: any): string[] {
    ...
    if (healthCheck.memory.heapUsed > 500 * 1024 * 1024) {
      // 500MB
