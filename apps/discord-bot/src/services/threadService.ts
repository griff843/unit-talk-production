...
export class ThreadService {
    ...
    try {
      ...
      if (threads) {
        ...
        threads.forEach(thread => {
          ...
          this.activeThreads.set(thread.gameId, gameThread);
        });
  ...
  async createGameThread(gameData: {
    ...
    try {
      ...
      // Get threads channel
      const threadsChannel = (await this.client.channels.fetch(
        botConfig.channels.threads
      )) as TextChannel;
      if (!threadsChannel) {
      ...
      const gameThread: GameThread = {
        ...
        game_time: gameData.gameTime
          ? typeof gameData.gameTime === 'string'
            ? gameData.gameTime
            : new Date(gameData.gameTime).toISOString()
      ...
      await thread.send({
        content:
          `Welcome to the **${gameData.teams}** discussion thread! 🏈\n\n` +
      ...
      switch (customId) {
        case 'thread_follow':
          ...
          await interaction.reply({
            content: '✅ You will now receive notifications for this thread!',
            ephemeral: true,
          });
          break;
        ...
        case 'thread_unfollow':
          ...
          await interaction.reply({
            content: '❌ Thread notifications disabled.',
            ephemeral: true,
          });
          break;
        ...
        case 'pick_track':
          if (await PermissionUtils.canSubmitPicks(member)) {
            ...
            await interaction.reply({
              content: '📊 Pick tracked! Check your stats with `/mystats`',
              ephemeral: true,
            });
          } else {
            await interaction.reply({
              content: '❌ VIP+ required to track picks.',
              ephemeral: true,
            });
          }
          break;
        ...
        case 'pick_analyze':
          if (await PermissionUtils.canAccessCoaching(member)) {
            ...
            await interaction.reply({
              content: '❌ VIP+ required for pick analysis.',
              ephemeral: true,
            });
          }
          break;
  ...
  private createPickEmbed(pickData: any): EmbedBuilder {
    return new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '⏰ Posted', value: toLocaleTimeString(new Date()), inline: true }
      )
  ...
  private createLiveUpdateEmbed(updateData: any): EmbedBuilder {
    return new EmbedBuilder()
      ...
      .addFields(
        { name: '📊 Score', value: updateData.professional_score, inline: true },
  ...
  private createGradingEmbed(gradingData: any): EmbedBuilder {
    const color =
      gradingData.result === 'win' ? 0x00ff00 : gradingData.result === 'loss' ? 0xff0000 : 0xffff00;
   ...
   */
  private createThreadButtons(_gameId: string): ActionRowBuilder<ButtonBuilder> {
   ...
   */
  private createPickButtons(_pickId: string): ActionRowBuilder<ButtonBuilder> {
    return new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
  ...
  private async saveGameThread(gameThread: GameThread): Promise<void> {
    try {
      await this.supabaseService.client.from('game_threads').upsert({
        id: gameThread.id,
        thread_id: gameThread.thread_id,
        game_id: (gameThread as any).gameId || gameThread.id,
  ...
  private async updateThreadStats(gameId: string, action: string): Promise<void> {
    try {
      ...
      if ('last_activity' in gameThread) {
        (gameThread as any).last_activity = toISOString(new Date());
      }
      ...
      await this.supabaseService.client
        .from('game_threads')
        .update({
          last_activity: (gameThread as any).last_activity || toISOString(new Date()),
   ...
   * Notify VIP users in thread
   */
  private async notifyVIPUsers(thread: ThreadChannel, _pickData: any): Promise<void> {
    try {
  ...
  private async followThread(userId: string, threadId: string): Promise<void> {
    try {
      await this.supabaseService.client.from('thread_followers').upsert({
        user_id: userId,
        thread_id: threadId,
        followed_at: toISOString(new Date()),
      });
    } catch (error) {
  ...
  private async trackUserPick(userId: string, pickEmbed: any): Promise<void> {
    try {
      ...
      await this.supabaseService.client.from('user_picks').insert({
        user_id: userId,
    ...
    try {
      ...
      const analysis = {
        ...
        factors: [
          ...
          'Historical matchup data positive',
   ...
   */
  async handleThreadUpdate(_oldThread: any, newThread: any): Promise<void> {
    try {
  ...
  private extractGameInfo(content: string): any | null {
    ...
    const patterns = [/(\w+)\s+vs\s+(\w+)/i, /(\w+)\s+@\s+(\w+)/i, /(\w+)\s+-\s+(\w+)/i];
    ...
    try {
      ...
      for (const [gameId, thread] of this.activeThreads) {
        const lastActivity = new Date(
          (thread as any).lastActivity ||
            (thread as any).last_activity ||
            thread.created_at ||
            Date.now()
        ).getTime();
