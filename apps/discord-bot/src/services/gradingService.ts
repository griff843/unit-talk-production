/* eslint-disable max-lines, max-lines-per-function, complexity, @typescript-eslint/no-unused-vars, import/order */
...
export class PickGradingService {
  ...
  private async extractGradingFactors(pick: UserPickSubmission): Promise<GradingFactor[]> {
    ...
    factors.push({
      name: 'Odds Value',
      category: 'value',
      value: oddsAnalysis.professional_score,
      weight: 0.25,
      score: oddsAnalysis.professional_score,
    ...
    factors.push({
      name: 'Pick Timing',
      category: 'timing',
      value: timingAnalysis.professional_score,
      weight: 0.15,
      score: timingAnalysis.professional_score,
    ...
    factors.push({
      ...
      category: 'history',
      value: userHistory.professional_score,
      weight: 0.3,
      score: userHistory.professional_score,
    ...
    factors.push({
      name: 'Risk Level',
      category: 'risk',
      value: riskAssessment.professional_score,
      weight: 0.2,
      score: riskAssessment.professional_score * 100,
  ...
  private async calculateEdge(pick: UserPickSubmission, factors: GradingFactor[]): Promise<number> {
    ...
    factors.forEach(factor => {
      weightedScore += (factor.professional_score || 0) * factor.weight;
  ...
  private async generateCoachingNotes(
    ...
    // Identify strengths
    const strongFactors = factors.filter(f => (f.professional_score || 0) >= 70);
    ...
    const weakFactors = factors.filter(f => (f.professional_score || 0) < 50);
  ...
  private identifyImprovementAreas(factors: GradingFactor[]): string[] {
    ...
    factors.forEach(factor => {
      if ((factor.professional_score || 0) < 50) {
        switch (factor.name) {
          case 'Odds Value':
  ...
  public analyzePickTiming(pick: UserPickSubmission): {
    ...
    return {
      professional_score: 0.7, // Default professional_score
  ...
  private async analyzeContextualFactors(
    ...
    const professional_score = 60 + Math.random() * 30;
  ...
  private async getUserHistoricalPerformance(
    ...
    try {
      ...
      const professional_score = winRate * 100;
