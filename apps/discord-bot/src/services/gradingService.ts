...
export class PickGradingService {
  ...
  async gradeUserPick(pickSubmission: UserPickSubmission): Promise<GradingResult> {
    try {
      ...
      logger.info(
        `Pick grading_status successfully: Edge ${edge}%, Tier ${tier}, Confidence ${confidence}%`
      );
      return result;
    } catch (error) {
  ...
  private async extractGradingFactors(pick: UserPickSubmission): Promise<GradingFactor[]> {
    ...
    const oddsValue = pick.odds
      ? typeof pick.odds === 'string'
        ? parseFloat(pick.odds)
        : pick.odds
      : 0;
    ...
    const userHistory = await this.getUserHistoricalPerformance(
      pick.user_id || '',
      pick.description || ''
    );
    factors.push({
      ...
      category: 'history',
      value: (userHistory as any).professional_score ?? (userHistory as any).score ?? 50,
      weight: 0.3,
      score: (userHistory as any).professional_score ?? (userHistory as any).score ?? 50,
      description: (userHistory as any).description,
    });
  ...
  private async calculateEdge(pick: UserPickSubmission, factors: GradingFactor[]): Promise<number> {
    ...
    factors.forEach(factor => {
      const val = (factor as any).professional_score ?? factor.score ?? factor.value ?? 0;
      weightedScore += val * factor.weight;
    ...
    const unitPenalty =
      (pick.units || 1) > 3 ? Math.max(0.8, 1 - ((pick.units || 1) - 3) * 0.1) : 1;
  ...
  private calculateConfidence(factors: GradingFactor[]): number {
    const avgScore = factors.reduce((sum, factor) => sum + ((factor as any).professional_score ?? factor.score ?? 0), 0) / factors.length;
  ...
  /**
   * Calculate consistency professional_score across factors
   */
  private calculateConsistency(factors: GradingFactor[]): number {
    const scores = factors.map(f => (f as any).professional_score ?? f.score ?? 0);
    const mean = scores.reduce((sum: number, s) => sum + (s || 0), 0) / scores.length;
    const variance = scores.reduce((sum: number, s) => sum + Math.pow((s || 0) - mean, 2), 0) / scores.length;
  ...
  private async generateFeedback(
    ...
    factors.forEach(factor => {
      const score = (factor as any).professional_score ?? factor.score ?? 0;
      const emoji = (score || 0) >= 70 ? '✅' : (score || 0) >= 50 ? '⚠️' : '❌';
      feedback += `${emoji} ${factor.name}: ${Math.round(score || 0)}% - ${factor.description}\n`;
    });
  ...
  private async generateCoachingNotes(
    ...
    // Identify strengths
    const strongFactors = factors.filter(f => (((f as any).professional_score ?? f.score ?? 0) >= 70));
    ...
    const weakFactors = factors.filter(f => (((f as any).professional_score ?? f.score ?? 0) < 50));
  ...
  private identifyImprovementAreas(factors: GradingFactor[]): string[] {
    ...
    factors.forEach(factor => {
      const s = (factor as any).professional_score ?? factor.score ?? 0;
      if ((s || 0) < 50) {
        switch (factor.name) {
          case 'Odds Value':
  ...
  private async getMarketData(
    description: string
  ): Promise<{ favorability: number; movement: number } | null> {
  ...
  public analyzePickTiming(pick: UserPickSubmission): { score: number; timing: string } {
    ...
    const submittedAt = pick.submitted_at ? new Date(pick.submitted_at) : new Date(now);
   ...
   */
  private async analyzeSharpMoney(
    description: string
  ...
  private async saveGradingResult(result: GradingResult): Promise<void> {
    try {
      await this.supabaseService.client.from('pick_gradings').insert({
        ...
        graded_at: toISOString(new Date()),
      });
    } catch (error) {
...
export class CoachingService {
  ...
  async generateBettingAnalysis(userId: string, period: string = '30d'): Promise<BettingAnalysis> {
    try {
      ...
      const riskAssessment =
        typeof riskAssessmentObj === 'string' ? riskAssessmentObj : riskAssessmentObj.level;
      ...
      return {
        ...
        risk_assessment:
          riskAssessment === 'reckless' || riskAssessment === 'aggressive'
            ? 'high'
  ...
  private async getUserPicks(userId: string, period: string): Promise<any[]> {
    ...
    const { data: picks } = await this.supabaseService.client
      ...
      .gte(
        'submitted_at',
        typeof cutoffDate === 'string' ? cutoffDate : new Date(cutoffDate).toISOString()
      )
  ...
  private analyzeBettingPatterns(picks: any[]): any {
    ...
    picks.forEach(pick => {
      ...
      sportBreakdown[sport].units +=
        pick.result === 'win' ? pick.units : pick.result === 'loss' ? -pick.units : 0;
    });
  ...
  private async generateRecommendations(
    ...
    // Sport focus
    const bestSport = Object.entries(analysis.sportBreakdown).sort(
      ([, a], [, b]) => (b as any).units - (a as any).units
    )[0];
  ...
  private assessRisk(analysis: any): RiskAssessment {
    ...
    const maxRecommendedUnits =
      level === 'reckless' ? 1 : level === 'aggressive' ? 2 : level === 'moderate' ? 3 : 5;
  ...
  async scheduleCoachingSession(userId: string, preferences: any): Promise<void> {
    try {
      await this.supabaseService.client.from('coaching_sessions').insert({
        user_id: userId,
