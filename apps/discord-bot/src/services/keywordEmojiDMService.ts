...
export class KeywordEmojiDMService {
  ...
  async loadTriggers(): Promise<void> {
    try {
      ...
      logger.info(
        `Loaded ${keywordTriggers?.length || 0} keyword triggers and ${emojiTriggers?.length || 0} emoji triggers`
      );
    } catch (error) {
  ...
  private async handleKeywordTrigger(
    ...
    try {
      ...
      const processedContent = this.processTemplateVariables(template.content, {
        ...
        timestamp: toISOString(new Date()).toLocaleString(),
      });
  ...
  async updateTriggerStatus(
    ...
    try {
      ...
      logger.info(
        `Admin ${adminId} ${isActive ? 'activated' : 'deactivated'} ${triggerType} trigger ${triggerId}`
      );
      return true;
    } catch (error) {
  ...
  async getTriggerStatistics(): Promise<any> {
    try {
      ...
      return {
        ...
        topKeywordTriggers: keywordStats
          .sort((a: any, b: any) => b.activation_count - a.activation_count)
          .slice(0, 5),
        topEmojiTriggers: emojiStats
          .sort((a: any, b: any) => b.activation_count - a.activation_count)
          .slice(0, 5),
      };
    } catch (error) {
  ...
  private shouldProcessTrigger(trigger: any, userTier: string, member: GuildMember): boolean {
    ...
    if (trigger.conditions.timeRestrictions) {
      ...
      if (
        trigger.conditions.timeRestrictions.startHour &&
        hour < trigger.conditions.timeRestrictions.startHour
      ) {
        return false;
      }
      if (
        trigger.conditions.timeRestrictions.endHour &&
        hour > trigger.conditions.timeRestrictions.endHour
      ) {
        return false;
      }
    }
  ...
  private async logTriggerActivation(
    ...
    try {
      await this.supabaseService.client.from('trigger_activation_logs').insert({
