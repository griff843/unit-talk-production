...
export class FeedbackService {
  ...
  private async recordFeedback(feedback: FeedbackData): Promise<void> {
    try {
      await supabaseService.client.from('message_feedback').upsert({
   ...
   * Store message for feedback tracking
   */
  private async storeMessageForFeedback(
    messageId: string,
    messageType: 'recap' | 'alert' | 'notification' | 'command_response'
  ): Promise<void> {
    try {
      await supabaseService.client.from('feedback_messages').upsert({
   ...
   */
  private async getMessageType(
    messageId: string
  ): Promise<'recap' | 'alert' | 'notification' | 'command_response' | null> {
    try {
      ...
      return (
        (data?.message_type as 'recap' | 'alert' | 'notification' | 'command_response') || null
      );
    } catch (error) {
  ...
  async getFeedbackSummary(days: number = 30): Promise<FeedbackSummary[]> {
    try {
      ...
      for (const feedback of data) {
        const messageType = feedback.message_type as
          | 'recap'
          | 'alert'
          | 'notification'
          | 'command_response';
    ...
    try {
      ...
      if (trending.length > 0) {
        ...
        embed.addFields({
          ...
          value:
            `${recentPositive}/${recentTotal} positive (${Math.round((recentPositive / recentTotal) * 100)}%)\n` +
          ...
          value:
            recentNegativeFeedback.length > 0
              ? recentNegativeFeedback
                  .map(
                    f =>
                      `- ${this.getMessageTypeEmoji(f.messageType)} ${f.messageType.toUpperCase()} at ${new Date(f.timestamp).toLocaleString()}`
                  )
                  .join('\n')
