...
export class ABTestingService {
  ...
  async assignUserToCohort(userId: string, testType: string = 'default'): Promise<string> {
    try {
      ...
      // Store assignment
      await supabaseService.client.from('user_cohort_assignments').insert({
        user_id: userId,
  ...
  async getMessageTemplate(
    ...
    try {
      ...
      return {
        ...
        createdAt: toISOString(new Date(template.createdAt)),
        updatedAt: toISOString(new Date(template.updated_at)),
      };
    } catch (error) {
  ...
  async trackResult(
    ...
    try {
      ...
      await supabaseService.client.from('ab_test_results').insert({
        user_id: userId,
        cohort_id: cohortId,
        test_type: testType,
        metric,
  ...
  async createCohort(cohort: Omit<ABTestCohort, 'createdAt'>): Promise<boolean> {
    try {
      await supabaseService.client.from('ab_test_cohorts').insert({
        id: cohort.id,
        name: cohort.name,
  ...
  async createTemplate(
    ...
    try {
      await supabaseService.client.from('message_templates').insert({
  ...
  async getCohorts(testType?: string): Promise<ABTestCohort[]> {
    try {
      let query = supabaseService.client.from('ab_test_cohorts').select('*');
      ...
      return cohorts.map(c => {
        const cohort: ABTestCohort = {
          ...
          config: c.config,
          createdAt: toISOString(new Date(c.createdAt)),
        };
