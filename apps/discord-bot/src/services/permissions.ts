...
export class PermissionsService {
  ...
  getUserTier(member: GuildMember): UserTier {
    try {
      ...
      logger.info(`Getting tier for ${member.user.username}. Roles: ${roles}`);
      ...
      if (member.roles.cache.has(botConfig.roles.owner)) {
        return 'owner';
      }

      if (member.roles.cache.has(botConfig.roles.admin)) {
        return 'admin';
      }

      if (member.roles.cache.has(botConfig.roles.staff)) {
        return 'staff';
      }

      if (member.roles.cache.has(botConfig.roles.moderator)) {
        return 'staff';
      }
      ...
      if (
        member.roles.cache.some(
          role =>
            ...
            role.name.toLowerCase().includes('ut capper')
        )
      ) {
        return 'capper';
      }
  ...
  isAdmin(member: GuildMember): boolean {
    try {
      return (
        ...
        member.roles.cache.has(botConfig.roles.owner) ||
  ...
  isModerator(member: GuildMember): boolean {
    try {
      return (
        member.roles.cache.has(botConfig.roles.moderator) ||
        member.roles.cache.has(botConfig.roles.staff) ||
        member.roles.cache.has(botConfig.roles.admin) ||
        member.roles.cache.has(botConfig.roles.owner) ||
        member.permissions.has('Administrator')
      );
  ...
  hasRole(member: GuildMember, roleType: string): boolean {
    try {
      switch (roleType.toLowerCase()) {
        ...
        case 'vip':
          return (
            member.roles.cache.has(botConfig.roles.vip) ||
            member.roles.cache.has(botConfig.roles.vipPlus) ||
            this.isAdmin(member)
          );
        case 'vipplus':
        case 'vip_plus':
          return member.roles.cache.has(botConfig.roles.vipPlus) || this.isAdmin(member);
        case 'capper':
        case 'ut capper':
        ...
        default:
          ...
          return member.roles.cache.some(
            role => role.name.toLowerCase() === roleType.toLowerCase() || role.id === roleType
          );
      }
    } catch (error) {
      logger.error(`Error checking role ${roleType}:`, {
        error,
        userId: member.id,
        username: member.user.username,
        roleType,
  ...
  async syncUserTier(member: GuildMember): Promise<void> {
    try {
      ...
      if (profile && (profile as any).tier && (profile as any).tier !== newTier) {
        ...
        logger.info(
          `Updated user ${member.user.username} tier from ${(profile as any).tier} to ${newTier}`
        );
      }
    } catch (error) {
