...
class AICoachingService {
  ...
  private async generateClaudeAnalysis(request: AIAnalysisRequest): Promise<AIAnalysisResponse> {
    ...
    const message = await this.anthropic.messages.create({
      model:
        request.userTier === 'vip_plus' ? 'claude-3-opus-20240229' : 'claude-3-sonnet-20240229',
      max_tokens: request.userTier === 'vip_plus' ? 1500 : 800,
      temperature: 0.7,
      system: systemPrompt,
...
- Recent Performance: ${userHistory.recentPicks.length} recent picks\n\n`;
  ...
  private generateKeyInsights(userHistory: any, question: string): string[] {
    ...
    if (userHistory.winRate < 0.45) {
      insights.push(
        'Your recent win rate suggests focusing on bet selection quality over quantity'
      );
    } else if (userHistory.winRate > 0.6) {
      insights.push(
        'Strong win rate indicates good analytical skills - consider increasing unit sizes gradually'
      );
    }

    if (userHistory.totalProfit < 0) {
      insights.push(
        'Current negative ROI suggests reviewing bankroll management and bet sizing strategy'
      );
    }

    if (question.toLowerCase().includes('parlay') || question.toLowerCase().includes('combo')) {
      insights.push(
        'Parlays have exponentially lower win probability - consider single bets for better long-term results'
      );
    }
  ...
  private generateRecommendations(
    ...
    if (userHistory.totalPicks < 50) {
      recommendations.push(
        'Build a larger sample size before drawing conclusions about your strategy'
      );
    }

    if (userHistory.winRate > 0.55 && userHistory.totalProfit > 0) {
      recommendations.push(
        'Your strategy is working - maintain consistency and gradually scale up'
      );
    }
...
The key to long-term success in sports betting is maintaining discipline, proper bankroll management, and continuous learning from both wins and losses.`;
  ...
  async addQuestionToSession(
    ...
    try {
      ...
      const updatedFeedback = {
        ...
        lastActivity: toISOString(new Date()),
      };
  ...
  async completeCoachingSession(sessionId: string): Promise<boolean> {
    try {
      ...
      const success = await databaseService.updateCoachingSession(sessionId, {
        status: 'completed',
        completed_at: typeof endTime === 'string' ? endTime : new Date(endTime).toISOString(),
