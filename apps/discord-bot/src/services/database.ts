...
// Relaxed typing to accelerate production readiness; detailed DB typing will be restored after schema sync
// import { Database } from '../db/types/supabase-complete';
import { logger } from '../utils/logger';
import { botConfig } from '../config';

// Type aliases (temporarily any to avoid "never" errors from partial schema typing)
type UserProfileRow = any;
type UserProfileInsert = any;
type UserProfileUpdate = any;
type UserPicksRow = any;
type UserPicksInsert = any;
type UserPicksUpdate = any;
type UnifiedPicksRow = any;
type UnifiedPicksInsert = any;
type UnifiedPicksUpdate = any;
type GameThreadsRow = any;
type GameThreadsInsert = any;
type GameThreadsUpdate = any;
type UserCooldownsRow = any;
type UserCooldownsInsert = any;

// Temporarily set to any until types are regenerated
type AnalyticsEventsInsert = any;
type ThreadStatsRow = any;
type ThreadStatsInsert = any;
type ThreadStatsUpdate = any;
// Coaching Sessions types
type CoachingSessionsRow = any;
type CoachingSessionsInsert = any;
type CoachingSessionsUpdate = any;

/**
 * Capper types - temporarily using any until TypeScript recognizes the new database schema
 */
type CappersRow = any;
type CappersInsert = any;
type CappersUpdate = any;
type CapperEvaluationsRow = any;
type CapperEvaluationsInsert = any;
type CapperEvaluationsUpdate = any;
...
export class DatabaseService {
  ...
  async getUserPicks(
    ...
    try {
      const { limit = 10, offset = 0, result, orderBy = 'created_at', ascending = false } = options;
   ...
   */
  async createUnifiedPick(pick: UnifiedPicksInsert): Promise<UnifiedPicksRow | null> {
    try {
      const { data, error } = await this.client
        .from('unified_picks')
        .insert({
  ...
  async updateGameThread(
    threadId: string,
    updates: GameThreadsUpdate
  ): Promise<GameThreadsRow | null> {
    try {
      const { data, error } = await this.client
        .from('game_threads')
        .update({
  ...
  async upsertUserCooldown(cooldown: UserCooldownsInsert): Promise<boolean> {
    try {
      const { error } = await this.client.from('user_cooldowns').upsert({
        ...cooldown,
        created_at: toISOString(new Date()),
      });
   ...
   */
  async updateThreadStats(
    threadId: string,
    updates: ThreadStatsUpdate
  ): Promise<ThreadStatsRow | null> {
    try {
      const { data, error } = await this.client
        .from('thread_stats')
        .upsert({
  ...
  /**
   * Create a DM log entry
   */
  async createDMLog(log: {
    user_id: string;
    ...
    try {
      await this.client.from('dm_logs').insert({
        user_id: log.user_id,
      ...
      logger.info(`Created DM log for user ${log.user_id}`, {
        contentType: log.content_type,
        templateId: log.template_id,
      });
    } catch (error) {
      logger.error(`Failed to create DM log for user ${log.user_id}:`, {
  ...
  async createCapper(capper: CappersInsert): Promise<CappersRow | null> {
    try {
      const { data, error } = await this.client.from('cappers').insert(capper).select().single();
  ...
  async createCoachingSession(
    session: CoachingSessionsInsert
  ): Promise<CoachingSessionsRow | null> {
    try {
      const { data, error } = await this.client
  ...
  async updateCoachingSession(
    sessionId: string,
    updates: CoachingSessionsUpdate
  ): Promise<boolean> {
    try {
      const { error } = await this.client
        .from('coaching_sessions')
  ...
  async getUserCoachingSessions(
    ...
    try {
      ...
      let query = this.client.from('coaching_sessions').select('*').eq('discord_id', discordId);
