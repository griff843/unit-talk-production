...
export class AdvancedAnalyticsService {
   ...
   */
  async trackPreferenceSelection(
    userId: string,
  ...
  async getOnboardingAnalytics(
    ...
    try {
      ...
      // Daily breakdown
      const dailyBreakdown = starts.reduce(
        (acc: Record<string, { starts: number; completions: number }>, event) => {
  ...
  async getAnalyticsSummary(): Promise<{
    ...
    try {
      const oneDayAgo = toISOString(new Date());
      const newOneDayAgo = setDate(oneDayAgo, getDate(oneDayAgo) - 1);
      ...
      const { count: totalEvents, error: totalError } = await this.supabaseService.client
        ...
        .gte(
          'timestamp',
          typeof oneDayAgo === 'string' ? oneDayAgo : new Date(oneDayAgo).toISOString()
        );
      ...
      const activeUsers = activeUsersData
        ? new Set(activeUsersData.map(event => event.user_id)).size
        : 0;
  ...
  async logError(error: Error, context?: string): Promise<void> {
    try {
      ...
      console.error(
        `❌ Analytics error log: ${error.message}`,
