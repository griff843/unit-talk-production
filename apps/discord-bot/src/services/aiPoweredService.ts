...
export class AIPoweredService {
  ...
  async gradePickWithAI(pickData: any, actualResult: any): Promise<AIGradingResult> {
    try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are an expert sports betting analyst. Provide detailed analysis of betting picks including accuracy assessment, reasoning evaluation, and improvement suggestions.',
            },
            {
              role: 'user',
              content: prompt,
      ...
      const gradingResult: AIGradingResult = {
        ...
        gradedAt: toISOString(new Date()),
        gradedBy: 'ai' as const,
        accuracyScore: this.extractAccuracyScore(aiAnalysis),
      };

      // Store grading result
      await this.supabaseService.client.from('ai_grading_results').insert(gradingResult);
  ...
  async startCoachingSession(userId: string, sessionType: string): Promise<AICoachingSession> {
    try {
      ...
      // Store session
      await this.supabaseService.client.from('ai_coaching_sessions').insert({
        id: session.id,
        user_id: userId,
  ...
  async generateMultiLingualResponse(
    ...
    try {
      ...
      // Store translation
      await this.supabaseService.client.from('multi_lang_responses').insert(multiLangResponse);
  ...
  async analyzeBettingPatterns(userId: string): Promise<any> {
    try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are an expert sports betting analyst. Analyze betting patterns and provide actionable insights for improvement.',
            },
            {
              role: 'user',
      ...
      // Store analysis
      await this.supabaseService.client.from('betting_pattern_analyses').insert(patternAnalysis);
  ...
  async generatePickRecommendations(userId: string, gameData: any[]): Promise<any[]> {
    try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are an expert sports betting advisor. Generate personalized pick recommendations based on user history and preferences.',
            },
            {
              role: 'user',
      ...
      await this.supabaseService.client.from('ai_pick_recommendations').insert({
        user_id: userId,
  ...
  async provideLiveCoaching(
    ...
    try {
          ...
          messages: [
            {
              role: 'system',
              content:
                'You are a live sports betting coach. Provide real-time, specific advice for live betting situations.',
            },
            {
              role: 'user',
      ...
      await this.supabaseService.client.from('live_coaching_sessions').insert({
        user_id: userId,
        game_id: gameId,
  ...
  private createGradingPrompt(pickData: any, actualResult: any): string {
    return `Analyze this sports betting pick:
      ...
      Please provide:
      1. Overall grade (A-F)
      2. Accuracy professional_score (1-10)
      3. Reasoning quality professional_score (1-10)
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a professional sports betting coach. Create personalized, encouraging coaching messages.',
          },
          {
            role: 'user',
            content: prompt,
  ...
  private extractImprovementSuggestions(analysis: string): string[] {
    ...
    const suggestions = analysis
      .split('\n')
      .filter(
        line => line.includes('suggest') || line.includes('improve') || line.includes('consider')
      )
      .slice(0, 5);
    return suggestions;
  }
  ...
  private extractWeaknesses(analysis: string): string[] {
    return analysis
      .split('\n')
      .filter(line => line.toLowerCase().includes('weakness'))
      .slice(0, 3);
  }
  ...
  private calculateProfitabilityTrend(userHistory: any[]): string {
    ...
    const recentProfits = userHistory
      .slice(0, 10)
      .map(p => (p.result === 'won' ? p.units : -p.units));
