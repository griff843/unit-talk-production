...
export class ValidationService {
  ...
  /**
   * Validate business logic rules with professional-grade analysis
   */
  private validateBusinessLogic(
    ...
    // PROFESSIONAL KELLY CRITERION ANALYSIS
    const kellyAnalysis = this.calculateProfessionalKelly(pickData);

    if (kellyAnalysis.hasValue) {
      if (kellyAnalysis.edge > 0.05) { // 5%+ edge
        suggestions.push(`Strong value detected: ${(kellyAnalysis.edge * 100).toFixed(1)}% edge, Kelly suggests ${(kellyAnalysis.kellyFraction * 100).toFixed(1)}% of bankroll`);
      } else if (kellyAnalysis.edge > 0.02) { // 2%+ edge
        suggestions.push(`Moderate value: ${(kellyAnalysis.edge * 100).toFixed(1)}% edge detected`);
      }
    } else {
      if (kellyAnalysis.edge < -0.02) { // -2% edge
        warnings.push(`Negative expected value: ${(kellyAnalysis.edge * 100).toFixed(1)}% edge - consider avoiding`);
      } else {
        warnings.push('Marginal or no edge detected - ensure strong conviction');
      }
    }

    // STAKE SIZE VALIDATION AGAINST KELLY
    if (pickData.stake && typeof pickData.stake === 'number') {
      const recommendedStake = kellyAnalysis.kellyFraction * 1000; // Assuming $1000 bankroll
      const stakeRatio = pickData.stake / recommendedStake;

      if (stakeRatio > 2) {
        warnings.push(`Stake size (${pickData.stake}) significantly exceeds Kelly recommendation (${recommendedStake.toFixed(0)})`);
      } else if (stakeRatio < 0.5 && kellyAnalysis.hasValue) {
        suggestions.push(`Consider increasing stake - Kelly suggests ${recommendedStake.toFixed(0)} for optimal growth`);
      }
    }

    // CONFIDENCE CALIBRATION ANALYSIS
    const confidenceAnalysis = this.analyzeConfidenceCalibration(pickData);
    if (confidenceAnalysis.warning) {
      warnings.push(confidenceAnalysis.warning);
    }
    if (confidenceAnalysis.suggestion) {
      suggestions.push(confidenceAnalysis.suggestion);
    }

    // REASONING QUALITY VALIDATION
    const reasoningAnalysis = this.validateReasoningQuality(pickData.reasoning);
    if (reasoningAnalysis.errors.length > 0) {
      errors.push(...reasoningAnalysis.errors);
    }
    if (reasoningAnalysis.warnings.length > 0) {
      warnings.push(...reasoningAnalysis.warnings);
    }
    if (reasoningAnalysis.suggestions.length > 0) {
      suggestions.push(...reasoningAnalysis.suggestions);
    }

    // MARKET TIMING VALIDATION
    const timingAnalysis = this.validateMarketTiming(pickData);
    if (timingAnalysis.warning) {
      warnings.push(timingAnalysis.warning);
    }

    // Same game parlay warning (if applicable)
    if (
      pickData.bet_type &&
      pickData.bet_type.includes('parlay') &&
      pickData.selection.includes('same game')
    ) {
   ...
   */
  private validateEnhancedFormFields(
    formData: EnhancedTicketFormData,
    errors: ValidationError[]
  ): void {
    if (!formData.sport) {
  ...
  private validateTicketLegs(legs: any[], errors: ValidationError[], warnings: string[]): void {
    ...
    const duplicates = selections.filter(
      (selection, index) => selections.indexOf(selection) !== index
    );
  ...
  private validateParlayRules(
    ...
    const devigged_edge = this.calculateParlayEV(formData.legs);
    if (devigged_edge < -0.2) {
   ...
   */
  private validateImageAttachments(
    files: File[],
    errors: ValidationError[],
    warnings: string[]
  ): void {
  ...
  /**
   * Professional Kelly Criterion calculation with devigging
   */
  private calculateProfessionalKelly(pickData: PickData): {
    edge: number;
    ...
    const decimalOdds = pickData.odds > 0 ?
      (pickData.odds / 100) + 1 :
      (100 / Math.abs(pickData.odds)) + 1;

    // Convert confidence to probability with calibration
    // Professional cappers typically overestimate confidence
    const rawProbability = pickData.confidence / 10;
    const calibratedProbability = this.calibrateConfidenceToProbability(rawProbability, pickData.confidence);

    // Calculate implied probability (with vig)
    const impliedProbability = this.calculateImpliedProbability(pickData.odds);

    // Estimate fair probability (remove vig)
    // Assuming 4.5% vig on average
    const fairProbability = impliedProbability / 1.045;

    // Calculate edge
    const edge = calibratedProbability - fairProbability;

    // Kelly formula: f = (bp - q) / b
    const b = decimalOdds - 1; // Net odds
    const p = calibratedProbability;
    const q = 1 - p;

    let kellyFraction = (b * p - q) / b;

    // Apply fractional Kelly for risk management (25% Kelly)
    ...
    const expectedValue = (calibratedProbability * decimalOdds) - 1;
  ...
  /**
   * Calibrate confidence to actual probability based on historical data
   */
  private calibrateConfidenceToProbability(rawProbability: number, confidence: number): number {
    // Professional cappers tend to be overconfident
    // Apply calibration curve based on confidence level

    if (confidence >= 9) {
      // Very high confidence - typically overestimated
      return rawProbability * 0.85;
    } else if (confidence >= 7) {
      // High confidence - moderately overestimated
      return rawProbability * 0.90;
    } else if (confidence >= 5) {
      // Medium confidence - slightly overestimated
      return rawProbability * 0.95;
    } else {
      // Low confidence - often underestimated
      return rawProbability * 1.05;
    }
  }
   ...
   */
  private analyzeConfidenceCalibration(pickData: PickData): {
    warning?: string;
    suggestion?: string;
  } {
    const confidence = pickData.confidence;
    const impliedProbability = this.calculateImpliedProbability(pickData.odds);
    const impliedConfidence = impliedProbability * 10;

    const confidenceDiff = confidence - impliedConfidence;

    if (confidenceDiff > 3) {
      return {
        warning: `Confidence (${confidence}/10) significantly higher than market implies (${impliedConfidence.toFixed(1)}/10) - ensure conviction is justified`
      };
    } else if (confidenceDiff < -2) {
      return {
        suggestion: `Market confidence (${impliedConfidence.toFixed(1)}/10) higher than yours (${confidence}/10) - consider if you're being too conservative`
      };
    }
   ...
   */
  private validateReasoningQuality(reasoning?: string): {
    errors: ValidationError[];
    warnings: string[];
    suggestions: string[];
  } {
    ...
    if (!reasoning || reasoning.trim().length === 0) {
      errors.push({
        field: 'reasoning',
        message: 'Reasoning is required for all picks',
    ...
    if (reasoningLength < 20) {
      warnings.push('Reasoning is very brief - consider adding more detail for better tracking');
    } else if (reasoningLength < 50) {
      suggestions.push('Consider expanding reasoning with specific factors that influenced your decision');
    }

    // Quality indicators
    const hasNumbers = /\d/.test(reasoning);
    const hasStats = /\b(avg|average|%|percent|yards|points|games?|season)\b/i.test(reasoning);
    const hasComparison = /\b(vs|versus|against|compared|better|worse|higher|lower)\b/i.test(reasoning);
    const hasRecent = /\b(recent|last|past|this season|lately)\b/i.test(reasoning);

    let qualityScore = 0;
    if (hasNumbers) qualityScore++;
    if (hasStats) qualityScore++;
    if (hasComparison) qualityScore++;
    if (hasRecent) qualityScore++;

    if (qualityScore === 0) {
      warnings.push('Reasoning lacks specific data - consider including stats, trends, or comparisons');
    } else if (qualityScore === 1) {
      suggestions.push('Good start - consider adding more specific data points to strengthen reasoning');
    } else if (qualityScore >= 3) {
      suggestions.push('Excellent detailed reasoning - this will help with long-term analysis');
    }

    // Check for emotional language
    const emotionalWords = /\b(love|hate|feel|gut|instinct|definitely|obviously|clearly)\b/i;
    if (emotionalWords.test(reasoning)) {
      warnings.push('Reasoning contains emotional language - focus on objective factors');
    }
   ...
   */
  private validateMarketTiming(pickData: PickData): {
    warning?: string;
  } {
    // Check if odds are at extreme values (potential line movement)
    const odds = Math.abs(pickData.odds);

    if (odds > 300) {
      return {
        warning: 'High odds detected - ensure line hasn\'t moved significantly since analysis'
      };
    } else if (odds < 110) {
      return {
        warning: 'Heavy favorite detected - consider if juice is worth the risk'
      };
    }
  ...
  /**
   * Calculate confidence professional_score based on validation results
   */
  private calculateConfidenceScore(
    pickData: PickData | EnhancedTicketFormData,
    errors: ValidationError[],
    warnings: string[]
  ): number {
    let baseScore = 100;

    // Reduce professional_score for errors
    baseScore -= errors.length * 20;

    // Reduce professional_score for warnings
    ...
    const odds =
      'odds' in pickData ? pickData.odds : 'legs' in pickData && pickData.legs?.length ? 100 : 100;
