...
export class EVService {
  ...
  async getEVSummary(
    ...
    try {
      ...
      const summary: EVSummary = {
        ...
        averageEV:
          analyses.length > 0
            ? analyses.reduce((sum, a) => sum + a.evPercentage, 0) / analyses.length
            : 0,
      ...
      summary.evByTimeRange = Object.entries(dateGroups)
        .map(([date, picks]) => ({
          date,
  ...
  async getTopEVPicks(options: {
    ...
    switch (timeRange) {
      case 'today':
        startDate = new Date(getFullYear(now), getMonth(now), getDate(now)).toISOString();
        break;
      case 'week':
        startDate = new Date(new Date(now).getTime() - 7 * 24 * 60 * 60 * 1000).toISOString();
        break;
      case 'month':
        startDate = new Date(new Date(now).getTime() - 30 * 24 * 60 * 60 * 1000).toISOString();
        break;
    }

    return this.getEVAnalysis({
      startDate,
      endDate: typeof now === 'string' ? now : new Date(now).toISOString(),
      minEV,
      limit,
    });
  }
  ...
  private async calculateUserEVStats(discordId: string, picks: EVAnalysis[]): Promise<UserEVStats> {
    ...
    return {
      ...
      averageEV:
        picks.length > 0 ? picks.reduce((sum, p) => sum + p.evPercentage, 0) / picks.length : 0,
      totalExpectedProfit: picks.reduce((sum, p) => sum + p.expectedProfit, 0),
      winRate: userStats.winRate,
      actualProfit: userStats.totalProfit,
      roi:
        userStats.averageStake > 0
          ? (userStats.totalProfit / (userStats.averageStake * userStats.totalPicks)) * 100
          : 0,
    };
  }
  ...
  private getStartDateForRange(timeRange: 'today' | 'week' | 'month'): string {
    ...
    switch (timeRange) {
      case 'today':
        return new Date(getFullYear(now), getMonth(now), getDate(now)).toISOString();
      case 'week':
