...
export class AdminDashboardService {
  ...
  private async loadConfigFromDB(): Promise<void> {
    try {
      ...
      if (data) {
        ...
        await this.saveConfigToDB(defaultOnboardingConfig);
  ...
  /**
   * Save onboarding configuration to database
   */
  private async saveConfigToDB(config: any): Promise<void> {
    try {
      await this.supabaseService.client.from('onboarding_config').upsert({
        id: 'default',
        config: config,
  ...
  async showAdminDashboard(interaction: any): Promise<void> {
    try {
      // Check permissions
      if (
        !this.permissionsService.hasRole(interaction.member, 'admin') &&
    ...
    try {
      ...
      if (dmFailures && Array.isArray(dmFailures)) {
        dmFailureStats = {
          ...
          recentFailures: dmFailures.filter(
            (f: any) => Date.now() - new Date(f.attemptedAt).getTime() < 24 * 60 * 60 * 1000
          ).length,
        };
      }
  ...
  private createDashboardComponents(): ActionRowBuilder<ButtonBuilder>[] {
    const mainActions = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
    ...
    const utilityActions = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
  ...
  async updateOnboardingFlow(flowId: string, updates: any, editedBy: string): Promise<boolean> {
    try {
      ...
      // Save to database
      await this.saveConfigToDB(this.currentConfig);
  ...
  private async logFlowEdit(edit: OnboardingFlowEdit): Promise<void> {
    try {
      const { error } = await this.supabaseService.client.from('onboarding_flow_edits').insert({
        flow_id: edit.flowId,
        field: edit.field,
