...
describe('TrendAnalysisService', () => {
                      ...
                      not: jest.fn().mockReturnValue({
                        order: jest.fn().mockReturnValue({
                          ilike: jest.fn().mockResolvedValue({
                            data: mockPicks.filter(p => p.pick_type.includes('nba')),
                            error: null,
                          }),
                        }),
                      }),
  ...
  describe('trend break analysis', () => {
    it('should detect performance decline trend breaks', async () => {
      ...
      const declineBreak = result.trend_breaks.find(
        tb => tb.trend_break_type === 'performance_decline'
      );
    ...
    it('should detect performance surge trend breaks', async () => {
      ...
      expect(result.trend_breaks.length).toBeGreaterThan(0);
      const surgeBreak = result.trend_breaks.find(
        tb => tb.trend_break_type === 'performance_surge'
      );
  ...
  describe('statistical outlier analysis', () => {
    ...
    it('should calculate z-scores correctly', async () => {
      ...
      const expectedZScore =
        Math.abs(outlier.current_line - outlier.historical_average) / outlier.standard_deviation;
  ...
  describe('regression analysis', () => {
    it('should detect over-performance regression candidates', async () => {
      ...
      const overPerformer = result.regression_candidates.find(
        r => r.current_performance > r.expected_regression
      );
    ...
    it('should detect under-performance regression candidates', async () => {
      ...
      expect(result.regression_candidates.length).toBeGreaterThan(0);
      const underPerformer = result.regression_candidates.find(
        r => r.current_performance < r.expected_regression
      );
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should respect confidence thresholds', async () => {
      ...
      expect(highConfidenceResult.streaks.length).toBeLessThanOrEqual(
        lowConfidenceResult.streaks.length
      );
      expect(highConfidenceResult.trend_breaks.length).toBeLessThanOrEqual(
        lowConfidenceResult.trend_breaks.length
      );
    });
  });
});
...
function createMockPicksData(): PickData[] {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    picks.push({
      ...
      created_at: new Date(new Date(baseDate).getTime() + i * 24 * 60 * 60 * 1000).toISOString(),
