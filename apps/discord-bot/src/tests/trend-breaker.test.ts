...
(TrendAnalysisService as jest.MockedClass<typeof TrendAnalysisService>).mockImplementation(
  () => mockTrendService
);
...
describe('trend-breaker command', () => {
        ...
        embeds: [
          expect.objectContaining({
            data: {
              title: '🔒 VIP+ Exclusive',
              description:
                "This command is for VIP+ members only. Type `/vip-info` to see what you're missing.",
              color: 0xff0000,
            },
          }),
        ],
  ...
  describe('error handling', () => {
    ...
    it('should handle trend analysis service errors', async () => {
      mockTrendService.performTrendAnalysis = jest
        .fn()
    ...
    it('should handle getUserTier errors', async () => {
      mockGetUserTier.mockImplementation(() => {
...
function createLargeAnalysisDataset() {
  ...
  base.trend_breaks = Array.from({ length: 12 }, (_, i) => ({
    ...
    sample_size: 20 + i,
    trend_break_type: (i % 2 === 0 ? 'performance_decline' : 'performance_surge') as
      | 'performance_decline'
      | 'performance_surge',
