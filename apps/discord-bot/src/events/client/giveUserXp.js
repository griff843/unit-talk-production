...
module.exports = {
  ...
  async execute(message, client) {
    ...
    const mysqlData = await mysql.queryExecute(
      `SELECT * FROM Level WHERE Guild_id = ? AND User_id = ?`,
      [guild.id, author.id]
    );

    if (mysqlData.length === 0) {
      await mysql.queryExecute(`INSERT INTO Level VALUES (?, ?, ?, ?)`, [
        guild.id,
        author.id,
        0,
        0,
    ...
    if (mysqlData[0].XP + give >= requiredXp) {
      // Level up
      await mysql.queryExecute(
        `UPDATE Level SET XP = ?, Level = ? WHERE Guild_id = ? AND User_id = ?`,
        [mysqlData[0].XP + give, mysqlData[0].Level + 1, guild.id, author.id]
      );
      ...
      const levelRoles = await mysql.queryExecute(
        `SELECT * FROM LevelRoles WHERE Guild_id = ? AND Level = ?`,
        [guild.id, mysqlData[0].Level + 1]
      );
      ...
      const levelEmbed = new EmbedBuilder()
        .setColor('Green')
        .setDescription(
          `⚡ Congratulations <@${message.author.id}> , you have levelled up to **level ${mysqlData[0].Level + 1}**`
        );

      message.reply({ embeds: [levelEmbed] });
    } else {
      await mysql.queryExecute(`UPDATE Level SET XP = ? WHERE Guild_id = ? AND User_id = ?`, [
        mysqlData[0].XP + give,
        guild.id,
        author.id,
      ]);
