      ...
      async function InsertData(gameData) {
        for (const game of gameData) {
          ...
          await mysql.queryExecute(
            `INSERT INTO LiveSports (Id, Sport_key, Home_team, Away_team, BookMakers, Commence_time)
            VALUES (?, ?, ?, ?, ?, ?)`,
            [
              game.id,
              game.sport_key,
          ...
          cron.schedule(cronExpression, async () => {
            // 15 minutes before the commence time
            ...
            let usDate = DateTime.fromISO(game.commence_time, { zone: 'utc' }).toFormat(
              'MM/dd/yyyy'
            );
            ...
            const startEmbed = new EmbedBuilder()
              .setDescription(
                `Heads up! The thread for [Team ${game.home_team} vs Team ${game.away_team}] is now open.\nJoin the convo and drop your picks before tipoff!`
              )
              .setColor(`Blue`);
            Thread.send({ embeds: [startEmbed] });
            await mysql.queryExecute(`UPDATE LiveSports SET ThreadId = ? WHERE Id = ?`, [
              Thread.id,
              game.id,
            ]);
          });
          ...
          cron.schedule(cronExpression, async () => {
            ...
            const data = await mysql.queryExecute(`SELECT * FROM LiveSports WHERE Id = ?`, [
              game.id,
            ]);
            if (!data) return;
          ...
          cron.schedule(cronExpression, async () => {
            // After 3 hours from commence time
