...
async function handleSubmitPick(interaction: ChatInputCommandInteraction) {
  ...
  if (!isOwner) {
    ...
    if (!hasPermissions) {
      await interaction.reply({
        content:
          '❌ You need to be an approved capper to submit picks. Use `/capper-onboard` to get started.',
        ephemeral: true,
      });
      return;
    }
  ...
  const modal = new ModalBuilder().setCustomId('submit_pick_modal').setTitle('Submit Your Pick');
...
async function handleCapperStats(interaction: ChatInputCommandInteraction) {
  ...
  const member = interaction.member;
  const isOwner =
    member && typeof member === 'object' && 'roles' in member && member.roles instanceof Map
      ? Array.from(member.roles.values()).some((role: any) =>
          role?.name?.toLowerCase()?.includes('owner')
        )
      : member && member.roles && 'cache' in member.roles
  ...
  if (!isOwner) {
    ...
    if (!capper) {
      await interaction.reply({
        content:
          '❌ You need to be an approved capper to view stats. Use `/capper-onboard` to get started.',
        ephemeral: true,
      });
      return;
    }
  ...
  if (isOwner) {
    ...
    if (!capper) {
      ...
      await interaction.reply({
        content:
          '👑 **Owner Access**: You have full access to all capper features. No capper profile found - you can view all capper data through admin commands.',
        ephemeral: true,
      });
      return;
    }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `Statistics for **${capper.display_name || capper.username}**${isOwner ? ' (Owner Access)' : ''}`
    )
    .addFields(
      ...
      { name: 'Win Rate', value: `${(stats.winRate * 100).toFixed(1)}%`, inline: true },
      {
        name: 'Total Units',
        value:
          stats.totalUnits > 0 ? `+${stats.totalUnits.toFixed(1)}` : stats.totalUnits.toFixed(1),
        inline: true,
      },
      ...
      {
        name: 'Tier',
        value: capper.tier.charAt(0).toUpperCase() + capper.tier.slice(1),
        inline: true,
      },
      {
        name: 'Status',
        value: capper.status.charAt(0).toUpperCase() + capper.status.slice(1),
        inline: true,
      }
    )
