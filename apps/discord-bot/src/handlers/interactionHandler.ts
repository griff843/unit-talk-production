...
export class InteractionHandler {
  ...
  constructor(
    ...
    this.commandHandler = new CommandHandler(supabaseService);
  }
  ...
  private async handleButton(interaction: ButtonInteraction): Promise<void> {
    ...
    try {
      ...
      // Check if this is a personalized onboarding button (WHY discovery and personalized journey)
      const personalizedOnboardingPrefixes = [
        'why_', // WHY discovery buttons
        'personalized_', // Personalized journey buttons
        'trial_', // Trial onboarding buttons
        'admin_', // Admin onboarding buttons (different from admin action buttons)
        'capper_', // Capper onboarding buttons (different from capper action buttons)
        'action_', // Action buttons from personalized journeys
        'deep_dive_', // Deep dive buttons from Day 2
        'upgrade_', // Upgrade buttons from conversion flow
      ...
      if (personalizedOnboardingPrefixes.some(prefix => customId.startsWith(prefix))) {
        ...
        await onboardingHandler.handleButtonInteraction(interaction);
        return;
      }

      // AI onboarding integration removed as part of clean slate rebuild
      ...
      // Check if this is a Black Label button
      if (
        customId.startsWith('black_label_') ||
        customId === 'enhanced_analytics' ||
        customId === 'risk_assessment' ||
        customId === 'portfolio_impact' ||
        customId === 'refresh_dashboard' ||
        customId === 'export_report' ||
        customId === 'detailed_analytics' ||
        customId === 'market_alerts' ||
        customId === 'portfolio_details' ||
        customId === 'optimize_portfolio' ||
        customId === 'risk_analysis' ||
        customId === 'rebalance' ||
        customId === 'detailed_insights' ||
        customId === 'trend_analysis' ||
        customId === 'ai_predictions' ||
        customId === 'contact_vip_support' ||
        customId === 'black_label_info'
      ) {
        logger.info(`🖤 Routing to Black Label handler: ${customId}`);
        const { BlackLabelButtonHandler } = await import('./blackLabelButtonHandler');
        const blackLabelHandler = BlackLabelButtonHandler.getInstance();
        await blackLabelHandler.handleButton(interaction);
        return;
      }

      // Check if this is a tutorial button
      if (customId.startsWith('tutorial_')) {
        logger.info(`🎓 Routing to tutorial handler: ${customId}`);
        const { handleTutorialButton } = await import('../commands/interactive-tutorial');
      ...
      if (customId.startsWith('help_')) {
        logger.info(`❓ Routing to help handler: ${customId}`);
        const { handleHelpButton } = await import('../commands/enhanced-help');
      ...
      const legacyOnboardingButtons = [
        ...
        'view_trending_picks',
        'whats_new',
        'upgrade_for_more_wins',
        'upgrade_to_catch_up',
        'refresh_heat_signal',
        'heat_signal_settings',
        'heat_signal_demo',
        'vip_plus_tour_start',
        'vip_plus_settings',
        'vip_tour_start',
      ...
      switch (action) {
        ...
        default:
          ...
          await interaction.reply({
            content:
              '❌ This button is not yet implemented. Please contact support if you need assistance.',
            ephemeral: true,
          });
      }
    } catch (error) {
   ...
   */
  private async handleAnalyticsButton(
  ...
  private async handleAdminButton(interaction: ButtonInteraction, params: string[]): Promise<void> {
    ...
    switch (subAction) {
      case 'maintenance':
        await this.services.adminOverrideService.toggleMaintenanceMode(
          interaction.user.id,
          'Button toggle'
        );
        await interaction.reply({
      ...
      case 'health':
        const healthCheck = await this.services.adminOverrideService.performSystemHealthCheck(
          interaction.user.id
        );
        await interaction.reply({
   ...
   */
  private async handleConfigSelect(
   ...
   */
  private async handleAnalyticsSelect(
    interaction: StringSelectMenuInteraction,
    params: string[],
    values: string[]
  ): Promise<void> {
   ...
   */
  private async handleAdminSelect(
    interaction: StringSelectMenuInteraction,
    params: string[],
    values: string[]
  ): Promise<void> {
   ...
   */
  private async handleAdminModal(
    interaction: ModalSubmitInteraction,
    params: string[]
  ): Promise<void> {
    const [action] = params;
  ...
  private async handleAIModal(
    ...
    const response = await this.services.aiPoweredService.processCoachingMessage(
      sessionId,
      message
    );
