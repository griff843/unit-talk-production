...
export function EnhancedSubmitTicketForm({
  ...
  const [searchResults, setSearchResults] = useState<
    Record<number, { players: PlayerSearchResult[]; games: GameSearchResult[] }>
  >({});
  ...
  const currentSportConfig = useMemo(() => ENHANCED_SPORT_CONFIGS[watchedSport], [watchedSport]);

  // Dynamic field visibility based on sport and bet type
  const getVisibleFields = useCallback(
    (legIndex: number, betType: BetType) => {
  ...
  const handleEnhancedSearch = useCallback(
    async (legIndex: number, query: string, searchType: 'player' | 'game') => {
      ...
      try {
        if (searchType === 'player') {
          ...
          `
            ...
            .or(
              `home_team_info.name.ilike.%${query}%,away_team_info.name.ilike.%${query}%,matchup.ilike.%${query}%`
            )
            .eq('sport', watchedSport)
  ...
  const analyzePickWithAI = useCallback(
  ...
  const validateLegInRealTime = useCallback(
    async (legIndex: number) => {
      const leg = watchedLegs[legIndex];
      const config = currentSportConfig;
      ...
      setValidationErrors(prev => [...prev.filter(e => e.legIndex !== legIndex), ...errors]);
  ...
  const handleSelectPlayer = useCallback(
    async (legIndex: number, player: PlayerSearchResult) => {
      const leg = watchedLegs[legIndex];
      const updatedLeg = {
        ...leg,
  ...
  const handleSelectGame = useCallback(
    (legIndex: number, game: GameSearchResult) => {
      const leg = watchedLegs[legIndex];
      const updatedLeg = {
        ...leg,
      ...
      if (game.weather && ['NFL', 'MLB'].includes(watchedSport)) {
        if (game.weather.conditions.includes('rain') || game.weather.wind_speed > 15) {
          toast.info(
            `🌤️ Weather alert: ${game.weather.conditions}, Wind: ${game.weather.wind_speed}mph`
          );
        }
      }
    },
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {userTier === 'vip_plus' && <option value="AI Optimized">🤖 AI Optimized</option>}
            </select>
          </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Pick Details</h2>
                    ...
                    <div
                      className={`w-3 h-3 rounded-full ${
                        leg.validation_status === 'valid'
                          ? 'bg-green-500'
                          : leg.validation_status === 'invalid'
                            ? 'bg-red-500'
                            : 'bg-yellow-500'
                      }`}
                    />

                    {/* Edge professional_score for VIP+ users */}
                ...
                {userTier === 'vip_plus' && leg.ai_analysis && (
                  <div className="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-lg p-4">
                    ...
                    <div className="text-sm text-gray-700 dark:text-gray-300 space-y-1">
                      <p>
                        <strong>Recommendation:</strong> {leg.ai_analysis.recommendation}
                      </p>
                      <p>
                        <strong>Key Factors:</strong> {leg.ai_analysis.key_factors?.join(', ')}
                      </p>
                      <p>
                        <strong>Confidence:</strong> {leg.ai_analysis.confidence}%
                      </p>
                    </div>
                  </div>
                )}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Ticket Preview</h2>
        ...
        {/* Preview content */}
        <div className="space-y-4">{/* Implementation for ticket preview */}</div>
