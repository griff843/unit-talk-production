...
export function createUserProfileEmbed(profile: UserProfile): EmbedBuilder {
  const embed = new EmbedBuilder()
    ...
    .addFields({
      ...
      value: [
        ...
        `**Member Since:** <t:${Math.floor(new Date(profile.createdAt || Date.now()).getTime() / 1000)}:R>`,
        `**Last Active:** <t:${Math.floor(new Date(profile.last_active || profile.createdAt || Date.now()).getTime() / 1000)}:R>`,
      ].join('\n'),
      inline: true,
    });
  ...
  if (profile.trial_ends_at) {
    ...
    const hoursLeft = Math.max(
      0,
      Math.floor((new Date(trialEnd).getTime() - new Date(now).getTime()) / (1000 * 60 * 60))
    );

    embed.addFields({
      name: '⏰ Trial Status',
      value: `**Time Remaining:** ${hoursLeft} hours\n**Expires:** <t:${Math.floor(new Date(trialEnd).getTime() / 1000)}:R>`,
      inline: true,
    });
  }
...
export function createHelpEmbed(userTier: UserTier): EmbedBuilder {
  ...
  return embed.setFooter({ text: 'Need more help? Contact our support team!' }).setTimestamp();
}
...
export function createTrialStatusEmbed(hoursRemaining: number): EmbedBuilder {
  const embed = new EmbedBuilder().setTitle('⏰ Trial Status').setColor(COLORS.trial);
 ...
 */
export function createFreeWelcomeEmbed(username: string): {
  embed: EmbedBuilder;
...
export function createTrialWelcomeEmbed(username: string): {
  embed: EmbedBuilder;
  buttons: ActionRowBuilder<ButtonBuilder>;
} {
  const embed = new EmbedBuilder()
...
export function createTrialReminderEmbed(hoursRemaining: number): EmbedBuilder {
  const embed = new EmbedBuilder().setTitle('⏰ Trial Reminder').setColor(COLORS.warning);
...
export function createReEngagementEmbed(username: string, daysSinceActive: number): EmbedBuilder {
  return new EmbedBuilder()
    ...
    .setColor(COLORS.info)
    .setDescription(
      `It's been ${daysSinceActive} days since your last visit. Here's what you've missed:`
    )
    .addFields(
      {
 ...
 */
export function createMissedValueEmbed(
  username: string,
  missedWins: number,
  missedProfit: number
): EmbedBuilder {
...
export function createUserStatsEmbed(profile: UserProfile, stats?: any): EmbedBuilder {
  ...
  embed.setFooter({ text: 'Stats updated in real-time' }).setTimestamp();
...
export function createStaffWelcomeEmbed(username: string): EmbedBuilder {
  return new EmbedBuilder()
    ...
    .setDescription(
      'Welcome to the Unit Talk staff team! You have access to all moderation and management tools.'
    )
    .addFields(
      {
        name: '🛠️ Staff Tools',
...
export function createAdminWelcomeEmbed(username: string): EmbedBuilder {
  return new EmbedBuilder()
    ...
    .setDescription(
      'Welcome to the Unit Talk administration team! You have full system access and control.'
    )
    .addFields(
      {
        name: '🔧 Admin Powers',
...
export function createOwnerWelcomeEmbed(username: string): EmbedBuilder {
  return new EmbedBuilder()
    ...
    .setDescription(
      'Welcome back, boss! You have ultimate control over the entire Unit Talk platform.'
    )
    .addFields(
      {
