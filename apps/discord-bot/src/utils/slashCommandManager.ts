...
export class SlashCommandManager {
  ...
  public async autoRegisterCommands(): Promise<CommandRegistrationResult> {
    ...
    try {
      ...
      const { toRegister, toUpdate, toDelete } = this.compareCommands(
        discoveredCommands,
        currentCommands
      );
      ...
      result.unchanged = currentCommands
        .filter(
          cmd => !toUpdate.some(u => u.name === cmd.name) && !toDelete.includes((cmd as any).id)
        )
        .map(cmd => cmd.name);
      ...
      logger.info(
        `Command registration complete. Registered: ${result.registered.length}, Failed: ${result.failed.length}, Unchanged: ${result.unchanged.length}`
      );
  ...
  private extractCommandDefinition(module: any): CommandDefinition | null {
    ...
    const candidates = [module.default, module.command, module.data, module];
  ...
  private async registerSingleCommand(command: CommandDefinition): Promise<void> {
    await this.rest.post(Routes.applicationCommands(this.client.user!.id), { body: command });
  }
