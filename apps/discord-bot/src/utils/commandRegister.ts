  ...
  try {
    ...
    if (commands.length > 0) {
      ...
      if (botConfig.discord!.guildId) {
        ...
        await rest.put(Routes.applicationCommands(botConfig.discord!.clientId), { body: commands });
  ...
  return [
    ...
    },
    {
      ...
      data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
          option.setName('pick').setDescription('Your pick (e.g., "Chiefs -3.5")').setRequired(true)
        )
        ...
        .addStringOption(option =>
          option.setName('book').setDescription('Sportsbook').setRequired(true)
        )
        .addStringOption(option =>
