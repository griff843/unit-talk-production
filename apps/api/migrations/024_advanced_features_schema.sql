-- Migration 024: Advanced Features Schema
...
-- ============================================================================
-- 1. PLAYERS TABLE (Foundation for injury analysis)
-- ============================================================================
CREATE TABLE players (
    ...
    external_id VARCHAR(200), -- For third-party player IDs
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT now(),

    -- Ensure uniqueness per sport
    UNIQUE(name, sport, team)
);

-- Create indexes for performance
CREATE INDEX idx_players_name_sport ON players(name, sport);
...
-- ============================================================================
-- 2. STEAM_MOVES TABLE (Enhanced steam detection tracking)
-- ============================================================================
CREATE TABLE steam_moves (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    prop_id UUID NOT NULL REFERENCES raw_props(id) ON DELETE CASCADE,
...
-- ============================================================================
-- 3. BEST_ODDS TABLE (Line shopping optimizer)
-- ============================================================================
...
-- ============================================================================
-- 4. INJURY_IMPACTS TABLE (Enhanced injury analysis)
-- ============================================================================
...
-- ============================================================================
-- 5. ARBITRAGE_OPPORTUNITIES TABLE (Arbitrage scanner)
-- ============================================================================
CREATE TABLE arbitrage_opportunities (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    prop_ids UUID[] NOT NULL, -- Array of prop IDs involved
...
-- ============================================================================
-- 6. ADD PLAYER REFERENCE TO RAW_PROPS (if needed)
-- ============================================================================
-- Add optional player_id to raw_props for direct player prop tracking
ALTER TABLE raw_props
ADD COLUMN player_id UUID REFERENCES players(id) ON DELETE SET NULL;

-- Create index for player prop queries
CREATE INDEX idx_raw_props_player_id ON raw_props(player_id) WHERE player_id IS NOT NULL;
...
COMMENT ON TABLE players IS 'Player master table for injury analysis and player prop tracking';
...
COMMENT ON COLUMN arbitrage_opportunities.prop_ids IS 'Array of prop UUIDs involved in arbitrage';
COMMENT ON COLUMN arbitrage_opportunities.combined_odds IS 'JSON object with odds from each book';

-- ============================================================================
-- 9. GRANT PERMISSIONS (if using RLS or specific users)
-- ============================================================================

-- Enable RLS on new tables if needed
-- ALTER TABLE players ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE steam_moves ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE best_odds ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE injury_impacts ENABLE ROW LEVEL SECURITY;
