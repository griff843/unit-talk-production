...
// Use same config as the API
const supabaseUrl = process.env.SUPABASE_URL || 'https://default.supabase.co';
const supabaseKey = process.env.SUPABASE_ANON_KEY || 'default-key';
...
async function testScoringPipeline() {
  ...
  try {
    ...
    // 6. Get recent props count
    console.log('🔢 Counting total unprocessed props...');
    const { count: totalUnprocessed, error: countError } = await supabase
      .from('raw_props')
      .select('id', { count: 'exact' })
      .is('professional_score', null);

    if (!countError) {
      console.log(`📊 Total unprocessed props: ${totalUnprocessed?.toLocaleString() || 0}`);
    }
    ...
    console.log(`📊 Unprocessed props: ${totalUnprocessed?.toLocaleString() || 'Unknown'} total`);
    console.log('🚀 ScoringAgent pipeline is ready for processing!');

    console.log('\n📋 NEXT STEPS:');
    console.log('1. Environment variables: USE_PRO_SCORER=true ✅');
    console.log('2. Environment variables: USE_ENHANCED_45_FACTOR=true ✅');
    console.log('3. Database schema: All columns added ✅');
    console.log('4. Ready to run ScoringAgent to process props!');
