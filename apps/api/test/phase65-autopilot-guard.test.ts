...
jest.mock('../src/services/supabaseClient', () => ({
  supabase: {
    from: jest.fn(() => ({
      insert: jest.fn(() => ({
        select: jest.fn(() => ({
          single: jest.fn(() => Promise.resolve({ data: { id: 'test-evidence-id' }, error: null }))
        }))
      }))
    }))
  }
...
// Mock shadowMode
jest.mock('../src/shadow/ShadowMode', () => ({
  shadowMode: {
...
import { AutopilotGuard, SideEffectContext, GuardResult, AutopilotMode } from '../src/lib/AutopilotGuard';
...
describe('Phase 6.5: AutopilotGuard Integration Points', () => {
  describe('Service Integration Verification', () => {
    it('should have AutopilotGuard available for DiscordPromotionAgent', async () => {
      const { autopilotGuard } = await import('../src/lib/AutopilotGuard');
