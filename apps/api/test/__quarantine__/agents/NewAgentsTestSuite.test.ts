...
describe('New Agents Test Suite', () => {
  ...
  describe('AutomatedOnboardingAgent', () => {
    ...
    test('should generate conversation responses', async () => {
      ...
      generateResponseSpy.mockResolvedValue({
        content:
          "Hello! I'd be happy to help you with betting. What specific questions do you have?",
        intent: 'help_request',
        confidence: 0.9,
      });
    ...
    test('should cleanup resources properly', async () => {
      ...
      expect(mockDeps.logger.info).toHaveBeenCalledWith(expect.stringContaining('cleanup'));
    });
  });
  ...
  describe('RiskManagementAgent', () => {
    ...
    test('should optimize portfolio correctly', async () => {
      ...
      const mockPortfolio = {
        positions: [
          ...
          { symbol: 'ML_HOME', allocation: 0.3, expectedReturn: 0.1, volatility: 0.18 },
        ],
        totalValue: 10000,
        riskTolerance: 0.25,
      };
    ...
    test('should calculate position sizes using Kelly Criterion', async () => {
      ...
      expect(positionSize).toBeLessThanOrEqual(
        mockPosition.bankroll * mockPosition.maxPositionSize
      );
    });
