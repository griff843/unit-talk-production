...
import {
  ...
  clearRateLimitCache,
  clearPropTypeCache,
} from '../../../src/agents/FeedAgent/optimal';
...
describe('Optimal API Integration', () => {
  ...
  describe('fetchOptimalProps', () => {
    ...
    it('should handle API errors', async () => {
      ...
      await expect(fetchOptimalProps('NBA', '2024-01-15')).rejects.toThrow(
    ...
    it('should handle missing API key', async () => {
      ...
      await expect(fetchOptimalProps('NBA', '2024-01-15')).rejects.toThrow(
        'OPTIMAL_API_KEY environment variable is required'
      );
  ...
  describe('Deduplication', () => {
    it('should remove duplicate props', async () => {
      const mockPropTypesResponse = {
        prop_types: [{ api_key: 'points', display_name: 'Points', abbreviation: 'PTS' }],
      };
