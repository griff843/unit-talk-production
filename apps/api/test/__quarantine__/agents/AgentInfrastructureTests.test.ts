...
describe('Agent Infrastructure Integration Tests', () => {
  ...
  describe('Redis Cache Integration Tests', () => {
    test('should handle Redis cache operations correctly', async () => {
      ...
      (redisCache.getPattern as jest.Mock).mockResolvedValue(
        new Map([
          ['key1', 'value1'],
          ['key2', 'value2'],
        ])
    ...
    test('should handle Redis connection failures gracefully', async () => {
      ...
      expect(mockDeps.logger.warn).toHaveBeenCalledWith(expect.stringContaining('Redis'));
    });
  ...
  describe('Circuit Breaker Pattern Tests', () => {
    ...
    test('should handle circuit breaker recovery correctly', async () => {
      ...
      // Initial failures
      await expect(agent.callWithCircuitBreaker('recovery_service', serviceMock)).rejects.toThrow(
  ...
  describe('Health Check Integration Tests', () => {
    ...
    test('should implement health check timeouts', async () => {
      ...
      healthCheckWithTimeout.mockImplementation(async (timeoutMs = 5000) => {
        ...
        try {
          return await Promise.race([agent.performHealthCheck(), timeoutPromise]);
        } catch (error) {
          return {
            status: 'timeout',
            timestamp: new Date(),
  ...
  describe('Error Handling and Resilience Tests', () => {
    test('should implement exponential backoff for retries', async () => {
      ...
      retryWithBackoffSpy.mockImplementation(
        async (operation: () => Promise<any>, maxAttempts = 3) => {
