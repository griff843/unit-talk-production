...
Provide a brief analysis of the matchup and the pick's value.`;
    ...
    it('should handle analysis failure gracefully', async () => {
      ...
      try {
        ...
        const { error } = await supabase.from('pick_analysis').insert({
          pick_id: testPick.id,
...
Average Odds: ${calculateAverageOdds(picks)}`;
      ...
      // Store insights
      const { error } = await supabase.from('capper_insights').insert({
...
${content}`,
  ...
  describe('AI Model Management', () => {
    it('should track model performance', async () => {
      ...
      // Store analyses
      const { error } = await supabase.from('model_performance').insert([
        {
          pick_id: testPick.id,
          model: 'gpt-4',
          response_time: 500,
    ...
    it('should select optimal model', async () => {
      ...
      const { data: performance } = await supabase.from('model_performance').select();

      // Calculate average response times
      const avgTimes = performance?.reduce(
        (acc, curr) => {
