...
describe('AI Response Cache Tests', () => {
  ...
  describe('Cache Hit/Miss Logic', () => {
    ...
    test('should trigger MISS when odds move beyond threshold', () => {
      ...
      const initialKey = aiResponseCache.generateAdviceCacheKey(
        'test-prop-odds',
        initialOdds,
        28.5,
        context
      );
      const movedKey = aiResponseCache.generateAdviceCacheKey(
        'test-prop-odds',
        movedOdds,
        28.5,
        context
      ...
      // Check requery logic
      const shouldRequery = aiResponseCache.shouldRequeryAdvice(
        'test-prop-odds',
        movedOdds,
        28.5,
        context
    ...
    test('should trigger MISS when context changes', () => {
      ...
      const changedKey = aiResponseCache.generateAdviceCacheKey(
        'test-prop-context',
        -110,
        32.5,
        changedContext
  ...
  describe('Cache Metrics and Analysis', () => {
    ...
    test('should identify popular cache entries', () => {
      ...
      expect(popularEntries[0].entry.hitCount).toBeGreaterThanOrEqual(
        popularEntries[1].entry.hitCount
      );
      expect(popularEntries[1].entry.hitCount).toBeGreaterThanOrEqual(
        popularEntries[2].entry.hitCount
      );
    });
  });
