...
describe('Supabase Mock', () => {
  ...
  describe('Basic CRUD Operations', () => {
    it('should handle insert operations', async () => {
      ...
      const result = await supabase.from<TestUser>('users').insert(newUser).execute();
    ...
    it('should handle delete operations', async () => {
      const result = await supabase.from<TestUser>('users').delete().eq('id', 'user-1').execute();
  ...
  describe('Query Filters', () => {
    it('should handle eq filter', async () => {
      const result = await supabase.from<TestUser>('users').select().eq('role', 'admin').execute();
    ...
    it('should handle neq filter', async () => {
      const result = await supabase.from<TestUser>('users').select().neq('role', 'admin').execute();
  ...
  describe('Storage Operations', () => {
    it('should handle storage operations', async () => {
      const uploadResult = await supabase.storage.from('bucket').upload('test.txt', 'test data');

      expect(uploadResult.error).toBeNull();
      expect(uploadResult.data).toBeTruthy();

      const downloadResult = await supabase.storage.from('bucket').download('test.txt');
