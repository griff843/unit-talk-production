...
describe('Advanced Feature Usage E2E', () => {
  ...
  describe('Advanced Analytics', () => {
    ...
    it('should view advanced metrics', async () => {
      ...
      const metrics = ['roi-by-sport', 'win-rate-by-odds', 'units-by-bet-type', 'profit-by-day'];
  ...
  describe('API Access', () => {
    it('should manage API keys', async () => {
      ...
      // Verify key created
      const { data: keys } = await supabase.from('api_keys').select().eq('user_id', testMember.id);
    ...
    it('should view API usage', async () => {
      ...
      // View usage metrics
      const metrics = ['total-requests', 'requests-by-endpoint', 'average-latency', 'error-rate'];
