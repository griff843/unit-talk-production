...
describe('promotionPolicy', () => {
  ...
  describe('findCriticalDataGaps', () => {
    ...
    it('detects fallback on core group features', () => {
      const audit = {
        expectedValue: { present: false, fallbackUsed: true, fallbackPolicy: 'neutral', rawValue: undefined },
        matchupRating: { present: true, fallbackUsed: false, fallbackPolicy: 'neutral', rawValue: 80 },
      };
    ...
    it('detects fallback on market group features', () => {
      const audit = {
        marketIntelligence: { present: false, fallbackUsed: true, fallbackPolicy: 'neutral', rawValue: undefined },
        sharpMoney: { present: false, fallbackUsed: true, fallbackPolicy: 'neutral', rawValue: undefined },
      };
      const gaps = findCriticalDataGaps(audit);
    ...
    it('ignores fallback on non-critical groups', () => {
      const audit = {
        playerFatigue: { present: false, fallbackUsed: true, fallbackPolicy: 'neutral', rawValue: undefined },
        correlationRisk: { present: false, fallbackUsed: true, fallbackPolicy: 'neutral', rawValue: undefined },
      };
      expect(findCriticalDataGaps(audit)).toEqual([]);
    });
  });
  ...
  describe('classifyBand', () => {
    ...
    it('NONE band when critical data gaps exist', () => {
      const result = makeV2Result({
        ...
        ev: 10,
        feature_audit: {
          expectedValue: { present: false, fallbackUsed: true, fallbackPolicy: 'neutral', rawValue: undefined as any },
          matchupRating: { present: true, fallbackUsed: false, fallbackPolicy: 'neutral', rawValue: 80 },
        },
      });
