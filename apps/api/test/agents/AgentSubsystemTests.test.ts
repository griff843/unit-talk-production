...
describe('Agent Subsystem Specialized Tests', () => {
    ...
    test('should handle engagement analysis with multiple dimensions', async () => {
      const mockUserActivity = {
        ...
        activities: [
          ...
          { type: 'bet_placed', timestamp: new Date(), metadata: { amount: 50, sport: 'football' } },
          { type: 'message_sent', timestamp: new Date(), metadata: { channel: 'general', length: 100 } },
  ...
  describe('RiskManagementAgent - Portfolio Optimization', () => {
    ...
    test('should perform Monte Carlo risk simulations', async () => {
      const mockPortfolio = {
        positions: [
          { symbol: 'OVER_45.5', allocation: 0.3, expectedReturn: 0.08, volatility: 0.15, correlation: 0.2 },
          { symbol: 'UNDER_52.5', allocation: 0.4, expectedReturn: 0.06, volatility: 0.12, correlation: -0.1 },
          { symbol: 'ML_HOME', allocation: 0.3, expectedReturn: 0.10, volatility: 0.18, correlation: 0.3 }
        ],
        totalValue: 50000,
        timeHorizon: 30
      };
    ...
    test('should optimize portfolio using Black-Litterman model', async () => {
      const mockMarketData = {
        ...
        expectedReturns: [0.08, 0.06, 0.10],
        covarianceMatrix: [
          [0.0225, 0.002, 0.004],
          [0.002, 0.0144, -0.001],
          [0.004, -0.001, 0.0324]
  ...
  describe('PredictiveAnalyticsAgent - ML Model Management', () => {
    ...
    test('should perform automated model training and evaluation', async () => {
      const mockTrainingData = {
        features: [
          { gameId: 'game-1', homeTeam: 'LAL', awayTeam: 'BOS', spread: -3.5, total: 215.5, weather: 'clear' },
          { gameId: 'game-2', homeTeam: 'GSW', awayTeam: 'MIA', spread: -7.0, total: 220.0, weather: 'indoor' }
        ],
        targets: [
    ...
    test('should detect market anomalies and opportunities', async () => {
      const mockMarketData = [
        { marketId: 'market-1', odds: 1.85, volume: 50000, timestamp: new Date(), line_movement: 0.05 },
        { marketId: 'market-2', odds: 3.20, volume: 15000, timestamp: new Date(), line_movement: -0.15 },
        { marketId: 'market-3', odds: 1.45, volume: 80000, timestamp: new Date(), line_movement: 0.02 }
      ];
  ...
  describe('PerformanceOptimizationAgent - System Optimization', () => {
    ...
    test('should perform intelligent bottleneck analysis', async () => {
      ...
      const bottleneckAnalysisSpy = jest.spyOn(agent as any, 'performIntelligentBottleneckAnalysis');
    ...
    test('should execute automated optimization with safety validation', async () => {
      ...
      const safeOptimizationSpy = jest.spyOn(agent as any, 'executeOptimizationWithSafetyValidation');
  ...
  describe('Cross-Agent Data Flow Tests', () => {
    test('should handle agent-to-agent data pipeline', async () => {
      ...
      const marketPredictionSpy = jest.spyOn(predictiveAgent as any, 'generatePersonalizedPredictions');
      ...
      const predictions = await predictiveAgent.generatePersonalizedPredictions(userProfile, riskAssessment);
