...
import { sendDiscordAlert, sendBatchDiscordAlerts } from '../../src/agents/AlertAgent/integrations/discord';
...
describe('Shadow Mode System Tests', () => {
  ...
  describe('PublishGuard Shadow Integration', () => {
    it('should route promotion decisions to shadow mode when enabled', async () => {
      ...
      const mockShadowWritePick = jest.spyOn(shadowModeService, 'shadowWritePick').mockResolvedValue();
    ...
    it('should handle alert decisions in shadow mode', async () => {
      ...
      const mockShadowPublishPreview = jest.spyOn(shadowModeService, 'shadowPublishPreview').mockResolvedValue();
  ...
  describe('Shadow Mode Configuration Validation', () => {
    ...
    it('should validate shadow mode database operations', async () => {
      ...
      await expect(service.shadowWritePick({
        id: 'test',
        propId: 'test',
        tier: 'A',
        confidence: 0.8,
        ...
        line: 20.5,
        sport: 'NBA'
      }, 'instant', ['test'])).resolves.not.toThrow();
    });
  });
  ...
  describe('Performance and Memory Tests', () => {
    it('should not leak memory with repeated shadow operations', async () => {
      ...
      const promises = [];
      for (let i = 0; i < 100; i++) {
        promises.push(service.shadowWritePick({
          id: `test-${i}`,
          propId: `prop-${i}`,
          tier: 'A',
          confidence: 0.8,
