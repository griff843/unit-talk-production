...
describe('Zone Threat Rating Integration', () => {
  ...
  describe('Score Impact Analysis', () => {
    test('should demonstrate meaningful professional_score improvement', () => {
      ...
      // Extreme prop should professional_score higher due to Zone Threat boost
      expect(extremeResult.professional_score).toBeGreaterThanOrEqual(baseResult.professional_score);
      if (extremeResult.breakdown['zone_threat_boost']) {
        expect(extremeResult.professional_score - baseResult.professional_score).toBe(2); // Boost amount
      }
    });
    ...
    test('should potentially change tier with boost', () => {
      ...
      expect(result.professional_score).toBeGreaterThanOrEqual(2); // At least the boost
  ...
  describe('Real-world Scenario Simulation', () => {
    test('should handle typical game scenario', () => {
      ...
      expect(result.professional_score).toBeGreaterThan(5); // Should be decent professional_score
    ...
    test('should handle edge case scenarios', () => {
      ...
      edgeCases.forEach((prop, index) => {
        ...
        // Should not crash
        expect(result.professional_score).toBeGreaterThanOrEqual(0);
        expect(internalDetails.zoneThreatAnalysis?.eligible).toBe(true);
