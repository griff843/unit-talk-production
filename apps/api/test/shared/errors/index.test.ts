...
import { 
  AgentError,
  NetworkError,
} from '../../../src/shared/errors/agent-errors';
import {
  RetryableError,
  ErrorHandler,
  isAgentError,
  isRetryableError,
  isNetworkError,
} from '../error-handler-mocks';

describe('Error Classes', () => {
  describe('AgentError', () => {
    it('should create an AgentError with correct properties', () => {
      const error = new AgentError('Test error', { agentName: 'test-agent', operation: 'test-op' });
    ...
    it('should create an AgentError with details', () => {
      ...
      const error = new AgentError('Test error', { agentName: 'test-agent', operation: 'test-op', details });
      expect(error.message).toBe('Test error');
      expect(error.agentName).toBe('test-agent');
      expect(error.details).toEqual({ userId: '123', action: 'test' });
    });
  });
