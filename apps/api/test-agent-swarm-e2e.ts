#!/usr/bin/env node
/**
 * E2E Test for Agent Swarm Pipeline Orchestration
...
async function validatePipelineFlow(): Promise<void> {
  ...
  try {
    ...
    for (const prop of rawProps.slice(0, 20)) {  // Process first 20 for validation
    ...
    console.log(`\n📈 Enhanced45FactorEngine processed ${processedProps.length}/${rawProps.slice(0, 20).length} props for promotion\n`);
    ...
    console.log(`✅ Pipeline validation complete! ${finalPicks?.length || 0} total picks in unified_picks`);

    if (finalPicks && finalPicks.length > 0) {
      console.log('\n📊 Latest Pipeline Results:');
      finalPicks.slice(0, 5).forEach((pick: any, index: number) => {
        console.log(`   ${index + 1}. ${pick.sport} ${pick.prop_type} - ${pick.tier}-tier (Score: ${pick.score})`);
      });
    }
    ...
    console.log(`   • Promotion Rate: ${((promotedCount / rawProps.slice(0, 20).length) * 100).toFixed(1)}%`);
