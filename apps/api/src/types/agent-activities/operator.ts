...
export interface OperatorAgentActivities {
  ...
  checkApiEndpoints(params: BaseParams & { endpoints: string[] }): Promise<ActivityResult<any[]>>;

  getWorkflowMetrics(params: BaseParams & { timeWindow: number }): Promise<ActivityResult<any>>;
  ...
  cleanupOldMetrics(params: BaseParams & { retentionDays: number }): Promise<MaintenanceResult>;

  cleanupOldRawProps(params: BaseParams & { retentionDays: number }): Promise<MaintenanceResult>;
