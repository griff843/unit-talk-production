...
import { getMlbRosters, convertMlbRosterPlayer } from '../agents/enrichment/mlbEnrichment';
import { getNbaRosters, convertNbaRosterPlayer } from '../agents/enrichment/nbaEnrichment';
import { getNflRosters, convertNflRosterPlayer } from '../agents/enrichment/nflEnrichment';
import { getNhlRosters, convertNhlRosterPlayer } from '../agents/enrichment/nhlEnrichment';
...
async function getExistingPlayers(league: string): Promise<Set<string>> {
  const { data, error } = await supabase.from('players').select('player_name').eq('sport', league);
...
async function discoverLeaguePlayers(
  ...
  try {
    ...
    // Find new players
    const newPlayers = rosterPlayers.filter(
      player => !existingPlayerNames.has(player.player_name.toLowerCase())
    );
    ...
    if (newPlayers.length > 0) {
      ...
      logger.info(
        `${dryRun ? 'Would add' : 'Added'} ${summary.addedPlayers} new ${league} players`
      );
    } else {
...
async function main() {
  ...
  try {
    if (all) {
      ...
      console.error(
        'Usage: npx ts-node -r dotenv/config src/scripts/discoverPlayers.ts [LEAGUE] [OPTIONS]'
      );
      console.error('');
