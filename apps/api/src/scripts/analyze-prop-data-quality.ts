...
async function analyzePropDataQuality() {
  ...
  try {
    ...
    if (sportData) {
      const sportQuality = sportData.reduce(
        (acc, prop) => {
          if (!acc[prop.sport]) {
    ...
    if (recentProps) {
      ...
      recentProps.forEach((prop, index) => {
        ...
        console.log(
          `  ${index + 1}. ${prop.sport} | ${prop.player_name} | Gradable: ${gradable ? '✅' : '❌'}`
        );
      });
    }
    ...
    if (enhancedCount === 0) {
      ...
      console.log(
        'The database lacks expected_value and sharp_money data required for sophisticated scoring.'
      );
      console.log('');
      console.log('Options:');
