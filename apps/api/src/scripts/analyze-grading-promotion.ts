...
async function analyzeGradingPromotion() {
  ...
  try {
    ...
    if (promotionStats?.length) {
      ...
      const edgeScores = promotionStats.map(p => p.edge_score).filter(Boolean).sort((a, b) => b - a);
    ...
    const { data: recentlyGraded } = await supabaseClient
      .from('raw_props')
      .select('id, sport, player_name, stat_type, edge_score, tier, promoted, created_at, updated_at')
      .not('edge_score', 'is', null)
    ...
    if (recentlyGraded?.length) {
      recentlyGraded.forEach(prop => {
        ...
        console.log(`    Created: ${prop.created_at?.split('T')[0]}, Updated: ${prop.updated_at?.split('T')[0] || 'Never'}`);
      });
    } else {
    ...
    const { data: highQualityProps } = await supabaseClient
      .from('raw_props')
      .select('id, sport, player_name, stat_type, edge_score, tier, promoted, promoted_to_picks, is_promoted')
    ...
    if (highQualityProps?.length) {
      ...
      highQualityProps.forEach(prop => {
        ...
        console.log(`    Flags: promoted=${prop.promoted}, promoted_to_picks=${prop.promoted_to_picks}, is_promoted=${prop.is_promoted}`);
      });
    } else {
    ...
    if (allGradedProps?.length) {
      ...
      const promotableByTier = allGradedProps.filter(p => ['S', 'A'].includes(p.tier) && !p.promoted).length;
    ...
    if (promotionStats?.length) {
      const unpromoted = promotionStats.filter(p => !p.promoted && !p.is_promoted && !p.promoted_to_picks).length;
