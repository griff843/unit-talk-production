...
function main() {
  ...
  for (const pick of PICKS) {
    ...
    else if (highestGate === 'steam-hunter')
      promotionLane = 'hold'; // steam requires additional data
  ...
  for (const gate of V2_GATES) {
    ...
    log(
      `    ${gate.gateId}: ${count}/${scoredPicks.length} (${((count / scoredPicks.length) * 100).toFixed(1)}%)`
    );
  }
  log('');
  ...
  const promoted = scoredPicks.filter(
    p => p.promotionLane === 'instant' || p.promotionLane === '10am'
  );
  log('  Promoted Picks (shadow — would be published if shadow mode OFF):');
  for (const p of promoted) {
    log(
      `    ${p.id} ${p.player.padEnd(25)} ${p.score.toFixed(1)} ${p.tier} ${p.promotionLane.padEnd(8)} gate:${p.highestGate}`
    );
  }
  log('');
  ...
  log(
    `  ${'Sport'.padEnd(8)} | ${'Win'.padStart(4)} | ${'Loss'.padStart(4)} | ${'Push'.padStart(4)} | ${'Void'.padStart(4)} | ${'Win%'.padStart(6)}`
  );
  log('  ' + '-'.repeat(50));

  const sportResults: Record<string, Record<string, number>> = {};
  for (const p of scoredPicks) {
    if (!sportResults[p.sport])
      sportResults[p.sport] = { win: 0, loss: 0, push: 0, void: 0, total: 0 };
  ...
  for (const [sport, counts] of Object.entries(sportResults).sort()) {
    ...
    log(
      `  ${sport.padEnd(8)} | ${String(counts.win).padStart(4)} | ${String(counts.loss).padStart(4)} | ${String(counts.push).padStart(4)} | ${String(counts.void || 0).padStart(4)} | ${winRate.padStart(5)}%`
    );
  }
  log('');
  ...
  const promotedPicks = scoredPicks.filter(
    p => p.promotionLane === 'instant' || p.promotionLane === '10am'
  );
  const rejectedPicks = scoredPicks.filter(
    p => p.promotionLane === 'reject' || p.promotionLane === 'hold'
  );
  ...
  log('  ' + '-'.repeat(65));
  log(
    `  ${'Cohort'.padEnd(12)} | ${'Count'.padStart(5)} | ${'Wins'.padStart(5)} | ${'Settled'.padStart(7)} | ${'Win Rate'.padStart(8)} | ${'Avg Score'.padStart(9)}`
  );
  log('  ' + '-'.repeat(65));

  const promotedAvgScore =
    promotedPicks.length > 0
      ? (promotedPicks.reduce((s, p) => s + p.score, 0) / promotedPicks.length).toFixed(1)
      : '0.0';
  const rejectedAvgScore =
    rejectedPicks.length > 0
      ? (rejectedPicks.reduce((s, p) => s + p.score, 0) / rejectedPicks.length).toFixed(1)
      : '0.0';

  const promotedWinRate =
    promotedSettled > 0 ? ((promotedWins / promotedSettled) * 100).toFixed(1) : '0.0';
  const rejectedWinRate =
    rejectedSettled > 0 ? ((rejectedWins / rejectedSettled) * 100).toFixed(1) : '0.0';

  log(
    `  ${'Promoted'.padEnd(12)} | ${String(promotedPicks.length).padStart(5)} | ${String(promotedWins).padStart(5)} | ${String(promotedSettled).padStart(7)} | ${promotedWinRate.padStart(7)}% | ${promotedAvgScore.padStart(9)}`
  );
  log(
    `  ${'Rejected'.padEnd(12)} | ${String(rejectedPicks.length).padStart(5)} | ${String(rejectedWins).padStart(5)} | ${String(rejectedSettled).padStart(7)} | ${rejectedWinRate.padStart(7)}% | ${rejectedAvgScore.padStart(9)}`
  );
  log('');

  const winRateDelta = Number(promotedWinRate) - Number(rejectedWinRate);
  log(
    `  Win rate delta (promoted - rejected): ${winRateDelta > 0 ? '+' : ''}${winRateDelta.toFixed(1)}%`
  );
  log(
    `  Average score delta: ${(Number(promotedAvgScore) - Number(rejectedAvgScore)).toFixed(1)} points`
  );
  log('');
  ...
  for (const gate of V2_GATES) {
    ...
    log(
      `    ${gate.gateId}: ${gatePicks.length} picks, ${gateWins}W/${gateSettled} settled (${gateWinRate}%)`
    );
  }
  log('');
  ...
  log(
    `  ${'ID'.padEnd(8)} | ${'Player'.padEnd(22)} | ${'Score'.padStart(6)} | ${'Tier'.padStart(4)} | ${'Lane'.padEnd(8)} | ${'Result'.padEnd(6)}`
  );
  log('  ' + '-'.repeat(70));
  for (const p of scoredPicks) {
    log(
      `  ${p.id.padEnd(8)} | ${p.player.padEnd(22)} | ${p.score.toFixed(1).padStart(6)} | ${p.tier.padStart(4)} | ${p.promotionLane.padEnd(8)} | ${p.simulatedResult.padEnd(6)}`
    );
  }
  log('');
  ...
  log(
    `    3. Promoted picks (avg score ${promotedAvgScore}) vs rejected (avg score ${rejectedAvgScore})`
  );
  ...
  const proofData = {
    ...
    day1_scoring: {
      ...
      gate_pass_rates: Object.fromEntries(
        V2_GATES.map(g => [
          g.gateId,
          {
            count: gateCounts[g.gateId] || 0,
            rate: `${(((gateCounts[g.gateId] || 0) / scoredPicks.length) * 100).toFixed(1)}%`,
          },
        ])
      ),
      lane_distribution: laneCounts,
      promoted_picks: promoted.map(p => ({
        id: p.id,
        player: p.player,
        score: p.score,
        tier: p.tier,
        lane: p.promotionLane,
        gate: p.highestGate,
      })),
    },
    day2_settlement: {
    ...
    day3_metrics: {
      promoted_cohort: {
        count: promotedPicks.length,
        wins: promotedWins,
        settled: promotedSettled,
        win_rate: promotedWinRate,
        avg_score: promotedAvgScore,
      },
      rejected_cohort: {
        count: rejectedPicks.length,
        wins: rejectedWins,
        settled: rejectedSettled,
        win_rate: rejectedWinRate,
        avg_score: rejectedAvgScore,
      },
  ...
  fs.writeFileSync(path.join(outDir, 'shadow_run_proof.json'), JSON.stringify(proofData, null, 2));
