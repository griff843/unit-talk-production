...
async function diagnoseGradingSystem() {
  ...
  try {
    ...
    console.log('\n📊 SCENARIO ANALYSIS:');
    console.log(
      `  Bull Case: ${result.scenarioAnalysis.bullCase.score.toFixed(1)} (${(result.scenarioAnalysis.bullCase.probability * 100).toFixed(1)}%)`
    );
    console.log(
      `  Base Case: ${result.scenarioAnalysis.baseCase.score.toFixed(1)} (${(result.scenarioAnalysis.baseCase.probability * 100).toFixed(1)}%)`
    );
    console.log(
      `  Bear Case: ${result.scenarioAnalysis.bearCase.score.toFixed(1)} (${(result.scenarioAnalysis.bearCase.probability * 100).toFixed(1)}%)`
    );
    ...
    results.forEach(r => {
      console.log(
        `  ${r.id}: ${r.sport} - ${r.player} | Score: ${r.finalScore.toFixed(1)} | Edge: ${r.edgeScore.toFixed(1)} | Tier: ${r.tier} | Conf: ${(r.confidence * 100).toFixed(1)}%`
      );
    });
    ...
    console.log('\n📊 VARIETY ANALYSIS:');
    console.log(
      `Unique Final Scores: ${uniqueScores.size} (${Array.from(uniqueScores).join(', ')})`
    );
