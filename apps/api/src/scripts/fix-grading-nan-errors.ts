...
async function fixGradingNaNErrors() {
  ...
  try {
    ...
    const { data: testProp } = await supabaseClient
      .from('raw_props')
      .select('*')
      .limit(1)
      .single();
    ...
    console.log(`Market Intelligence: ${features.marketIntelligence} (${typeof features.marketIntelligence})`);
...
function convertToFeatureSet(rawProp: any): GradingFeatureSet {
  ...
  return {
    ...
    sharpMoney: parseFloat(rawProp.sharp_money) || parseFloat(rawProp.sharp_betting_percentage) || 50,
