...
async function checkCurrentGradingScores() {
  ...
  try {
    ...
    if (edgeScoreStats) {
      ...
      // Group by sport
      const sportBreakdown = edgeScoreStats.reduce(
        (acc, prop) => {
      ...
      for (const [sport, scores] of Object.entries(sportBreakdown)) {
        ...
        console.log(
          `  ${sport}: ${scores.length} props, avg: ${avgScore.toFixed(2)}, unique scores: ${uniqueScores.length}`
        );
    ...
    if (recentProps) {
      ...
      recentProps.forEach(prop => {
        console.log(
          `  ${prop.sport} | ${prop.player_name} | ${prop.stat_type} | Score: ${prop.edge_score} | Tier: ${prop.tier}`
        );
      });
    }
    ...
    if (invalidScores) {
      ...
      invalidScores.forEach(prop => {
        console.log(
          `  ${prop.sport} | ${prop.player_name} | Score: ${prop.edge_score} | Tier: ${prop.tier}`
        );
      });
    }
    ...
    if (tierStats) {
      const tierBreakdown = tierStats.reduce(
        (acc, prop) => {
    ...
    if (promotableProps) {
      ...
      const sportEdgeBreakdown = promotableProps.reduce(
        (acc, prop) => {
      ...
      for (const [sport, data] of Object.entries(sportEdgeBreakdown)) {
        ...
        console.log(
          `    Tiers: ${Object.entries(data.tiers)
    ...
    const { data: testProps } = await supabaseClient.from('raw_props').select('*').limit(3);
