...
const V2_GATES: GateConfig[] = [
  {
    ...
    name: 'Instant S-Tier Gate (V2)',
    minProfessionalScore: 62,
    minTier: 'A',
  },
  {
    gateId: '10am-premium',
    name: '10am Premium Release (V2)',
    minProfessionalScore: 58,
    minTier: 'A',
  },
  {
    gateId: 'steam-hunter',
    name: 'Steam Detection Gate (V2)',
    minProfessionalScore: 55,
    minTier: 'A',
  },
];
...
function main() {
  ...
  for (const gate of V1_GATES) {
    ...
    log(
      `    Score pass: ${scorePass}/${results.length} (${((scorePass / results.length) * 100).toFixed(1)}%)`
    );
    log(
      `    Tier pass:  ${tierPass}/${results.length} (${((tierPass / results.length) * 100).toFixed(1)}%)`
    );
    log(
      `    Combined:   ${combinedPass}/${results.length} (${((combinedPass / results.length) * 100).toFixed(1)}%)`
    );
    log('');
  }
  ...
  log(
    `  instant-s-tier: ${instantRate.toFixed(1)}% → ${instantInBand ? '✅ IN BAND' : '⚠️  CHECK'} (target 5-15%)`
  );
  log(
    `  10am-premium:   ${premiumRate.toFixed(1)}% → ${premiumInBand ? '✅ IN BAND' : '⚠️  CHECK'} (target 15-30%)`
  );
  log(
    `  steam-hunter:   ${steamRate.toFixed(1)}% score+tier pass → ${steamInBand ? '✅ WITHIN RANGE' : '⚠️  CHECK'} (further filtered by steam)`
  );
  log('');
  ...
  for (const variant of controlVariants) {
    for (const val of variant.values) {
      const passed = results.filter(
        r => r.score >= val && tierRank(r.tier) >= tierRank('A')
      ).length;
      ...
      log(
        `  ${variant.envVar.padEnd(32)}| ${String(val).padEnd(8)}| ${rate.padStart(5)}% ${isDefault ? '← DEFAULT' : ''}`
      );
    }
    log('');
  }
  ...
  log(
    '    - Env var overrides: V2_GATE_INSTANT_S_TIER, V2_GATE_10AM_PREMIUM, V2_GATE_STEAM_HUNTER'
  );
  ...
  log(
    `  [${allControllable ? 'CONFIRMED' : 'FAILED'}] Pass rates are controllable via env var overrides`
  );
  ...
  log('');
  log(
    `  STAGE 1 STATUS: ${allNonZero && allControllable ? 'PASS' : 'NOT PROVEN'} (proceed to Stage 2)`
  );
  log('');
  ...
  const proofData = {
    ...
    v2_gates: {
      ...
      results: Object.fromEntries(
        Object.entries(v2GateResults).map(([k, v]) => [
          k,
          {
  ...
  fs.writeFileSync(path.join(outDir, 'gate_recal_proof.json'), JSON.stringify(proofData, null, 2));
