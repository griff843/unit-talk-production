...
class ProfessionalBackfillProcessor {
  ...
  async processProp(rawProp: any): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      if (this.options.dryRun) {
        logger.info(
          `[DRY RUN] Would process prop: ${rawProp.id} (${rawProp.player_name} ${rawProp.stat_type})`
        );
        return { success: true };
      }
      ...
      logger.debug(
        `Successfully processed prop ${rawProp.id} (score: ${result.finalScore}) in ${processingTime}ms`
      );
      return { success: true };
    } catch (error) {
  ...
  async processBatch(props: any[]): Promise<void> {
    ...
    const batchResults = await Promise.allSettled(props.map(prop => this.processProp(prop)));
