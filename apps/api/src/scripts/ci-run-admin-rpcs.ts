...
if (!SUPABASE_URL || !SUPA@ASE_SERVICE_ROLE_KEY) {
...
async function runBackfill(days: number = 3): Promise<void> {
  console.log(`\n 👄 Running admin_backfill_market_props(${days})...`);
  ...
  if (result.total_market_props < 1000) {
    console.warn(`⚠️  Warning: Only ${result.total_market_props} market_props (target: 1000+)`);
  }
}

async function runScoring(): Promise<void> {
  console.log(`\n 🎈 Running scoring loop...`);
  ...
  while (iteration <= 20) {  // Max 20 iterations to prevent infinite loop
    ...
    if (scored === 0 || remaining === 0) {
      console.log(`\n✅ Scoring complete! Total scored: ${totalScored}`);
      break;
    }
  ...
  if (iteration > 20) {
    console.warn(`⚠️  Warning: Reached max iterations (20)`);
  }
}
...
async function main() {
  ...
  try {
    ...
    console.log('\n' + '='.repeat(60));
    console.log('✅ All admin RPCs completed successfully!');
    console.log('='.repeat(60));
    console.log('');
    ...
    console.error('\n' + '='.repeat(60));
    console.error('❭ Admin RPC execution failed!');
