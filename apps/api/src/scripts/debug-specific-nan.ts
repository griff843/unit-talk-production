...
function testCalculateCoreScore(features: GradingFeatureSet, weights: any): number {
  ...
  console.log(
    `    EV Value: ${evValue}, EV Score: ${evScore}, Weight: ${safeWeight(weights, 'expectedValue', 0.3)}, Contribution: ${evContribution}`
  );
  ...
  console.log(
    `    Line Value: ${lineValue}, Line Score: ${lineScore}, Contribution: ${lineContribution}`
  );

  // Other components...
  const matchupValue = safeNumber(features.matchupRating, 50);
  const matchupScore = matchupValue / 10;
  const matchupContribution = safeMultiply(
    matchupScore,
    safeWeight(weights, 'matchupRating', 0.25)
  );
  professional_score += matchupContribution;
  console.log(
    `    Matchup Rating: ${matchupValue}, Score: ${matchupScore}, Contribution: ${matchupContribution}`
  );
...
function testCalculateMarketIntelligenceScore(features: GradingFeatureSet, weights: any): number {
  ...
  const marketContribution = safeMultiply(
    marketScore,
    safeWeight(weights, 'marketIntelligence', 0.3)
  );
  professional_score += marketContribution;
  console.log(
    `    Market Intelligence: ${marketValue}, Score: ${marketScore}, Contribution: ${marketContribution}`
  );
  ...
  console.log(
    `    Sharp Money: ${sharpValue}, Score: ${sharpScore}, Contribution: ${sharpContribution}`
  );
