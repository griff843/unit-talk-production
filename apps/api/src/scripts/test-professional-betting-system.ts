...
class ProfessionalBettingSystemTester {
  ...
  private async runDeviggingTests(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Two-way market devigging', async () => {
    ...
    tests.push(await this.runTest('Multi-way market devigging', async () => {
      const market = {
        option1: { odds: 200 },
        option2: { odds: 300 },
        option3: { odds: 400 }
    ...
    tests.push(await this.runTest('Edge calculation accuracy', async () => {
    ...
    tests.push(await this.runTest('Devigging performance', async () => {
      ...
      return { markets: 100, duration: `${duration.toFixed(1)}ms`, avgPerMarket: `${(duration/100).toFixed(2)}ms` };
    }));
  ...
  private async runCLVTrackingTests(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Pick tracking', async () => {
    ...
    tests.push(await this.runTest('CLV calculation', async () => {
    ...
    tests.push(await this.runTest('Stats aggregation', async () => {
  ...
  private async runFeedbackLoopTests(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Feedback loop execution', async () => {
      try {
        const results = await this.feedbackLoopService.runFeedbackLoop();
        
        // Verify structure
        if (!results.weightAdjustments || !results.bookAdjustments || !results.marketAdjustments) {
    ...
    tests.push(await this.runTest('Optimization history', async () => {
  ...
  private async runAlertSystemTests(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Threshold configuration', async () => {
  ...
  private async runSchedulerTests(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Scheduler lifecycle', async () => {
      // Test start
    ...
    tests.push(await this.runTest('Manual task triggers', async () => {
  ...
  private async runIntegrationTests(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('E2E professional flow', async () => {
      // 1. Devig odds
    ...
    tests.push(await this.runTest('Database consistency', async () => {
  ...
  private async runPerformanceTests(): Promise<TestSuite> {
    ...
    tests.push(await this.runTest('Devigging performance', async () => {
      ...
      return { operations: 1000, totalTime: `${duration.toFixed(1)}ms`, avgPerOp: `${avgPerOp.toFixed(3)}ms` };
    }));

    // Test 2: CLV tracking performance
    tests.push(await this.runTest('CLV tracking performance', async () => {
      const operations = 10;
      const start = performance.now();
      ...
      return { operations, totalTime: `${duration.toFixed(1)}ms`, avgPerOp: `${avgPerOp.toFixed(1)}ms` };
    }));
  ...
  private generateFinalReport(testSuites: TestSuite[]): void {
    ...
    testSuites.forEach(suite => {
      ...
      console.log(`${icon} ${suite.name}: ${passedCount}/${suite.tests.length} (${(suite.totalDuration / 1000).toFixed(2)}s)`);
    });
