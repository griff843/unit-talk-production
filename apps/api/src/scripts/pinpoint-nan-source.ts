...
async function pinpointNaNSource() {
  ...
  try {
    ...
    const calculateCompositeScore = (gradingEngine as any).calculateCompositeScore.bind(
      gradingEngine
    );
    ...
    const calculateProfessionalInsights = (gradingEngine as any).calculateProfessionalInsights.bind(
      gradingEngine
    );
    ...
    const calculateFeatureContributions = (gradingEngine as any).calculateFeatureContributions.bind(
      gradingEngine
    );
    ...
    const determineTierAndConfidence = (gradingEngine as any).determineTierAndConfidence.bind(
      gradingEngine
    );
    ...
    const calculateKellyFraction = (gradingEngine as any).calculateKellyFraction.bind(
      gradingEngine
    );
    ...
    const rawPositionSize = await riskManager.calculatePositionSize(
      kellyFraction,
    ...
    if (tierResult.tier === 'S' && positionSize < 0.05) {
      positionSize = 0.05;
    }
    if (tierResult.tier === 'A' && positionSize < 0.03) {
      positionSize = 0.03;
    }
    if (tierResult.tier === 'B' && positionSize < 0.015) {
      positionSize = 0.015;
    }
    ...
    console.log('\n🔍 FINAL ANALYSIS:');
    console.log(
      `Final Score: ${finalResult.finalScore} ${isNaN(finalResult.finalScore) ? '❌ NaN' : '✅ Valid'}`
    );
    console.log(
      `Edge Score: ${finalResult.edgeScore} ${isNaN(finalResult.edgeScore) ? '❌ NaN' : '✅ Valid'}`
    );
    console.log(
      `Confidence: ${finalResult.confidence} ${isNaN(finalResult.confidence) ? '❌ NaN' : '✅ Valid'}`
    );
    console.log(
      `Kelly Fraction: ${finalResult.kellyFraction} ${isNaN(finalResult.kellyFraction) ? '❌ NaN' : '✅ Valid'}`
    );
    console.log(
      `Position Size: ${finalResult.positionSize} ${isNaN(finalResult.positionSize) ? '❌ NaN' : '✅ Valid'}`
    );
    ...
    console.log(
      `Actual Final Score: ${actualResult.finalScore} ${isNaN(actualResult.finalScore) ? '❌ NaN' : '✅ Valid'}`
    );
    ...
    if (!isNaN(manualFinalScore) && isNaN(actualFinalScore)) {
      ...
      console.log(
        '   This suggests there is validation or processing logic in gradeProp that we missed'
      );
