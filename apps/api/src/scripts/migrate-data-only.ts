      ...
      try {
        ...
        if (insertError) {
          if (insertError.message.includes('duplicate key') || insertError.message.includes('already exists')) {
            console.log('  ℹ️ Records already exist in historical table, proceeding with deletion...');
          } else {
        ...
        const progressPercent = ((totalMigrated / Math.min(oldProps || 0, maxBatches * BATCH_SIZE)) * 100).toFixed(1);
