  ...
  try {
    ...
    for (const { name, prop } of testProps) {
      ...
      try {
        ...
        console.log(
          `    Neural Network: professional_score=${nnResult.score}, confidence=${nnResult.confidence}`
        );

        const gbResult = await mlManager.scoreWithGradientBoosting(features);
        console.log(
          `    Gradient Boosting: professional_score=${gbResult.score}, confidence=${gbResult.confidence}`
        );

        const rfResult = await mlManager.scoreWithRandomForest(features);
        console.log(
          `    Random Forest: professional_score=${rfResult.score}, confidence=${rfResult.confidence}`
        );

        const ensembleResult = await mlManager.scoreWithEnsemble(features);
        console.log(
          `    Ensemble: professional_score=${ensembleResult.score}, confidence=${ensembleResult.confidence}, agreement=${ensembleResult.agreement}`
        );
        ...
        const hasMLNaN =
          isNaN(mlPredictions.neuralNetwork) ||
        ...
        const calculateMarketIntelligenceScore = (
          gradingEngine as any
        ).calculateMarketIntelligenceScore.bind(gradingEngine);
        const calculateMLScore = (gradingEngine as any).calculateMLScore.bind(gradingEngine);
        const calculateContextScore = (gradingEngine as any).calculateContextScore.bind(
          gradingEngine
        );
        const calculateRiskScore = (gradingEngine as any).calculateRiskScore.bind(gradingEngine);
        const calculateProfessionalCapperScore = (
          gradingEngine as any
        ).calculateProfessionalCapperScore?.bind(gradingEngine);
        ...
        console.log(
          `    Professional Contribution: ${professionalScore} * 1.2 = ${professionalContribution}`
        );
