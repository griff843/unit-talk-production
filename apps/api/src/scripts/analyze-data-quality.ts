      ...
      Object.entries(sportAnalysis.mismatches).forEach(([sport, issues]) => {
        ...
        issues.forEach((issue, index) => {
          if (index < 5) {
            ...
            console.log(
              `    - Player: "${issue.player}" | Stat: "${issue.stat}" | League: "${issue.league}"`
            );
          }
        });
        if (issues.length > 5) {
...
function analyzeSportIssues(data: any[]): { mismatches: Record<string, any[]> } {
  ...
  data.forEach(row => {
    ...
    if (
      (sport === 'nba' || sport === 'nhl') &&
      (player.includes('Total') || stat.includes('Team'))
    ) {
      isMismatch = true;
    }

    // Generic mismatch indicators
    if (sport === 'nfl' && stat?.includes('Goals')) {
      // Hockey stat in NFL
      isMismatch = true;
    }
...
function analyzePlayerVsTeamProps(data: any[]): {
  ...
  data.forEach(row => {
    ...
    const hasTeamKeyword = teamKeywords.some(
      keyword => playerName.includes(keyword) || statType.includes(keyword)
    );

    const hasPlayerKeyword = playerKeywords.some(keyword => statType.includes(keyword));
