...
export class VIPPlusChannelService {
        ...
        .addFields(
          {
            ...
            value: picks.slice(0, 3).map((pick, index) => 
   ...
   */
  async postPerPeriodUpdate(gameId: string, threadId: string, updateData: any, correlationId: string): Promise<void> {
   ...
   */
  async handleUserCoachingRequest(userId: string, userPicks: any[], correlationId: string): Promise<void> {
   ...
   */
  async postGameTrivia(gameId: string, threadId: string, triviaData: any, correlationId: string): Promise<void> {
  ...
  private getRiskLevel(riskFactors: string[]): string {
    ...
    if (riskCount <= 1) {return 'Low Risk';}
    if (riskCount <= 2) {return 'Low-Medium Risk';}
    if (riskCount <= 3) {return 'Medium Risk';}
  ...
  private async storeTriviaQuestion(gameId: string, threadId: string, triviaData: any): Promise<void> {
  ...
  async postLiveGameUpdate(gameData: any, correlationId: string): Promise<void> {
    ...
    try {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: '📊 Live Lines', value: `Spread: ${gameData.liveSpread}\nTotal: ${gameData.liveTotal}`, inline: true },
          { name: '🎯 Opening Lines', value: `Spread: ${gameData.openingSpread}\nTotal: ${gameData.openingTotal}`, inline: true },
    ...
    try {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '📊 Current Performance', value: `Win Rate: ${coachingData.winRate}\nROI: ${coachingData.roi}`, inline: true }
        )
        .setColor(0x00AA55)
        .setTimestamp()
