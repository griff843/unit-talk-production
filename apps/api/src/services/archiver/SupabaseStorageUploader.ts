...
export class SupabaseStorageUploader {
   ...
   */
  private async resumableUpload(params: {
    filePath: string;
    bucketName: string;
    objectPath: string;
    ...
    const parts: Array<{ partNumber: number; path: string; size: number }> = [];
   ...
   */
  private async cleanupPartialUploads(bucketName: string, partPaths: string[]): Promise<void> {
    try {
  ...
  async verifyUpload(params: {
    ...
    try {
      ...
      const fileInfo = fileList?.find((file: any) => file.name === fileName);
