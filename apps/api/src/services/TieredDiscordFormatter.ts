...
export class TieredDiscordFormatter {
   ...
   */
  private static createFreeEmbed(alertData: AlertData, alertType: AlertType): EmbedBuilder {
   ...
   */
  private static createBlackLabelEmbed(alertData: AlertData, alertType: AlertType): EmbedBuilder {
   ...
   */
  private static getVIPTierEmoji(tier?: string): string {
    const emojis = {
      'S-tier': '⭐', // Star for VIP
      'A-tier': '💎',
      'B-tier': '🔥',
  ...
  private static getVIPPlusTierEmoji(tier?: string): string {
    const emojis = {
