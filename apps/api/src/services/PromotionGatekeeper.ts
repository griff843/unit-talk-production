...
class PromotionGatekeeper {
  ...
  private async evaluateGate(pick: PickForPromotion, gate: PromotionGate): Promise<GateResult> {
    ...
    const results: Array<{ test: string; passed: boolean; score: number; threshold: number; weight: number }> = [];
  ...
  private makePromotionDecision(
    ...
    gateResults: GateResult[],
    highestPriorityGate: PromotionGate | null,
    riskScore: number,
    blockerCount: number
  ): PromotionDecision {
   ...
   */
  private getGateImpact(gate: PromotionGate, failedTestCount: number): 'blocking' | 'warning' | 'informational' {
