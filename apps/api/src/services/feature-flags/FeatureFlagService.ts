...
export class FeatureFlagService {
  private logger = createLogger('FeatureFlagService');
  private supabase: SupabaseClient;
  private flagConfigs: Map<string, FeatureFlagConfig> = new Map();
  private metricsBuffer: Map<string, RolloutMetrics> = new Map();
  
  // Performance monitoring
  private evaluationCache: Map<string, { result: FeatureFlagEvaluation; expiry: number }> = new Map();
    ...
    try {
      ...
      if (flagConfig.dependencies && flagConfig.dependencies.length > 0) {
        const dependencyResults = await Promise.all(
          flagConfig.dependencies.map(dep => this.evaluateFlag(dep, context))
        );
   ...
   */
  async updateFlag(flagName: string, updates: Partial<FeatureFlagConfig>): Promise<void> {
    try {
   ...
   */
  async graduateRollout(flagName: string, targetPercentage: number, options: {
    ...
    try {
      ...
      for (const [index, stepPercentage] of Array.from(steps.entries())) {
   ...
   */
  async emergencyRollback(flagName: string, reason: string): Promise<void> {
    try {
   ...
   */
  async getRolloutMetrics(flagName: string): Promise<RolloutMetrics | null> {
    try {
  ...
  private async storeFlagConfig(config: FeatureFlagConfig): Promise<void> {
  ...
  private calculateRolloutBucket(userId: string, flagName: string): number {
  ...
  private checkSegmentMatch(config: FeatureFlagConfig, userSegment?: string): boolean {
  ...
  private assignABTestGroup(groups: ABTestGroup[], userId: string, flagName: string): ABTestGroup {
  ...
  private createDefaultEvaluation(flagName: string, enabled: boolean, rolloutBucket?: number): FeatureFlagEvaluation {
    return {
      flagName,
      enabled,
      segmentMatch: true,
  ...
  private getCacheKey(flagName: string, context: any): string {
    return `${flagName}-${JSON.stringify(context)}`;
  }

  private clearFlagCache(flagName: string): void {
    for (const [key] of Array.from(this.evaluationCache)) {
  ...
  private calculateRolloutSteps(current: number, target: number, step: number): number[] {
    const steps: number[] = [];
  ...
  private async validateRolloutMetrics(flagName: string): Promise<boolean> {
    ...
    for (const gate of config.validationGates) {
      const metricValue = (metrics as any)[gate.metric];
  ...
  private evaluateValidationGate(value: number, gate: ValidationGate): boolean {
  ...
  private async rollbackToPercentage(flagName: string, percentage: number): Promise<void> {
    await this.updateFlag(flagName, { rolloutPercentage: percentage });
  }

  private async validateFlagUpdate(config: FeatureFlagConfig): Promise<void> {
  ...
  private async emitFlagChangeEvent(flagName: string, oldConfig: FeatureFlagConfig, newConfig: FeatureFlagConfig): Promise<void> {
    try {
      await this.supabase
        .from('events')
        .insert({
  ...
  private async emitEmergencyEvent(flagName: string, reason: string): Promise<void> {
    try {
      await this.supabase
        .from('events')
        .insert({
  ...
  private calculateConfigChanges(oldConfig: FeatureFlagConfig, newConfig: FeatureFlagConfig): Record<string, any> {
  ...
  private cleanExpiredCache(): void {
    const now = Date.now();
    for (const [key, cached] of Array.from(this.evaluationCache)) {
