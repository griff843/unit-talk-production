...
class PortfolioRiskManager {
  ...
  private readonly LIMITS: PortfolioLimits = {
    ...
    maxGameExposure: 0.4, // 40% max single game
    maxPlayerExposure: 0.3, // 30% max single player

    maxCorrelationThreshold: 0.7, // 70% max correlation
    maxCorrelatedPositions: 3, // Max 3 correlated
    maxSameGamePositions: 4, // Max 4 per game
    maxSamePlayerPositions: 2, // Max 2 per player

    maxSportConcentration: 0.6, // 60% max sport concentration
    maxTierConcentration: 0.8, // 80% max S-tier
    maxBookmakerExposure: 0.5, // 50% max single book

    dailyVARLimit: 0.15, // 15% daily VaR limit
    weeklyDrawdownLimit: 0.25, // 25% weekly drawdown
    monthlyVolatilityLimit: 0.3, // 30% monthly volatility
  };
  ...
  private async analyzeCorrelations(pick: any, portfolio: any[]): Promise<CorrelationAnalysis> {
    ...
    const averageCorrelation =
      correlatedPositions.length > 0 ? totalCorrelation / correlatedPositions.length : 0;

    const correlationRisk = this.classifyCorrelationRisk(
      maxCorrelation,
      correlatedPositions.length
    );
  ...
  private determineCorrelationType(
    pick1: any,
    pick2: any
  ): 'same_game' | 'same_player' | 'statistical' | 'market_driven' {
    if (pick1.game_id === pick2.game_id) {
    ...
    if (
      pick1.steam_strength &&
      pick2.steam_strength &&
   ...
   */
  private async calculateRiskImpact(
    pick: any,
    size: number,
    portfolio: any[]
    ...
    for (const position of portfolio) {
      const correlation = await this.calculateCorrelation(pick, position);
      correlationImpact +=
        correlation * size * position.kelly_fraction * (position.variance || 0.2);
    }

    const newTotalRisk = Math.sqrt(
      Math.pow(currentRisk.totalRisk, 2) + Math.pow(positionRisk, 2) + 2 * correlationImpact
    );
  ...
  private async determinePositionSize(
    ...
    if (correlationAnalysis.maxCorrelation > this.LIMITS.maxCorrelationThreshold) {
      const correlationAdjustment =
        1 - (correlationAnalysis.maxCorrelation - this.LIMITS.maxCorrelationThreshold);
    ...
    if (gameExposure > this.LIMITS.maxGameExposure) {
      const gameAdjustedSize =
        (this.LIMITS.maxGameExposure - this.getCurrentGameExposure(pick, portfolio)) / 1;
  ...
  private calculateDiversificationScore(correlatedPositions: CorrelatedPosition[]): number {
    ...
    const avgCorrelation =
      correlatedPositions.reduce((sum, pos) => sum + Math.abs(pos.correlation), 0) /
      correlatedPositions.length;
  ...
  private calculateCurrentConcentrationRisk(portfolio: any[]): number {
    const sportExposures = portfolio.reduce(
      (acc, pick) => {
  ...
  private classifyOverallRisk(
    ...
    if (var95 > 0.1 || totalRisk > 0.2) return 'medium';
    return 'low';
  }
  ...
  private async storeRiskAssessment(
    ...
    try {
      await supabaseClient.from('portfolio_risk_assessments').insert({
        pick_id: pick.id,
