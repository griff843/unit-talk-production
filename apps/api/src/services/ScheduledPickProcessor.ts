...
export class ScheduledPickProcessor {
    ...
    try {
      ...
      for (const pick of scheduledPicks) {
        try {
          ...
          results.failed++;
          results.errors.push(
            `${(pick as any).capper}: ${error instanceof Error ? error.message : String(error)}`
          );
  ...
  private async markPickAsPosted(pickId: string, messageId: string): Promise<void> {
    // SPRINT-RUNTIME-TRUTH-008: Use posted_to_discord flag instead of status
    const result = await lifecycleUpdate(
      ...
      { writerRole: 'poster', traceId: `scheduled-post-${pickId}` }
    );

    if (!result.success) {
      throw new Error(`Failed to update pick status: ${result.error}`);
    }
  }
