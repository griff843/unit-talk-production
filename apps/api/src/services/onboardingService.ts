...
export class OnboardingService {
  ...
  async handleUserOnboarding(user: User, tier: UserTier, testMode: boolean = false): Promise<void> {
    try {
      ...
      if (!sequence || sequence.messages.length === 0) {
        logger.warn('No onboarding sequence available', { tier });
        return;
      }
      ...
      logger.info('Completed user onboarding', {
        userId: user.id,
        username: user.username,
        tier,
        testMode,
      ...
      logger.error('Failed to handle user onboarding', {
        userId: user.id,
        username: user.username,
        tier,
  ...
  private async cancelOnboarding(userId: string): Promise<void> {
    const timeouts = this.activeSequences.get(userId);
    if (timeouts) {
      timeouts.forEach(clearTimeout);
      this.activeSequences.delete(userId);
      logger.info('Cancelled existing onboarding sequence', { userId });
    }
  }
  ...
  private async sendSequenceWithDelays(user: User, sequence: OnboardingSequence): Promise<void> {
    ...
    for (let i = 0; i < sequence.messages.length; i++) {
      ...
      const timeout = setTimeout(async () => {
        try {
          ...
          logger.info('Sent onboarding message', {
            userId: user.id,
            username: user.username,
            messageNumber: i + 1,
            totalMessages: sequence.messages.length,
    ...
    setTimeout(() => {
      ...
      logger.info('Completed onboarding sequence', {
        userId: user.id,
        username: user.username,
        tier: sequence.tier,
        totalMessages: sequence.messages.length,
  ...
  private createOnboardingSequence(user: User, tier: UserTier): OnboardingSequence {
    logger.info('Creating onboarding sequence', { userId: user.id, tier });
