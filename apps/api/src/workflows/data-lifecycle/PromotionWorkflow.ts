...
// Import activity types for promotion management
interface PromotionActivities {
  fetchPromotionCandidates(params: {
    minGrade: 'S' | 'A' | 'B' | 'C';
    minScore: number;
    minEdge: number;
    ...
    candidates: Array<{
      ...
      capperAttribution: string;
      featureContributions: Record<string, number>;
    }>;
    totalFound: number;
  }>;

  performRiskAssessment(params: {
    candidates: Array<{
      propId: string;
      professionalScore: number;
      deviggedEdge: number;
  ...
  validatePromotionCriteria(params: {
    candidates: Array<{
      propId: string;
      grade: string;
      professionalScore: number;
  ...
  sendPromotionAlert(params: {
    alertType: 'tier_promotion' | 'risk_rejection' | 'portfolio_limit' | 'completion';
    message: string;
    details: object;
...
const activities = proxyActivities<PromotionActivities>({
 ...
 */
export async function PromotionWorkflow(params: {
  minGrade?: 'S' | 'A' | 'B' | 'C';
  minScore?: number;
  minEdge?: number;
  minKellyFraction?: number;
  ...
  try {
    ...
    promotions.forEach(p => {
      result.tierDistribution[p.tier as keyof typeof result.tierDistribution]++;
    });
 ...
 */
export async function ExpressPromotionWorkflow(params: {
  propIds: string[];
