import { proxyActivities, sleep, log, workflowInfo, startChild, executeChild } from '@temporalio/workflow';
import type {
 ...
 */
export async function massiveParallelSGOBackfill(options: ParallelBackfillOptions): Promise<BackfillProgress> {
  const {
    sports = ['MLB', 'NFL', 'NBA', 'NCAAF', 'NCAAB', 'WNBA', 'NHL'],
    days = 365, // Full year for comprehensive backfill
  ...
  log.info('🚀 MASSIVE PARALLEL SGO BACKFILL STARTED', {
    workflowId,
  ...
  try {
    ...
    log.info(`📋 Created ${sportTasks.length} sport tasks with ${sportTasks.reduce((sum, task) => sum + task.dateRanges.length, 0)} total date ranges`);
    ...
    const sportWorkflows = sportTasks.map(async (task, index) => {
      ...
      if (index > 0) {
        await sleep(`${index * 2}s`);
      }

      return startChild('parallelSportBackfill', {
        args: [task, dryRun],
    ...
    log.info(`🏃 Launched ${sportWorkflows.length} concurrent sport workflows`);

    // Start concurrent settlement processing
    const settlementWorkflow = startChild('concurrentSettlementProcessor', {
    ...
    const sportResults = await Promise.all(
      allWorkflows.map(workflow => workflow.result())
    );
    ...
    log.info('🎯 MASSIVE PARALLEL SGO BACKFILL COMPLETED!', {
 ...
 */
export async function parallelSportBackfill(task: SportBackfillTask, dryRun: boolean = false): Promise<BackfillProgress> {
  const { sport, dateRanges, batchSize, rateLimit } = task;
  const workflowId = workflowInfo().workflowId;
  const startTime = new Date();

  log.info(`🏀 Starting parallel ${sport} backfill`, {
    sport,
  ...
  let sportProgress: BackfillProgress = {
    workflowId,
    startedAt: startTime,
  ...
  try {
    ...
    const dateRangeWorkflows = dateRanges.map(async (dateRange, index) => {
      ...
      if (index > 0) {
        await sleep(`${index * 1}s`);
      }

      return executeChild('processDateRangeBatch', {
        args: [sport, dateRange, batchSize, rateLimit, dryRun],
        workflowExecutionTimeout: '2 hours'
      });
    });

    log.info(`📅 Launched ${dateRangeWorkflows.length} concurrent date range processors for ${sport}`);
    ...
    log.info(`✅ ${sport} backfill completed`, {
      duration: `${Math.round(duration / 1000 / 60)} minutes`,
 ...
 */
export async function processDateRangeBatch(
  sport: string,
  ...
  try {
    log.info(`📅 Processing ${sport} date range: ${dateRange.start} to ${dateRange.end}`);
    ...
    for (let currentDate = new Date(startDate); currentDate <= endDate; currentDate.setDate(currentDate.getDate() + 1)) {
      try {
        ...
        log.warn(`Failed to fetch games for ${sport} on ${currentDate.toISOString().split('T')[0]}: ${error}`);
      }
    }

    log.info(`🎮 Found ${allGames.length} games for ${sport} in date range`);
    ...
    log.info(`✅ Date range completed for ${sport}`, {
    ...
    progress.status = 'failed';
    progress.duration = new Date().getTime() - startTime.getTime();
    log.error(`❌ Date range failed for ${sport}: ${error}`);
  }
 ...
 */
async function processConcurrentGameBatch(
  games: GameData[],
  sport: string,
  batchSize: number,
  rateLimit: number,
  dryRun: boolean
): Promise<{ gamesProcessed: number; propsProcessed: number; apiCallsUsed: number }> {
  ...
  try {
    ...
    const duplicateCheck = await checkDuplicates(
      games.map(g => g.external_game_id),
      'games'
    );
    ...
    const propPromises = newGames.map(async (game) => {
      try {
        ...
        log.warn(`Failed to process props for game ${game.external_game_id}: ${error}`);
        return 0;
      }
    });
    ...
    if (apiCallsUsed > 10) {
      await sleep(`${Math.ceil((apiCallsUsed * 60000) / rateLimit)}ms`);
    }
 ...
 */
export async function concurrentSettlementProcessor(options: {
  batchSize: number;
  ...
  log.info('🔧 Starting concurrent settlement processor', {
    workflowId,
    batchSize,
    maxConcurrent
  });
  ...
  try {
    ...
    while (true) {
      ...
      await sleep('30s'); // Process every 30 seconds
    ...
    log.error(`Settlement processor error: ${error}`);
  }

  log.info('🔧 Settlement processor completed', {
 ...
 */
async function createSportBackfillTasks(
  sports: string[],
  totalDays: number,
  batchSize: number
): Promise<SportBackfillTask[]> {

  const tasks: SportBackfillTask[] = [];
 ...
 */
function getSportOptimization(sport: string): { batchSize: number; rateLimit: number } {
  const optimizations: Record<string, { batchSize: number; rateLimit: number }> = {
