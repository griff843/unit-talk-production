...
interface ESPNAthlete {
  ...
  stats: string[]; // Array of stat values
}
...
export class NFLAdapter extends BaseAdapter {
  ...
  constructor() {
    super();
    this.logger = createLogger('NFLAdapter');
  ...
  private matchesGame(event: any, awayCode: string, homeCode: string): boolean {
    ...
    for (const teamPlayers of boxscore.players) {
      for (const category of teamPlayers.statistics || []) {
        ...
        for (const athleteData of category.athletes || []) {
          ...
          const parsedStats = this.parseStatCategory(
            category.name,
            athleteData.stats,
            statMapping
          );
  ...
  private getStatMapping(category: string): string[] | null {
