 ...
 */
function extractStatementType(statement: string): string {
  const normalizedStatement = statement.trim().toLowerCase();
  
  if (normalizedStatement.startsWith('select')) return 'SELECT';
  if (normalizedStatement.startsWith('insert')) return 'INSERT';
  if (normalizedStatement.startsWith('update')) return 'UPDATE';
  if (normalizedStatement.startsWith('delete')) return 'DELETE';
  if (normalizedStatement.startsWith('create')) return 'CREATE';
  if (normalizedStatement.startsWith('alter')) return 'ALTER';
  if (normalizedStatement.startsWith('drop')) return 'DROP';
 ...
 */
export function traceAsync(operationName: string, attributes: Record<string, any> = {}) {
