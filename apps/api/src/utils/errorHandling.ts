...
export class ErrorHandler {
  ...
  constructor(context?: string, supabase?: SupabaseClient) {
    ...
    this.logger = createLogger(`ErrorHandler${context ? ':' + context : ''}`);
  ...
  private async recordError(error: AgentError): Promise<void> {
    try {
      if (this.supabase) {
        const { error: dbError } = await this.supabase.from('agent_errors').insert([
          {
            message: error.message,
            code: error.code,
            stack: error.stack,
  ...
  private async handleCriticalError(error: AgentError): Promise<void> {
    try {
      if (this.supabase) {
        const { error: dbError } = await this.supabase.from('critical_errors').insert([
          {
            message: error.message,
            code: error.code,
            stack: error.stack,
      ...
      this.logger.error('Failed to handle critical error:', {
        error: err instanceof Error ? err.message : String(err),
      });
    }
  }
  ...
  public async withRetry<T>(fn: () => Promise<T>, operation: string): Promise<T> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        this.logger.warn(
          `Operation '${operation}' failed on attempt ${attempt}, retrying in ${delay}ms`,
          {
            error: error instanceof Error ? error.message : String(error),
            attempt,
            delay,
