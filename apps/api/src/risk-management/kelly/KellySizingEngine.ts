...
export interface KellyCalculationInput {
  ...
  variance?: number;              // Estimated variance
  correlatedExposure?: number;    // Existing correlated exposure
  portfolioContext?: {
...
export class KellySizingEngine {
  private logger = createLogger('KellySizingEngine');
  private config: KellyConfig;
  private riskProfile: RiskProfile;

  // Kelly calculation statistics
  private calculationHistory: KellyResult[] = [];
   ...
   */
  async calculateOptimalSize(input: KellyCalculationInput): Promise<KellyResult> {
   ...
   */
  async batchCalculateKelly(inputs: KellyCalculationInput[]): Promise<KellyResult[]> {
    ...
    for (let i = 0; i < inputs.length; i += batchSize) {
      ...
      const batchResults = await Promise.all(
        batch.map(input => this.calculateOptimalSize(input))
      );
   ...
   */
  private validateInput(input: KellyCalculationInput): KellyValidationResult {
   ...
   */
  private calculateBaseKelly(odds: number, winProbability: number, expectedValue: number): number {
   ...
   */
  private async calculateRiskAdjustments(input: KellyCalculationInput): Promise<{
   ...
   */
  private applyAdjustments(baseFraction: number, adjustments: any): number {
   ...
   */
  private async applyPortfolioConstraints(kellyFraction: number, input: KellyCalculationInput): Promise<number> {
   ...
   */
  private calculatePortfolioAdjustment(context: any): number {
    let adjustment = 1.0;
  ...
  private validateFinalResult(
    ...
    input: KellyCalculationInput
  ): KellyValidationResult {
    const violations: string[] = [];
   ...
   */
  private calculateRiskScore(input: KellyCalculationInput, adjustments: any): number {
    let riskScore = 0;

    // Base risk from Kelly fraction
    if (adjustments.overallAdjustment < 0.5) riskScore += 3;
    else if (adjustments.overallAdjustment < 0.8) riskScore += 2;
    else if (adjustments.overallAdjustment < 0.9) riskScore += 1;

    // Confidence risk
    if (input.confidence < 60) riskScore += 3;
    else if (input.confidence < 75) riskScore += 2;
    else if (input.confidence < 85) riskScore += 1;

    // Expected value risk
    if (input.expectedValue < 3) riskScore += 2;
    else if (input.expectedValue < 5) riskScore += 1;

    // Odds risk (extreme odds = higher risk)
    const absOdds = Math.abs(input.odds);
    if (absOdds > 300) riskScore += 2;
    else if (absOdds > 200) riskScore += 1;
   ...
   */
  private createRejectedResult(input: KellyCalculationInput, violations: string[]): KellyResult {
    return {
   ...
   * Create error result
   */
  private createErrorResult(input: KellyCalculationInput, errorMessage: string): KellyResult {
    return {
   ...
   */
  private updateStatistics(result: KellyResult): void {
    ...
    this.performanceMetrics.maxKellyUsed = Math.max(
      this.performanceMetrics.maxKellyUsed,
      result.fractionalKelly
    );
