...
export interface CircuitBreakerStatus {
  ...
  currentValue: number;
  triggeredAt?: string;
  duration: number;               // Seconds until reset
  actionsBlocked: string[];       // Blocked action types
}
...
export interface RiskControlEvent {
  ...
  triggeredBy: string;           // What triggered the control
  executedAt: string;
  success: boolean;
  impact: {
  ...
  overrideRequired: boolean;     // Whether manual override was needed
}
...
export interface EmergencyStopStatus {
  ...
  overrideCode?: string;         // Required for manual override
}

export class AutomatedRiskControls {
  private logger = createLogger('AutomatedRiskControls');
  private config: AlertConfig;
  private riskEngine: any; // Reference to main risk engine

  // Control system state
  private circuitBreakers = new Map<string, CircuitBreakerStatus>();
  private emergencyStop: EmergencyStopStatus = {
    isActive: false,
    reason: '',
    severity: 'HIGH',
    estimatedDuration: 0,
  ...
  private controlHistory: RiskControlEvent[] = [];
   ...
   */
  async evaluatePosition(position: Position): Promise<RiskControlAction[]> {
   ...
   */
  async executeAction(action: RiskControlAction): Promise<boolean> {
    ...
    try {
      ...
      const controlEvent: RiskControlEvent = {
        id: `control-${Date.now()}`,
   ...
   */
  async triggerEmergencyStop(reason: string, severity: 'HIGH' | 'CRITICAL' = 'CRITICAL'): Promise<void> {
   ...
   */
  async overrideEmergencyStop(overrideCode: string, operatorId: string): Promise<boolean> {
    if (!this.emergencyStop.isActive) {
      return false;
    }
  ...
  private initializeCircuitBreakers(): void {
    ...
    breakerTypes.forEach(type => {
      this.circuitBreakers.set(type, {
        type: type as any,
        isTriggered: false,
        threshold: this.CIRCUIT_BREAKER_THRESHOLDS[type as keyof typeof this.CIRCUIT_BREAKER_THRESHOLDS].WARNING,
        currentValue: 0,
        duration: 0,
        actionsBlocked: []
      });
   ...
   * Check position-level limits
   */
  private async checkPositionLimits(position: Position): Promise<RiskControlAction[]> {
    const actions: RiskControlAction[] = [];
   ...
   * Check portfolio-level limits
   */
  private async checkPortfolioLimits(): Promise<RiskControlAction[]> {
    const actions: RiskControlAction[] = [];
   ...
   */
  private async checkCircuitBreakers(): Promise<RiskControlAction[]> {
    const actions: RiskControlAction[] = [];
      ...
      for (const check of checks) {
        ...
        if (check.value > check.thresholds.EMERGENCY) {
          ...
          await this.triggerEmergencyStop(
            `${check.type} exceeded emergency threshold: ${(check.value * 100).toFixed(1)}%`,
            'CRITICAL'
          );
   ...
   */
  private async reducePosition(action: RiskControlAction): Promise<{
    success: boolean;
    impact: { positionsAffected: number; exposureReduced: number; riskReduction: number };
  }> {
    // Implementation would integrate with position management system
    // For now, simulate the action
    const targetSize = action.parameters.targetSize as number;
   ...
   * Hedge position
   */
  private async hedgePosition(action: RiskControlAction): Promise<{
    success: boolean;
    impact: { positionsAffected: number; exposureReduced: number; riskReduction: number };
  }> {
    // Implementation would create hedge positions
    const hedgeRatio = action.parameters.hedgeRatio as number;
   ...
   * Halt trading
   */
  private async haltTrading(action: RiskControlAction): Promise<{
    success: boolean;
    impact: { positionsAffected: number; exposureReduced: number; riskReduction: number };
  }> {
    const duration = action.parameters.duration as number;
   ...
   */
  private async reduceExposure(action: RiskControlAction): Promise<{
    success: boolean;
    impact: { positionsAffected: number; exposureReduced: number; riskReduction: number };
  }> {
    // Implementation would reduce positions proportionally
    const targetExposure = action.parameters.targetExposure as number;
   ...
   * Rebalance portfolio
   */
  private async rebalancePortfolio(action: RiskControlAction): Promise<{
    success: boolean;
    impact: { positionsAffected: number; exposureReduced: number; riskReduction: number };
  }> {
   ...
   */
  private isEmergencyOverride(action: RiskControlAction): boolean {
   ...
   */
  private getControlEventType(actionType: string): RiskControlEvent['type'] {
    switch (actionType) {
      case 'HALT_TRADING':
   ...
   */
  private addControlEvent(event: RiskControlEvent): void {
   ...
   */
  private async notifyEmergencyStop(): Promise<void> {
