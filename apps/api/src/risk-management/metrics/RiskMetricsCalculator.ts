...
export interface VaRCalculationOptions {
  ...
  timeHorizon: number;             // Days (typically 1)
  methodology: 'HISTORICAL' | 'PARAMETRIC' | 'MONTE_CARLO';
  windowSize?: number;             // Days of historical data
  simulations?: number;            // Monte Carlo simulations
}

export class RiskMetricsCalculator {
  private logger = createLogger('RiskMetricsCalculator');
  private riskProfile: RiskProfile;

  // Historical performance data
  private performanceHistory: PerformanceData[] = [];
  private readonly MAX_HISTORY_DAYS = 365;

  // Calculation cache
  private metricsCache = new Map<string, { metrics: any; timestamp: number; ttl: number }>();
   ...
   */
  async calculateComprehensiveMetrics(positions: Position[]): Promise<RiskMetrics> {
   ...
   */
  async calculateVaR(positions: Position[], options: VaRCalculationOptions): Promise<VaRResult> {
    const portfolioValue = this.calculatePortfolioValue(positions);
  ...
  ): Promise<ExpectedShortfallResult> {
    const portfolioValue = this.calculatePortfolioValue(positions);
  ...
  async calculateSharpeRatio(positions: Position[]): Promise<number> {
    ...
    if (returns.length < 30) return 0; // Need minimum data
   ...
   */
  async calculateSortinoRatio(positions: Position[]): Promise<number> {
    if (positions.length === 0) return 0;
   ...
   */
  async calculateCalmarRatio(positions: Position[]): Promise<number> {
    if (positions.length === 0) return 0;
   ...
   */
  async calculateMaxDrawdown(positions: Position[]): Promise<number> {
    const returns = this.getHistoricalReturns(252);
   ...
   */
  async calculateCurrentDrawdown(positions: Position[]): Promise<number> {
    const returns = this.getHistoricalReturns(252);
    if (returns.length === 0) return 0;
   ...
   */
  private async calculateAverageDrawdown(positions: Position[]): Promise<number> {
    const returns = this.getHistoricalReturns(252);
    if (returns.length === 0) return 0;
   ...
   */
  private async calculateRiskAdjustedReturn(positions: Position[]): Promise<number> {
    if (positions.length === 0) return 0;
   ...
   */
  private async calculateInformationRatio(positions: Position[]): Promise<number> {
   ...
   */
  private async calculateTreynorRatio(positions: Position[]): Promise<number> {
   ...
   */
  private async runStressTests(positions: Position[]): Promise<StressTestResult[]> {
   ...
   */
  private async calculateHistoricalVaR(positions: Position[], options: VaRCalculationOptions): Promise<VaRResult> {
    const portfolioValue = this.calculatePortfolioValue(positions);
    ...
    const worstScenarios: ScenarioResult[] = sortedReturns
   ...
   */
  private async calculateParametricVaR(positions: Position[], options: VaRCalculationOptions): Promise<VaRResult> {
    const portfolioValue = this.calculatePortfolioValue(positions);
    const returns = this.getHistoricalReturns(options.windowSize || 252);
    ...
    const zScore = zScores[options.confidenceLevel as keyof typeof zScores] || 1.645;
   ...
   */
  private async calculateMonteCarloVaR(positions: Position[], options: VaRCalculationOptions): Promise<VaRResult> {
    const portfolioValue = this.calculatePortfolioValue(positions);
    const historicalReturns = this.getHistoricalReturns(options.windowSize || 252);
  ...
  private getHistoricalReturns(days: number): number[] {
  ...
  private calculateSportExposures(positions: Position[]): Record<string, number> {
  ...
  private calculateLargestClusterExposure(positions: Position[]): { exposure: number; positions: string[] } {
    // Simplified clustering based on same game
    const gameGroups: Record<string, Position[]> = {};
  ...
  private calculateVolatilityStressLoss(positions: Position[]): number {
  ...
  private createFallbackVaR(portfolioValue: number, options: VaRCalculationOptions): VaRResult {
  ...
  private createFallbackMetrics(): RiskMetrics {
    return {
