...
// Legacy scorer. The canonical scorer is computeScoreV2 in
...
interface PitcherStats {
  ...
  meatballPercent: number;
  hittableCountPct: number;
  recentHRs: number;
  walkRate: number;
}
...
interface MatchupData {
  batterBarrel: number;
  batterLaunch: number;
  batter_zone_contact_pct?: number;
  pitcher_zone_weakness?: number;
    ...
    if (pitcherStats && matchupData) {
      ...
      if (zoneThreatBoost > 0) {
        ...
        if (config.zoneThreat.logDecisions) {
          logZoneThreatDecision({ pitcherStats, matchupData, threatResult });
        }
      }
