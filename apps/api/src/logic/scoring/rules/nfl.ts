...
export function nflSynergy(prop: PropObject): ScoreBreakdown {
  ...
  if (prop['tags'] && prop['tags'].length > 0) {
    const hasWeatherTag = prop['tags'].some(
      tag => typeof tag === 'string' && tag.toLowerCase().includes('weather')
    );
    if (hasWeatherTag) {
      breakdown['weather_impact'] = -0.08; // Negative for bad weather
    }

    const hasPrimeTimeTag = prop['tags'].some(
      tag => typeof tag === 'string' && tag.toLowerCase().includes('primetime')
    );
    if (hasPrimeTimeTag) {
      breakdown['prime_time_boost'] = 0.1;
    }

    const hasInjuryTag = prop['tags'].some(
      tag => typeof tag === 'string' && tag.toLowerCase().includes('injury')
    );
    if (hasInjuryTag) {
