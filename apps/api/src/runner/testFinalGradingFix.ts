...
async function testFinalGradingFix() {
  ...
  try {
    ...
    if (updatedProp) {
      ...
      console.log(
        `Original edge_score: ${originalEdgeScore} (${updatedProp.edge_score} basis points)`
      );
      ...
      if (batchProps && batchProps.length > 0) {
        ...
        console.log(
          `✅ Batch update: ${successCount}/${batchProps.length} props grading_status successfully`
        );
