...
async function findUnifiedPicksColumns() {
  ...
  try {
    const { data, error } = await supabase.from('unified_picks').insert(basicColumns).select();
    ...
    if (error) {
      ...
      // Clean up
      await supabase.from('unified_picks').delete().eq('user_id', 'test-basic');
    }
  } catch (e: any) {
  ...
  for (const column of testColumns) {
    const testData = {
      ...
      [column]:
        column === 'confidence_score'
          ? 0.75
          : column === 'professional_score'
            ? 85.5
            : column === 'devigged_edge'
              ? 12.3
    ...
    try {
      const { error } = await supabase.from('unified_picks').insert(testData).select();
      ...
      if (error) {
        ...
        // Clean up
        await supabase.from('unified_picks').delete().eq('user_id', `test-${column}`);
      }
    } catch (e: any) {
