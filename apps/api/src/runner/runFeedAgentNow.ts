    ...
    if (process.env.OPTIMAL_API_KEY) {
      ...
      try {
        ...
        if (optimalProps.length > 0) {
          const { error } = await supabase.from('raw_props').insert(optimalProps);
    ...
    if (process.env.ODDS_API_KEY) {
      ...
      try {
        ...
        for (const sport of sports) {
          const props = await oddsApiClient.fetchOddsApiProps(sport as any, [
            'h2h',
            'spreads',
            'totals',
          ]); // Use correct method
          if (props.length > 0) {
            const { error } = await supabase.from('raw_props').insert(props);
...
async function runProfessionalProcessing() {
  ...
  try {
    ...
    console.log(
      `• Average Score: ${results.reduce((sum, r) => sum + r.professionalScore, 0) / results.length || 0}`
    );
  } catch (error) {
