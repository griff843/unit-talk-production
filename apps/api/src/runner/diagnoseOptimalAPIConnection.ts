...
async function diagnoseOptimalAPI() {
    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        if (response.data.length > 0) {
          ...
          const oddsFields = Object.keys(sample).filter(
            key =>
              key.includes('odd') ||
              key.includes('over') ||
              key.includes('under') ||
  ...
  const specificEndpoints = ['/v1/props/MLB', '/v1/odds/MLB', '/v1/sportsbooks', '/v1/sports'];
  ...
  for (const endpoint of specificEndpoints) {
    ...
    try {
      ...
      console.log(
        `✅ ${endpoint}: ${response.status} - ${Array.isArray(response.data) ? response.data.length + ' items' : 'object'}`
      );
    } catch (error: any) {
      console.log(
        `❌ ${endpoint}: ${error.response?.status || 'Network Error'} - ${error.message}`
      );
    }
  }
}
