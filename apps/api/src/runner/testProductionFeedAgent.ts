...
async function testProductionFeedAgent() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   ✅ Fetched ${result.data.length} props from ${result.source} in ${result.metadata.processingTimeMs}ms`
      );

      // Analyze odds quality
      const propsWithBothOdds = result.data.filter(
        prop =>
          prop.over_odds &&
          prop.under_odds &&
          prop.over_odds !== 0 &&
          prop.under_odds !== 0 &&
          !isNaN(prop.over_odds) &&
          !isNaN(prop.under_odds)
      );

      const oddsQuality =
        result.data.length > 0 ? (propsWithBothOdds.length / result.data.length) * 100 : 0;

      console.log(
        `   📈 Odds Quality: ${oddsQuality.toFixed(1)}% (${propsWithBothOdds.length}/${result.data.length})`
      );
        ...
        if (insertError) {
          ...
          if (insertedProps && insertedProps.length > 0) {
            ...
            insertedProps.forEach((prop, i) => {
              console.log(
                `      ${i + 1}. ${prop.player_name} ${prop.stat_type} - Over: ${prop.over_odds}, Under: ${prop.under_odds}`
              );
            });
          }

          // Clean up test data
          await supabase.from('raw_props').delete().eq('source', 'production-test');
      ...
      if (oddsQuality >= 95 && result.data.length > 1000) {
        ...
        if (oddsQuality < 95)
          console.log(`   - Odds quality ${oddsQuality.toFixed(1)}% below 95% target`);
    ...
    console.error(
      '❌ Production FeedAgent test failed:',
