...
async function confirmDataPipeline() {
  ...
  try {
    ...
    console.log(
      `📈 Grading rate: ${totalProps ? (((gradedCount || 0) / totalProps) * 100).toFixed(2) : 0}%`
    );
    ...
    if (gradedProps && gradedProps.length > 0) {
      ...
      gradedProps.forEach((prop, i) => {
        ...
        console.log(
          `      Confidence: ${prop.confidence} (0/1), Edge: ${prop.edge_score} (per-mille)`
        );
    ...
    if (testProp) {
      ...
      console.log(
        `Player: ${testProp.player_name}, Stat: ${testProp.stat_type}, Line: ${testProp.line}`
      );
      ...
      if (gradingError) {
        ...
        console.log(
          `✅ Grading test successful: ${mockGradingResult.tier} tier, ${mockGradingResult.confidence}% confidence`
        );
        ...
        if (gradedProp) {
          console.log(
            `   Stored: tier=${gradedProp.tier}, confidence=${gradedProp.confidence}, edge=${gradedProp.edge_score}`
          );
        }
      }
    } else {
    ...
    console.log(
      `   Grading Progress: ${gradedCount || 0}/${totalProps || 0} (${(gradingRate * 100).toFixed(2)}%)`
    );
    console.log(
      `   Promotion Rate: ${promotedCount || 0}/${gradedCount || 0} (${(promotionRate * 100).toFixed(2)}%)`
    );
