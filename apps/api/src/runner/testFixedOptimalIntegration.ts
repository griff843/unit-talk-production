...
async function testFixedOptimalIntegration() {
  ...
  try {
    ...
    // Check for odds data
    const propsWithOdds = rawProps.filter(
      prop => prop.over_odds && prop.under_odds && prop.over_odds !== 0 && prop.under_odds !== 0
    );

    console.log(
      `   Props with complete odds: ${propsWithOdds.length}/${rawProps.length} (${((propsWithOdds.length / rawProps.length) * 100).toFixed(1)}%)`
    );
    ...
    console.log(
      `   Types: ${propTypes.slice(0, 10).join(', ')}${propTypes.length > 10 ? '...' : ''}`
    );
    ...
    if (propsWithOdds.length > 0) {
      ...
      console.log(
        `   Testing insert of: ${testProp.player_name} ${testProp.stat_type} ${testProp.line}`
      );
