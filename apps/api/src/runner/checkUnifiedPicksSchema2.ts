...
async function checkUnifiedPicksSchemaProper() {
  ...
  try {
    ...
    if (error) {
      ...
      // Clean up
      await supabase.from('unified_picks').delete().eq('id', testRecord.id);
    }
  } catch (e: any) {
  ...
  try {
    const { data, error } = await supabase.from('unified_picks').insert(expectedRecord).select();
    ...
    if (error) {
      ...
      // Clean up
      await supabase.from('unified_picks').delete().eq('id', expectedRecord.id);
    }
  } catch (e: any) {
