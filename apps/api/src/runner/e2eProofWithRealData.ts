...
function calculateProfessionalScore(prop: any): {
  score: number;
  tier: string;
  confidence: number;
} {
...
async function runE2EProofWithRealData() {
  ...
  try {
    ...
    console.log(
      `Sample: ${recentProps[0].player_name} ${recentProps[0].stat_type} ${recentProps[0].line} (${recentProps[0].sport})`
    );
    ...
    if (professionalResults.length > 0) {
      const avgScore =
        professionalResults.reduce((sum, r) => sum + r.professionalScore, 0) /
        professionalResults.length;
      const avgEdge =
        professionalResults.reduce((sum, r) => sum + r.devigedEdge, 0) / professionalResults.length;
      const autoApproved = professionalResults.filter(r => r.autoApproved).length;
      const avgProcessingTime =
        professionalResults.reduce((sum, r) => sum + r.processingTime, 0) /
        professionalResults.length;

      console.log(`📈 Average Professional Score: ${avgScore.toFixed(2)}`);
      console.log(`⚡ Average Devigged Edge: ${(avgEdge * 100).toFixed(2)}%`);
      console.log(
        `🏆 Auto-Approval Rate: ${((autoApproved / professionalResults.length) * 100).toFixed(1)}%`
      );
    ...
    console.log(
      `   🔄 Processing: ${professionalResults.length} props through professional system`
    );
