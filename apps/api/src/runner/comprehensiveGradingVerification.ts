...
async function comprehensiveGradingVerification(): Promise<GradingVerificationResult> {
  ...
  console.log(
    `Grading completion: ${totalRawProps ? (((gradedCount || 0) / totalRawProps) * 100).toFixed(1) : 0}%`
  );
  ...
  const tierDistribution =
    tierData?.reduce((acc: any, prop: any) => {
  ...
  if (unifiedPicks && unifiedPicks.length > 0) {
    ...
    const clvTracking = unifiedPicks.filter(
      pick => pick.kelly_bet_size !== null && pick.kelly_bet_size > 0
    ).length;

    const kellyFractions = unifiedPicks.filter(pick => pick.kelly_bet_size !== null).length;

    const confidenceScores = unifiedPicks.filter(
      pick => pick.confidence !== null && pick.confidence > 0
    ).length;

    const tierAssignments = unifiedPicks.filter(pick => pick.tier_when_placed !== null).length;

    console.log(
      `   CLV tracking: ${clvTracking}/${unifiedPicks.length} (${((clvTracking / unifiedPicks.length) * 100).toFixed(1)}%)`
    );
    console.log(
      `   Kelly fractions: ${kellyFractions}/${unifiedPicks.length} (${((kellyFractions / unifiedPicks.length) * 100).toFixed(1)}%)`
    );
    console.log(
      `   Confidence scores: ${confidenceScores}/${unifiedPicks.length} (${((confidenceScores / unifiedPicks.length) * 100).toFixed(1)}%)`
    );
    console.log(
      `   Tier assignments: ${tierAssignments}/${unifiedPicks.length} (${((tierAssignments / unifiedPicks.length) * 100).toFixed(1)}%)`
    );
  ...
  console.log(
    `   Promotion rate: ${gradedCount ? (((promotedCount || 0) / gradedCount) * 100).toFixed(1) : 0}%`
  );
  ...
  const clvCompliance = unifiedPicks
    ? (unifiedPicks.filter(pick => pick.kelly_bet_size !== null).length / unifiedPicks.length) * 100
    : 0;
  console.log(`   Rule #1: CLV Tracking: ${clvCompliance.toFixed(1)}% (Target: 100%)`);

  // Rule #2: Professional Grading
  const gradingCompliance = gradedCount && totalRawProps ? (gradedCount / totalRawProps) * 100 : 0;
  ...
  const kellyCompliance = unifiedPicks
    ? (unifiedPicks.filter(pick => pick.kelly_bet_size && pick.kelly_bet_size > 0).length /
        unifiedPicks.length) *
      100
    : 0;
  ...
  return {
    ...
    professionalCompliance: {
      ...
      confidenceScores: unifiedPicks
        ? (unifiedPicks.filter(pick => pick.confidence !== null).length / unifiedPicks.length) * 100
        : 0,
      tierAssignments: unifiedPicks
        ? (unifiedPicks.filter(pick => pick.tier_when_placed !== null).length /
            unifiedPicks.length) *
          100
        : 0,
    },
    gradingMetrics: {
      avgConfidence:
        unifiedPicks && unifiedPicks.length > 0
          ? unifiedPicks
              .filter(pick => pick.confidence !== null)
              .reduce((sum, pick) => sum + (pick.confidence || 0), 0) /
            unifiedPicks.filter(pick => pick.confidence !== null).length
          : 0,
      tierDistribution,
...
async function main() {
  try {
    ...
    console.log('📋 Summary:');
    console.log(
      `   • ${results.rawPropsGraded}/${results.rawPropsTotal} props grading_status (${results.rawPropsTotal ? ((results.rawPropsGraded / results.rawPropsTotal) * 100).toFixed(1) : 0}%)`
    );
