...
async function checkSystemState() {
  ...
  rawPropsStatus?.forEach((prop, i) => {
    const status = prop.processed_at
      ? '✅ Processed'
      : prop.error_message
        ? '❌ Error'
        : '⏳ Pending';
  ...
  if (unifiedPicksCount && unifiedPicksCount > 0) {
    ...
    recentPicks?.forEach((pick, i) => {
      ...
      console.log(
        `${i + 1}. ${pick.player_name} ${pick.stat_type} | ${pick.tier || 'N/A'}-Tier | Score: ${score} | Edge: ${edge}`
      );
    });
  }
  ...
  console.log(
    `✅ Data Pipeline: ${unifiedPicksCount && unifiedPicksCount > 0 ? 'OPERATIONAL' : 'NO DATA'}`
  );
