...
async function processRealPicksThroughGrading() {
  ...
  try {
    ...
    for (let i = 0; i < realRawProps.length; i++) {
      ...
      try {
        ...
        if (typeof scoreToUse !== 'number') {
          ...
          console.log(`   ⚠️ Using fallback score: ${scoreToUse} (finalScore was ${typeof gradingResult.finalScore})`);
        }
    ...
    if (processedPicks.length > 0) {
      console.log(`✅ Successfully processed ${processedPicks.length} REAL picks through grading system`);
      
      // Show statistics
      const avgScore = processedPicks.reduce((sum, p) => sum + p.professional_score, 0) / processedPicks.length;
      ...
      const avgProcessingTime = processedPicks.reduce((sum, p) => sum + p.processing_time, 0) / processedPicks.length;
      
      console.log(`📊 Average Professional Score: ${avgScore.toFixed(2)}`);
      console.log(`⚡ Average Devigged Edge: ${(avgEdge * 100).toFixed(2)}%`);
      console.log(`🏆 Auto-Approval Rate: ${((autoApproved/processedPicks.length)*100).toFixed(1)}%`);
      ...
      processedPicks.slice(0, 10).forEach((pick, i) => {
        ...
        console.log(`   Score: ${pick.score.toFixed(2)} | ${pick.tier}-Tier | Edge: ${(pick.edge * 100).toFixed(2)}% | Kelly: ${(pick.kelly * 100).toFixed(2)}% | Auto: ${pick.approved ? '✅' : '❌'}`);
      });
...
processRealPicksThroughGrading().then(() => {
