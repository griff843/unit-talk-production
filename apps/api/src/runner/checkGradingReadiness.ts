...
async function checkGradingReadiness() {
  ...
  if (sampleProps && sampleProps.length > 0) {
    sampleProps.forEach((prop, i) => {
      ...
      console.log(`${i+1}. ${prop.player_name} ${prop.stat_type} ${prop.line} - Over: ${prop.over_odds}, Under: ${prop.under_odds} (${prop.source}) ${canGrade ? '✅' : '❌'}`);
    });
  } else {
  ...
  const nullPercentage = totalProps && nullOddsProps ? (nullOddsProps / totalProps * 100).toFixed(1) : '0.0';
  ...
  console.log(`✅ Props with odds: ${propsWithOdds || 0} ${hasEnoughProps ? '✅' : '❌'} (need 100+)`);
...
checkGradingReadiness().then(() => process.exit(0)).catch(console.error);