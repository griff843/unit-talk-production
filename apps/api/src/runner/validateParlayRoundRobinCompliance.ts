...
async function processCombinationBet(type: 'parlay' | 'round_robin', legs: any[]) {
  ...
  legs.forEach((leg, i) => {
    console.log(`   Leg ${i+1}: ${leg.player_name} ${leg.stat_type} | Edge: ${(leg.devigged_edge * 100).toFixed(2)}% | Score: ${leg.professional_score}`);
  });
  ...
  if (type === 'parlay') {
    ...
    combinations.forEach((combo, i) => {
      ...
      console.log(`        Edge: ${(totalEdge * 100).toFixed(2)}% | Kelly: ${(kellyFraction * 100).toFixed(2)}%`);
    });
...
validateParlayRoundRobinCompliance().then(() => {
