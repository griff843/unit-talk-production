...
async function testGradingDatabase() {
  ...
  try {
    ...
    const { data, error } = await supabase.from('grading_results').insert(testResult).select();
    ...
    if (error) {
      ...
      await supabase.from('grading_results').delete().eq('prop_id', 'test-prop-123');
    }
  } catch (e: any) {
  ...
  try {
    ...
    if (error) {
      ...
      // Clean up test record
      await supabase.from('unified_picks').delete().eq('raw_prop_id', 'test-prop-456');
    }
  } catch (e: any) {
  ...
  try {
    ...
    const { data, error } = await supabase.from('unified_picks').insert(testUnifiedPick).select();
