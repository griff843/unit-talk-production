...
async function checkPropSources() {
  ...
  const { data: sampleProps } = await supabase
    .from('raw_props')
    .select(
      'id, player_name, stat_type, over_odds, under_odds, source, confidence_score, tier_tag, promoted'
    )
    .eq('source', 'production-feedagent')
    .limit(3);
  ...
  if (sampleProps && sampleProps.length > 0) {
    ...
    sampleProps.forEach((prop, i) => {
      console.log(
        `${i + 1}. ${prop.player_name} ${prop.stat_type} - Over: ${prop.over_odds}, Under: ${prop.under_odds}, Graded: ${prop.confidence_score ? 'Yes' : 'No'}`
      );
    });
  }
  ...
  if (needsSourceFix) {
    ...
    console.log(
      `🔄 Need to update source from 'production-feedagent' to 'feedagent-pipeline-test'`
    );
