...
class SimpleHistoricalTester {
  ...
  private async testServiceComponents(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `✅ Devigging Service: ${testOdds.odds1}/${testOdds.odds2} → Fair odds: ${result.outcome1.fairOdds}/${result.outcome2.fairOdds}`
      );
    ...
    try {
      ...
      console.log(
        `   - Positive CLV rate: ${((stats.clvPositive / Math.max(stats.totalBets, 1)) * 100).toFixed(1)}%`
      );
    } catch (error) {
    ...
    try {
      ...
      console.log(
        `✅ Kelly Calculation: 5% edge at 1.9 odds → ${(kellyFraction * 100).toFixed(2)}% of bankroll`
      );
  ...
  private async testRuleComplianceLogic(): Promise<void> {
    ...
    console.log(
      `✅ Kelly Fraction Calculated: ${compliance.kellyFractionCalculated ? 'PASS' : 'FAIL'}`
    );
