...
async function checkAndProcessRealData() {
  ...
  recentProps?.forEach((prop, i) => {
    ...
    console.log(
      `${i + 1}. ${prop.player_name} ${prop.stat_type} ${prop.line} (${prop.sport}) - ${date}`
    );
  });
  ...
  if (unprocessedCount && unprocessedCount > 0) {
    ...
    try {
      ...
      results.slice(0, 5).forEach((result, i) => {
        console.log(
          `${i + 1}. Score: ${result.professionalScore.toFixed(2)} | ${result.tier}-Tier | Edge: ${(result.devigged_edge * 100).toFixed(2)}%`
        );
        console.log(
          `   Auto-Approved: ${result.published ? '✅' : '❌'} | Processing: ${result.processing_time}ms`
        );
      });
      ...
      console.log(
        `🎯 Professional Rules: ${results.length > 0 ? '✅ FOLLOWED' : '❌ NOT VERIFIED'}`
      );
