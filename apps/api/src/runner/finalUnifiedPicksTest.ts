...
async function finalUnifiedPicksTest() {
  ...
  try {
    const { data, error } = await supabase.from('unified_picks').insert(allRequiredFields).select();
    ...
    if (error) {
      ...
      await supabase.from('unified_picks').delete().eq('id', allRequiredFields.id);
