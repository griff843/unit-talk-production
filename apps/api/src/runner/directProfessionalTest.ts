...
async function runDirectProfessionalTest() {
  ...
  try {
    ...
    console.log(
      `Sample prop: ${rawProps[0].player_name} ${rawProps[0].stat_type} ${rawProps[0].line} (${rawProps[0].sport})`
    );
    ...
    for (let i = 0; i < Math.min(5, rawProps.length); i++) {
      ...
      try {
        ...
        const kellyFraction = Math.min(
          0.25,
          devigged.deviggedEdge * 0.5 * gradingResult.confidence
        );
    ...
    if (results.length > 0) {
      ...
      results.forEach((r, i) => {
        ...
        console.log(
          `   Score: ${r.professionalScore.toFixed(2)} | ${r.tier}-Tier | Edge: ${(r.devigged_edge * 100).toFixed(2)}% | Kelly: ${(r.kelly_fraction * 100).toFixed(2)}%`
        );
        console.log(
          `   CLV: ${r.clv_tracking_id.substring(0, 8)}... | Auto: ${r.published ? '✅' : '❌'} | Time: ${r.processing_time}ms`
        );
      });
