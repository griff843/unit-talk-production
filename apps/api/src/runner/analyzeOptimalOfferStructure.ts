...
async function analyzeOptimalOfferStructure() {
  ...
  try {
    ...
    console.log(
      `🎯 Analyzing event: ${testEvent.id} (${testEvent.away_display} @ ${testEvent.home_display})`
    );
    ...
    Object.entries(propTypeStats).forEach(([propType, stats]) => {
      ...
      console.log(
        `     Total: ${stats.total} | Over: ${stats.over} (${overPct}%) | Under: ${stats.under} (${underPct}%)`
      );
    ...
    const completePairs = Array.from(offersByPropAndBook.entries()).filter(
      ([key, pair]) => pair.over && pair.under
    );

    console.log(
      `   Found ${completePairs.length} complete over/under pairs out of ${offersByPropAndBook.size} unique prop/book combinations`
    );
