...
async function runProductionFeedAgent() {
  ...
  try {
    ...
    for (const sport of sports) {
      ...
      // Verify odds quality
      const propsWithOdds = result.data.filter(prop => 
      ...
      for (let i = 0; i < propsWithOdds.length; i += batchSize) {
        ...
        console.log(`   Inserted batch ${Math.floor(i/batchSize) + 1}: ${insertedBatch?.length || 0} props`);
      }
    }
    ...
    const finalOddsQuality = finalTotalProps && finalPropsWithOdds ? 
      (finalPropsWithOdds / finalTotalProps * 100).toFixed(1) : '0.0';
    
    console.log(`✅ Final props with odds: ${finalPropsWithOdds || 0}/${finalTotalProps || 0} (${finalOddsQuality}%)`);
...
runProductionFeedAgent().then(success => {
