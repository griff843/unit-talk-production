...
async function testMinimalUnifiedPicks() {
  ...
  try {
    const { data, error } = await supabase.from('unified_picks').insert(minimalPick).select();
    ...
    if (error) {
      ...
      await supabase.from('unified_picks').delete().eq('id', minimalPick.id);
      ...
      if (profError) {
        ...
        for (const col of professionalColumns) {
          const testRec = {
            ...minimalPick,
            id: randomUUID(),
            [col]: col === 'tier' ? 'A' : col === 'confidence' ? 0.85 : 0.05,
          };

          const { error: colError } = await supabase.from('unified_picks').insert(testRec).select();
          ...
          if (colError) {
            ...
            // Clean up
            await supabase.from('unified_picks').delete().eq('id', testRec.id);
          }
        }
      } else {
        ...
        // Clean up
        await supabase.from('unified_picks').delete().eq('id', professionalPick.id);
      }
    }
  } catch (e: any) {
