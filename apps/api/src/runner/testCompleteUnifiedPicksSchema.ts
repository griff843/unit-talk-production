...
async function testCompleteUnifiedPicksSchema() {
  ...
  try {
    const { data, error } = await supabase.from('unified_picks').insert(completePick).select();
    ...
    if (error) {
      ...
      const metaFields = sortedColumns.filter(
        col =>
          !requiredFields.includes(col) && !gradingFields.includes(col) && !gameFields.includes(col)
      );

      console.log('\n  🔴 REQUIRED FIELDS:');
      requiredFields
        .filter(col => col in record)
        .forEach(col => {
      ...
      await supabase.from('unified_picks').delete().eq('id', completePick.id);
      ...
      if (gradingError) {
        ...
        // Clean up
        await supabase.from('unified_picks').delete().eq('id', perfectGradingPick.id);
      }
    }
  } catch (e: any) {
...
testCompleteUnifiedPicksSchema()
  .then(() => process.exit(0))
  .catch(console.error);
