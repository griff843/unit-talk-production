...
async function testCompleteOptimalE2E() {
  ...
  try {
    ...
    console.log(
      `   Odds Quality: ${oddsQuality.toFixed(1)}% (${propsWithBothOdds.length}/${rawProps.length})`
    );
    ...
    if (insertError) {
      ...
      console.log(
        `   Testing: ${singleProp.player_name} ${singleProp.stat_type} ${singleProp.line}`
      );
    ...
    if (verifyCount && verifyCount > 0) {
      ...
      if (verifyProps && verifyProps.length > 0) {
        const avgOverOdds =
          verifyProps.reduce((sum, p) => sum + (p.over_odds || 0), 0) / verifyProps.length;
        const avgUnderOdds =
          verifyProps.reduce((sum, p) => sum + (p.under_odds || 0), 0) / verifyProps.length;
