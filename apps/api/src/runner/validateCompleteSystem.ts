...
async function validateCompleteSystem() {
  ...
  try {
    ...
    if (games) {
      const sportCounts = games.reduce(
        (acc, game) => {
    ...
    if (linkedProps && games) {
      ...
      const linkageRate =
        linkedProps.length > 0 ? (linkableProps.length / linkedProps.length) * 100 : 0;
    ...
    console.log(
      `   Props with Odds: ${propCount || 0} (Target: 1000+ ${propCount && propCount >= 1000 ? '✅' : '❌'})`
    );
    console.log(
      `   Odds Quality: ${oddsQuality.toFixed(1)}% (Target: 95%+ ${oddsQuality >= 95 ? '✅' : '❌'})`
    );
