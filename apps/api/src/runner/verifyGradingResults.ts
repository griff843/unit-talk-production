...
async function verifyGradingResults() {
  ...
  if (recentPicks && recentPicks.length > 0) {
    ...
    recentPicks.forEach((pick, i) => {
      ...
      console.log(
        `${i + 1}. ${pick.player_name} ${pick.stat_type} - Tier: ${pick.tier}, Confidence: ${pick.confidence_score}, Kelly: ${pick.kelly_fraction}, CLV: ${hasClv}`
      );
    });
  }
  ...
  console.log(
    `   CLV tracking: ${clvTracking || 0}/${unifiedPicks || 0} picks (${unifiedPicks ? (((clvTracking || 0) / unifiedPicks) * 100).toFixed(1) : '0.0'}%)`
  );
  console.log(
    `   Tier assignment: ${tieredPicks || 0}/${unifiedPicks || 0} picks (${unifiedPicks ? (((tieredPicks || 0) / unifiedPicks) * 100).toFixed(1) : '0.0'}%)`
  );
  ...
  if (processingStats && processingStats.length > 0) {
    const avgProcessingTime =
      processingStats.reduce((sum, p) => sum + (p.processing_time || 0), 0) /
