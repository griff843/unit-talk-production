...
async function monitorOptimalOddsQuality() {
  ...
  for (const sport of sports) {
    ...
    try {
      ...
      const propsWithBothOdds = rawProps.filter(
        prop =>
          prop.over_odds &&
          prop.under_odds &&
          prop.over_odds !== 0 &&
      ...
      // Calculate averages
      const avgOverOdds =
        propsWithBothOdds.reduce((sum, p) => sum + (p.over_odds || 0), 0) /
        propsWithBothOdds.length;
      const avgUnderOdds =
        propsWithBothOdds.reduce((sum, p) => sum + (p.under_odds || 0), 0) /
        propsWithBothOdds.length;
      ...
      // Log results
      console.log(
        `   📈 Quality: ${oddsQuality.toFixed(1)}% (${propsWithBothOdds.length}/${rawProps.length})`
      );
  ...
  try {
    const { error } = await supabase.from('optimal_quality_metrics').insert(allMetrics);
    ...
    console.log(
      `⚠️  Database storage skipped: ${dbError instanceof Error ? dbError.message : 'Unknown error'}`
    );
  }
