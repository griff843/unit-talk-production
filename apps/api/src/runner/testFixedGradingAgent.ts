  ...
  try {
    ...
    for (const prop of testProps.slice(0, 3)) {
      ...
      try {
        ...
        console.log(`   ❌ Failed to grade: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
    ...
    if (gradedAfter && gradedAfter.length > 0) {
      ...
      gradedAfter.slice(0, 5).forEach((prop, index) => {
        console.log(`   ${index + 1}. ${prop.player_name} ${prop.stat_type}: ${prop.tier} tier, ${prop.confidence}% confidence`);
      });
    }
