...
async function checkGradedProps() {
  ...
  const { data: gradedSample } = await supabase
    .from('raw_props')
    .select(
      'player_name, stat_type, tier_tag, confidence_score, edge_score, promoted, clv_tracking_id'
    )
    .gt('confidence_score', 0)
    .limit(5);
  ...
  if (gradedSample && gradedSample.length > 0) {
    ...
    gradedSample.forEach((prop, i) => {
      ...
      console.log(
        `${i + 1}. ${prop.player_name} ${prop.stat_type} - Tier: ${prop.tier_tag}, Confidence: ${prop.confidence_score}, Edge: ${prop.edge_score}, Promoted: ${prop.promoted}, CLV: ${hasClv}`
      );
    });
  } else {
