...
class E2ERealDataTester {
  ...
  private async testFeedAgentIngestion(): Promise<void> {
    ...
    try {
      ...
      stepResult.metrics = {
        ...
        propsPerGame:
          todaysGames.reduce((sum, game) => sum + game.propsCount, 0) /
          Math.max(todaysGames.length, 1),
      };
      ...
      if (stepResult.success) {
        ...
        todaysGames.forEach(game => {
          console.log(
            `   🏈 ${game.sport}: ${game.awayTeam} @ ${game.homeTeam} (${game.propsCount} props)`
          );
        });
      } else {
  ...
  private async testPropsProcessing(): Promise<void> {
    ...
    try {
      ...
      if (stepResult.success) {
        console.log(
          `✅ Props Processing SUCCESS: ${validationResults.valid}/${sampleProps.length} valid props`
        );
        console.log(
          `   📊 Validation Rate: ${((validationResults.valid / sampleProps.length) * 100).toFixed(1)}%`
        );
        console.log(
          `   📈 Avg Field Completion: ${(validationResults.avgFieldCompletion * 100).toFixed(1)}%`
        );
      } else {
  ...
  private validatePropStructures(props: any[]): any {
    ...
    props.forEach(prop => {
      ...
      const hasAllRequired = requiredFields.every(
        field => prop[field] !== null && prop[field] !== undefined && prop[field] !== ''
      );
  ...
  private async testGradingAgent(): Promise<void> {
    ...
    try {
      ...
      if (stepResult.success) {
        console.log(
          `✅ GradingAgent SUCCESS: ${gradingResults.successfulGrades}/${testProps.length} props graded`
        );
        ...
        console.log(
          `   🎯 Score Distribution: ${JSON.stringify(gradingResults.scoreDistribution)}`
        );
      } else {
    ...
    for (const prop of props) {
      try {
        ...
        if (gradingResult.success) {
          ...
          console.log(
            `   ✅ ${prop.player_name} ${prop.stat_type}: Score ${gradingResult.score.toFixed(2)} (${tier} tier)`
          );
        } else {
  ...
  private async testProfessionalSystemIntegration(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `${complianceTest.success ? '✅' : '❌'} Rule Compliance: ${complianceTest.message}`
      );

      const overallSuccess =
        deviggingTest.success && clvTest.success && kellyTest.success && complianceTest.success;
      ...
      stepResult.metrics = {
        componentSuccessRate:
          ([deviggingTest, clvTest, kellyTest, complianceTest].filter(t => t.success).length / 4) *
          100,
      };
  ...
  private async testPromotionReadiness(): Promise<void> {
    ...
    try {
      ...
      if (stepResult.success) {
        console.log(
          `✅ Promotion Readiness SUCCESS: ${promotionTests.promotableProps} props ready for promotion`
        );
        console.log(
          `   📈 Promotion Rate: ${((promotionTests.promotableProps / promotionTests.totalProps) * 100).toFixed(1)}%`
        );
  ...
  private async runPromotionTests(): Promise<any> {
    ...
    const promotableProps = mockGradedProps.filter(
      prop => prop.professional_score >= promotionThreshold
    ).length;

    const tierDistribution = mockGradedProps.reduce(
      (dist, prop) => {
    ...
    const avgPromotionScore =
      promotableProps > 0
        ? mockGradedProps
            .filter(prop => prop.professional_score >= promotionThreshold)
            .reduce((sum, prop) => sum + prop.professional_score, 0) / promotableProps
        : 0;
  ...
  private async generateE2EReport(): Promise<void> {
    ...
    console.log(
      `📈 Success Rate: ${successfulSteps}/${totalSteps} steps (${((successfulSteps / totalSteps) * 100).toFixed(1)}%)`
    );
