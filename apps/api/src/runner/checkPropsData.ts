...
async function checkPropsData() {
  ...
  if (allProps) {
    const sourceCounts = allProps.reduce(
      (acc, prop) => {
    ...
    Object.entries(sourceCounts).forEach(([source, counts]) => {
      const percentage =
        counts.total > 0 ? ((counts.withOdds / counts.total) * 100).toFixed(1) : '0.0';
      console.log(
        `   ${source}: ${counts.total} props (${counts.withOdds} with odds = ${percentage}%)`
      );
    });
  }
  ...
  if (goodProps && goodProps.length > 0) {
    ...
    goodProps.forEach((prop, i) => {
      console.log(
        `${i + 1}. ${prop.player_name} ${prop.stat_type} - Over: ${prop.over_odds}, Under: ${prop.under_odds} (${prop.source})`
      );
    });
  }
}
