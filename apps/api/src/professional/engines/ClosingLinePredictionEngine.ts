...
import type {
  ClosingLinePredictionEngine as IClosingLinePredictionEngine,
  ClosingLinePredictionConfig,
  ...
  EngineHealthStatus,
  FactorContribution,
  ClosingLineRecommendation,
  PriceAlert
} from '../types/closing-line-prediction';
import { createLogger } from '../../utils/logger';

export class ClosingLinePredictionEngine implements IClosingLinePredictionEngine {
  private logger: any;
  private config: ClosingLinePredictionConfig;
  private metrics: PredictionPerformanceMetrics;
  private modelInfo: ModelInfo;
  private predictionHistory: Map<string, ClosingLinePredictionResult[]> = new Map();

  constructor(config?: Partial<ClosingLinePredictionConfig>) {
    this.logger = createLogger('ClosingLinePredictionEngine');
   ...
   */
  async predictClosingLine(input: ClosingLinePredictionInput): Promise<ClosingLinePredictionResult> {
    ...
    try {
      ...
      const result: ClosingLinePredictionResult = {
   ...
   */
  async batchPredictLines(inputs: ClosingLinePredictionInput[]): Promise<ClosingLinePredictionResult[]> {
    this.logger.info(`🔄 Batch predicting ${inputs.length} closing lines`);

    const results = await Promise.allSettled(
      inputs.map(input => this.predictClosingLine(input))
    );

    const successful = results
      .filter(result => result.status === 'fulfilled')
      .map(result => (result as PromiseFulfilledResult<ClosingLinePredictionResult>).value);
   ...
   */
  private validateInput(input: ClosingLinePredictionInput): void {
    if (!input.propId) throw new Error('PropId is required');
    if (!input.sport) throw new Error('Sport is required');
    if (typeof input.currentLine !== 'number') throw new Error('Current line must be a number');
    if (input.hoursUntilGame < 0) throw new Error('Hours until game cannot be negative');
   ...
   */
  private async analyzeHistoricalPatterns(input: ClosingLinePredictionInput): Promise<FactorContribution> {
    ...
    if (input.similarGames && input.similarGames.length > 0) {
      const avgMovement = input.similarGames.reduce((sum, game) =>
        sum + (game.closingLine - game.openingLine), 0) / input.similarGames.length;

      const avgSimilarity = input.similarGames.reduce((sum, game) =>
        sum + game.similarity, 0) / input.similarGames.length;

      impact = avgMovement * avgSimilarity * 100;
      confidence = Math.min(0.95, avgSimilarity);

      evidence.push(
        `${input.similarGames.length} similar games found`,
        `Average line movement: ${avgMovement.toFixed(2)} points`,
   ...
   */
  private analyzeMarketDynamics(input: ClosingLinePredictionInput): FactorContribution {
   ...
   */
  private analyzeInjuryImpact(input: ClosingLinePredictionInput): FactorContribution {
    let impact = 0;
    let maxConfidence = 0;
    const evidence: string[] = [];
    ...
    for (const injury of input.injuryReports) {
      ...
      evidence.push(
        `${injury.player} ${injury.severity}: ${injuryImpact.toFixed(1)} point impact`
      );
    }
   ...
   */
  private analyzeWeatherImpact(input: ClosingLinePredictionInput): FactorContribution {
   ...
   */
  private analyzePublicSentiment(input: ClosingLinePredictionInput): FactorContribution {
    let impact = 0;
    let avgConfidence = 0;
    const evidence: string[] = [];
   ...
   */
  private analyzeSharpMoney(input: ClosingLinePredictionInput): FactorContribution {
    const factors = input.marketFactors;
    let impact = 0;
    const evidence: string[] = [];
   ...
   */
  private async runMLPrediction(input: ClosingLinePredictionInput, factors: any) {
  ...
  private calculateOptimalTiming(
    input: ClosingLinePredictionInput,
    prediction: any,
    sportConfig: any
  ) {
   ...
   */
  private estimateCLVPotential(input: ClosingLinePredictionInput, prediction: any, timing: any) {
   ...
   */
  private assessPredictionRisks(input: ClosingLinePredictionInput, prediction: any) {
  ...
  private generateRecommendation(
    ...
    let action: 'BET_NOW' | 'WAIT_FOR_OPTIMAL' | 'MONITOR_AND_DECIDE' | 'AVOID';
   ...
   */
  private getRecommendationReasoning(action: string, prediction: any, clv: any, risk: any): string {
    switch (action) {
      case 'BET_NOW':
   ...
   */
  private updateMetrics(result: ClosingLinePredictionResult, processingTime: number): void {
  ...
  async updateModel(modelData: any): Promise<void> {
  ...
  async backtestModel(period: string): Promise<BacktestResult> {
  ...
  async getHistoricalAccuracy(filters?: AccuracyFilters): Promise<AccuracyMetrics> {
    return {
      overall: 0.78,
  ...
  async getPerformanceMetrics(): Promise<PredictionPerformanceMetrics> {
    return { ...this.metrics };
  }
