...
export interface CLVValidationMetrics {
  ...
  // Percentage metrics
  positiveCLVRate: number; // Target: >= 50%
  avgCLV: number;
  medianCLV: number;
  ...
  targetExceeded: boolean; // 60%+ positive CLV (excellent)

  // Quality metrics
  dataQuality: number; // 0-1
  trackingAccuracy: number; // 0-1
  reportingCompleteness: number; // 0-1
...
export interface CLVValidationConfig {
  targetPositiveRate: number; // Default: 0.5 (50%)
  excellentPositiveRate: number; // Default: 0.6 (60%)
  minSampleSize: number; // Default: 100
  validationInterval: number; // Default: 3600000 (1 hour)
  dataRetentionDays: number; // Default: 90
  alertThresholds: {
    criticalCLVRate: number; // Below this = critical alert
    warningCLVRate: number; // Below this = warning alert
    poorProfitability: number; // Below this = poor performance
  };
}

export class CLVTrackingValidator extends EventEmitter {
  private logger: any;
  private config: CLVValidationConfig;
  private clvResults: CLVTrackingResult[] = [];
  private validationTimer?: NodeJS.Timeout;
  private startTime: number;

  constructor(config?: Partial<CLVValidationConfig>) {
    super();
    this.logger = createLogger('CLVTrackingValidator');
   ...
   */
  async performValidation(): Promise<CLVValidationMetrics> {
   ...
   */
  private calculateCLVMetrics(): CLVValidationMetrics {
   ...
   */
  private createEmptyMetrics(): CLVValidationMetrics {
    return {
      totalTracks: 0,
      positiveCLVCount: 0,
      negativeCLVCount: 0,
   ...
   */
  private calculateSportBreakdown(): Record<string, SportCLVMetrics> {
    const sportGroups = this.groupBySport();
    const breakdown: Record<string, SportCLVMetrics> = {};
   ...
   */
  private calculatePeriodMetrics(period: string, results: CLVTrackingResult[]): CLVPeriodMetrics {
    if (results.length === 0) {
    ...
    if (positiveCLVRate > 0.55) trend = 'IMPROVING';
    else if (positiveCLVRate < 0.45) trend = 'DECLINING';
   ...
   */
  private createEmptyPeriodMetrics(period: string): CLVPeriodMetrics {
    return {
      period,
      totalTracks: 0,
      positiveCLVRate: 0,
   ...
   */
  private checkCLVAlerts(metrics: CLVValidationMetrics): void {
   ...
   * Helper methods
   */
  private groupBySport(): Record<string, CLVTrackingResult[]> {
    const groups: Record<string, CLVTrackingResult[]> = {};
  ...
  private filterByTime(milliseconds: number): CLVTrackingResult[] {
    const cutoff = Date.now() - milliseconds;
    return this.clvResults.filter(result =>
      new Date(result.timestamp).getTime() >= cutoff
    );
  }
  ...
  private calculateDataQuality(): number {
    ...
    this.clvResults.forEach(result => {
      requiredFields.forEach(field => {
        totalChecks++;
        if (result[field as keyof CLVTrackingResult] !== undefined) {
          passedChecks++;
        }
      });
    });
  ...
  private cleanupOldData(): void {
    ...
    this.clvResults = this.clvResults.filter(result =>
      new Date(result.timestamp).getTime() >= cutoff
    );
  }
...
SPORT BREAKDOWN
---------------
${Object.values(metrics.sportBreakdown).map(sport =>
  `${sport.sport}: ${(sport.positiveCLVRate * 100).toFixed(1)}% positive (${sport.totalTracks} tracks)`
).join('\n')}
