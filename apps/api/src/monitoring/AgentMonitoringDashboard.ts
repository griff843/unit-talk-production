...
export class AgentMonitoringDashboard extends EventEmitter {
  ...
  async getPerformanceReport(agentName?: string, days: number = 7): Promise<any> {
    ...
    const agentsToAnalyze = agentName
      ? [agentName]
      : Object.keys(await this.metricsCollector.getAllAgentsStatus());
    ...
    report.summary.systemReliability =
      totalPeriods > 0 ? (healthyPeriods / totalPeriods) * 100 : 100;
  ...
  registerConnection(connectionId: string): void {
    ...
    this.logger.debug('Dashboard connection registered', {
      connectionId,
  ...
  unregisterConnection(connectionId: string): void {
    ...
    this.logger.debug('Dashboard connection unregistered', {
      connectionId,
      totalConnections: this.activeConnections.size,
    });
  }
  ...
  broadcastUpdate(data: any): void {
    if (this.activeConnections.size > 0) {
      ...
      this.logger.debug('Dashboard update broadcasted', {
  ...
  private calculateAgentInsights(metrics: any[]): any {
    ...
    const avgErrorRate =
      metrics.reduce((sum, m) => sum + m.performance.errorRate, 0) / metrics.length;
