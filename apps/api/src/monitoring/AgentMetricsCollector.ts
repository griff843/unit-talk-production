...
export class AgentMetricsCollector extends EventEmitter {
  ...
  async getAggregatedMetrics(
    agentName: string,
    timeframe: string
  ): Promise<MetricsAggregation | null> {
    try {
  ...
  async getAllAgentsStatus(): Promise<
    Record<string, { status: string; score: number; lastUpdate: Date }>
  > {
  ...
  private async checkAlertRules(metrics: AgentMetrics): Promise<void> {
    const relevantRules = Array.from(this.alertRules.values()).filter(
      rule => rule.enabled && rule.agentName === metrics.agentName
    );
  ...
  private calculateAggregation(
    ...
    const totalErrors = metrics.reduce(
      (sum, m) => sum + m.performance.errorRate * m.business.operationsCompleted,
      0
    );
