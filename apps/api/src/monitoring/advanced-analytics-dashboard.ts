...
export class AdvancedAnalyticsDashboard extends EventEmitter {
  ...
  private metrics: UnitTalkMetrics;
  private server: http.Server | null = null;
  private wsServer: WebSocket.Server | null = null;
  private updateInterval: NodeJS.Timeout | null = null;
  private currentData: LiveMonitoringData | null = null;
    ...
    return new Promise((resolve, reject) => {
      this.server = http.createServer(async (req, res) => {
        ...
        try {
          ...
          logger.error('Analytics server request error:', error);
          res.writeHead(500);
      ...
      this.server.listen(port, () => {
        logger.info(`🌐 Advanced Analytics Server started on port ${port}`);
  ...
  private handleWebSocketConnection(ws: WebSocket): void {
    ...
    logger.info(`📡 New WebSocket client connected (${this.connectedClients.size} total)`);
   ...
   */
  private async getAnalyticsOverview(): Promise<any> {
    return {
      timestamp: new Date().toISOString(),
      summary: {
        totalAgents: 5,
   ...
   */
  private async getAgentAnalytics(agentName?: string): Promise<any> {
    if (!agentName) {
   ...
   */
  private async checkAlertConditions(data: LiveMonitoringData): Promise<void> {
   ...
   */
  private getAdvancedDashboardHTML(): string {
    return `
<!DOCTYPE html>
<html>
<head>
...
export async function createAdvancedAnalyticsDashboard(
