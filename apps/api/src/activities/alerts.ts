...
const logger = makeLogger('AlertActivities');
...
export async function sendDiscordAlert(params: {
  ...
  try {
    ...
    const prefixedMessage = params.channel === 'operator' ? `🚨 ${params.message}` : params.message;
...
export async function sendApprovedPicksAlert(params: {
  ...
  try {
    const message =
      `**New Approved Picks - Cycle ${params.cycleCount}**\n\n` +
      ...
      embeds: [
        {
          ...
          fields: [
            ...
            {
              name: 'Promoted Picks',
              value: String(params.report.promotedPicks || 0),
              inline: true,
            },
