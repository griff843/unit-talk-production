...
export async function generateConversation(data: {
  ...
  try {
    ...
    const message = await conversationEngine.generateResponse(
      data.userId,
      data.context,
      { 
        sentiment: 'neutral',
...
export async function scheduleIntervention(data: {
  ...
  try {
    ...
    const interventionId = await interventionSystem.scheduleIntervention(
      data.userId,
      {
...
export async function analyzeOnboardingProgress(data: {
  userId: string;
}): Promise<{ 
  success: boolean; 
  analysis?: {
...
export async function processConversionOpportunity(data: {
  ...
  try {
    ...
    const strategy = await conversationEngine.generateConversionMessage(
      data.userId,
      data.context
    );
