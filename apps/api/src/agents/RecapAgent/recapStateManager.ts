...
export class RecapStateManager {
  ...
  async isMicroRecapInCooldown(): Promise<boolean> {
    try {
      ...
      this.logger.error(
        'Failed to check micro-recap cooldown',
        error instanceof Error ? error : new Error(String(error))
      );
  ...
  async initializeStateTable(): Promise<boolean> {
    try {
      ...
      const { error: checkError } = await this.supabase.from(this.tableName).select('key').limit(1);
      ...
      this.logger.error(
        'Failed to initialize state table',
        error instanceof Error ? error : new Error(String(error))
      );
      return false;
    }
