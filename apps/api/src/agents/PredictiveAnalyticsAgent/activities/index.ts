...
export async function updateModelEnsemble(data: {
  ...
  try {
    ...
    const combinedAccuracy = models.reduce((sum, m) => 
      sum + (m.weight / totalWeight) * m.performance, 0
    );
    
    // Consensus professional_score (how much models agree)
    const performanceVariance = models.reduce((sum, m) => 
      sum + Math.pow(m.performance - combinedAccuracy, 2), 0
    ) / models.length;
