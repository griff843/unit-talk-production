...
// Simple interface for PerformanceTracker since the original is missing
interface PerformanceTracker {
  // Minimal interface - would be implemented by actual performance tracker
...
export class HealthChecker {
  ...
  private async checkPerformance(): Promise<ComponentHealth> {
    try {
      ...
      const failureRate = (metrics?.failed || 0) / (metrics?.total_processed || 1);

      return {
        status: failureRate < 0.05 ? 'healthy' : failureRate < 0.1 ? 'degraded' : 'unhealthy',
        latency_ms: (metrics?.processing_time_ms || 0) / (metrics?.total_processed || 1),
        last_success: new Date().toISOString()
      };
    } catch (error) {
      return {
