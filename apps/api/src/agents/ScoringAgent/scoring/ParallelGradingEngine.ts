...
export interface ParallelScoringTask<T = any> {
  ...
  fallback?: T;
  timeout?: number;
  critical?: boolean; // If true, failure will fail the entire scoring
}
...
export class ParallelScoringEngine {
  ...
  async calculateProfessionalInsightsParallel(
    ...
    const tasks: ParallelScoringTask[] = [
      {
        name: 'steamAnalysis',
   ...
   */
  async executeTasksInParallel(tasks: ParallelScoringTask[]): Promise<ParallelScoringResult> {
    const startTime = Date.now();
  ...
  private transformParallelResults(
    results: Record<string, any>, 
    features: ScoringFeatureSet
  ): ScoringResult['professionalInsights'] {
  ...
  async calculateProfessionalScoreParallel(
    ...
    const scoringTasks: ParallelScoringTask[] = [
      {
        name: 'steamScore',
        task: async () => {
