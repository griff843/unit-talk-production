...
import { Enhanced45FactorEngine, Enhanced45FactorResult, Factor45Config } from './Enhanced45FactorEngine';
...
describe('Enhanced45FactorEngine', () => {
  let engine: Enhanced45FactorEngine;
  let mockFeatureStore: jest.Mocked<FeatureStoreIntegration>;
  let mockChangeDetector: jest.Mocked<MaterialChangeDetector>;
  let mockFeatureStoreService: jest.Mocked<FeatureStoreService>;
  ...
  beforeEach(() => {
    ...
    engine = new Enhanced45FactorEngine(mockFeatureStore, mockChangeDetector);
  });
  ...
  describe('Batch Processing', () => {
    it('should process multiple props efficiently', async () => {
      ...
      const propsList = Array.from({ length: 100 }, (_, i) => 
        createMockGradingFeatureSet({ propId: `prop_${i}` })
      );
  ...
  describe('Performance Benchmarks', () => {
    ...
    it('should process 1000 props within performance targets', async () => {
      ...
      const propsList = Array.from({ length: 1000 }, (_, i) => 
        createMockGradingFeatureSet({ propId: `perf_test_${i}` })
      );
  ...
  describe('Configuration Management', () => {
    it('should accept custom factor weights', async () => {
      ...
      const customConfig: Partial<Factor45Config> = {
        marketFactors: {
...
function createMockGradingFeatureSet(overrides: Partial<GradingFeatureSet> = {}): GradingFeatureSet {
  return {
...
function createMockEnhancedFeatures(): any {
  return {
    lineHistory: [
    ...
    volumeData: [
      { timestamp: Date.now() - 1800000, volume: 1000, direction: 'up' as const },
      { timestamp: Date.now(), volume: 1200, direction: 'up' as const }
    ],
    relatedMarkets: [],
...
function createEmptyEnhancedFeatures(): any {
  return {
    lineHistory: [],
