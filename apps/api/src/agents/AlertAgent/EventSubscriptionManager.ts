...
export class EventSubscriptionManager {
  ...
  private async triggerImmediateAlert(alertType: string, alertData: any, aggregateId: string): Promise<void> {
  ...
  private calculateLineMovement(lineHistory: any[]): { original: number; current: number; change: number; direction: string } {
   ...
   */
  private async recordBridgeOutboxProcessing(outboxId: string, status: 'processed' | 'failed'): Promise<void> {
    try {
      await withCircuitBreaker.supabase(
        async () => {
          await this.supabase
