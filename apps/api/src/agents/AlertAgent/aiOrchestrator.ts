...
export class AIOrchestrator {
  ...
  async getAdviceForPick(pick: UnifiedPick, _context?: MarketContext): Promise<AIAdvice> {
  ...
  async getConsensusAdvice(pick: UnifiedPick): Promise<ConsensusAdvice> {
  ...
  private async selectOptimalModel(pick: UnifiedPick): Promise<ModelConfig | null> {
    const availableModels = this.getAvailableModels();
  ...
  private calculateModelScore(model: ModelConfig, pick: UnifiedPick): number {
    let modelScore = 0;
  ...
  private async queryModel(model: ModelConfig, pick: UnifiedPick): Promise<AIAdvice> {
    const startTime = Date.now();
...
Be concise but thorough. Focus on actionable insights that can improve betting outcomes.`;
  ...
  private buildPrompt(pick: UnifiedPick): string {
    const basePrompt = `
...
`;
  ...
  private async getFallbackAdvice(pick: UnifiedPick): Promise<AIAdvice> {
