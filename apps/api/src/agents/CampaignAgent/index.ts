...
let instance: CampaignAgent | null = null;
...
export class CampaignAgent extends BaseAgent {
  ...
  public async executeCampaign(params: CampaignParams): Promise<void> {
    await this.runCampaign(params);
  }

  public async validateCampaign(params: CampaignParams): Promise<void> {
  ...
  private async runCampaign(config: CampaignConfig): Promise<void> {
  ...
  // Public API
  public static getInstance(dependencies: BaseAgentDependencies): CampaignAgent {
    if (!instance) {
...
export function initializeCampaignAgent(dependencies: BaseAgentDependencies): CampaignAgent {
  // Create a default config since logger doesn't have config property
  const config: BaseAgentConfig = {
    name: 'CampaignAgent',
    enabled: true,
