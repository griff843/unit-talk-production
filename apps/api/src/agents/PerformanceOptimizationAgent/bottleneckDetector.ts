...
export class BottleneckDetector {
  ...
  async detectBottlenecks(metrics: PerformanceMetrics): Promise<PerformanceBottleneck[]> {
  ...
  private generateDescription(type: string, value: number, severity: string): string {
    const descriptions: Record<string, (v: number, s: string) => string> = {
  ...
  private generateRecommendations(type: string, severity: string, _value: number): string[] {
    // Value unused in recommendations
    ...
    const baseRecommendations = recommendationMap[type] || [
      'Review performance metrics and optimize accordingly',
    ];
  ...
  private calculateSystemDegradationScore(metrics: PerformanceMetrics): number {
    ...
    if (metrics.errorRate >= this.thresholds.errorRate.warning) {
      professional_score +=
        Math.min(1, metrics.errorRate / this.thresholds.errorRate.critical) * 0.3;
      factorCount++;
    }
    ...
    if (metrics.database >= this.thresholds.database.warning) {
      professional_score +=
        Math.min(1, metrics.database / this.thresholds.database.critical) * 0.15;
      factorCount++;
    }
