...
export class AdvancedBacktestingSystem {
  ...
  private calculateTierPerformance(results: any[]): Record<string, any> {
    ...
    Object.entries(tierGroups).forEach(([tier, tierResults]) => {
      ...
      const totalStake = tierResults.reduce(
        (sum, r) => sum + (r.prediction.positionSize || 0.01),
        0
      );
      const avgConfidence =
        tierResults.reduce((sum, r) => sum + r.prediction.confidence, 0) / tierResults.length;
  ...
  private calculateSportPerformance(results: any[]): Record<string, any> {
    ...
    Object.entries(sportGroups).forEach(([sport, sportResults]) => {
      ...
      const totalStake = sportResults.reduce(
        (sum, r) => sum + (r.prediction.positionSize || 0.01),
        0
      );
  ...
  private calculateMarketPerformance(results: any[]): Record<string, any> {
    ...
    Object.entries(marketGroups).forEach(([market, marketResults]) => {
      ...
      const totalStake = marketResults.reduce(
        (sum, r) => sum + (r.prediction.positionSize || 0.01),
        0
      );
      const avgOdds =
        marketResults.reduce((sum, r) => sum + Math.abs(r.pick.odds), 0) / marketResults.length;
  ...
  private calculateMonthlyPerformance(results: any[]): Array<any> {
    ...
    return Object.entries(monthlyGroups)
      .map(([month, monthResults]) => {
        ...
        const totalStake = monthResults.reduce(
          (sum, r) => sum + (r.prediction.positionSize || 0.01),
          0
        );
  ...
  private calculateWeeklyPerformance(results: any[]): Array<any> {
    ...
    return Object.entries(weeklyGroups)
      .map(([week, weekResults]) => {
        ...
        const totalStake = weekResults.reduce(
          (sum, r) => sum + (r.prediction.positionSize || 0.01),
          0
        );
  ...
  private calculateStreakAnalysis(results: any[]): any {
    ...
    const avgWinStreak =
      winStreaks.length > 0 ? winStreaks.reduce((sum, s) => sum + s, 0) / winStreaks.length : 0;
    const avgLoseStreak =
      loseStreaks.length > 0 ? loseStreaks.reduce((sum, s) => sum + s, 0) / loseStreaks.length : 0;
  ...
  private calculateOddsAnalysis(results: any[]): any {
    ...
    const favoriteStake = favorites.reduce(
      (sum, r) => sum + (r.prediction.positionSize || 0.01),
      0
    );
    const underdogStake = underdogs.reduce(
      (sum, r) => sum + (r.prediction.positionSize || 0.01),
      0
    );
  ...
  private calculateConfidenceAnalysis(results: any[]): any {
    ...
    const medium = results.filter(
      r => r.prediction.confidence >= 60 && r.prediction.confidence < 80
    );
    const low = results.filter(r => r.prediction.confidence < 60);

    const analyzeGroup = (group: any[]) => {
      if (group.length === 0) {
        return { winRate: 0, roi: 0, count: 0 };
      }
    ...
    return [
      {
        ...
        importance: 0.25,
        correlation: 0.78,
        performance: featurePerformance.expectedValue || 0.82,
      },
      {
        feature: 'marketIntelligence',
        importance: 0.2,
        correlation: 0.72,
        performance: featurePerformance.marketIntelligence || 0.79,
      },
      {
        feature: 'playerForm',
        importance: 0.15,
        correlation: 0.65,
        performance: featurePerformance.playerForm || 0.75,
      },
      {
        feature: 'matchupRating',
        importance: 0.15,
        correlation: 0.68,
        performance: featurePerformance.matchupRating || 0.77,
      },
      {
        feature: 'lineMovement',
        importance: 0.12,
        correlation: 0.58,
        performance: featurePerformance.lineMovement || 0.71,
      },
    ];
  }
  ...
  private generateRecommendations(metrics: AdvancedPerformanceMetrics): string[] {
    ...
    if (metrics.sharpeRatio < 1.0) {
      recommendations.push(
        'Focus on risk-adjusted returns by reducing position sizes in volatile markets'
      );
    }
  ...
  private generateOptimizationSuggestions(metrics: AdvancedPerformanceMetrics): Array<any> {
    ...
    const bestTier = Object.entries(metrics.tierPerformance).sort((a, b) => b[1].roi - a[1].roi)[0];
    ...
    // Sport optimization
    const bestSport = Object.entries(metrics.sportPerformance).sort(
      (a, b) => b[1].roi - a[1].roi
    )[0];
