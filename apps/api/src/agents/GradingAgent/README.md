...
The GradingAgent is the core business intelligence engine that grades and scores
...
```typescript
interface GradingAgentConfig extends BaseAgentConfig {
...
```typescript
const gradingAgent = new GradingAgent(config, dependencies);

// Grade a single pick
const result = await gradingAgent.gradePick(pick);

// Batch process multiple picks
const results = await gradingAgent.processBatch(picks);
```
...
const gradingResult = await proxyActivities<GradingActivities>({
  startToCloseTimeout: '5m',
  retry: { maximumAttempts: 3 },
}).gradeAndPromotePicks(picks);
```
...
```bash
# Unit tests
npm test src/agents/GradingAgent

# Integration tests
npm run test:integration -- --grep "GradingAgent"

# Performance tests
npm run test:performance -- --agent=grading
```
...
```bash
# Agent health check
npm run agents:test -- --agent=grading

# Performance analysis
npm run scripts:performance -- --focus=grading

# Batch processing test
npm run test:grading-batch
```
