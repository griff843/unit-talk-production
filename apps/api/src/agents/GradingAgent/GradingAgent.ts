...
export class GradingAgent extends BaseAgent {
  ...
  // Enhanced grading methods with parallel processing optimization
  async gradeProp(features: GradingFeatureSet): Promise<GradingResult> {
    ...
    try {
      ...
      this.gradingMetrics.avgGradingTimeMs =
        (this.gradingMetrics.avgGradingTimeMs + executionTime) / 2;
  ...
  private async processBatched(props: GradingFeatureSet[]): Promise<GradingResult[]> {
    ...
    const failedProps: GradingFeatureSet[] = [];

    // Process all batches in parallel for maximum throughput
    const batchPromises = batches.map(async (batch, index) => {
      try {
        const results = await this.gradeProps(batch);
        ...
        const recoveredResults: GradingResult[] = [];
        const stillFailedProps: GradingFeatureSet[] = [];

        for (const prop of batch) {
          try {
            const result = await this.gradeProp(prop);
            recoveredResults.push(result);
          } catch (propError) {
            this.logger.warn('⚠️ Individual prop grading failed, marking for retry', {
              propId: prop.propId,
   ...
   */
  private async scheduleFailedPropsForRetry(failedProps: GradingFeatureSet[]): Promise<void> {
    if (!this.hasSupabase()) {
    ...
    if (this.USE_PRO_SCORER) {
      ...
      try {
        ...
        if (rawProps && rawProps.length > 0) {
          const validation = DataValidationGates.validatePropBatch(rawProps);
