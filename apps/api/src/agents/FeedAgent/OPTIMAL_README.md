...
This document describes the implementation of the Optimal API feed integration
for the Unit Talk platform.

## Overview

The Optimal API integration provides a new feed source for sports betting props,
implementing batch fetching, normalization, and rate limiting according to the
...
- Maps `api_key` to `stat_type`, `display_name` to `label`, `abbreviation` to
  `abbr`
...
- Handles all required fields: `player_id`, `player_name`, `stat_type`, `label`,
  `abbr`, `line`, `odds`, `bookmaker`, `start_time`
...
The Optimal integration is designed to work seamlessly with the existing
FeedAgent workflow:

1. **Provider Addition**: Added 'Optimal' to the `ProviderSchema` enum
2. **Type Compatibility**: Returns `RawProp[]` compatible with existing
   processing
