...
export async function calculatePortfolioRisk(data: {
  ...
  logger.info('📊 Calculating portfolio risk', { portfolioId: data.portfolioId });
  ...
  try {
    ...
    const weightedVolatility = positions.reduce(
      (sum, pos) => sum + (pos.allocation / totalAllocation) * pos.volatility,
      0
    );

    // Calculate diversification professional_score (inverse of concentration)
    const concentrationIndex = positions.reduce(
      (sum, pos) => sum + Math.pow(pos.allocation / totalAllocation, 2),
      0
    );
    ...
    const riskLevel =
      totalRisk > 0.3 ? 'extreme' : totalRisk > 0.2 ? 'high' : totalRisk > 0.1 ? 'medium' : 'low';
...
export async function optimizePortfolio(data: {
  ...
  try {
    ...
    const totalRecommended = recommendedAllocations.reduce(
...
export async function assessDrawdownRisk(data: {
  userId: string;
  ...
  logger.info('📉 Assessing drawdown risk', {
    userId: data.userId,
  ...
  try {
    ...
    const maxDrawdown = Math.min(
      ...historicalReturns.map((_, i) => Math.min(...historicalReturns.slice(0, i + 1)))
    );
    ...
    logger.error('❌ Failed to assess drawdown risk', { error });
    return { success: false };
  }
}

export async function checkRiskManagementAgentHealth(): Promise<{
