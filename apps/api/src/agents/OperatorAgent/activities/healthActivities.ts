 ...
 */
export async function executeHealthSnapshot(): Promise<{
  success: boolean;
  ...
  try {
    ...
    logger.info('🔍 Executing system_health_snapshot query');
    ...
    if (error) {
      logger.error('❌ Health snapshot query failed', { 
        error: error.message,
        code: error.code,
    ...
    const healthData: HealthSection[] = data.health_data;
    const executionTime = Date.now() - startTime;

    logger.info('✅ Health snapshot query successful', {
    ...
    logger.error('❌ Health snapshot execution failed', {
      error: errorMessage,
      executionTime
    });
...
export async function logJobRun(params: {
  ...
  try {
    ...
    if (error) {
      logger.error('❌ Failed to log job run', { 
        error: error.message,
        params
      });
    ...
    logger.info('📝 Job run logged successfully', {
      jobId,
      agent: params.agent,
      workflow: params.workflow,
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error('❌ Job run logging failed', { error: errorMessage, params });
...
export async function completeJobRun(params: {
  ...
  try {
    ...
    if (error) {
      logger.error('❌ Failed to complete job run', { 
        error: error.message,
        jobId: params.jobId,
        status: params.status
      });
    ...
    logger.info('✅ Job run completed successfully', {
      jobId: params.jobId,
      status: params.status,
      duration,
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error('❌ Job run completion failed', { 
      error: errorMessage, 
      jobId: params.jobId,
      status: params.status
    });
 ...
 */
export async function createIncidents(params: {
  incidents: Array<{
    kind: string;
  ...
  try {
    ...
    logger.info('🚨 Creating health incidents', {
    ...
    for (const incident of params.incidents) {
      ...
      const { data, error } = await supabaseClient
        ...
        if (data) {
          ...
          logger.info('⏭️ Incident skipped (duplicate)', {
            kind: incident.kind,
            section: incident.section
          });
        }
        ...
        logger.error('❌ Individual incident creation failed', {
          error: errorMessage,
          incident: incident.kind
        });
      }
    ...
    logger.info('📊 Incident creation summary', {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error('❌ Incident creation batch failed', { error: errorMessage });
 ...
 */
export async function pushToCommandCenter(params: {
  healthData: HealthSection[];
  timestamp: string;
  executionMetrics: {
  ...
  try {
    logger.info('📡 Pushing health data to Command Center', {
      endpoint,
    ...
    try {
      ...
      logger.info('✅ Command Center push successful', {
        endpoint,
        responseTime,
      ...
      logger.error('❌ Command Center push failed', {
        endpoint,
        responseTime,
        error: errorMessage
      });
    ...
    logger.error('❌ Unexpected error in pushToCommandCenter', {
      endpoint,
      responseTime,
      error: errorMessage
    });
 ...
 */
export async function sendHealthAlert(params: {
  ...
  try {
    ...
    const embedColor = params.priority === 'high' ? 0xFF0000 : params.priority === 'medium' ? 0xFFA500 : 0x00FF00;
    const emoji = params.alertType === 'critical_incidents' ? '🚨' : params.alertType === 'system_degraded' ? '⚠️' : 'ℹ️';
    ...
    if (webhookUrl) {
      ...
      logger.warn('⚠️ No Discord webhook URL configured - alert not sent', {
        alertType: params.alertType,
        priority: params.priority
      });
    }
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error('❌ Health alert failed', {
      error: errorMessage,
      alertType: params.alertType,
      priority: params.priority
    });
...
// Helper functions
function getAlertColor(priority: string): number {
  switch (priority) {
