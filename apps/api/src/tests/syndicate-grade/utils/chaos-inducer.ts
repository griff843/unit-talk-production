...
export class ChaosInducer extends EventEmitter {
  private config: ChaosInducerConfig;
  private activeChaosEvents: Map<string, ChaosEvent> = new Map();
  private chaosHistory: ChaosEvent[] = [];
  ...
  async stop(): Promise<void> {
    ...
    const stopPromises = Array.from(this.activeChaosEvents.values()).map(event => 
   ...
   */
  async induceMildChaos(chaosType: string, duration: number = 5000): Promise<string> {
    if (!this.isActive) {
   ...
   */
  async injectNetworkPartition(config: any): Promise<void> {
    const event: ChaosEvent = {
   ...
   */
  async injectMemoryPressure(config: any): Promise<void> {
    const event: ChaosEvent = {
   ...
   */
  async injectCPUStarvation(config: any): Promise<void> {
    const event: ChaosEvent = {
   ...
   */
  async injectDatabaseConnectionFailure(config: any): Promise<void> {
    const event: ChaosEvent = {
   ...
   */
  async injectAPIRateLimiting(config: any): Promise<void> {
    const event: ChaosEvent = {
   ...
   */
  async injectProgressiveFailures(config: any): Promise<void> {
   ...
   */
  async injectServiceFailures(config: any): Promise<void> {
    const event: ChaosEvent = {
  ...
  private async executeChaosEvent(event: ChaosEvent): Promise<void> {
    switch (event.type) {
  ...
  private async stopChaosEvent(eventId: string): Promise<void> {
  ...
  private async cleanupChaosEvent(event: ChaosEvent): Promise<void> {
    switch (event.type) {
      case 'memory_pressure':
  ...
  private getChaosParameters(chaosType: string, intensity: number): any {
  ...
  private async simulateNetworkLatency(event: ChaosEvent): Promise<void> {
  ...
  private async simulateNetworkPartition(event: ChaosEvent): Promise<void> {
  ...
  private networkPartitionActive: boolean = false;
  private networkPartitionConfig: any = {};
  
  private async simulateMemoryPressure(event: ChaosEvent): Promise<void> {
    ...
    (event as any).memoryChunks = chunks;
  }
  
  private async simulateCPUStarvation(event: ChaosEvent): Promise<void> {
    ...
    (event as any).cpuWorkerActive = true;
  }
  
  private async simulateDatabaseFailures(event: ChaosEvent): Promise<void> {
  ...
  private databaseFailureActive: boolean = false;
  private databaseFailureRate: number = 0;
  private databaseAffectedOperations: string[] = [];
  
  private async simulateAPIRateLimiting(event: ChaosEvent): Promise<void> {
  ...
  private apiRateLimitActive: boolean = false;
  private apiRateLimitConfig: any = {};
  
  private async simulateServiceFailures(event: ChaosEvent): Promise<void> {
  ...
  private serviceFailuresActive: boolean = false;
  private serviceFailureConfig: any = {};
  
  private stopCPUIntensiveOperations(eventId: string): void {
    // Stop CPU intensive operations for the given event
    console.log(`🛑 Stopping CPU intensive operations for ${eventId}`);
  }
  
  private restoreNetworkConnectivity(eventId: string): void {
    // Restore network connectivity
  ...
  public isNetworkPartitionActive(): boolean {
    return this.networkPartitionActive;
  }
  
  public isDatabaseFailureActive(): boolean {
    return this.databaseFailureActive;
  }
  
  public isAPIRateLimitActive(): boolean {
    return this.apiRateLimitActive;
  }
  
  public isServiceFailureActive(): boolean {
    return this.serviceFailuresActive;
  }
  
  public shouldSimulateNetworkFailure(): boolean {
    if (!this.networkPartitionActive) return false;
    return this.random() < this.networkPartitionConfig.affectedPercentage;
  }
  
  public shouldSimulateDatabaseFailure(operation: string): boolean {
    if (!this.databaseFailureActive) return false;
    if (!this.databaseAffectedOperations.includes(operation)) return false;
    return this.random() < this.databaseFailureRate;
  }
  
  public shouldSimulateAPIRateLimit(apiName: string): boolean {
    if (!this.apiRateLimitActive) return false;
    if (!this.apiRateLimitConfig.affectedAPIs.includes(apiName)) return false;
    return this.random() < 0.8; // 80% chance of rate limiting when active
  }
  
  public shouldSimulateServiceFailure(serviceName: string): boolean {
    if (!this.serviceFailuresActive) return false;
    if (!this.serviceFailureConfig.services.includes(serviceName)) return false;
