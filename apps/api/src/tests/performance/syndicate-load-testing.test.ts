...
describe('Syndicate-Grade Load Testing', () => {
    ...
    test('should process 8K+ props within 30 seconds', async () => {
      ...
      const processingPromises = batches.map(async (batch, batchIndex) => {
        try {
          ...
          if (batchIndex % 10 === 0) {
            console.log(
              `✅ Batch ${batchIndex + 1}/${batches.length} processed in ${batchDuration.toFixed(2)}ms`
            );
            console.log(
              `📈 Progress: ${processedCount}/${propCount} props (${((processedCount / propCount) * 100).toFixed(1)}%)`
            );
          }
    ...
    test('should maintain <1s alert latency under full load', async () => {
      ...
      const alertMonitoring = setInterval(async () => {
        ...
        try {
          ...
          console.log(
            `🚨 Alert latency: ${latency.toFixed(2)}ms (max: ${maxLatency.toFixed(2)}ms)`
          );
        } catch (error) {
      ...
      const p95Latency = alertLatencies.sort((a, b) => a - b)[
        Math.floor(alertLatencies.length * 0.95)
      ];
  ...
  describe('Resource Management Under Load', () => {
    test('should maintain memory usage within limits', async () => {
      ...
      const memoryMonitoring = setInterval(() => {
        ...
        console.log(
          `💾 Memory: ${(memory.heapUsed / 1024 / 1024).toFixed(2)}MB (growth: ${(growth / 1024 / 1024).toFixed(2)}MB)`
        );
      }, snapshotInterval);
    ...
    test('should handle database connection pooling efficiently', async () => {
      ...
      console.log(
        `🗄️  Testing database connection pooling with ${concurrentQueries} concurrent queries`
      );
      ...
      const queryPromises = Array.from({ length: concurrentQueries }, async (_, index) => {
        ...
        try {
          ...
          if (index % 10 === 0) {
            console.log(
              `🔄 Query batch ${index + 1}/${concurrentQueries} completed in ${queryTime.toFixed(2)}ms`
            );
          }
    ...
    test('should maintain <100ms agent coordination overhead', async () => {
      ...
      for (let i = 0; i < testIterations; i++) {
        ...
        try {
          ...
          if (i % 20 === 0 && i > 0) {
            const avgTime =
              coordinationTimes.reduce((sum, time) => sum + time, 0) / coordinationTimes.length;
            console.log(
              `🔄 Iteration ${i}/${testIterations}, avg coordination: ${avgTime.toFixed(2)}ms`
            );
          }
        } catch (error) {
      ...
      const avgCoordination =
        coordinationTimes.reduce((sum, time) => sum + time, 0) / coordinationTimes.length;
      const p95Coordination = coordinationTimes.sort((a, b) => a - b)[
        Math.floor(coordinationTimes.length * 0.95)
      ];
  ...
  async function cleanupLoadTestData() {
    ...
    try {
      ...
      await supabaseClient.from('raw_props').delete().like('id', 'load-test-prop-%');

      await supabaseClient.from('unified_picks').delete().like('prop_id', 'load-test-prop-%');
