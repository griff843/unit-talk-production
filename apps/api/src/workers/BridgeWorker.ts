...
export class BridgeWorker extends BaseAgent {
  ...
  private async triggerGradingWorkflow(ticketId: string, ticketData: any, idempotencyKey: string): Promise<void> {
  ...
  private async checkForHedgeMiddleOpportunities(gradingId: string, gradingData: any): Promise<void> {
  ...
  private async emitHighTierAlert(gradingId: string, gradingData: any, cooldownMultiplier: number = 1.0): Promise<void> {
  ...
  private async handleEventProcessingError(event: EventRecord, error: any, processingTime: number): Promise<void> {
    const errorMessage = error instanceof Error ? error.message : String(error);
  ...
  private async logProcessingFailure(event: EventRecord, errorMessage: string, processingTime: number): Promise<void> {
    if (!this.hasSupabase()) return;
