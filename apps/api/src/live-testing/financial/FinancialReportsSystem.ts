...
export class FinancialReportsSystem extends EventEmitter implements FinancialReports {
  private logger = new Logger('FinancialReportsSystem');

  public realTimePnL: RealTimePnL;
  public transactionCosts: TransactionCostAnalysis;
  public attribution: PerformanceAttribution;
  public riskReports: RiskReport[];

  private betPnLHistory: BetPnL[] = [];
  ...
  constructor(initialBankroll: number = 2000) {
    super();
  ...
  private initializeFinancialReports(initialBankroll: number): void {
    this.realTimePnL = {
      sessionPnL: 0,
      unrealizedPnL: 0,
      realizedPnL: 0,
  ...
  private startReportingUpdates(): void {
  ...
  public trackBetPlacement(bet: LiveBet, transactionCosts?: TransactionCosts): void {
  ...
  private recordTransactionCosts(betId: string, costs: TransactionCosts): void {
  ...
  private updateRealTimePnL(): void {
    ...
    const sessionBets = settledBets.filter(bet =>
      new Date(bet.timestamp) >= sessionStart
    );
  ...
  private calculateRiskAdjustedMetrics(settledBets: BetPnL[]): void {
  ...
  private calculateMaxDrawdown(settledBets: BetPnL[]): number {
    let peak = 0;
    let maxDrawdown = 0;
    let runningPnL = 0;
  ...
  private calculateBetaAlpha(returns: number[]): { beta: number; alpha: number } {
  ...
  private updatePnLByCategory(): void {
    // P&L by sport
  ...
  private updateTransactionCosts(): void {
  ...
  private getTimingRecommendations(): string[] {
  ...
  private updatePerformanceAttribution(): void {
  ...
  private calculateFeatureAttribution(): void {
    const settledBets = this.betPnLHistory.filter(bet => bet.result !== 'PENDING');
  ...
  private calculateTimingAttribution(): void {
    const settledBets = this.betPnLHistory.filter(bet => bet.result !== 'PENDING');
  ...
  private calculateKellyAttribution(): void {
  ...
  private calculateLineShoppingAttribution(): void {
  ...
  private calculateProfessionalFeaturesAttribution(): void {
  ...
  private generateAttributionExplanations(): void {
    const explanations: string[] = [];

    // Best performing sport
    const sportPnL = this.attribution.attributions.sport;
    const bestSport = Object.entries(sportPnL).reduce((best, [sport, pnl]) =>
      pnl > (sportPnL[best[0]] || -Infinity) ? [sport, pnl] : best
    , ['', -Infinity]);
  ...
  public generateRiskReport(date: string = new Date().toISOString().split('T')[0]): RiskReport {
    const settledBets = this.betPnLHistory.filter(bet =>
      bet.result !== 'PENDING' && bet.settledAt?.startsWith(date)
    );
  ...
  private calculatePortfolioVolatility(bets: BetPnL[]): number {
  ...
  private calculateCorrelationRisk(bets: BetPnL[]): number {
  ...
  private calculateConcentrationRisk(bets: BetPnL[]): number {
  ...
  private calculateDaysSinceStart(): number {
    if (this.betPnLHistory.length === 0) return 0;
  ...
  public exportPnLData(format: 'json' | 'csv' = 'json'): string {
    if (format === 'csv') {
