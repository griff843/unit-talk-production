...
import { supabaseClient, requireSupabase } from '../../utils/supabaseUtils';
import {
  LiveBet,
  LiveTestingConfig,
  DailyResult,
...
export class LiveResultsStorage {
  private logger = new Logger('LiveResultsStorage');
  ...
  public async storeLiveBet(bet: LiveBet): Promise<void> {
    try {
      const betRecord = {
        id: bet.id,
        prop_id: bet.propId,
  ...
  public async updateLiveBetResult(
    betId: string,
  ...
  private async getBetStake(betId: string): Promise<number> {
    if (!supabaseClient) {
      throw new Error('Supabase client not initialized');
    }
  ...
  public async storeDailyReport(report: DailyResult): Promise<void> {
    try {
      const reportRecord = {
        date: report.date,
  ...
  public async storeWeeklyReport(report: WeeklyResult): Promise<void> {
    try {
      const reportRecord = {
        week: report.week,
        date: report.date,
  ...
  public async storePhaseReport(report: PhaseResult): Promise<void> {
    try {
      const reportRecord = {
        phase: report.phase,
  ...
  public async storeCLVTrack(track: CLVTrack): Promise<void> {
    try {
      const trackRecord = {
        bet_id: track.betId,
  ...
  public async storeRiskReport(report: RiskReport): Promise<void> {
    try {
      const reportRecord = {
  ...
  public async getPhaseReports(): Promise<PhaseResult[]> {
    try {
      if (!supabaseClient) {
        throw new Error('Supabase client not initialized');
      }
  ...
  public async getPerformanceByPhase(): Promise<PhaseResult[]> {
    try {
      if (!supabaseClient) {
        throw new Error('Supabase client not initialized');
      }
  ...
  public async getPerformanceBySport(): Promise<SportResult[]> {
    try {
      if (!supabaseClient) {
        throw new Error('Supabase client not initialized');
      }
      ...
      return (data || []).map(row => ({
        ...
        confidence: this.calculateConfidence(
          parseInt(row.total_bets),
          parseFloat(row.win_rate),
          parseFloat(row.roi_stddev)
        )
      }));
  ...
  public async exportAllData(format: 'json' | 'csv' = 'json'): Promise<string> {
    try {
      const data = {
        metadata: {
  ...
  private convertDataToCSV(data: any): string {
    ...
    if (data.liveBets.length > 0) {
      ...
      const betRows = data.liveBets.map((bet: LiveBet) => [
        bet.id,
        bet.sport,
        bet.market,
        bet.stake,
    ...
    if (data.dailyReports.length > 0) {
      ...
      const dailyRows = data.dailyReports.map((report: DailyResult) => [
        report.date,
        report.totalBets,
  ...
  public async createTablesIfNotExist(): Promise<void> {
    try {
      ...
      for (const table of tables) {
        try {
          const supabase = requireSupabase();
          await supabase.rpc('execute_sql', { sql: table.schema });
  ...
  private convertToLiveBet(record: any): LiveBet {
    return {
      id: record.id,
  ...
  private convertToDailyResult(record: any): DailyResult {
    return {
      date: record.date,
  ...
  private convertToPhaseResult(record: any): PhaseResult {
    return {
      phase: record.phase,
  ...
  private convertToCLVTrack(record: any): CLVTrack {
    return {
      betId: record.bet_id,
      propId: record.prop_id,
