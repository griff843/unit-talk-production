/* eslint-disable max-lines, @typescript-eslint/no-unused-vars, no-unused-vars, no-undef, import/order, no-script-url, max-depth */
...
export class SecurityTester {
  ...
  private async testPasswordStrengthRequirements(): Promise<QATestResult[]> {
    ...
    try {
      ...
      // Test weak passwords
      const weakPasswords = ['123456', 'password', 'qwerty', 'abc123', '12345678', 'password123'];
      ...
      for (const weakPassword of weakPasswords) {
        ...
        const errorVisible = await page.isVisible('[data-testid="password-error"]', {
          timeout: 2000,
        });
  ...
  private async testBruteForceProtection(): Promise<QATestResult[]> {
    ...
    try {
      ...
      for (let i = 1; i <= maxAttempts; i++) {
        ...
        const isBlocked = await page.isVisible(
          '[data-testid="account-blocked"], [data-testid="rate-limited"]',
          { timeout: 3000 }
        );
      ...
      for (const test of csrfTests) {
        try {
          ...
          const response = await page.evaluate(
            async (testData: { url: string; method: string }) => {
  ...
  private async testContentSecurityPolicy(): Promise<QATestResult[]> {
    ...
    try {
      ...
      if (!cspHeader) {
        ...
        const unsafeDirectives = Object.entries(cspAnalysis).filter(
  ...
  private async loginAsTestUser(page: any): Promise<void> {
    ...
    await page.fill(
      ...
      this.config.environment.credentials?.testUser?.password || 'defaultPassword123'
    );
    await page.click('[data-testid="login-submit"]');
