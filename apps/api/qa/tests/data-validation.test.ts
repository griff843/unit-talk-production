/* eslint-disable max-lines, @typescript-eslint/no-unused-vars, no-unused-vars, complexity */
...
export class DataValidationTester {
    ...
    for (const test of userValidationTests) {
      ...
      try {
        ...
        const validationResult = await this.validateInputs(
          ...
          test.validInputs,
          test.invalidInputs,
          test.name.toLowerCase().includes('username')
            ? 'input[name="username"]'
            : test.name.toLowerCase().includes('email')
              ? 'input[name="email"]'
              : 'input[name="password"]'
        );
        ...
        results.push(
          createQATestResult(`User Data - ${test.name}`, status, message, Date.now() - startTime, {
    ...
    for (const test of picksValidationTests) {
      ...
      try {
        ...
        const validationResult = await this.validateInputs(
          ...
          test.validInputs,
          test.invalidInputs,
          test.name.toLowerCase().includes('odds')
            ? 'input[name="odds"]'
            : test.name.toLowerCase().includes('stake')
              ? 'input[name="stake"]'
        ...
        results.push(
          createQATestResult(`Picks Data - ${test.name}`, status, message, Date.now() - startTime, {
            details: validationResult,
          })
        );
  ...
  private async testAnalyticsDataValidation(): Promise<QATestResult[]> {
    ...
    try {
      ...
      results.push(
        createQATestResult('Analytics Data Validation', status, message, Date.now() - startTime, {
  ...
  private async testDataConsistency(): Promise<QATestResult[]> {
    ...
    try {
      ...
      results.push(
        createQATestResult('Data Consistency', status, message, Date.now() - startTime, {
  ...
  private async testDataIntegrity(): Promise<QATestResult[]> {
    ...
    try {
      ...
      results.push(
        createQATestResult('Data Integrity', status, message, Date.now() - startTime, {
  ...
  private async validateInputs(
    ...
    for (const input of validInputs) {
      try {
        ...
        const hasError =
          (await page.locator('.error, .invalid, [aria-invalid="true"]').count()) > 0;
        if (hasError) {
