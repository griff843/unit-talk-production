...
export class WorkflowTester {
  ...
  private async testPickSubmissionWorkflow(): Promise<QATestResult[]> {
    ...
    try {
      ...
      const pickExists = await page.isVisible(
        '[data-testid="pick-item"]:has-text("Chiefs vs Bills")'
      );
  ...
  private async testParlayCreationWorkflow(): Promise<QATestResult[]> {
    ...
    try {
      ...
      const parlayExists = await page.isVisible(
        '[data-testid="parlay-item"]:has-text("QA Test Parlay")'
      );
  ...
  private async loginAsTestUser(page: any): Promise<void> {
    ...
    await page.fill(
      ...
      this.config.environment.credentials?.testUser?.email || 'test@example.com'
    );
    await page.fill(
      '[data-testid="password-input"]',
      this.config.environment.credentials?.testUser?.password || 'password'
    );
    await page.click('[data-testid="login-submit"]');
    await page.waitForSelector('[data-testid="dashboard"]');
  }
  ...
  private async loginAsAdminUser(page: any): Promise<void> {
    ...
    await page.fill(
      '[data-testid="email-input"]',
      this.config.environment.credentials?.adminUser?.email || 'admin@example.com'
    );
    await page.fill(
      '[data-testid="password-input"]',
      this.config.environment.credentials?.adminUser?.password || 'admin123'
    );
    await page.click('[data-testid="login-submit"]');
    await page.waitForSelector('[data-testid="dashboard"]');
  }
  ...
  private async simulateEmailVerification(page: any, email: string): Promise<void> {
    ...
    if (this.config.environment.name !== 'production') {
      await page.goto(
        `${this.config.environment.baseUrl}/verify-email?token=test-token&email=${encodeURIComponent(email)}`
      );
