 ...
 * Generate comprehensive report for all games on 2025-11-28
...
async function generateReport() {
  ...
  try {
    ...
    // SQL #1: All games for today
    console.log('Running SQL #1: All games for today...');
    const query1 = `
      SELECT
        id,
        league,
    ...
    // SQL #4: Per-league props counts for today
    ...
    const reportPath = join(outputDir, 'TODAYS_GAMES_ACROSS_ALL_SPORTS.md');
    writeFileSync(reportPath, markdown);

    console.log(`\n✓ Report saved to: ${reportPath}`);
    console.log('\nDB_TODAYS_GAMES_REPORT_COMPLETE for 2025-11-28');
...
function generateMarkdown(data: {
  ...
  return `# Today's Games Across All Sports
...
**Date Stability**: Fixed to DATE '2025-11-28' for consistency
`;
}
...
function generateExplicitFindings(data: {
  ...
  if (data.games.length === 0) {
    findings.push('- **ZERO games** found in the database for 2025-11-28');
  } else {
    findings.push(`- **${data.games.length} total games** found for 2025-11-28`);
  }
  ...
  if (data.leaguePropsCount.length === 0) {
    findings.push('- **ZERO props** found for 2025-11-28');
  } else {
