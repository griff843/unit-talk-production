...
class AIModelRouter {
     ...
     */
    async callModel(modelName, request) {
        const startTime = Date.now();
     ...
     */
    async simulateModelCall(config, request) {
     ...
     */
    generateAdviceResponse(request, config) {
     ...
     */
    generateRecapResponse(request, config) {
     ...
     */
    calculateOddsBpsDifference(currentOdds, lastOdds) {
        if (lastOdds === 0)
            return Infinity;
     ...
     */
    shouldRequery(currentOdds, lastOdds, contextHash, lastContextHash) {
...
exports.aiModelRouter = AIModelRouter.getInstance();
