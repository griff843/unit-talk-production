...
const logger = (0, logger_1.createLogger)('OpsRouter');
...
router.post('/ingest-now', async (req, res) => {
    const runId = `ingest-${Date.now()}-${crypto_1.default.randomBytes(4).toString('hex')}`;
    ...
    try {
        ...
        const response = {
            ...
            monitoringUrl: env.TEMPORAL_SERVER_URL ? `${env.TEMPORAL_SERVER_URL}/namespaces/default/workflows/${workflowId}` : undefined,
            timestamp: new Date().toISOString(),
            endpoints: {
    ...
    try {
        ...
        try {
            ...
            const response = {
                ...
                // runTime not available in typed SDK, compute approx from executionTime/startTime if needed
