...
exports.ABTestingEngine = void 0;
const logger_1 = require("../../utils/logger");
class ABTestingEngine {
    constructor(supabase, featureFlagService) {
        this.logger = (0, logger_1.createLogger)('ABTestingEngine');
        // Statistical thresholds
        this.SIGNIFICANCE_THRESHOLD = 0.05; // 95% confidence
        this.MINIMUM_SAMPLE_SIZE = 1000;
        this.MINIMUM_TEST_DURATION_HOURS = 24;
        this.POWER_THRESHOLD = 0.8; // 80% statistical power
     ...
     */
    async analyzeTest(flagName, options = {}) {
        const { minHours = this.MINIMUM_TEST_DURATION_HOURS, minSampleSize = this.MINIMUM_SAMPLE_SIZE } = options;
        try {
            ...
            const { pValue, confidenceInterval, testPower } = this.calculateStatisticalSignificance(controlConversions, controlResults.length, treatmentConversions, treatmentResults.length);
     ...
     */
    async comparePerformance(flagName, metric, thresholds) {
        try {
            ...
            const significancePValue = this.performTTest(controlValues, treatmentValues);
     ...
     */
    async checkRollbackConditions(flagName) {
        const reasons = [];
        let severity = 'low';
        try {
     ...
     */
    async generateTestReport(flagName) {
        try {
            ...
            const validComparisons = performanceComparisons.filter(c => c !== null);
            // Generate recommendations
            const recommendations = this.generateRecommendations(analysis, validComparisons, rollbackAssessment);
            return {
                analysis,
    ...
    calculatePower(effectSize, sampleSize) {
        // Simplified power calculation
    ...
    performTTest(sample1, sample2) {
        ...
        if (n1 < 2 || n2 < 2)
            return 1.0; // No significance with small samples
    ...
    determineSignificanceLevel(pValue) {
        if (pValue < 0.01)
            return 'high';
        if (pValue < 0.05)
            return 'medium';
    ...
    determineRecommendedAction(params) {
    ...
    async checkCriticalMetrics(flagName) {
        const issues = [];
        try {
    ...
    generateRecommendations(analysis, comparisons, rollbackAssessment) {
            ...
            for (const flag of activeFlags) {
                if (flag.ab_test_groups && Array.isArray(flag.ab_test_groups) && flag.ab_test_groups.length > 0) {
                    ...
                    if (rollbackCheck.shouldRollback && rollbackCheck.severity === 'critical') {
                        ...
                        await this.featureFlagService.emergencyRollback(flag.flag_name, `Automatic rollback triggered: ${rollbackCheck.reasons.join(', ')}`);
                    }
                }
            }
        }
