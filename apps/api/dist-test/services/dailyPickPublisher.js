...
class DailyPickPublisher {
    ...
    async publishDailyPicks() {
        ...
        try {
            ...
            const picks = await capperService_1.capperService.getCapperPicks('', today, 'pending');
            if (picks.length === 0) {
            ...
            for (const [capperId, capperPicks] of Array.from(picksByCapper)) {
                ...
                logger_1.logger.info(`Processing picks for capper: ${capperId}`);
    ...
    async publishCapperPicks(channel, picks) {
        try {
            ...
            if (!firstPick) {
                logger_1.logger.error('No picks provided to publish');
                return;
            }
            // TypeScript assertion since we've already checked for null
            const capperProfile = await capperService_1.capperService.getCapperById(firstPick.capper_id);
            if (!capperProfile) {
            ...
            // Log analytics
            await capperService_1.capperService.logAnalyticsEvent('picks_published', {
