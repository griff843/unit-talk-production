...
exports.VIPPlusChannelService = void 0;
...
class VIPPlusChannelService {
    constructor() {
        ...
        this.discordBot = DiscordBotService_1.DiscordBotService.getInstance();
    }
    /**
     * Post advanced analytics to exclusive-insights (S/A-tier picks only)
     */
    async postExclusiveAnalysis(pick, insights, correlationId) {
        const analysisLogger = logger_1.logger.child({ correlationId, pickId: pick.id });
        try {
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .setDescription(`**${pick.player || pick.team} ${pick.statType || 'Pick'}** | ${insights.systemGrade}`)
                ...
                value: insights.marketIntelligence || 'Advanced algorithmic analysis indicates positive expected value with multiple confirming indicators aligning for optimal betting opportunity.',
                inline: false
            }, {
     ...
     */
    async postMarketMovement(marketData, correlationId) {
        const marketLogger = logger_1.logger.child({ correlationId });
        try {
            const embed = new discord_js_1.EmbedBuilder()
     ...
     */
    async postDailyBestBets(picks, correlationId) {
        const curatedLogger = logger_1.logger.child({ correlationId });
        try {
            const embed = new discord_js_1.EmbedBuilder()
                ...
                value: picks.slice(0, 3).map((pick, index) => `**${index + 1}.** ${pick.description || `${pick.team} ${pick.line}`} | **${pick.capper}**\n` +
                    `    └── Confidence: ${pick.confidence}% | Edge: +${pick.expectedValue?.toFixed(1) || '2.4'}%`).join('\n\n'),
                inline: false
            }, {
     ...
     */
    async routeToGameThread(pick, threadId, correlationId) {
        const routeLogger = logger_1.logger.child({ correlationId, threadId });
        try {
    ...
    createPickEmbed(pick) {
        ...
        return new discord_js_1.EmbedBuilder()
            ...
            name: '📊 **Analysis**',
            value: pick.reasoning || `Strong ${sport.toUpperCase()} analysis indicates value at current line. Key factors align for optimal betting opportunity.`,
            inline: false
        })
     ...
     */
    async postPerPeriodUpdate(gameId, threadId, updateData, correlationId) {
        const updateLogger = logger_1.logger.child({ correlationId, gameId, threadId });
        try {
     ...
     */
    async handleUserCoachingRequest(userId, userPicks, correlationId) {
        const coachingLogger = logger_1.logger.child({ correlationId, userId });
        try {
     ...
     */
    async postGameTrivia(gameId, threadId, triviaData, correlationId) {
        const triviaLogger = logger_1.logger.child({ correlationId, gameId });
        try {
            const embed = new discord_js_1.EmbedBuilder()
    ...
    generatePeriodStats(sport, updateData) {
        switch (sport) {
            case 'nba':
                return [
    ...
    async analyzeUserPerformance(userPicks) {
    ...
    async storeTriviaQuestion(gameId, threadId, triviaData) {
     ...
     */
    async postLiveGameUpdate(gameData, correlationId) {
        const updateLogger = logger_1.logger.child({ correlationId, action: 'post_live_game_update' });
        try {
            const embed = new discord_js_1.EmbedBuilder()
                .setTitle(`🔴 LIVE: ${gameData.matchup}`)
                .setDescription(`**${gameData.quarter} | ${gameData.time}**\n${gameData.awayTeam} ${gameData.awayScore} - ${gameData.homeScore} ${gameData.homeTeam}`)
                .addFields({ name: '📊 Live Lines', value: `Spread: ${gameData.liveSpread}\nTotal: ${gameData.liveTotal}`, inline: true }, { name: '🎯 Opening Lines', value: `Spread: ${gameData.openingSpread}\nTotal: ${gameData.openingTotal}`, inline: true }, { name: '⚡ Key Events', value: gameData.keyEvents || 'No major events', inline: false })
                .setColor(0xFF4444)
                .setTimestamp()
     ...
     */
    async postAICoaching(coachingData, correlationId) {
        const coachingLogger = logger_1.logger.child({ correlationId, action: 'post_ai_coaching' });
        try {
            const embed = new discord_js_1.EmbedBuilder()
                .setTitle('🤖 Personal AI Coaching Session')
                .setDescription(`Performance Analysis & Strategy Recommendations`)
                .addFields({ name: '💪 Your Strengths', value: coachingData.strengths, inline: false }, { name: '🎯 Areas for Improvement', value: coachingData.improvements, inline: false }, { name: '📊 Current Performance', value: `Win Rate: ${coachingData.winRate}\nROI: ${coachingData.roi}`, inline: true })
                .setColor(0x00AA55)
                .setTimestamp()
                .setFooter({ text: 'VIP+ AI Coaching | Personalized Strategy' });
            if (coachingData.bankrollAdvice) {
                embed.addFields({ name: '💰 Bankroll Management', value: coachingData.bankrollAdvice, inline: false });
            }
            if (coachingData.marketAdvice) {
                embed.addFields({ name: '📈 Market Focus', value: coachingData.marketAdvice, inline: false });
            }
            if (coachingData.actionPlan) {
                embed.addFields({ name: '📋 4-Week Action Plan', value: coachingData.actionPlan, inline: false });
            }
     ...
     */
    async runTestPosts(correlationId) {
        const testLogger = logger_1.logger.child({ correlationId });
        try {
            ...
            const testInsights = {
                ...
                marketIntelligence: 'Strong sharp money alignment with historical trends suggesting excellent value at current line.'
            };
            ...
            const testGameData = {
                ...
                analysis: 'Lakers defensive intensity increasing, Warriors missing open shots. Pace dropped 15% from Q1. Under 215.5 presents strong value as both teams prioritizing defense.'
            };
...
exports.VIPPlusChannelService = VIPPlusChannelService;
