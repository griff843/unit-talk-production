...
class RollbackProcedures {
    constructor(supabase) {
        ...
        this.averageRollbackTime = 0;
        this.supabase = supabase;
        this.featureFlagService = new FeatureFlagService_1.FeatureFlagService(supabase);
        this.validationGates = new AutomatedValidationGates_1.AutomatedValidationGates(supabase);
     ...
     */
    async executeRollback(flagName, trigger, strategyName) {
     ...
     */
    async executeRollbackPhases(executionId) {
     ...
     */
    async executeRollbackPhase(execution, phase, phaseResult) {
        ...
        try {
            ...
            for (let actionIndex = 0; actionIndex < phase.actions.length; actionIndex++) {
                ...
                const actionSuccess = await this.executeRollbackAction(execution.flagName, action, actionResult);
            ...
            for (const checkName of phase.validationChecks) {
                const validationResult = await this.runValidationCheck(execution.flagName, checkName);
     ...
     */
    async executeRollbackAction(flagName, action, actionResult) {
        const startTime = Date.now();
     ...
     */
    async executeFlagUpdateAction(flagName, parameters) {
     ...
     */
    async executeDatabaseRollbackAction(flagName, parameters) {
     ...
     */
    async executeCacheClearAction(parameters) {
     ...
     */
    async executeServiceRestartAction(parameters) {
     ...
     */
    async executeNotificationAction(flagName, parameters) {
     ...
     */
    async executeCustomAction(flagName, parameters) {
     ...
     */
    async runValidationCheck(flagName, checkName) {
        try {
            let passed = false;
            let result = null;
            switch (checkName) {
     ...
     * Select appropriate rollback strategy
     */
    async selectRollbackStrategy(flagName, trigger, strategyName) {
     ...
     */
    async assessRollbackImpact(flagName, trigger, strategy) {
        ...
        let serviceDegradation = 'minimal';
        if (trigger.severity === 'critical') {
     ...
     * Create recovery plan
     */
    async createRecoveryPlan(flagName, strategy, impact) {
        return {
    ...
    async storeRollbackExecution(execution) {
        try {
    ...
    async emitRollbackEvent(executionId, eventType, execution) {
        try {
            await this.supabase
                .from('events')
                .insert({
    ...
    async requestRollbackApproval(execution) {
    ...
    updateSuccessMetrics(success, duration) {
...
exports.RollbackProcedures = RollbackProcedures;
