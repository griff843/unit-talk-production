...
exports.AutomatedValidationGates = void 0;
...
class AutomatedValidationGates {
    constructor(supabase) {
        this.logger = (0, logger_1.createLogger)('AutomatedValidationGates');
        // Validation state tracking
        this.evaluationIntervals = new Map();
        this.metricCache = new Map();
        this.lastEvaluations = new Map();
        // Performance tracking
        this.evaluationLatencies = [];
    ...
    async validateRollout(flagName) {
        ...
        try {
            ...
            const recommendation = this.determineRecommendation(overallScore, criticalFailures, gateResults, rolloutInfo.rolloutPercentage);
            // Assess risk
     ...
     */
    async evaluateGate(gate, rolloutPercentage) {
        // Collect metric data
        const metricData = await this.collectMetricData(gate.flagName, gate.metricName, gate.evaluationWindowMinutes);
        ...
        const statisticalSignificance = this.calculateStatisticalSignificance(metricData, gate.thresholdValue, gate.comparisonOperator);
        // Evaluate threshold
        ...
        const contextualPassed = this.evaluateWithContext(passed, actualValue, gate, rolloutPercentage, confidenceInterval, statisticalSignificance);
        let reason = '';
        if (!contextualPassed) {
            if (!passed) {
     ...
     */
    async collectMetricData(flagName, metricName, windowMinutes) {
        const cacheKey = `${flagName}-${metricName}`;
        try {
            // First try to get from cache
            const cached = this.metricCache.get(cacheKey) || [];
            const cutoffTime = new Date(Date.now() - windowMinutes * 60 * 1000);
            const validCached = cached.filter(m => new Date(m.timestamp) >= cutoffTime);
            // If we have sufficient cached data, use it
            if (validCached.length >= 50) {
                return validCached;
            }
            // Collect from multiple sources
            const [flagMetrics, systemMetrics, applicationMetrics] = await Promise.all([
     ...
     */
    async collectFromFeatureFlagMetrics(flagName, metricName, windowMinutes) {
        const cutoffTime = new Date(Date.now() - windowMinutes * 60 * 1000);
        ...
        return data.map(row => ({
            metricName,
            value: this.extractMetricValue(row, metricName),
     ...
     */
    async collectFromSystemMetrics(metricName, windowMinutes) {
     ...
     */
    async collectFromApplicationMetrics(flagName, metricName, windowMinutes) {
     ...
     */
    async collectABTestMetrics(flagName, metricName, windowMinutes) {
        const cutoffTime = new Date(Date.now() - windowMinutes * 60 * 1000);
        const { data, error } = await this.supabase
     ...
     */
    extractMetricValue(row, metricName) {
        const metricMap = {
     ...
     */
    generateSimulatedMetricValue(metricName) {
        const baseValues = {
     ...
     */
    calculateStatisticalSignificance(data, threshold, operator) {
        if (data.length < 2)
            return 0;
        const values = data.map(d => d.value);
        const n = values.length;
     ...
     */
    evaluateWithContext(basicPassed, actualValue, gate, rolloutPercentage, confidenceInterval, statisticalSignificance) {
        if (!basicPassed)
            return false;
        ...
        if (gate.required) {
            const thresholdInInterval = this.thresholdInConfidenceInterval(gate.thresholdValue, gate.comparisonOperator, confidenceInterval);
     ...
     */
    determineRecommendation(overallScore, criticalFailures, gateResults, rolloutPercentage) {
     ...
     * Assess risk level
     */
    assessRisk(gateResults, rolloutPercentage) {
        const factors = [];
        const mitigations = [];
     ...
     */
    async handleCriticalValidationFailure(summary) {
        ...
        try {
            ...
            await this.featureFlagService.emergencyRollback(summary.flagName, `Critical validation failure: ${summary.criticalFailures} gates failed, score: ${summary.overallScore.toFixed(1)}%`);
    ...
    // Helper methods
    async getValidationGates(flagName) {
        const { data, error } = await this.supabase
    ...
    async getCurrentRolloutInfo(flagName) {
        const { data, error } = await this.supabase
            .from('feature_flags')
    ...
    getRequiredConfidence(rolloutPercentage) {
    ...
    thresholdInConfidenceInterval(threshold, operator, interval) {
        switch (operator) {
            case 'gt':
            case 'gte':
    ...
    createDefaultSummary(flagName, reason) {
        return {
            flagName,
            rolloutPercentage: 0,
            totalGates: 0,
    ...
    async storeValidationResults(summary) {
        try {
    ...
    cleanupMetricCache() {
        ...
        for (const [key, metrics] of this.metricCache) {
            const validMetrics = metrics.filter(m => new Date(m.timestamp) >= cutoffTime);
     ...
     */
    getLastValidationSummary(flagName) {
...
exports.AutomatedValidationGates = AutomatedValidationGates;
