...
exports.FeatureFlagDeploymentOrchestrator = void 0;
...
class FeatureFlagDeploymentOrchestrator {
    constructor(supabase) {
        this.logger = (0, logger_1.createLogger)('FeatureFlagDeploymentOrchestrator');
        // Active deployments tracking
        this.activeDeployments = new Map();
        this.deploymentTimers = new Map();
        // Default rollout configuration
        this.DEFAULT_ROLLOUT_STEPS = [5, 25, 50, 75, 100];
        this.DEFAULT_STABILIZATION_PERIOD = 300; // 5 minutes
        this.DEFAULT_MAX_ROLLOUT_DURATION = 240; // 4 hours
     ...
     */
    async executeRolloutStep(deploymentId, step, config) {
        const deployment = this.activeDeployments.get(deploymentId);
        if (!deployment)
            return;
     ...
     */
    async collectStepMetrics(flagName) {
        try {
            ...
            if (metrics) {
                return {
                    ...
                    latencyP95: metrics.latency_p95_ms || 0,
     ...
     * Run validation gates
     */
    async runValidationGates(gates, flagName) {
        const results = [];
     ...
     */
    async checkRollbackConditions(thresholds, step) {
     ...
     */
    async validateStepCompletion(step, gates) {
        ...
        const requiredGatesFailed = step.validationResults.some(r => gates.find(g => g.name === r.gateName)?.required && !r.passed);
     ...
     * Store deployment status
     */
    async storeDeploymentStatus(deployment) {
        try {
     ...
     */
    async emitDeploymentEvent(deploymentId, eventType, deployment) {
        try {
            await this.supabase
                .from('events')
                .insert({
     ...
     */
    createSyndicateGradeDeploymentConfigs() {
        return {
...
exports.FeatureFlagDeploymentOrchestrator = FeatureFlagDeploymentOrchestrator;
