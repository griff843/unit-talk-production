...
exports.SmartFormBridge = void 0;
const supabase_js_1 = require("@supabase/supabase-js");
const ScoringAgent_1 = require("../agents/ScoringAgent");
const ProfessionalPropProcessor_1 = require("./ProfessionalPropProcessor");
 ...
 */
class SmartFormBridge {
    constructor() {
        this.bridgeLogger = logger_1.logger.child({ service: 'SmartFormBridge' });
     ...
     */
    async processSubmission(ticketId) {
        const correlationId = `bridge-${ticketId}-${Date.now()}`;
        const bridgeLogger = logger_1.logger.child({ correlationId, ticketId });
        try {
            ...
            logger_1.logger.error('Smart form bridge processing failed', {
                ticketId,
    ...
    async getSmartTicket(ticketId) {
        const { data, error } = await this.supabase
            ...
            .single();
        if (error) {
            logger_1.logger.error('Failed to fetch smart ticket', { ticketId, error });
            return null;
        }
        return data;
    }
     ...
     */
    getCapperThreadId(capperName) {
        const threadId = env_1.env.capperThreads[capperName];
     ...
     */
    async determineRoutingTarget(smartTicket, defaultCapperThread) {
        try {
            // Check if this is a game-specific pick (has teams/matchup info)
            const hasGameContext = smartTicket.legs.some(leg => leg.teams || leg.matchup || leg.game_id);
            if (hasGameContext) {
                ...
                if (gameThreadId) {
                    logger_1.logger.info('Routing pick to game thread', {
                        gameInfo,
                        gameThreadId,
            ...
            logger_1.logger.info('Routing pick to capper thread', {
                capper: smartTicket.capper,
            ...
            logger_1.logger.warn('Error determining routing target, using capper thread', {
                error: error instanceof Error ? error.message : String(error),
     ...
     */
    extractGameInfo(smartTicket) {
        const firstLeg = smartTicket.legs[0];
        return {
    ...
    async findGameThread(gameInfo) {
        try {
            ...
            if (!error && data?.thread_id) {
                logger_1.logger.info('Found existing game thread', {
                    gameInfo,
            ...
            logger_1.logger.info('No game thread found, creating new one', { gameInfo });
            return await this.createGameThread(gameInfo);
        }
        catch (error) {
            logger_1.logger.warn('Error finding/creating game thread', {
                gameInfo,
                error: error instanceof Error ? error.message : String(error)
            });
            return null;
     ...
     */
    async createGameThread(gameInfo) {
        try {
            ...
            const { DiscordBotService } = await Promise.resolve().then(() => __importStar(require('../services/DiscordBotService')));
            ...
            let thread = null; // Placeholder - implement thread creation logic
            try {
                ...
                logger_1.logger.info('Game thread creation disabled (placeholder mode)', { gameData });
            }
            catch (error) {
                logger_1.logger.warn('Failed to create game thread', { error, gameData });
                thread = null;
            }
            if (thread) {
                logger_1.logger.info('Auto-created game thread from pick submission', {
                    gameInfo,
                    threadId: thread.id,
            ...
            logger_1.logger.error('Failed to create game thread from pick submission', {
                gameInfo,
                error: error instanceof Error ? error.message : String(error)
            });
            return null;
     ...
     */
    async transformToPlatformFormat(smartTicket, _correlationId) {
        // Transform to match actual daily_picks table schema
        const leg = smartTicket.legs[0];
        return {
     ...
     */
    async generateInsights(smartTicket, dailyPick) {
        try {
            ...
            const gradingResult = await this.scoringAgent.gradePick?.(pickForGrading) || {
                tier: 'B',
                score: 50,
                expectedValue: 0,
                riskFactors: [],
            ...
            logger_1.logger.warn('Failed to generate insights', {
     ...
     */
    async storeInsights(pickId, insights) {
        ...
        if (error) {
            logger_1.logger.warn('Failed to store insights', { pickId, error: error.message });
        }
    }
    /**
     * Process live pick - immediate Discord posting
     */
    async processLivePick(pickId, threadId) {
        ...
        logger_1.logger.info('Live pick marked for immediate posting', { pickId, threadId });
    }
    /**
     * Schedule pick for 10 AM batch posting
     */
    async scheduleForBatchPosting(pickId) {
        ...
        logger_1.logger.info('Pick scheduled for batch posting', {
            pickId,
     ...
     */
    async updateSmartTicketStatus(ticketId, status, insights, errorMessage) {
        const updateData = {
            status,
        ...
        const { error } = await this.supabase
            ...
            .eq('id', ticketId);
        if (error) {
            logger_1.logger.error('Failed to update smart ticket status', {
                ticketId,
                status,
                error: error.message
            });
     ...
     */
    async processThroughProfessionalGrading(pickId, dailyPick, insights) {
        try {
            ...
            const professionalProcessor = ProfessionalPropProcessor_1.ProfessionalPropProcessor.getInstance();
     ...
     */
    validateSharpGradingCompliance(professionalResult) {
        const violations = [];
     ...
     */
    async promoteWithProfessionalResults(pickId, dailyPick, professionalResult) {
        const finalPick = {
            id: `final-${pickId}`,
     ...
     */
    async markForManualReview(pickId, dailyPick, error) {
        try {
            await this.supabase
                .from('daily_picks')
                .update({
     ...
     */
    async promoteToUnifiedPicks(pickId, dailyPick, insights) {
        try {
            ...
            await this.supabase
                ...
                .eq('id', pickId);
            logger_1.logger.info('Pick successfully promoted to unified_picks', {
                dailyPickId: pickId,
            ...
            logger_1.logger.error('Failed to promote pick to unified_picks', {
                pickId,
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined
            });
     ...
     */
    async sendSystemAlert(title, details) {
...
exports.SmartFormBridge = SmartFormBridge;
