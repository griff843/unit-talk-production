...
exports.rollingMetricsService = void 0;
...
class RollingMetricsService {
    constructor() {
        ...
        this.logger = (0, logger_1.createLogger)('RollingMetricsService');
     ...
     */
    async calculateMetricsForWindow(window, sport) {
        const now = new Date();
     ...
     */
    async calculateWindowMetrics(picks) {
     ...
     * Calculate ROI metrics
     */
    calculateROIMetrics(picks) {
        const calculateROI = (picksSubset) => {
     ...
     * Calculate risk metrics
     */
    calculateRiskMetrics(picks) {
        // Sort picks by date for drawdown calculation
        const sortedPicks = [...picks].sort((a, b) => new Date(a.settled_at).getTime() - new Date(b.settled_at).getTime());
     ...
     */
    async analyzeSportPerformance(sport) {
     ...
     */
    analyzeTrend(recent, historical) {
        const evTrend = this.getTrendDirection(recent.metrics.actualEV, historical.metrics.actualEV);
        const clvTrend = this.getTrendDirection(recent.metrics.averageCLV, historical.metrics.averageCLV);
     ...
     */
    createEVAdjustment(sport, metrics) {
     ...
     */
    createCLVAdjustment(sport, metrics) {
        const avgCLV = metrics.metrics.averageCLV;
        if (avgCLV > 10)
            return null; // Acceptable CLV
        const adjustmentFactor = Math.min(this.LEARNING_CONFIG.adjustmentRate, Math.abs(avgCLV - 15) / 100);
        return {
            timestamp: new Date(),
            sport,
            adjustmentType: 'threshold',
     ...
     */
    createKellyAdjustment(sport, metrics) {
        const efficiency = metrics.metrics.kellyEfficiency;
        if (efficiency > 0.8)
            return null; // Good efficiency
     ...
     */
    async applyGlobalAdjustments(metrics) {
        const targets = this.LEARNING_CONFIG.performanceTarget;
        // Check if global adjustments are needed
        const needsAdjustment = metrics.metrics.roi < targets.minROI ||
     ...
     * Generate performance alert
     */
    async generatePerformanceAlert(alert) {
        const fullAlert = {
            id: `alert_${Date.now()}`,
            timestamp: new Date(),
            ...alert
    ...
    async getPicksForWindow(startDate, endDate, sport) {
        try {
    ...
    async getActiveSports() {
        try {
            ...
            return [...new Set((data || []).map((p) => String(p.sport)))];
        }
        catch (error) {
    ...
    async storeMetrics(metrics) {
        try {
            ...
            if ((0, ShadowMode_1.isShadowMode)()) {
                await (0, ShadowMode_1.shadowWriteMetrics)({
                    window: metrics.window,
    ...
    getTrendDirection(recent, historical) {
    ...
    async getSportMetrics(sport) {
        const windows = [];
        for (const window of ['7d', '30d', 'lifetime']) {
        ...
        const { data: adjustments } = await supabaseClient_1.supabaseClient
            .from('learning_adjustments')
            .select('*')
            .eq('sport', sport)
    ...
    async getPerformanceSummary() {
        ...
        // Get recent alerts
        const { data: alerts } = await supabaseClient_1.supabaseClient
            .from('performance_alerts')
            .select('*')
            .order('timestamp', { ascending: false })
            .limit(10);
...
exports.rollingMetricsService = RollingMetricsService.getInstance();
