...
exports.autoRecheckService = void 0;
const logger_1 = require("../utils/logger");
const supabaseClient_1 = require("./supabaseClient");
const PromotionGatekeeper_1 = require("./PromotionGatekeeper");
const STierEnforcer_1 = require("./STierEnforcer");
const PortfolioRiskManager_1 = require("./PortfolioRiskManager");
...
class AutoRecheckService {
    constructor() {
        ...
        this.DEFAULT_RECHECK_INTERVALS = [
            { minutes: 15, type: 'final_validation' },
            { minutes: 10, type: 'market_check' },
            { minutes: 5, type: 'emergency_check' },
            { minutes: -30, type: 'post_game_settlement' }, // 30 min after game
            { minutes: -60, type: 'final_settlement' } // 1 hour after game
        ];
        this.logger = (0, logger_1.createLogger)('AutoRecheckService');
     ...
     */
    async scheduleRecheckForPick(pick) {
     ...
     */
    async processScheduledRechecks() {
        const now = new Date();
        const dueSchedules = [];
     ...
     */
    async executeRecheckPoint(schedule, checkPoint) {
        ...
        try {
            ...
            const action = await this.determineRecheckAction(pick, validationSnapshot, oddsMovement, checkPoint);
     ...
     * Perform comprehensive validation
     */
    async performValidation(pick, _checkType) {
        const timestamp = new Date();
        // Promotion gate validation
        const promotionValidation = await PromotionGatekeeper_1.promotionGatekeeper.evaluatePromotion({
            id: pick.id,
            propId: pick.prop_id,
            tier: pick.tier,
        ...
        let sTierValidation;
        if (pick.tier === 'S') {
            const sTierResult = await STierEnforcer_1.sTierEnforcer.enforceSTierStandards(pick);
            sTierValidation = {
        ...
        const portfolioResult = await PortfolioRiskManager_1.portfolioRiskManager.assessPositionRisk(pick, pick.kelly_fraction || 0.1);
     ...
     */
    async validateMarketConditions(pick) {
        try {
            // Get current market data
            const { data: marketData } = await supabaseClient_1.supabaseClient
                .from('market_data')
                .select('*')
                ...
                .single();
            // Get steam data
            const { data: steamData } = await supabaseClient_1.supabaseClient
                .from('steam_tracking')
                .select('*')
                .eq('prop_id', pick.prop_id)
                .order('created_at', { ascending: false })
     ...
     */
    async getOddsMovementData(pick) {
        try {
            // Get initial and current odds
            const { data: oddsHistory } = await supabaseClient_1.supabaseClient
                .from('odds_tracking')
                .select('*')
                .eq('prop_id', pick.prop_id)
     ...
     */
    async determineRecheckAction(pick, validation, oddsMovement, _checkPoint) {
     ...
     */
    async executeRecheckAction(pick, action) {
        ...
        await PublishGuard_1.publishGuard.handleRecheckDecision(pick.id, action.type, action.type === 'cancel' ? 'invalid' : 'valid', action.type, {
     ...
     * Helper methods
     */
    calculateOddsMovementBps(initialOdds, currentOdds) {
        if (initialOdds === 0)
            return 0;
        const difference = Math.abs(currentOdds - initialOdds);
        return (difference / Math.abs(initialOdds)) * 10000;
    }
    getNextCheckTime(preGameChecks, postGameChecks) {
        const allChecks = [...preGameChecks, ...postGameChecks];
        const pendingChecks = allChecks.filter(c => c.status === 'pending');
        if (pendingChecks.length === 0)
            return null;
        return pendingChecks.reduce((earliest, check) => check.scheduledTime < earliest ? check.scheduledTime : earliest, pendingChecks[0].scheduledTime);
    }
    isScheduleComplete(schedule) {
        const allChecks = [...schedule.preGameChecks, ...schedule.postGameChecks];
        return allChecks.every(check => check.status === 'completed' || check.status === 'skipped' || check.status === 'failed');
    }
    async getCurrentPickData(pickId) {
        try {
    ...
    async loadActiveSchedules() {
        try {
            const { data: schedules } = await supabaseClient_1.supabaseClient
    ...
    async storeRecheckSchedule(schedule) {
        try {
            await supabaseClient_1.supabaseClient
                .from('recheck_schedules')
                .insert({
    ...
    async updateRecheckSchedule(schedule) {
        try {
            await supabaseClient_1.supabaseClient
                .from('recheck_schedules')
                .update({
    ...
    async triggerRepublication(pick, reason) {
     ...
     */
    async getRecheckStats(_timeframe = 'day') {
...
exports.autoRecheckService = AutoRecheckService.getInstance();
