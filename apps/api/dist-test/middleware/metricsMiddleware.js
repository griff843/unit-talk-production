...
function metricsMiddleware() {
    ...
    return (req, res, next) => {
        ...
        metricsServer_1.activeConnections.set(currentActiveConnections);
        ...
        const requestSize = req.headers['content-length']
            ...
            : 0;
        if (requestSize > 0) {
            metricsServer_1.httpRequestSize.observe({ method, route }, requestSize);
        }
        ...
        res.send = function (data) {
            ...
            // Track HTTP metrics
            metricsServer_1.httpRequestsTotal.inc({ method, route, status_code: statusCode });
            metricsServer_1.httpRequestDuration.observe({ method, route, status_code: statusCode }, duration);
            // Track response size
            const responseSize = typeof data === 'string'
                ? Buffer.byteLength(data, 'utf8')
                : JSON.stringify(data).length;
            metricsServer_1.httpResponseSize.observe({ method, route, status_code: statusCode }, responseSize);
        ...
        req.on('close', () => {
            currentActiveConnections--;
            metricsServer_1.activeConnections.set(Math.max(0, currentActiveConnections));
        });
        req.on('error', () => {
            currentActiveConnections--;
            metricsServer_1.activeConnections.set(Math.max(0, currentActiveConnections));
...
function errorMetricsMiddleware() {
    return (error, req, res, next) => {
        ...
        metricsServer_1.httpRequestsTotal.inc({
            method,
            route,
        ...
        if (req.startTime) {
            ...
            metricsServer_1.httpRequestDuration.observe({ method, route, status_code: statusCode.toString() }, duration);
        }
        next(error);
    };
}
