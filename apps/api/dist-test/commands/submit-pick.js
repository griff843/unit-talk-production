...
exports.handlePickTypeSelect = handlePickTypeSelect;
exports.handleSinglePickModal = handleSinglePickModal;
exports.handleParlayPickModal = handleParlayPickModal;
exports.handlePickConfirmation = handlePickConfirmation;
...
async function execute(interaction) {
    try {
        ...
        const existingPicks = await capperService_1.capperService.getCapperPicks(capperProfile.id, today, 'pending');
        ...
        logger_1.logger.error('Error in submit-pick command', { error });
        await interaction.reply({
...
async function handlePickTypeSelect(interaction) {
    try {
        const pickType = interaction.values[0];
        ...
        logger_1.logger.error('Error handling pick type selection', { error });
        await interaction.reply({
...
async function showSinglePickForm(interaction) {
    const modal = new discord_js_1.ModalBuilder()
        .setCustomId('single_pick_modal')
        .setTitle('Submit Single Pick');
    const gameInput = new discord_js_1.TextInputBuilder()
        .setCustomId('game')
        .setLabel('Game/Match')
        .setStyle(discord_js_1.TextInputStyle.Short)
        .setPlaceholder('e.g., Lakers vs Warriors')
        .setRequired(true);
    const betTypeInput = new discord_js_1.TextInputBuilder()
        .setCustomId('bet_type')
        .setLabel('Bet Type')
        .setStyle(discord_js_1.TextInputStyle.Short)
        .setPlaceholder('e.g., Moneyline, Spread, Over/Under')
        .setRequired(true);
    const selectionInput = new discord_js_1.TextInputBuilder()
        .setCustomId('selection')
        .setLabel('Your Pick')
        .setStyle(discord_js_1.TextInputStyle.Short)
        .setPlaceholder('e.g., Lakers -5.5, Over 220.5')
        .setRequired(true);
    const oddsInput = new discord_js_1.TextInputBuilder()
        .setCustomId('odds')
        .setLabel('Odds')
        .setStyle(discord_js_1.TextInputStyle.Short)
        .setPlaceholder('e.g., -110, +150')
        .setRequired(true);
    const unitsInput = new discord_js_1.TextInputBuilder()
        .setCustomId('units')
        .setLabel('Units (1-5)')
        .setStyle(discord_js_1.TextInputStyle.Short)
    ...
    //   .setRequired(false);
    const rows = [
        new discord_js_1.ActionRowBuilder().addComponents(gameInput),
        new discord_js_1.ActionRowBuilder().addComponents(betTypeInput),
        new discord_js_1.ActionRowBuilder().addComponents(selectionInput),
        new discord_js_1.ActionRowBuilder().addComponents(oddsInput),
        new discord_js_1.ActionRowBuilder().addComponents(unitsInput)
    ];
    modal.addComponents(...rows);
    await interaction.showModal(modal);
}
async function showParlayForm(interaction) {
    const modal = new discord_js_1.ModalBuilder()
        .setCustomId('parlay_pick_modal')
        .setTitle('Submit Parlay Pick');
    const leg1Input = new discord_js_1.TextInputBuilder()
        .setCustomId('leg1')
        .setLabel('Leg 1')
        .setStyle(discord_js_1.TextInputStyle.Short)
        .setPlaceholder('Game | Bet Type | Selection | Odds')
        .setRequired(true);
    const leg2Input = new discord_js_1.TextInputBuilder()
        .setCustomId('leg2')
        .setLabel('Leg 2')
        .setStyle(discord_js_1.TextInputStyle.Short)
        .setPlaceholder('Game | Bet Type | Selection | Odds')
        .setRequired(true);
    const leg3Input = new discord_js_1.TextInputBuilder()
        .setCustomId('leg3')
    ...
    const unitsInput = new discord_js_1.TextInputBuilder()
        ...
        .setPlaceholder('e.g., 2')
        .setRequired(true);
    const analysisInput = new discord_js_1.TextInputBuilder()
        .setCustomId('analysis')
        .setLabel('Analysis (Optional)')
        ...
        .setRequired(false);
    const rows = [
        new discord_js_1.ActionRowBuilder().addComponents(leg1Input),
        new discord_js_1.ActionRowBuilder().addComponents(leg2Input),
        new discord_js_1.ActionRowBuilder().addComponents(leg3Input),
        new discord_js_1.ActionRowBuilder().addComponents(unitsInput),
        new discord_js_1.ActionRowBuilder().addComponents(analysisInput)
    ];
    modal.addComponents(...rows);
    await interaction.showModal(modal);
}
async function handleSinglePickModal(interaction) {
    try {
        ...
        logger_1.logger.error('Error handling single pick modal', { error });
        await interaction.reply({
...
async function handleParlayPickModal(interaction) {
    try {
        ...
        logger_1.logger.error('Error handling parlay pick modal', { error });
        await interaction.reply({
...
async function showPickPreview(interaction, pickData) {
    const embed = new discord_js_1.EmbedBuilder()
        .setTitle('📝 Pick Preview')
        .setColor(0x00ff00)
        .addFields({ name: 'Type', value: pickData.pick_type.toUpperCase(), inline: true }, { name: 'Units', value: pickData.total_units.toString(), inline: true }, { name: 'Total Odds', value: pickData.total_odds > 0 ? `+${pickData.total_odds}` : pickData.total_odds.toString(), inline: true });
    // Add legs
    const legs = pickData.legs;
...
async function handlePickConfirmation(interaction) {
    try {
        ...
        if (interaction.customId === 'confirm_pick') {
            ...
            // Submit the pick
            const pick = await capperService_1.capperService.createDailyPick(userData.pickData);
            if (!pick) {
        ...
        logger_1.logger.error('Error handling pick confirmation', { error });
        await interaction.reply({
