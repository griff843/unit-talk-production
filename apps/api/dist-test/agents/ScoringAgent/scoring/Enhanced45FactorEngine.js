...
exports.Enhanced45FactorEngine = void 0;
const logger_1 = require("../../../utils/logger");
class Enhanced45FactorEngine {
    constructor(featureStore, changeDetector) {
        this.logger = (0, logger_1.createLogger)('Enhanced45FactorEngine');
     ...
     */
    async calculate45FactorScore(features, config) {
        ...
        try {
            ...
            const enhancedFeatures = await this.featureStore.getEnhancedFeatures(features.propId, features.sport, features.player || 'unknown');
            ...
            const [marketScores, playerScores, matchupScores, priceScores, metaScores] = await Promise.all([
            ...
            const totalScore = marketScores.categoryScore * categoryWeights.market +
            ...
            const riskAdjustedScore = this.applyRiskAdjustments(totalScore, priceScores.factors.correlationRisk, metaScores.factors.confidenceInterval);
            const result = {
     ...
     */
    async calculateMarketFactors(features, enhancedFeatures, weights) {
        const factors = {
     ...
     */
    async calculatePlayerFactors(features, enhancedFeatures, weights) {
        const factors = {
     ...
     */
    async calculateMatchupFactors(features, enhancedFeatures, weights) {
        const factors = {
     ...
     */
    async calculatePriceFactors(features, enhancedFeatures, weights) {
        const factors = {
     ...
     */
    async calculateMetaFactors(features, enhancedFeatures, weights) {
        const factors = {
    ...
    // ========================================
    calculateDeviggedEV(features, enhancedFeatures) {
    ...
    calculateLineVelocity(lineHistory) {
    ...
    calculateCLV(features, predictedClosingLine) {
        if (!predictedClosingLine)
            return 50;
    ...
    calculateBettingSplit(bettingData) {
    ...
    calculateVolumeProfile(volumeData) {
    ...
    calculateCrossMarketOpportunity(relatedMarkets) {
    ...
    calculateMarketResistance(priceAction) {
        ...
        if (supportLevel && resistanceLevel) {
            const proximity = Math.min(Math.abs(supportLevel.price - (priceAction[priceAction.length - 1].price || 0)), Math.abs(resistanceLevel.price - (priceAction[priceAction.length - 1].price || 0)));
            return Math.max(0, Math.min(100, 80 - (proximity * 20)));
        }
        return 50;
    }
    calculateOptimalTiming(features, timingData) {
        const hoursToGame = this.calculateHoursToGame(features);
        const optimalWindow = timingData.optimalWindow || { start: 24, end: 2 };
        if (hoursToGame >= optimalWindow.start)
            return 90; // Early value
        if (hoursToGame <= optimalWindow.end)
            return 20; // Too close
    ...
    // ========================================
    calculatePlayerForm(recentGames) {
    ...
    calculateRoleStability(roleData) {
    ...
    calculateMatchupHistory(headToHead) {
    ...
    calculatePropTendencies(features, propHistory) {
    ...
    calculateSituationalPerformance(situationalData) {
    ...
    calculateKellyOptimal(features, enhancedFeatures) {
    ...
    calculateRiskAdjustedReturn(features, enhancedFeatures) {
    ...
    calculateLiquidityPremium(liquidityData) {
        const depth = liquidityData.depth || 50;
        return Math.max(0, Math.min(100, depth));
    }
    calculateMarketTimingValue(features, timingData) {
        const hoursToGame = this.calculateHoursToGame(features);
    ...
    calculateDataQuality(features, enhancedFeatures) {
    ...
    calculateModelAgreement(modelOutputs) {
    ...
    calculateConfidence(score, modelAgreement) {
    ...
    calculateKellyFraction(score, riskAdjustedReturn) {
        const edge = (score - 50) / 100; // Convert score to edge
        const kelly = edge * 0.25; // Conservative Kelly multiplier
        return Math.max(0, Math.min(0.25, kelly));
    }
    applyRiskAdjustments(score, correlationRisk, confidenceInterval) {
     ...
     */
    async batchProcess45Factor(propsList, config) {
        const batchSize = 100; // Process in batches of 100
        const results = [];
        for (let i = 0; i < propsList.length; i += batchSize) {
            const batch = propsList.slice(i, i + batchSize);
            const batchResults = await Promise.all(batch.map(features => this.calculate45FactorScore(features, config)));
...
exports.Enhanced45FactorEngine = Enhanced45FactorEngine;
