...
exports.AdvancedBacktestingSystem = void 0;
...
class AdvancedBacktestingSystem {
     ...
     */
    async runAdvancedBacktest(historicalData, gradingEngine) {
        ...
        for (const data of historicalData) {
            ...
            results.push({
                ...
                timestamp: data.features.date || (0, dateUtils_1.toISOString)(new Date())
            });
        }
     ...
     */
    async calculateAdvancedMetrics(results) {
     ...
     */
    calculateTierPerformance(results) {
     ...
     */
    calculateSportPerformance(results) {
     ...
     */
    calculateMarketPerformance(results) {
     ...
     */
    calculateMonthlyPerformance(results) {
     ...
     */
    calculateWeeklyPerformance(results) {
    ...
    calculateStreakAnalysis(results) {
    ...
    calculateOddsAnalysis(results) {
        ...
        const oddsRanges = [
            { range: '-200 to -150', filter: (r) => r.pick.odds >= -200 && r.pick.odds < -150 },
            { range: '-150 to -110', filter: (r) => r.pick.odds >= -150 && r.pick.odds < -110 },
            { range: '-110 to +110', filter: (r) => r.pick.odds >= -110 && r.pick.odds <= 110 },
            { range: '+110 to +200', filter: (r) => r.pick.odds > 110 && r.pick.odds <= 200 },
            { range: '+200+', filter: (r) => r.pick.odds > 200 }
        ];
    ...
    calculateConfidenceAnalysis(results) {
    ...
    calculateModelAccuracy(results) {
        ...
        const correctPredictions = results.filter(r => (r.prediction.confidence > 0.5 && r.actual.outcome > 0) ||
            (r.prediction.confidence < 0.5 && r.actual.outcome <= 0)).length;
    ...
    calculateFeatureAttribution(results) {
    ...
    analyzeFeaturePerformance(results) {
    ...
    calculateProjectedPerformance(results) {
