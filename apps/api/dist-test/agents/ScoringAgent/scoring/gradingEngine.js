...
exports.SyndicateGradingEngine = void 0;
...
const weights_1 = require("../../../scoring/config/weights");
const enhancedScoringEngine_1 = require("./enhancedScoringEngine");
const featureEngineer_1 = require("./featureEngineer");
const mlModelManager_1 = require("./mlModelManager");
...
class SyndicateGradingEngine {
    constructor() {
        this.scoringConfigs = new Map();
        this.activeConfig = 'default';
        this._performanceHistory = new Map();
        // 🆕 NEW: Professional Capper Data Tracking
        this.lineMovementHistory = new Map();
        this.bettingPercentages = new Map();
        this.bookLines = new Map();
        this.injuryNews = new Map();
        this.crossMarketData = new Map();
        // Initialize performance tracking
        this.initializePerformanceTracking();
        this.mlModelManager = new mlModelManager_1.MLModelManager();
        this.featureEngineer = new featureEngineer_1.FeatureEngineer();
    ...
    async scoreProp(features) {
        ...
        const sportConfig = (0, weights_1.getScoringConfig)(features.sport || 'NBA');
        ...
        try {
            ...
            let enhancedCapperAnalysis;
            try {
                if (enrichedFeatures.sport && ['MLB', 'NBA', 'NFL', 'NHL'].includes(enrichedFeatures.sport)) {
                    enhancedCapperAnalysis = await enhancedScoringEngine_1.enhancedScoringEngine.calculateEnhancedScore(enrichedFeatures.sport, {
                    ...
                    if (enhancedCapperAnalysis) {
                        ...
                        console.log(`🔍 Enhanced Capper Analysis for ${features.sport}:`);
                        console.log(`   Original finalScore: ${compositeScore.finalScore}, isNaN: ${isNaN(compositeScore.finalScore)}`);
                        console.log(`   Enhanced score: ${enhancedCapperAnalysis.enhancedScore}, isNaN: ${isNaN(enhancedCapperAnalysis.enhancedScore)}`);
                        const originalScore = compositeScore.finalScore;
                        const enhancedScore = safeNumber(enhancedCapperAnalysis.enhancedScore, 0); // Use safe number
                        const newFinalScore = (originalScore * (1 - enhancedWeight)) + (enhancedScore * enhancedWeight);
                        console.log(`   Calculated newFinalScore: ${newFinalScore}, isNaN: ${isNaN(newFinalScore)}`);
                        compositeScore.finalScore = safeNumber(newFinalScore, originalScore); // Fallback to original if NaN
                        console.log(`   Final finalScore after enhancement: ${compositeScore.finalScore}, isNaN: ${isNaN(compositeScore.finalScore)}`);
                    }
                }
            }
            catch (error) {
     ...
     */
    convertToLegacyWeights(sportWeights) {
        return {
            // Core Components
    ...
    async getMLPredictions(features) {
        ...
        // Calculate model agreement (how much models agree) - use consistent .score property
        const scores = [safeNumber(nn?.score, 0), safeNumber(gb?.score, 0), safeNumber(rf?.score, 0)];
        const mean = scores.reduce((a, b) => a + b, 0) / scores.length;
        const variance = scores.reduce((acc, score) => acc + Math.pow(score - mean, 2), 0) / scores.length;
        const agreement = Math.max(0, 1 - (variance / 100)); // Normalize to 0-1
        return {
            neuralNetwork: safeNumber(nn?.score, 0),
            gradientBoosting: safeNumber(gb?.score, 0),
            randomForest: safeNumber(rf?.score, 0),
            ensemble: safeNumber(ensemble?.score, 0),
            contributions: {
                'Neural Network': safeNumber(nn?.score, 0) * 0.2,
                'Gradient Boosting': safeNumber(gb?.score, 0) * 0.25,
                'Random Forest': safeNumber(rf?.score, 0) * 0.15,
                'Ensemble': safeNumber(ensemble?.score, 0) * 0.3
            },
            agreement: safeNumber(agreement, 0.5)
        };
    }
    /**
     * Calculate composite professional_score with dynamic weight optimization
     */
    async calculateCompositeScore(features, mlPredictions) {
        const config = this.scoringConfigs.get(this.activeConfig);
        const weights = config.weights;
        // Debug logging to identify NaN source
        console.log(`🔍 calculateCompositeScore debug for ${features.sport || 'unknown'}:`);
        console.log(`   activeConfig: ${this.activeConfig}`);
        console.log(`   config exists: ${!!config}`);
        console.log(`   weights exists: ${!!weights}`);
        const breakdown = {};
        let compositeScore = 0;
        // Core features (max 35 points) - Increased weight
        const coreScore = this.calculateCoreScore(features, weights);
        console.log(`   coreScore: ${coreScore}, isNaN: ${isNaN(coreScore)}`);
        const coreContribution = safeMultiply(coreScore, 3.5);
        console.log(`   coreContribution: ${coreContribution}, isNaN: ${isNaN(coreContribution)}`);
        compositeScore = safeNumber(compositeScore + coreContribution, 0);
        console.log(`   compositeScore after core: ${compositeScore}, isNaN: ${isNaN(compositeScore)}`);
        ...
        console.log(`   marketScore: ${marketScore}, isNaN: ${isNaN(marketScore)}`);
        const marketContribution = safeMultiply(marketScore, 3);
        console.log(`   marketContribution: ${marketContribution}, isNaN: ${isNaN(marketContribution)}`);
        compositeScore = safeNumber(compositeScore + marketContribution, 0);
        console.log(`   compositeScore after market: ${compositeScore}, isNaN: ${isNaN(compositeScore)}`);
        ...
        console.log(`   contextScore: ${contextScore}, isNaN: ${isNaN(contextScore)}`);
        const contextContribution = safeMultiply(contextScore, 1.5);
        compositeScore = safeNumber(compositeScore + contextContribution, 0);
        console.log(`   compositeScore after context: ${compositeScore}, isNaN: ${isNaN(compositeScore)}`);
        ...
        console.log(`   riskScore: ${riskScore}, isNaN: ${isNaN(riskScore)}`);
        const riskContribution = riskScore;
        compositeScore = safeNumber(compositeScore + riskContribution, 0);
        console.log(`   compositeScore after risk: ${compositeScore}, isNaN: ${isNaN(compositeScore)}`);
        ...
        console.log(`   professionalScore: ${professionalScore}, isNaN: ${isNaN(professionalScore)}`);
        ...
        console.log(`   compositeScore after professional: ${compositeScore}, isNaN: ${isNaN(compositeScore)}`);
        ...
        console.log(`   mlScore: ${mlScore}, isNaN: ${isNaN(mlScore)}`);
        const mlContribution = mlScore;
        compositeScore = safeNumber(compositeScore + mlContribution, 0);
        console.log(`   compositeScore after ML: ${compositeScore}, isNaN: ${isNaN(compositeScore)}`);
        ...
        console.log(`   Final compositeScore before processing: ${compositeScore}, isNaN: ${isNaN(compositeScore)}`);
        const safeFinalScore = safeNumber(compositeScore, 0);
        console.log(`   safeFinalScore: ${safeFinalScore}, isNaN: ${isNaN(safeFinalScore)}`);
        const clampedFinalScore = Math.max(0, Math.min(100, safeFinalScore));
        console.log(`   clampedFinalScore: ${clampedFinalScore}, isNaN: ${isNaN(clampedFinalScore)}`);
        ...
        console.log(`   edgeScore: ${edgeScore}, isNaN: ${isNaN(edgeScore)}`);
     ...
     */
    calculateCoreScore(features, weights) {
     ...
     */
    calculateMarketIntelligenceScore(features, weights) {
        let professional_score = 0;
     ...
     */
    calculateMLScore(mlPredictions, weights) {
        let professional_score = 0;
     ...
     */
    calculateContextScore(features, weights) {
        let professional_score = 0;
        // Player Fatigue: 0 is good, higher is bad
        const fatigueValue = safeNumber(features.playerFatigue, 0);
        const fatigueScore = Math.max(0, 10 - fatigueValue / 10);
     ...
     */
    async assessRisk(features, _compositeScore, confidence) {
     ...
     */
    async generateScenarioAnalysis(features, compositeScore) {
     ...
     */
    calculateFeatureContributions(_features, compositeScore) {
        const contributions = {};
        ...
        Object.entries(compositeScore.breakdown).forEach(([category, score]) => {
            contributions[category] = safeDivide(score, totalScore, 0) * 100;
        });
     ...
     */
    determineTierAndConfidence(compositeScore, riskAssessment, features) {
        ...
        if (professional_score >= 70 && edge >= 20 && risk <= 4) {
            tier = 'S';
        }
        else if (professional_score >= 50 && edge >= 0 && risk <= 5) {
            tier = 'A';
        } // Lowered A-tier threshold
        else if (professional_score >= 40 && risk <= 6) {
            tier = 'B';
        }
        else if (professional_score >= 30 && risk <= 7) {
            tier = 'C';
        }
        else {
            tier = 'D';
     ...
     */
    calculateKellyFraction(features, compositeScore, riskAssessment) {
     ...
     */
    async getHistoricalAccuracy(_features) {
     ...
     */
    async logPerformance(_features, result, _processingTime) {
        await this.performanceAnalyzer.logGradingPerformance(result, 0, // actualOutcome - will be updated when result is known
    ...
    async optimizeWeights(timeframe = '30d') {
        ...
        Object.keys(optimizedWeights).forEach(key => {
            optimizedWeights[key] /= totalWeight;
        });
    ...
    analyzeFeaturePerformance(performanceData) {
    ...
    calculatePerformanceCorrelation(_feature, performanceData) {
    ...
    async scoreProps(propsList) {
        const results = await Promise.all(propsList.map(features => this.scoreProp(features)));
     ...
     */
    async calculateProfessionalInsights(features) {
     ...
     */
    async calculateProfessionalCapperScore(features, weights) {
        let professional_score = 0;
        const propId = features.propId;
        try {
     ...
     * 2. Closing Line Prediction
     */
    async predictClosingLine(propId, features) {
     ...
     */
    calculateOptimalBettingTime(features) {
        const hoursToGame = this.calculateHoursToGame(features);
     ...
     */
    async findBestAvailableLine(propId) {
     ...
     */
    async getBettingPercentages(propId) {
     ...
     */
    calculateInjuryTimingAdvantage(features) {
        const hasRecentInjuryNews = features.injuryImpact > 3;
        const hoursToGame = this.calculateHoursToGame(features);
        if (!hasRecentInjuryNews)
            return 2; // Base professional_score
     ...
     */
    async calculateCrossMarketArbitrage(propId) {
    ...
    calculateHoursToGame(features) {
        try {
            ...
            if (!gameDate)
                return 12; // Default to 12 hours if no date
    ...
    calculateLineTrend(lineHistory) {
        if (lineHistory.length < 2)
            return 0;
     ...
     */
    updateLineMovement(propId, line, volume) {
     ...
     */
    updateBettingPercentages(propId, publicPercentage, sharpPercentage) {
        this.bettingPercentages.set(propId, {
            public: publicPercentage,
            sharp: sharpPercentage,
            timestamp: Date.now()
     ...
     */
    updateBookLines(propId, book, line, odds) {
        const bookData = this.bookLines.get(propId) || [];
     ...
     */
    addInjuryNews(propId, severity, newsBreakTimestamp) {
     ...
     */
    addCrossMarketRelationship(propId, relatedPropId, correlation) {
    ...
    async devigOdds(features) {
        ...
        const edgeResult = DeviggingService_1.deviggingService.calculateEdge(modelProbability, marketOdds, true // Include Kelly multiplier
        );
        return {
            originalEdge,
     ...
     */
    async startCLVTracking(features) {
        try {
            await CLVTrackingService_1.clvTrackingService.trackPick({
                propId: features.propId,
                sport: features.sport,
     ...
     */
    calculateModelProbability(features) {
     ...
     */
    calculateOppositeOdds(_primaryOdds, modelProb) {
     ...
     */
    probToAmericanOdds(probability) {
        if (probability >= 0.5) {
     ...
     */
    async triggerFeedbackLoop() {
        try {
            // Dynamically import to avoid circular dependency
            const { feedbackLoopService } = await Promise.resolve().then(() => __importStar(require('../../../services/feedback/FeedbackLoopService')));
...
exports.SyndicateGradingEngine = SyndicateGradingEngine;
