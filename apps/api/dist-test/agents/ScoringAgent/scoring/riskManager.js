...
class RiskManager {
     ...
     */
    async calculateKellyPositionSize(winProbability, odds, confidence, riskFactors = {}) {
     ...
     */
    applyRiskAdjustments(kellyFraction, confidence, riskFactors) {
     ...
     */
    async calculatePositionSize(kellyFraction, riskScore, portfolioContext) {
    ...
    async assessPropRisk(prop) {
        ...
        const overallRisk = (riskMetrics.individualRisk * 0.3 +
     ...
     */
    async calculatePositionRiskMetrics(prop) {
     ...
     */
    calculateIndividualRisk(odds, confidence) {
     ...
     */
    async calculateCorrelationRisk(prop) {
     ...
     */
    async getCorrelation(prop1, prop2) {
     ...
     */
    async calculatePortfolioContribution(prop) {
     ...
     */
    calculateVaR(prop, confidenceLevel) {
     ...
     */
    calculateCVaR(prop, confidenceLevel) {
     ...
     */
    async calculateConcentrationRisk(prop) {
     ...
     */
    getMarketTypeExposure(marketType) {
        let totalExposure = 0;
        for (const [, position] of this.positions) {
     ...
     */
    async calculateCurrentPortfolioRisk() {
     ...
     */
    async simulatePortfolioRisk(newProp) {
     ...
     */
    async adjustPortfolioRisk(gradingResults) {
        const portfolioRisk = await this.calculateCurrentPortfolioRisk();
    ...
    async calculatePortfolioVariance(_positions) {
        // Simplified portfolio variance calculation
        return 0.10; // 10% default portfolio risk
    }
    async calculateSharpeRatio(_positions) {
    ...
    async calculatePortfolioVaR(_positions) {
        // Simplified portfolio VaR
        return 0.04; // 4% default VaR
    }
    async buildCorrelationMatrix(_positions) {
        // Simplified correlation matrix
        return {};
    }
    calculateRiskByCategory(positions) {
    ...
    generateRiskRecommendations(portfolioRisk, alerts) {
