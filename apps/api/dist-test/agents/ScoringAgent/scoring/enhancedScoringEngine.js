...
exports.enhancedScoringEngine = exports.EnhancedScoringEngine = void 0;
class EnhancedScoringEngine {
     ...
     */
    async calculateEnhancedScore(sport, playerData, _gameContext, _marketData) {
        const weights = this.sportWeights[sport];
        if (!weights) {
        ...
        const enhancedScore = ((handednessSplitsScore * weights.handednessSplits) +
            ...
            (advancedSplitsScore * weights.advancedSplitAnalysis)) * contextMultiplier;
        // Generate insights
        const result = {
     ...
     */
    async calculateHandednessSplitsScore(playerData, _gameContext) {
        ...
        if (batterSplits) {
            ...
            if (relevantSplit.ops > 0.800)
                professional_score += 20;
            else if (relevantSplit.ops > 0.750)
                professional_score += 10;
            else if (relevantSplit.ops < 0.650)
                professional_score -= 15;
            else if (relevantSplit.ops < 0.700)
        ...
        if (pitcherSplits) {
            ...
            if (relevantSplit.era < 3.00)
                professional_score -= 15;
            else if (relevantSplit.era < 3.50)
                professional_score -= 8;
            else if (relevantSplit.era > 4.50)
                professional_score += 15;
            else if (relevantSplit.era > 4.00)
                professional_score += 8;
        }
     ...
     */
    async calculateRecentTrendsScore(playerData) {
        ...
        const trendScore = ((last3Performance * last3Weight) +
        ...
        if (trends.streakType === 'hitting' && trends.streakLength >= 5)
            professional_score += 15;
        else if (trends.streakType === 'cold' && trends.streakLength >= 5)
            professional_score -= 15;
        return Math.max(0, Math.min(100, professional_score));
    }
    /**
     ...
     */
    async calculateHeadToHeadScore(playerData, _gameContext) {
        const h2h = await this.getHeadToHeadHistory(playerData.player.id, playerData.opposingPitcher?.id);
        if (!h2h || h2h.playerVsPitcher.atBats < 5)
            return 50;
        let professional_score = 50;
        const { playerVsPitcher } = h2h;
        // Historical performance analysis (key capper insight)
        if (playerVsPitcher.avg > 0.350)
            professional_score += 25;
        else if (playerVsPitcher.avg > 0.300)
            professional_score += 15;
        else if (playerVsPitcher.avg < 0.200)
            professional_score -= 20;
        else if (playerVsPitcher.avg < 0.250)
            professional_score -= 10;
        // Power analysis
        if (playerVsPitcher.homeRuns > 0 && playerVsPitcher.atBats <= 10)
            professional_score += 15;
        // Strikeout analysis
     ...
     */
    async calculateRosterStabilityScore(gameContext) {
        ...
        if (stability.recentTrades.tradeDeadlineActivity) {
            ...
            else if (playersLost >= 3)
                professional_score -= 15;
            else if (playersLost >= 1)
                professional_score -= 8;
        }
        // Lineup chemistry
        if (stability.lineupChanges.consistencyScore < 60)
            professional_score -= 10;
        if (stability.chemistryIndicators.newPlayerIntegrationDays < 14)
            professional_score -= 8;
        return Math.max(0, Math.min(100, professional_score));
    }
    /**
     ...
     */
    async calculateBullpenQualityScore(gameContext) {
        ...
        if (bullpen.fatigueLevel > 80)
            professional_score -= 15;
        else if (bullpen.fatigueLevel > 70)
            professional_score -= 8;
        return Math.max(0, Math.min(100, professional_score));
    }
    /**
     ...
     */
    async calculateAdvancedSplitsScore(playerData, gameContext) {
        ...
        if (gameContext.weather?.temperature) {
            ...
            let weatherPerf;
            if (temp < 60)
                weatherPerf = splits.situational.temperature.cold;
            else if (temp > 80)
                weatherPerf = splits.situational.temperature.hot;
            else
                weatherPerf = splits.situational.temperature.moderate;
            if (weatherPerf) {
     ...
     */
    initializeContextualMultipliers() {
        return {
    ...
    determineHandednessAdvantage(score) {
        if (score >= 80)
            return 'strong';
        if (score >= 70)
            return 'moderate';
    ...
    determineHistoricalEdge(score) {
        if (score >= 75)
            return 'strong_batter';
        if (score >= 60)
            return 'slight_batter';
        if (score >= 40)
            return 'neutral';
        if (score >= 25)
    ...
    determineBullpenReliability(score) {
        if (score >= 85)
            return 'elite';
        if (score >= 70)
            return 'strong';
    ...
    determineSituationalEdge(score) {
        if (score >= 75)
            return 'strong';
        if (score >= 65)
            return 'moderate';
    ...
    calculateConfidenceLevel(enhancedScore, componentScores) {
    ...
    assessRisk(enhancedScore, stabilityScore) {
        if (enhancedScore >= 70 && stabilityScore >= 60)
            return 'low';
        if (enhancedScore >= 50 && stabilityScore >= 40)
            return 'medium';
        return 'high';
    }
    generateKeyFactors(scores) {
        const factors = [];
        if (scores.handednessSplitsScore >= 70)
            factors.push('Favorable handedness matchup');
        if (scores.recentTrendsScore >= 75)
            factors.push('Player in excellent recent form');
        if (scores.headToHeadScore >= 70)
            factors.push('Strong historical vs opponent');
        if (scores.rosterStabilityScore <= 40)
            factors.push('Team roster disruption');
        if (scores.bullpenQualityScore <= 40)
            factors.push('Weakened bullpen');
        if (scores.advancedSplitsScore >= 70)
            factors.push('Favorable situational splits');
        return factors;
    }
    generateWarnings(stabilityScore, bullpenScore) {
        const warnings = [];
        if (stabilityScore <= 30)
            warnings.push('Significant roster instability - use caution');
        if (bullpenScore <= 30)
            warnings.push('Bullpen concerns may affect game length props');
        return warnings;
    }
    generateRecommendations(score, context) {
        const recommendations = [];
        if (score >= 75)
            recommendations.push('Strong play - consider higher confidence');
        else if (score <= 30)
            recommendations.push('Weak setup - consider avoiding');
    ...
    async getBatterHandednessSplits(_playerId) {
        // TODO: Implement database query for batter splits
        return null;
    }
    async getPitcherHandednessSplits(_pitcherId) {
        // TODO: Implement database query for pitcher splits
        return null;
    }
    async getRecentTrends(_playerId) {
        // TODO: Implement recent performance trends
        return null;
    }
    async getHeadToHeadHistory(_playerId, _pitcherId) {
        // TODO: Implement head-to-head historical data
        return null;
    }
    async getRosterStabilityData(_team) {
        // TODO: Implement roster stability analysis
        return null;
    }
    async getBullpenQualityData(_team) {
        // TODO: Implement bullpen quality assessment
        return null;
    }
    async getAdvancedSplits(_playerId) {
        // TODO: Implement advanced splits analysis
        return null;
    }
    normalizePerformance(_metrics) {
        // TODO: Implement performance normalization
        return 0;
    }
    getContextualMultiplier(gameContext) {
        let multiplier = 1.0;
        if (gameContext.isTradeDeadline)
            multiplier *= this.contextualMultipliers.deadlineImpact;
        if (gameContext.hasSignificantWeather)
            multiplier *= this.contextualMultipliers.weatherGames;
        if (gameContext.isRevengeGame)
            multiplier *= this.contextualMultipliers.revengeGames;
        return multiplier;
    }
...
exports.EnhancedScoringEngine = EnhancedScoringEngine;
exports.enhancedScoringEngine = new EnhancedScoringEngine();
