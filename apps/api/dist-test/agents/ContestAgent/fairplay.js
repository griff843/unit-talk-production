...
exports.FairPlayMonitor = void 0;
const prom_client_1 = require("prom-client");
const zod_1 = require("zod");
const errorHandling_1 = require("../../utils/errorHandling");
const logger_1 = require("../../utils/logger");
// Validation schemas
const fairPlayViolationSchema = zod_1.z.object({
    ruleId: zod_1.z.string().uuid(),
    participantId: zod_1.z.string().uuid(),
    timestamp: zod_1.z.date(),
    severity: zod_1.z.enum(['low', 'medium', 'high', 'critical']),
    evidence: zod_1.z.record(zod_1.z.string(), zod_1.z.any()),
    action: zod_1.z.string(),
    status: zod_1.z.enum(['pending', 'resolved', 'appealed'])
});
class FairPlayMonitor {
    constructor(supabase, _config) {
        this.supabase = supabase;
        this.logger = (0, logger_1.makeLogger)('FairPlayMonitor');
        this.errorHandler = new errorHandling_1.ErrorHandler('FairPlayMonitor', supabase);
        this.patternCache = new Map();
        // Initialize Prometheus metrics
        this.metrics = {
            violationsDetected: new prom_client_1.Counter({
                name: 'fairplay_violations_total',
                help: 'Total number of fair play violations detected',
                labelNames: ['severity', 'type']
            }),
            activeMonitors: new prom_client_1.Gauge({
                name: 'fairplay_active_monitors',
                help: 'Number of active fair play monitoring sessions'
            }),
            checkLatency: new prom_client_1.Histogram({
                name: 'fairplay_check_duration_seconds',
                help: 'Duration of fair play checks',
                buckets: [0.1, 0.5, 1, 2, 5]
            }),
            falsePositiveRate: new prom_client_1.Gauge({
                name: 'fairplay_false_positive_rate',
                help: 'Rate of false positive detections'
            }),
            appealRate: new prom_client_1.Gauge({
    ...
    async initialize() {
        try {
            ...
            this.errorHandler.handleError(error, { context: 'Failed to initialize fair play monitor' });
            throw error;
        }
    }
    async cleanup() {
    ...
    async handleActivityUpdate(payload) {
        try {
            ...
            this.errorHandler.handleError(error, { context: 'Failed to check fair play' });
        }
    }
    async loadParticipantHistory(participantId) {
    ...
    async reportViolation(participant, violation) {
        try {
            const fullViolation = {
                ...violation,
            ...
            this.errorHandler.handleError(error, { context: 'Failed to report violation' });
        }
    }
    calculateScoreReduction(severity) {
        ...
        return reductions[severity] || 5;
    }
    async updateFairPlayScore(participantId, reduction) {
    ...
    async checkMultipleAccounts(participant) {
        try {
            const startTime = process.hrtime();
            ...
            this.errorHandler.handleError(error, { context: 'Failed to check multiple accounts' });
        }
    }
    async analyzeAccountRelationship(participantId, relatedId) {
    ...
    checkAlternatingActivity(patterns1, patterns2) {
        ...
        for (let i = 1; i < allTimestamps.length; i++) {
            ...
            const isAccount1 = patterns1.some(p => p.timestamps.some(t => t.getTime() === currentTimestamp.getTime()));
    ...
    checkBehaviorSimilarity(patterns1, patterns2) {
    ...
    compareValueDistributions(values1, values2) {
        // Create histograms
    ...
    async checkResourceSharing(participantId1, participantId2) {
    ...
    async checkBettingPatterns(participant) {
        try {
            ...
            this.errorHandler?.handleError(error, { context: 'Failed to check betting patterns' });
        }
    }
    analyzeBettingBehavior(patterns) {
        const evidence = {};
    ...
    findRapidSequences(timestamps, threshold) {
        const sequences = [];
        let currentSequence = [];
    ...
    async checkCollusion(participant) {
        try {
            ...
            this.errorHandler.handleError(error, { context: 'Failed to check collusion' });
        }
    }
    analyzeCollusionPatterns(participantBets, relatedBets) {
        const evidence = {};
        let suspiciousFactors = 0;
        // Group bets by event
        const eventBets = new Map();
        ...
        participantBets.forEach(bet => {
            ...
            const hasComplement = relatedEventBets.some(rb => rb.bet_type === this.getComplementaryBet(bet.bet_type) &&
    ...
    analyzeProfitPatterns(participantBets, relatedBets) {
        // Group by counterparty
        const profitsByCounterparty = new Map();
        participantBets.forEach(bet => {
        ...
        const suspiciousCorrelations = Array.from(profitsByCounterparty.entries())
            ...
            const correlation = this.calculateCorrelation(profits, relatedBets
    ...
    async checkTimeAnomaly(participant) {
        try {
            const patterns = await this.loadParticipantHistory(participant.id);
            ...
            this.errorHandler.handleError(error, { context: 'Failed to check time anomalies' });
        }
    }
...
exports.FairPlayMonitor = FairPlayMonitor;
