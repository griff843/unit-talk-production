...
exports.promoteToDiscord = promoteToDiscord;
...
// ---- IMAGE GENERATOR ----
async function generateEliteCard(_) {
...
// ---- POSTER ----
async function postEliteCardToDiscord(pick) {
    if (!DISCORD_WEBHOOK_URL) {
        logging_1.logger.error('No Discord webhook URL set!');
        return;
    }
    ...
    try {
        await axios_1.default.post(DISCORD_WEBHOOK_URL, form, {
        ...
        logging_1.logger.error({ id: pick.id, error: err?.message || err }, 'Discord image-card post error');
        console.error('Discord image-card post error:', err);
    }
}
// ---- AGENT ----
async function promoteToDiscord() {
    const { data: picks, error } = await supabaseClient_1.supabase
        ...
        .limit(10);
    if (error) {
        logging_1.logger.error(error, 'Error fetching picks for Discord promotion');
        throw error;
    }
    if (!picks || picks.length === 0) {
        logging_1.logger.info('No eligible picks found for Discord promotion.');
        return;
    }
    for (const pick of picks) {
        await postEliteCardToDiscord(pick);
        try {
            await supabaseClient_1.supabase.from('unified_picks').update({ posted_to_discord: true }).eq('id', pick.id);
            logging_1.logger.info({ id: pick.id }, 'Posted pick to Discord with image-card');
        }
        catch (err) {
            logging_1.logger.error({ id: pick.id, error: err?.message || err }, 'Failed to update posted_to_discord flag');
        }
    }
}
