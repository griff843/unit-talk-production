...
const fetchRawProps_1 = require("./fetchRawProps");
const types_1 = require("./types");
const validation_1 = require("./validation");
/**
 * Create a proper IngestionAgent configuration that extends BaseAgentConfig
 */
function createIngestionAgentConfig(config) {
 ...
 */
class IngestionAgent extends index_1.BaseAgent {
    constructor(config, dependencies) {
        ...
        let ingestionConfig;
        try {
            ingestionConfig = types_1.IngestionAgentConfigSchema.parse(config);
        }
        catch (error) {
    ...
    async fetchAllRawProps() {
        ...
        for (const provider of this.fullConfig.providers) {
            ...
            try {
                ...
                const props = await (0, fetchRawProps_1.fetchRawProps)(provider);
                allProps.push(...props);
     ...
     * Process props in batches
     */
    async processPropsBatch(rawProps) {
    ...
    async processSingleProp(prop) {
        try {
            ...
            // Validate the prop
            if (!(0, validation_1.validateRawProp)(prop)) {
            ...
            const isDuplicate = await this.checkForDuplicate(prop);
            if (isDuplicate) {
            ...
            // Normalize the prop
            const normalizationResult = (0, validation_1.normalizeRawProp)(prop);
