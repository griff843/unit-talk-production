...
class NotionSyncService {
                            ...
                            rich_text: [
                                {
                                    type: 'text',
                                    text: {
                                        content: entry.summary.capperBreakdown.map(capper => `${capper.capper}: ${capper.wins}W-${capper.losses}L, ${capper.netUnits.toFixed(1)}U, ${capper.roi.toFixed(1)}% ROI`).join('\n')
                                    }
                                }
                            ]
                        }
     ...
     */
    async searchRecaps(period, startDate, endDate) {
        try {
            const filter = {};
            if (period) {
     ...
     */
    mapNotionPageToRecapEntry(page) {
        const properties = page.properties;
        return {
            id: page.id,
            title: properties.Title?.title?.[0]?.text?.content || '',
            date: properties.Date?.date?.start || '',
            period: properties.Period?.select?.name || 'daily',
            summary: {
                date: properties.Date?.date?.start || '',
                period: properties.Period?.select?.name || 'daily',
