...
const oddsApi_1 = require("../FeedAgent/oddsApi");
class SettlementAgent extends index_1.BaseAgent {
    constructor(config, deps) {
        super(config, deps);
        this.SETTLEMENT_INTERVALS = {
    ...
    async checkHealth() {
        ...
        const overallStatus = healthyServices === checks.length ? 'healthy' :
     ...
     */
    async processGameSettlement(game) {
        ...
        try {
            ...
            this.logger.info(`✅ Game settlement completed: ${game.external_game_id} (${processingTime}ms)`);
        }
        catch (error) {
     ...
     */
    async fetchGameSettlementData(game) {
        try {
            ...
            const settlementData = await (0, oddsApi_1.fetchSettlementData)(oddsApiSport, 3); // 3 days back
            // Find the specific game in settlement data
            const gameData = settlementData.find(data => data.id === game.external_game_id ||
     ...
     */
    async updateGameResult(game, settlementData) {
        if (!this.hasSupabase())
            return;
        try {
            const updateData = {
            ...
            if (settlementData.scores && settlementData.scores.length >= 2) {
                const homeScore = parseInt(settlementData.scores.find((s) => s.name === game.home_team)?.professional_score || '0');
                const awayScore = parseInt(settlementData.scores.find((s) => s.name === game.away_team)?.professional_score || '0');
                if (homeScore !== game.home_score || awayScore !== game.away_score) {
                    updateData.home_score = homeScore;
                    updateData.away_score = awayScore;
                    this.logger.info(`📊 Score updated for ${game.external_game_id}: ${game.away_team} ${awayScore} - ${game.home_team} ${homeScore}`);
                }
            }
            const { error } = await this.requireSupabase()
                .from('game_results')
     ...
     */
    async processGameProps(game, settlementData) {
        if (!this.hasSupabase())
            return;
        try {
     ...
     */
    async settleProp(prop, game, settlementData) {
        if (!this.hasSupabase())
            return;
        try {
            ...
            this.logger.debug(`✅ Prop settled: ${prop.player_name} ${prop.stat_type} ${settlement.settlement_result}`);
        }
        catch (error) {
     ...
     */
    async calculatePropSettlement(prop, game, settlementData) {
     ...
     */
    async updateUnifiedPickSettlement(prop, settlement) {
        if (!this.hasSupabase())
            return;
        try {
            // Find corresponding final pick
            ...
            for (const finalPick of finalPicks) {
                const { error: updateError } = await this.requireSupabase()
                    ...
                    .eq('id', finalPick.id);
                if (updateError) {
                    this.logger.error(`❌ Failed to update final pick settlement: ${finalPick.id}`, {
     ...
     */
    async processDelayedSettlements() {
     ...
     */
    async validateExistingSettlements() {
     ...
     */
    async manualSettle(propId, result, actualValue, notes) {
        if (!this.hasSupabase()) {
