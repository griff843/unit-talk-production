...
exports.HedgeEngine = void 0;
const enhanced_cache_1 = require("../../cache/enhanced-cache");
class HedgeEngine {
    constructor(logger) {
        this.hedgeHistory = new Map();
        this.marketData = new Map();
        this.hedgePerformance = new Map();
    ...
    async identifyHedgeOpportunities(positions, correlations) {
    ...
    async analyzeHedgingNeed(positions, riskTolerance = 0.1) {
        ...
        try {
            ...
            const recommendedHedges = await this.selectOptimalHedges(hedgeOpportunities, portfolioRisk, riskTolerance);
            // Calculate metrics
    ...
    async executeHedgeRecommendation(hedgeOpportunity, userId) {
    ...
    async monitorActiveHedges(userId) {
    ...
    async identifyDirectHedges(positions) {
        const directHedges = [];
    ...
    async createDirectHedge(position) {
    ...
    async findOppositeHedge(position) {
    ...
    async getOppositeOdds(position) {
    ...
    async identifyCorrelationHedges(positions, correlations) {
        const correlationHedges = [];
    ...
    async createCorrelationHedge(pos1, pos2, correlation) {
    ...
    async identifyPortfolioHedges(positions) {
        if (positions.length < 3)
            return []; // Need multiple positions for portfolio hedge
    ...
    async identifyInsuranceHedges(positions) {
        const insuranceHedges = [];
        for (const position of positions) {
    ...
    async createInsuranceHedge(position) {
        const hedgeInstrument = {
            type: 'insurance_bet',
            gameId: position.gameId,
    ...
    async identifyDynamicHedges(positions) {
    ...
    async createDynamicHedge(position, marketConditions) {
    ...
    async calculateHedgeEffectiveness(position, hedgeInstrument) {
    ...
    async calculateTimeDecay(position) {
        ...
        if (hoursToEvent > 48)
            return 0.01; // Low decay for distant events
        if (hoursToEvent > 24)
            return 0.02; // Medium decay
        if (hoursToEvent > 12)
            return 0.05; // Higher decay
        if (hoursToEvent > 2)
            return 0.1; // High decay
        return 0.2; // Very high decay for imminent events
    }
    getTimeToEvent(position) {
        // This would get actual event time from game data
        // For now, assume events are within 24 hours
        return 24 * 60 * 60 * 1000; // 24 hours in milliseconds
    }
    async getMarketConditions(gameId) {
    ...
    async filterViableOpportunities(opportunities) {
    ...
    async selectOptimalHedges(opportunities, portfolioRisk, riskTolerance) {
        const selected = [];
    ...
    determineRiskProfile(portfolioRisk, riskTolerance) {
    ...
    determinePriority(effectiveness, cost, riskReduction) {
        const benefitCostRatio = riskReduction / cost;
        if (effectiveness > 0.8 && benefitCostRatio > 3)
            return 'high';
        if (effectiveness > 0.6 && benefitCostRatio > 2)
            return 'medium';
        return 'low';
    }
    // Recommendation Generation Methods
    async generateDirectHedgeRecommendation(effectiveness, cost) {
    ...
    async generateCorrelationHedgeRecommendation(correlation, effectiveness) {
    ...
    async generatePortfolioHedgeRecommendation(totalRisk, effectiveness) {
        if (totalRisk > 0.3) {
            return {
                action: 'hedge_now',
    ...
    async generateInsuranceHedgeRecommendation(position, effectiveness) {
        return {
            action: 'hedge_conditional',
    ...
    async generateDynamicHedgeRecommendation(marketConditions, effectiveness) {
    ...
    async validateHedgeViability(hedge) {
    ...
    async checkExecutionPermissions(hedge, userId) {
        // Check user permissions, available funds, risk limits, etc.
        return true; // Simplified for now
    }
    async executeHedge(hedge, userId) {
    ...
    async recordHedgeExecution(hedge, userId) {
        const execution = {
            hedgeId: hedge.id,
            userId,
        ...
        await enhanced_cache_1.redisCache.set(`hedge:execution:${hedge.id}`, JSON.stringify(execution), 86400 // 24 hours TTL
        );
    }
    async getActiveHedges(userId) {
        const cached = await enhanced_cache_1.redisCache.get(`hedge:active:${userId}`);
        return cached ? JSON.parse(cached) : [];
    }
    async updateHedgeEffectiveness(hedge) {
    ...
    async generateAdjustmentRecommendation(hedge) {
        return {
            action: 'monitor',
    ...
    async cacheHedgeOpportunities(userId, opportunities) {
        await enhanced_cache_1.redisCache.set(`hedge:opportunities:${userId}`, JSON.stringify(opportunities), 1800 // 30 minutes TTL
        );
    }
    async cacheActiveHedges(userId, hedges) {
        await enhanced_cache_1.redisCache.set(`hedge:active:${userId}`, JSON.stringify(hedges), 3600 // 1 hour TTL
        );
    }
    async loadHedgeStrategies() {
        const strategies = {
    ...
    async loadHedgeHistory() {
        try {
            const cachedHistory = await enhanced_cache_1.redisCache.getPattern('hedge:history:*');
    ...
    async cleanup() {
        ...
        for (const [userId, history] of this.hedgeHistory) {
            await enhanced_cache_1.redisCache.set(`hedge:history:${userId}`, JSON.stringify(history), 86400 // 24 hours TTL
            );
        }
...
exports.HedgeEngine = HedgeEngine;
