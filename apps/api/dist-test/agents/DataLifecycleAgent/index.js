...
exports.DataLifecycleAgent = void 0;
 ...
 */
class DataLifecycleAgent extends BaseAgent_1.BaseAgent {
    ...
    async archiveHotToWarm() {
        ...
        try {
            ...
            const { data, error } = await enhanced_circuit_breaker_1.withCircuitBreaker.supabase(async () => await this.requireSupabase().rpc('archive_hot_to_warm', {
    ...
    async archiveWarmToCold() {
        ...
        try {
            ...
            const { data, error } = await enhanced_circuit_breaker_1.withCircuitBreaker.supabase(async () => await this.requireSupabase().rpc('archive_warm_to_cold', {
                p_cutoff_date: cutoffDate
            }), async () => {
    ...
    async triggerParquetExport(archiveId) {
        ...
        try {
            ...
            await enhanced_cache_1.redisCache.set(`parquet_export:${archiveId}`, JSON.stringify({
                archiveId,
                status: 'pending',
    ...
    // Helper Methods
    async ensureHistoricalTablesExist() {
    ...
    async countRecordsToArchive(tableName, cutoffDate) {
        if (!this.hasSupabase())
            return 0;
        try {
            const { count, error } = await this.requireSupabase()
    ...
    async moveRecordsBatched(sourceTable, targetTable, cutoffDate, batchSize) {
        if (!this.hasSupabase())
            return;
        await enhanced_circuit_breaker_1.withCircuitBreaker.supabase(async () => {
    ...
    async checkRetentionViolations() {
        ...
        try {
            ...
            const hotViolations = await this.countRecordsToArchive('raw_props', this.calculateCutoffDate(this.retentionPolicy.hotTierDays));
            // Check if warm tier has old data
            const warmViolations = await this.countRecordsToArchive('raw_props_recent', this.calculateCutoffDate(this.retentionPolicy.warmTierDays));
    ...
    async loadLifecycleMetrics() {
        try {
            const cached = await enhanced_cache_1.redisCache.get('lifecycle:metrics');
            if (cached) {
    ...
    async generateLifecycleInsights() {
        ...
        await enhanced_cache_1.redisCache.set('lifecycle:insights', JSON.stringify(insights), 3600 // 1 hour TTL
        );
        // Cache metrics for next run
        await enhanced_cache_1.redisCache.set('lifecycle:metrics', JSON.stringify(this.lifecycleMetrics), 86400 // 24 hours TTL
        );
    ...
    async cleanup() {
        ...
        // Save final metrics
        await enhanced_cache_1.redisCache.set('lifecycle:metrics', JSON.stringify(this.lifecycleMetrics), 86400);
...
exports.DataLifecycleAgent = DataLifecycleAgent;
