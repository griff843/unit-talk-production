...
exports.PerformanceOptimizationAgent = void 0;
...
class PerformanceOptimizationAgent extends BaseAgent_1.BaseAgent {
    constructor(config, deps) {
        ...
        this.systemMonitor = new systemMonitor_1.SystemMonitor(this.logger);
        this.performanceOptimizer = new performanceOptimizer_1.PerformanceOptimizer(this.logger);
        this.bottleneckDetector = new bottleneckDetector_1.BottleneckDetector(this.logger);
        this.resourceManager = new resourceManager_1.ResourceManager(this.logger);
    }
    async initialize() {
    ...
    async generateOptimizationRecommendations() {
        ...
        try {
            ...
            const highPriorityRecs = prioritizedRecommendations.filter(r => r.priority === 'critical' || r.priority === 'high');
    ...
    async applyAutomatedOptimizations() {
        ...
        try {
            ...
            for (const recommendation of this.systemHealth.recommendations) {
                ...
                const automatedActions = recommendation.actions.filter(a => a.automated && a.safetyRating >= 0.8);
    ...
    async updateResourcePredictions() {
        ...
        try {
            ...
            await enhanced_cache_1.redisCache.set('performance:resource_predictions', JSON.stringify(predictions), 1800 // 30 minutes TTL
            );
        }
        catch (error) {
    ...
    async generatePerformanceReport() {
        ...
        try {
            ...
            // Cache for dashboard
            await enhanced_cache_1.redisCache.set('performance:latest_report', JSON.stringify(report), 1800 // 30 minutes TTL
            );
    ...
    async sendBottleneckAlert(bottleneck) {
    ...
    async logBottleneckResolution(bottleneck) {
    ...
    async sendResourceWarning(prediction) {
    ...
    async storeMetricsForTrendAnalysis(metrics) {
        const timestamp = Date.now();
        await enhanced_cache_1.redisCache.set(`performance:metrics:${timestamp}`, JSON.stringify({
            timestamp,
            cpu: metrics.cpu.usage,
            memory: metrics.memory.usage,
            disk: metrics.disk.usage,
    ...
    async getHistoricalMetrics() {
        ...
        const cached = await enhanced_cache_1.redisCache.getPattern(pattern);
        return Array.from(cached.values()).map(data => JSON.parse(data));
    }
    ...
    async getResourcePredictions() {
        const cached = await enhanced_cache_1.redisCache.get('performance:resource_predictions');
    ...
    async loadPerformanceBaseline() {
        try {
            const cached = await enhanced_cache_1.redisCache.get('performance:baseline');
            if (cached) {
    ...
    async loadOptimizationHistory() {
        try {
            const cached = await enhanced_cache_1.redisCache.get('performance:optimization_history');
            if (cached) {
    ...
    async loadActiveBottlenecks() {
        try {
            const cached = await enhanced_cache_1.redisCache.getPattern('performance:bottleneck:*');
            for (const [_key, data] of cached) {
                const bottleneck = JSON.parse(data);
    ...
    async cleanup() {
        ...
        await enhanced_cache_1.redisCache.set('performance:baseline', JSON.stringify(this.performanceBaseline), 86400 // 24 hours TTL
        );
        // Save optimization history
        await enhanced_cache_1.redisCache.set('performance:optimization_history', JSON.stringify(this.optimizationHistory), 86400 // 24 hours TTL
        );
        // Save active bottlenecks
        for (const [id, bottleneck] of this.activeBottlenecks) {
            await enhanced_cache_1.redisCache.set(`performance:bottleneck:${id}`, JSON.stringify(bottleneck), 86400 // 24 hours TTL
            );
        }
        await Promise.all([
...
exports.PerformanceOptimizationAgent = PerformanceOptimizationAgent;
