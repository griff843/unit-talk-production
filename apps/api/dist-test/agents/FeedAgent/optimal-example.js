...
exports.integrationExample = integrationExample;
/**
...
const dedupePublicProps_1 = require("./utils/dedupePublicProps");
async function exampleUsage() {
    try {
        // Check rate limit status before making requests
        const rateLimitStatus = (0, optimal_1.getRateLimitStatus)();
        ...
        const nbaProps = await (0, optimal_1.fetchOptimalProps)('NBA', '2024-01-15');
        ...
        for (const league of leagues) {
            try {
                ...
                const status = (0, optimal_1.getRateLimitStatus)();
                if (!status.canMakeRequest) {
                    console.log(`Rate limit reached, skipping ${league}`);
                    continue;
                }
                console.log(`\nFetching ${league} props...`);
                const props = await (0, optimal_1.fetchOptimalProps)(league, date);
...
async function integrationExample() {
    ...
    try {
        ...
        const optimalProps = await (0, optimal_1.fetchOptimalProps)(league, currentDate);
        ...
        // Deduplicate props
        const dedupedProps = await (0, dedupePublicProps_1.dedupePublicProps)(optimalProps, 'Optimal', supabase);
