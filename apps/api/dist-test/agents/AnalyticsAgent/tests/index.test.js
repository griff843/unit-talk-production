...
const testDependencies = {
    supabase: (0, supabase_js_1.createClient)('test-url', 'test-service-role-key'),
    logger: {
        debug: jest.fn(),
        info: jest.fn(),
        warn: jest.fn(),
...
describe('AnalyticsAgent', () => {
    let agent;
    beforeEach(() => {
        agent = new __1.AnalyticsAgent(testConfig, testDependencies);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe('process', () => {
        ...
        it('should handle empty data gracefully', async () => {
            ...
            testDependencies.supabase.from.mockImplementation(mockFrom);
            await expect(agent.__test__initialize()).resolves.not.toThrow();
        });
