...
class AnalyticsAgent extends index_1.BaseAgent {
    ...
    async processBatch(cappers, batchIndex) {
    ...
    async fetchPicksForCappers(capperIds) {
    ...
    async processCapperAnalytics(capperId, picks) {
    ...
    updateProcessingMetrics(result, processingTimeMs) {
    ...
    async storeAnalyticsSummary(capperId, summary) {
        try {
            if (!this.hasSupabase()) {
