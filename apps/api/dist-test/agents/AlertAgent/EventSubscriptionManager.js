...
class EventSubscriptionManager {
    ...
    async setupEventSubscriptions() {
        ...
        try {
            ...
            const eventsChannel = this.supabase
                ...
                schema: 'public',
                table: 'events',
                filter: 'event_type=in.(ticket.submitted.v1,grading.completed.v1,alert.reemit.v1,ticket.processing.completed.v1,ticket.processing.failed.v1)'
            ...
            const alertsChannel = this.supabase
                ...
                schema: 'public',
                table: 'events',
                filter: 'event_type=in.(alert.injury.detected.v1,alert.high_tier.v1,alert.hedge.opportunity.v1,alert.middle.opportunity.v1,alert.line_movement.detected.v1)'
    ...
    async processTicketSubmission(event) {
    ...
    async processGradingCompletion(event) {
    ...
    async processAlertReemission(event) {
    ...
    async analyzeTicketForAlerts(ticketData) {
        const opportunities = [];
        try {
            // Injury detection
    ...
    async analyzeForHedgeMiddle(gradingData) {
        const opportunities = [];
        try {
    ...
    async emitAlert(opportunity, aggregateId) {
    ...
    async emitHighTierAlert(gradingData, aggregateId) {
        const alertData = {
    ...
    async emitReemissionAlert(alertData, aggregateId) {
        await this.supabase
            .from('events')
            .insert({
    ...
    async triggerImmediateAlert(alertType, alertData, aggregateId) {
    ...
    addToProcessingQueue(queueName, event) {
    ...
    async processBatchEvent(event) {
        // Placeholder for batch event processing
        this.logger.debug('Processing batch event', { eventId: event.id });
    }
    // Utility methods
    calculateInjuryConfidence(ticketData) {
        let confidence = 0.5; // Base confidence
        if (ticketData.player_status === 'questionable')
            confidence += 0.2;
        if (ticketData.injury_reports?.length > 0)
            confidence += 0.1;
        if (ticketData.game_time && this.isGameTimeClose(ticketData.game_time))
            confidence += 0.15;
        return Math.min(confidence, 0.95);
    }
    calculateLineMovement(lineHistory) {
    ...
    calculateSteamScore(ticketData) {
    ...
    calculateHedgeOpportunity(gradingData) {
     ...
     */
    async handleBridgeOutboxCompletion(payload) {
     ...
     */
    async processBridgeTicketSubmission(outboxRecord) {
     ...
     */
    async processBridgeTicketCompletion(outboxRecord) {
        ...
        if (completionData.requires_immediate_alert) {
            await this.triggerImmediateAlert('bridge_completion', completionData, outboxRecord.unique_key);
        }
    }
    /**
     * Process bridge outbox grading completion
     */
    async processBridgeGradingCompletion(outboxRecord) {
     ...
     */
    async processWorkflowSuccess(workflowExecution) {
     ...
     */
    async processWorkflowFailure(workflowExecution) {
     ...
     */
    async analyzeWorkflowForAlerts(workflowExecution) {
        const opportunities = [];
        try {
     ...
     */
    async recordBridgeOutboxProcessing(outboxId, status) {
        try {
            await enhanced_circuit_breaker_1.withCircuitBreaker.supabase(async () => {
                await this.supabase
