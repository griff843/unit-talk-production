...
exports.TicketStateManager = exports.TicketType = exports.TicketState = void 0;
...
(function (TicketState) {
    ...
    TicketState["SWEAT"] = "SWEAT";
    TicketState["HEDGE_WINDOW"] = "HEDGE_WINDOW";
    TicketState["DONE"] = "DONE";
})(TicketState || (exports.TicketState = TicketState = {}));
var TicketType;
(function (TicketType) {
    TicketType["SINGLE"] = "single";
    TicketType["PARLAY"] = "parlay";
    TicketType["ROUND_ROBIN"] = "round_robin";
...
class TicketStateManager {
     ...
     */
    async evaluateStateTransition(currentState) {
     ...
     */
    async transitionToState(ticketId, newState, trigger, metadata = {}) {
        const currentState = await this.getTicketState(ticketId);
        if (!currentState) {
            throw new Error(`Ticket state not found: ${ticketId}`);
        }
        ...
        if (!this.isValidTransition(currentState.state, newState)) {
            throw new Error(`Invalid state transition: ${currentState.state} -> ${newState} for ticket ${ticketId}`);
        }
        ...
        return await enhanced_circuit_breaker_1.withCircuitBreaker.supabase(async () => {
            ...
            // Emit state transition event
            await this.emitStateTransitionEvent(ticketId, currentState.state, newState, trigger, metadata);
     ...
     */
    async handleStateActions(ticketState) {
     ...
     */
    async handleLiveState(ticketState) {
     ...
     */
    async handleSweatState(ticketState) {
     ...
     */
    async handleHedgeWindowState(ticketState) {
     ...
     */
    async handleDoneState(ticketState) {
     ...
     */
    async generateHedgeRecommendations(ticketState) {
     ...
     * Emit state transition event
     */
    async emitStateTransitionEvent(ticketId, fromState, toState, trigger, metadata) {
        try {
            await this.supabase
                .from('events')
                .insert({
     ...
     * Emit alert event
     */
    async emitAlertEvent(alertType, ticketId, alertData) {
        try {
            await this.supabase
                .from('events')
                .insert({
     ...
     */
    async getTicketsByState(state, limit = 100) {
        return await enhanced_circuit_breaker_1.withCircuitBreaker.supabase(async () => {
            const { data, error } = await this.supabase
                .from('ticket_states')
                .select('*')
