...
class AIOrchestrator {
    ...
    async getAdviceForPick(pick, _context) {
        ...
        try {
            ...
            if (!this.checkRateLimit(selectedModel.id)) {
                logging_1.logger.warn(`Rate limit exceeded for model ${selectedModel.id}, using fallback`);
            ...
            logging_1.logger.error('Primary advice generation failed, using fallback:', error);
            return this.getFallbackAdvice(pick);
        }
    }
    // Multi-model consensus for high-stakes decisions
    async getConsensusAdvice(pick) {
    ...
    calculateModelScore(model, pick) {
        let modelScore = 0;
        ...
        if (pick.tier === 'S' || pick.tier === 'A') {
            ...
            modelScore += model.performance.accuracy * 0.15;
        }
        return modelScore;
    }
    getAvailableModels() {
...
`;
    ...
    parseAdviceResponse(response, modelId, temperature, processingTime) {
    ...
    updateModelPerformance(modelId, success, latency) {
        const model = this.models.get(modelId);
        if (!model) {
            return;
        }
    ...
    async updateModelConfig(modelId, updates) {
        ...
        if (model) {
            ...
            logging_1.logger.info(`Updated configuration for model ${modelId}`, updates);
        }
    }
    resetCircuitBreaker(modelId) {
        this.circuitBreaker.reset(modelId);
        logging_1.logger.info(`Reset circuit breaker for model ${modelId}`);
    }
    // Methods required by FeedbackLoopAgent
    async initialize() {
        logging_1.logger.info('Initializing AI Orchestrator');
    ...
    async switchToBackupModel(primaryModelId) {
        ...
        if (availableModels.length > 0) {
            ...
            logging_1.logger.info(`Switching from ${primaryModelId} to backup model ${backupModelId}`);
