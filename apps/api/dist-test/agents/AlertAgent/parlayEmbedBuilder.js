...
exports.getPlayerHeadshotUrl = getPlayerHeadshotUrl;
exports.calculateParlayOdds = calculateParlayOdds;
exports.buildParlayAlertEmbed = buildParlayAlertEmbed;
exports.formatParlayPlayerList = formatParlayPlayerList;
const discord_js_1 = require("discord.js");
/**
 * Get player headshot URL based on sport and player ID/name
 */
function getPlayerHeadshotUrl(sport, playerId, playerName) {
 ...
 */
function calculateParlayOdds(odds) {
...
function getTierColor(tier) {
    if (['S+', 'S'].includes(tier))
        return 0xFF0000; // Red for top tier
    if (['A+', 'A'].includes(tier))
        return 0x00FF99; // Green for A tier
    if (['B+', 'B'].includes(tier))
        return 0x0099FF; // Blue for B tier
 ...
 */
function buildParlayAlertEmbed(picks, advice, _options = {}) {
    ...
    const sports = [...new Set(picks.map(p => p.sport || 'Unknown'))].join('/');
    ...
    const avgConfidence = picks.reduce((sum, p) => sum + (p.confidence || 75), 0) / picks.length;
    embed.addFields({
        name: '🏆 Parlay Details',
 ...
 */
function formatParlayPlayerList(picks) {
    return picks.map((pick) => {
        const sportEmoji = getSportEmoji(pick.sport || 'Unknown');
