...
exports.testPlayerEnrichment = testPlayerEnrichment;
 ...
 */
async function testPlayerEnrichment(playerName, league, headshotFn, physicalsFn) {
    const result = {
        player: playerName,
        league,
...
async function testLeague(league) {
    ...
    switch (league) {
        case 'MLB':
            headshotFn = mlbEnrichment_1.getMlbHeadshot;
            physicalsFn = mlbEnrichment_1.getMlbPhysicals;
            break;
        case 'NBA':
            headshotFn = nbaEnrichment_1.getNbaHeadshot;
            physicalsFn = nbaEnrichment_1.getNbaPhysicals;
            break;
        case 'NFL':
            headshotFn = nflEnrichment_1.getNflHeadshot;
            physicalsFn = nflEnrichment_1.getNflPhysicals;
            break;
        case 'NHL':
            headshotFn = nhlEnrichment_1.getNhlHeadshot;
            physicalsFn = nhlEnrichment_1.getNhlPhysicals;
            break;
    }
    // Test each player
...
async function runTests() {
    ...
    try {
        // Test each league
        for (const league of Object.keys(testPlayers)) {
