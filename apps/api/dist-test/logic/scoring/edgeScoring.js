...
exports.getInternalScoringDetails = getInternalScoringDetails;
const edgeConfig_1 = require("../config/edgeConfig");
const zoneThreat_1 = require("../zoneThreat");
/**
 * Extracts pitcher statistics from prop object
 * Returns null if insufficient data for Zone Threat analysis
 */
function extractPitcherStats(prop) {
 ...
 */
function extractMatchupData(prop) {
    const toNumber = (value) => {
        if (typeof value === 'number') {
            return value;
        }
 ...
 */
function isZoneThreatEligible(prop) {
    if (!edgeConfig_1.EDGE_CONFIG.zoneThreat.enabled) {
        return false;
    }
    // Check if market type is eligible for Zone Threat boost
    const eligibleMarkets = edgeConfig_1.EDGE_CONFIG.zoneThreat.hrMarkets;
...
function finalEdgeScore(prop, config, adminOverrideTier) {
    let professional_score = 0, breakdown = {}, tags = [];
    // Market type bonus
    ...
    if (prop['source'] && config.source[prop['source']]) {
        professional_score += config.source[prop['source']];
    ...
    // Tags + boosts
    if (prop['is_rocket']) {
        professional_score += config.tags.rocket;
        tags.push('rocket');
        breakdown['is_rocket'] = config.tags.rocket;
    }
    if (prop['is_ladder']) {
        professional_score += config.tags.ladder;
        tags.push('ladder');
    ...
    if (isZoneThreatEligible(prop)) {
        ...
        if (pitcherStats && matchupData) {
            const threatResult = (0, zoneThreat_1.zoneThreatRating)(matchupData, {
                thresholds: { low: 1, medium: 3, high: 5 },
                weights: { batterBarrel: 1, batterLaunch: 1, parkFactor: 0.5 }
            });
            const zoneThreatBoost = (0, zoneThreat_1.calculateZoneThreatBoost)(threatResult);
            if (zoneThreatBoost > 0) {
                professional_score += zoneThreatBoost;
                ...
                if (config.zoneThreat.logDecisions) {
                    (0, zoneThreat_1.logZoneThreatDecision)({ pitcherStats, matchupData, threatResult });
                }
            }
            // Add zone threat level to breakdown for internal analysis
            breakdown['zone_threat_level'] = threatResult.level;
        }
    }
    ...
    return { score: professional_score, tier, tags, breakdown, postable, solo_lock };
}
// Export alias for ScoringAgent compatibility
function scorePropEdge(prop) {
    const result = finalEdgeScore(prop, edgeConfig_1.EDGE_CONFIG);
 ...
 */
function getInternalScoringDetails(prop) {
    const result = finalEdgeScore(prop, edgeConfig_1.EDGE_CONFIG);
    ...
    if (isZoneThreatEligible(prop)) {
        ...
        zoneThreatAnalysis = {
            eligible: true,
            ...(result.breakdown['zone_threat_level'] && { threatLevel: result.breakdown['zone_threat_level'] }),
            ...(result.breakdown['zone_threat_boost'] !== undefined && { boostApplied: result.breakdown['zone_threat_boost'] || 0 }),
