...
exports.nhlCoreStats = nhlCoreStats;
exports.nhlSynergy = nhlSynergy;
/**
 * NHL Core Statistics Scoring
 * Evaluates fundamental hockey metrics
 */
function nhlCoreStats(prop) {
    const breakdown = {};
...
function nhlSynergy(prop) {
    ...
    if (prop['tags'] && prop['tags'].length > 0) {
        const hasRestTag = prop['tags'].some(tag => typeof tag === 'string' && tag.toLowerCase().includes('rest'));
        if (hasRestTag) {
            breakdown['fatigue_factor'] = -0.1; // Negative for tired teams
        }
        const hasHomeTag = prop['tags'].some(tag => typeof tag === 'string' && tag.toLowerCase().includes('home'));
        if (hasHomeTag) {
