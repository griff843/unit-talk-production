...
exports.mlbCoreStats = mlbCoreStats;
exports.mlbSynergy = mlbSynergy;
/**
 * MLB Core Statistics Scoring
 * Evaluates fundamental baseball metrics
 */
function mlbCoreStats(prop) {
    const breakdown = {};
 ...
 */
function mlbSynergy(prop) {
    const breakdown = {};
    // Use line value professional_score as weather/park factor
    ...
    if (prop['tags'] && prop['tags'].length > 0) {
        ...
        const hasParkTag = prop['tags'].some(tag => typeof tag === 'string' && tag.toLowerCase().includes('park'));
        if (hasParkTag) {
