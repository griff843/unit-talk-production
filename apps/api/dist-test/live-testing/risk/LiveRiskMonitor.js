...
exports.LiveRiskMonitor = void 0;
const events_1 = require("events");
const logger_1 = require("@shared/logger");
class LiveRiskMonitor extends events_1.EventEmitter {
    constructor(config) {
        super();
        this.logger = new logger_1.Logger('LiveRiskMonitor');
    ...
    // ========================================
    assessBetRisk(bet, professionalFeatures) {
        const reasons = [];
        let approved = true;
        let riskLevel = 'LOW';
        // Check basic limits
        ...
        if (concentrationRisk.excessive) {
            ...
            riskLevel = Math.max(riskLevel, concentrationRisk.severity);
        }
    ...
    updateBetStatus(betId, status, pnl) {
    ...
    // ========================================
    validateKellySizing(bet, professionalFeatures) {
    ...
    assessCorrelationRisk(bet) {
        ...
        const correlationScore = Math.max(sameGameExposure / maxSameGame, (playerKey ? samePlayerExposure / maxSamePlayer : 0), sameSportExposure / maxSameSport);
        return {
            excessive: false,
    ...
    // ========================================
    calculateOverallRiskLevel(bet, professionalFeatures) {
        let riskScore = 0;
        // Kelly fraction risk (0-3 points)
        if (bet.kellyFraction > 0.2)
            riskScore += 3;
        else if (bet.kellyFraction > 0.1)
            riskScore += 2;
        else if (bet.kellyFraction > 0.05)
            riskScore += 1;
        // Professional score risk (0-2 points)
        if (professionalFeatures.overallProfessionalScore < 65)
            riskScore += 2;
        else if (professionalFeatures.overallProfessionalScore < 75)
            riskScore += 1;
        // Portfolio concentration risk (0-2 points)
        const concentration = bet.stake / this.portfolioRisk.totalValue;
        if (concentration > 0.03)
            riskScore += 2;
        else if (concentration > 0.02)
            riskScore += 1;
        // Correlation risk (0-2 points)
        const correlationRisk = this.assessCorrelationRisk(bet);
        if (correlationRisk.correlationScore > 0.8)
            riskScore += 2;
        else if (correlationRisk.correlationScore > 0.6)
            riskScore += 1;
        // Market risk (0-1 point)
        if (bet.sport === 'TENNIS' || bet.market.includes('Live'))
            riskScore += 1;
    ...
    updateKellyMonitoring() {
        if (this.kellyMonitoring.currentPositions.length > 0) {
            ...
            this.kellyMonitoring.maxKellyFraction = Math.max(...this.kellyMonitoring.currentPositions.map(pos => pos.kellyFraction));
            // Check if all positions are optimally sized
            this.kellyMonitoring.optimalSizing = this.kellyMonitoring.currentPositions.every(pos => Math.abs(pos.actualFraction - pos.kellyFraction) / pos.kellyFraction < 0.1);
        }
    }
    // ========================================
    // Risk Limit Checking
    ...
    calculatePortfolioCorrelation(bets) {
    ...
    determineOverallRiskLevel() {
        let riskScore = 0;
        // Utilization risk
        if (this.realTimeRisk.utilizationRate > 0.8)
            riskScore += 3;
        else if (this.realTimeRisk.utilizationRate > 0.6)
            riskScore += 2;
        else if (this.realTimeRisk.utilizationRate > 0.4)
            riskScore += 1;
        // Correlation risk
        if (this.realTimeRisk.portfolioCorrelation > 0.7)
            riskScore += 2;
        else if (this.realTimeRisk.portfolioCorrelation > 0.5)
            riskScore += 1;
        // Drawdown risk
        if (this.portfolioRisk.currentDrawdown > 0.08)
            riskScore += 2;
        else if (this.portfolioRisk.currentDrawdown > 0.05)
            riskScore += 1;
        // Alert count
    ...
    calculateMarketDiversification() {
        const activeBets = Array.from(this.activeBets.values())
            .filter(bet => bet.status === 'CONFIRMED' || bet.status === 'PENDING');
        if (activeBets.length === 0)
            return 1;
...
exports.LiveRiskMonitor = LiveRiskMonitor;
