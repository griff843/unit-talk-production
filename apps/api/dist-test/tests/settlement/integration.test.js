...
describe('Settlement System Integration', () => {
    let agent;
    beforeAll(async () => {
        agent = new SettlementAgent_1.SettlementAgent();
    ...
    describe('Market Normalization', () => {
        it('should normalize MLB total bases correctly', () => {
            ...
            const result = (0, normalizers_1.normalizeMarket)('MLB', 'total_bases', stats);
            expect(result).toBe(5);
        });
        it('should normalize NFL passing yards', () => {
            const stats = { PASS_YDS: 287 };
            const result = (0, normalizers_1.normalizeMarket)('NFL', 'passing_yards', stats);
            expect(result).toBe(287);
        });
        it('should normalize NBA points + rebounds + assists', () => {
            const stats = { 'PTS+REB+AST': 45, PTS: 20, REB: 12, AST: 13 };
            const result = (0, normalizers_1.normalizeMarket)('NBA', 'PRA', stats);
            expect(result).toBe(45);
        });
        it('should handle unknown markets gracefully', () => {
            const stats = { UNKNOWN_STAT: 10 };
            const result = (0, normalizers_1.normalizeMarket)('MLB', 'unknown_market', stats);
            expect(result).toBeNull();
        });
    });
    describe('Settlement Agent', () => {
        it('should be instantiated correctly', () => {
            expect(agent).toBeInstanceOf(SettlementAgent_1.SettlementAgent);
        });
    ...
    describe('Error Handling', () => {
        it('should handle invalid sport gracefully', () => {
            const result = (0, normalizers_1.normalizeMarket)('INVALID_SPORT', 'points', { PTS: 20 });
            expect(result).toBeNull();
        });
        it('should handle empty stats object', () => {
            const result = (0, normalizers_1.normalizeMarket)('MLB', 'hits', {});
            expect(result).toBeNull();
        });
    });
...
describe('Settlement Performance', () => {
    it('should normalize stats quickly', () => {
        ...
        for (let i = 0; i < iterations; i++) {
            (0, normalizers_1.normalizeMarket)('NBA', 'points', stats);
            (0, normalizers_1.normalizeMarket)('NBA', 'rebounds', stats);
            (0, normalizers_1.normalizeMarket)('NBA', 'assists', stats);
        }
...
describe('Settlement Edge Cases', () => {
    it('should handle negative stats', () => {
        ...
        const result = (0, normalizers_1.normalizeMarket)('NFL', 'rushing_yards', stats);
        expect(result).toBe(-5);
    });
    it('should handle zero stats', () => {
        const stats = { INT: 0 };
        const result = (0, normalizers_1.normalizeMarket)('NFL', 'interceptions', stats);
        expect(result).toBe(0);
    });
    it('should handle decimal stats', () => {
        const stats = { SACKS: 1.5 };
        const result = (0, normalizers_1.normalizeMarket)('NFL', 'sacks', stats);
