...
const GradingAgent_1 = require("../../../agents/GradingAgent/GradingAgent");
...
const FeatureStoreService_1 = require("../../../services/feature-store/FeatureStoreService");
const supabaseClient_1 = require("../../../services/supabaseClient");
const metrics_collector_1 = require("../../../monitoring/metrics-collector");
const test_data_generator_1 = require("../utils/test-data-generator");
const performance_monitor_1 = require("../utils/performance-monitor");
const chaos_inducer_1 = require("../chaos/chaos-inducer");
...
const SYNDICATE_THRESHOLDS = {
    ...
    MIN_CACHE_HIT_RATE: 0.90, // 90% cache hit rate
    MAX_QUEUE_DEPTH: 1000 // Max items in processing queue
};
(0, globals_1.describe)('Syndicate-Grade High Volume Processing', () => {
    let gradingAgent;
    let alertAgent;
    let feedAgent;
    let propProcessor;
    ...
    (0, globals_1.beforeAll)(async () => {
        ...
        testDataGenerator = new test_data_generator_1.TestDataGenerator();
        chaosInducer = new chaos_inducer_1.ChaosInducer({ intensity: 'low' });
        ...
        console.log(`🎯 Performance Thresholds: Alert <${SYNDICATE_THRESHOLDS.ALERT_LATENCY}ms, Features <${SYNDICATE_THRESHOLDS.FEATURE_RETRIEVAL_P95}ms`);
    });
    ...
    (0, globals_1.describe)('Enterprise Volume Processing', () => {
        (0, globals_1.test)('should process 8K+ props within 30 seconds with <1% error rate', async () => {
            ...
            const processBatch = async (batch, semaphore) => {
                ...
                try {
                    ...
                    if (batch.id % 20 === 0) {
                        console.log(`✅ Batch ${batch.id + 1}/${batches.length} | ${batchDuration.toFixed(2)}ms | ${batchThroughput.toFixed(2)} props/sec`);
                        console.log(`📈 Progress: ${processedCount}/${propCount} props (${((processedCount / propCount) * 100).toFixed(1)}%) | Errors: ${errorCount}`);
                    }
                    return result;
                }
                catch (error) {
            ...
            const avgBatchDuration = batchMetrics.reduce((sum, m) => sum + m.duration, 0) / batchMetrics.length;
            const avgCacheHitRate = batchMetrics.reduce((sum, m) => sum + (m.cacheHitRate || 0), 0) / batchMetrics.length;
            ...
            (0, globals_1.expect)(overallThroughput).toBeGreaterThan(SYNDICATE_THRESHOLDS.MIN_THROUGHPUT_PPS);
            (0, globals_1.expect)(successRate).toBeGreaterThan(SYNDICATE_THRESHOLDS.MIN_SUCCESS_RATE);
            (0, globals_1.expect)(errorRate).toBeLessThan(SYNDICATE_THRESHOLDS.MAX_ERROR_RATE);
            (0, globals_1.expect)(avgCacheHitRate).toBeGreaterThan(SYNDICATE_THRESHOLDS.MIN_CACHE_HIT_RATE);
            // Validate professional features were used
            const professionalFeaturesUsage = batchMetrics.reduce((sum, m) => sum + (m.professionalFeaturesUsed || 0), 0) / batchMetrics.length;
            (0, globals_1.expect)(professionalFeaturesUsage).toBeGreaterThan(7); // At least 7/8 professional features used
        }, 120000); // 2 minute timeout for full enterprise test
        (0, globals_1.test)('should maintain <1s alert latency under peak load', async () => {
            ...
            let alertLatencies = [];
            let alertProcessingMetrics = [];
            let maxLatency = 0;
            let totalAlerts = 0;
            // Start continuous prop processing to create realistic load
            const loadSimulation = testDataGenerator.createContinuousLoadStream(propsPerSecond, propStreamDuration);
            ...
            const alertMonitoring = setInterval(async () => {
                ...
                try {
                    ...
                    console.log(`🚨 Alert latency: ${latency.toFixed(2)}ms | Max: ${maxLatency.toFixed(2)}ms | Total: ${totalAlerts}`);
                }
                catch (error) {
            ...
            console.log(`  ❌ Failures (>1s): ${alertFailures} (${(alertFailures / alertLatencies.length * 100).toFixed(2)}%)`);
            ...
            (0, globals_1.expect)(p99Latency).toBeLessThan(SYNDICATE_THRESHOLDS.ALERT_LATENCY * 1.5);
            (0, globals_1.expect)(maxLatency).toBeLessThan(SYNDICATE_THRESHOLDS.ALERT_LATENCY * 2);
            (0, globals_1.expect)(alertFailures / alertLatencies.length).toBeLessThan(0.02); // <2% failure rate under peak load
            (0, globals_1.expect)(alertsPerSecond).toBeGreaterThan(10); // Should process at least 10 alerts/second
        }, 90000); // 1.5 minute timeout
        (0, globals_1.test)('should maintain <50ms feature retrieval at 95th percentile under load', async () => {
            ...
            let featureRetrievals = [];
            let cacheHitRates = [];
            let featureStoreMetrics = [];
            console.log(`🔍 Testing feature retrieval for ${testDuration / 1000}s with ${concurrentRetrievals} concurrent threads`);
                ...
                while (Date.now() - startTime < testDuration) {
                    ...
                    try {
                        ...
                        const retrievalPromises = [
                            ...
                            featureStore.getPlayerFeatures('random_player_id', ['rushing_yards', 'receiving_yards']),
                            ...
                            featureStore.getAggregatedFeatures('clv_analysis', { groupBy: 'sport', period: '1d' })
                        ];
                        ...
                        const cacheHits = results.filter(r => r.status === 'fulfilled' && r.value?.fromCache).length;
                        ...
                        if (workerRetrievals.length % 100 === 0) {
                            const avgTime = workerRetrievals.reduce((sum, time) => sum + time, 0) / workerRetrievals.length;
                            console.log(`🔍 Worker ${workerId}: ${workerRetrievals.length} retrievals, avg: ${avgTime.toFixed(2)}ms`);
                        }
                    }
                    catch (error) {
            ...
            console.log(`  📊 Total retrievals: ${totalRetrievals} across ${concurrentRetrievals} workers`);
            ...
            (0, globals_1.expect)(p95Retrieval).toBeLessThan(SYNDICATE_THRESHOLDS.FEATURE_RETRIEVAL_P95);
            (0, globals_1.expect)(p99Retrieval).toBeLessThan(SYNDICATE_THRESHOLDS.FEATURE_RETRIEVAL_P99);
            (0, globals_1.expect)(avgRetrieval).toBeLessThan(SYNDICATE_THRESHOLDS.FEATURE_RETRIEVAL_P95 * 0.6); // Avg should be much better
            (0, globals_1.expect)(maxRetrieval).toBeLessThan(SYNDICATE_THRESHOLDS.FEATURE_RETRIEVAL_P95 * 4); // Max allowed spike
            (0, globals_1.expect)(avgCacheHitRate).toBeGreaterThan(SYNDICATE_THRESHOLDS.MIN_CACHE_HIT_RATE);
            (0, globals_1.expect)(retrievalsPerSecond).toBeGreaterThan(100); // Should handle >100 retrievals/second
        }, 60000); // 1 minute timeout
    });
    (0, globals_1.describe)('Resource Management and Optimization', () => {
        (0, globals_1.test)('should maintain memory usage within enterprise limits', async () => {
            ...
            const memoryMonitoring = setInterval(() => {
                ...
                console.log(`💾 RSS: ${rss.toFixed(2)}MB | Heap: ${heapUsed.toFixed(2)}/${heapTotal.toFixed(2)}MB | External: ${external.toFixed(2)}MB | Growth: ${(growth / 1024 / 1024).toFixed(2)}MB`);
                // Force garbage collection monitoring
                if (global.gc) {
                    const gcStart = perf_hooks_1.performance.now();
                    global.gc();
                    const gcDuration = perf_hooks_1.performance.now() - gcStart;
            ...
            const memoryVariance = memorySnapshots.reduce((sum, m) => sum + Math.pow(m.heapUsed - avgMemory, 2), 0) / memorySnapshots.length;
            ...
            (0, globals_1.expect)(avgGCDuration).toBeLessThan(SYNDICATE_THRESHOLDS.MAX_GC_PAUSE);
            (0, globals_1.expect)(maxGCDuration).toBeLessThan(SYNDICATE_THRESHOLDS.MAX_GC_PAUSE * 2);
            (0, globals_1.expect)(memoryStdDev / avgMemory).toBeLessThan(0.2); // Memory should be stable (within 20% variance)
        }, 180000); // 3 minute timeout
        (0, globals_1.test)('should handle database connection pooling at enterprise scale', async () => {
            ...
            console.log(`🗄️  Executing ${totalQueries} queries across ${concurrentConnections} concurrent connections`);
            // Monitor connection pool statistics
            const poolMonitoring = setInterval(async () => {
                try {
                    // Get connection pool statistics (implementation depends on your pool)
                    const poolStats = await supabaseClient_1.supabaseClient.from('pg_stat_database').select('*').limit(1);
            ...
            const connectionWorkers = Array.from({ length: concurrentConnections }, async (_, connectionId) => {
                const connectionStart = perf_hooks_1.performance.now();
                const workerQueryTimes = [];
                try {
                    ...
                    for (let i = 0; i < queriesPerConnection; i++) {
                        ...
                        switch (queryType) {
                            case 0: // Prop lookups
                                queryPromise = supabaseClient_1.supabaseClient
                                    .from('raw_props')
                                    .select('*')
                                    .eq('sport', 'NFL')
                                    .limit(10);
                    ...
                    const connectionDuration = perf_hooks_1.performance.now() - connectionStart;
                    const avgQueryTime = workerQueryTimes.reduce((sum, time) => sum + time, 0) / workerQueryTimes.length;
                    ...
                    if (connectionId % 20 === 0) {
                        console.log(`🔄 Connection ${connectionId}: ${queriesPerConnection} queries in ${connectionDuration.toFixed(2)}ms (avg: ${avgQueryTime.toFixed(2)}ms/query)`);
                    }
            ...
            const avgConnectionDuration = connectionMetrics.reduce((sum, m) => sum + m.totalDuration, 0) / connectionMetrics.length;
            console.log(`\n🎯 Enterprise Database Performance Analysis:`);
            console.log(`  📊 Successful connections: ${successfulConnections}/${concurrentConnections} (${(connectionSuccessRate * 100).toFixed(2)}%)`);
            ...
            (0, globals_1.expect)(connectionSuccessRate).toBeGreaterThan(SYNDICATE_THRESHOLDS.CONNECTION_POOL_EFFICIENCY);
            (0, globals_1.expect)(actualQueriesExecuted / totalQueries).toBeGreaterThan(0.98); // 98% query success rate
            (0, globals_1.expect)(avgQueryTime).toBeLessThan(SYNDICATE_THRESHOLDS.DATABASE_QUERY_P95);
            (0, globals_1.expect)(p95QueryTime).toBeLessThan(SYNDICATE_THRESHOLDS.DATABASE_QUERY_P95);
            (0, globals_1.expect)(p99QueryTime).toBeLessThan(SYNDICATE_THRESHOLDS.DATABASE_QUERY_P95 * 2);
            (0, globals_1.expect)(queriesPerSecond).toBeGreaterThan(100); // Should handle >100 queries/second
        }, 180000); // 3 minute timeout
        (0, globals_1.test)('should maintain event loop responsiveness under extreme load', async () => {
            ...
            let eventLoopMeasurements = [];
            let cpuUsageSnapshots = [];
            ...
            const eventLoopMonitor = setInterval(() => {
                ...
                setImmediate(() => {
                    ...
                    if (eventLoopMeasurements.length % 100 === 0) {
                        const avgLag = eventLoopMeasurements.reduce((sum, lag) => sum + lag, 0) / eventLoopMeasurements.length;
                        console.log(`⚡ Event loop measurements: ${eventLoopMeasurements.length}, avg lag: ${avgLag.toFixed(2)}ms`);
                    }
                });
            ...
            const loadTasks = [
                // CPU-intensive tasks
                ...Array.from({ length: cpuIntensiveTasks }, () => generateCPUIntensiveLoad(testDuration / cpuIntensiveTasks)),
                // I/O intensive tasks
                ...Array.from({ length: ioIntensiveTasks }, () => generateIOIntensiveLoad(testDuration / ioIntensiveTasks)),
            ...
            const avgEventLoopLag = eventLoopMeasurements.reduce((sum, lag) => sum + lag, 0) / eventLoopMeasurements.length;
            const p50EventLoopLag = eventLoopMeasurements[Math.floor(eventLoopMeasurements.length * 0.5)];
            const p95EventLoopLag = eventLoopMeasurements[Math.floor(eventLoopMeasurements.length * 0.95)];
            const p99EventLoopLag = eventLoopMeasurements[Math.floor(eventLoopMeasurements.length * 0.99)];
            const maxEventLoopLag = Math.max(...eventLoopMeasurements);
            const avgCPUUsage = cpuUsageSnapshots.reduce((sum, usage) => sum + usage, 0) / cpuUsageSnapshots.length;
            ...
            (0, globals_1.expect)(avgEventLoopLag).toBeLessThan(SYNDICATE_THRESHOLDS.MAX_EVENT_LOOP_LAG);
            (0, globals_1.expect)(p95EventLoopLag).toBeLessThan(SYNDICATE_THRESHOLDS.MAX_EVENT_LOOP_LAG * 2);
            (0, globals_1.expect)(p99EventLoopLag).toBeLessThan(SYNDICATE_THRESHOLDS.MAX_EVENT_LOOP_LAG * 5);
            (0, globals_1.expect)(maxEventLoopLag).toBeLessThan(SYNDICATE_THRESHOLDS.MAX_EVENT_LOOP_LAG * 10);
            (0, globals_1.expect)(avgCPUUsage).toBeLessThan(SYNDICATE_THRESHOLDS.MAX_CPU_USAGE);
        }, 90000); // 1.5 minute timeout
    });
    ...
    async function performComprehensiveWarmup() {
        ...
        await Promise.all([
            supabaseClient_1.supabaseClient.from('raw_props').select('*').limit(1),
            supabaseClient_1.supabaseClient.from('unified_picks').select('*').limit(1),
            supabaseClient_1.supabaseClient.from('users').select('*').limit(1),
            supabaseClient_1.supabaseClient.from('agent_health').select('*').limit(1)
        ]);
        console.log('✅ Comprehensive service warmup complete');
    }
    async function generateSustainedPropLoad(duration, propsPerSecond) {
        const endTime = Date.now() + duration;
        const interval = 1000 / propsPerSecond;
        while (Date.now() < endTime) {
            const props = testDataGenerator.generateRealisticProps(10);
            propProcessor.processBatchWithProfessionalFeatures(props, {
                enableCLVTracking: true,
                enableSteamDetection: true
    ...
    async function generateSustainedFeatureLoad(duration, retrievalsPerSecond) {
        const endTime = Date.now() + duration;
    ...
    async function generateSustainedAlertLoad(duration, checksPerSecond) {
        const endTime = Date.now() + duration;
    ...
    async function generateSustainedDatabaseLoad(duration, queriesPerSecond) {
        const endTime = Date.now() + duration;
        const interval = 1000 / queriesPerSecond;
        while (Date.now() < endTime) {
            const queries = [
                supabaseClient_1.supabaseClient.from('raw_props').select('*').limit(5),
                supabaseClient_1.supabaseClient.from('unified_picks').select('*').limit(5),
                supabaseClient_1.supabaseClient.from('agent_health').select('*').limit(3)
            ];
            Promise.all(queries).catch(err => console.warn('Sustained DB load error:', err.message));
            await new Promise(resolve => setTimeout(resolve, interval));
        }
    }
    async function generateCPUIntensiveLoad(duration) {
        const endTime = Date.now() + duration;
        while (Date.now() < endTime) {
    ...
    async function generateIOIntensiveLoad(duration) {
        const endTime = Date.now() + duration;
        while (Date.now() < endTime) {
    ...
    async function generateRealisticBettingLoad(duration) {
        const endTime = Date.now() + duration;
        while (Date.now() < endTime) {
            // Mixed betting operations
            const operations = [
                // Prop processing
                propProcessor.processBatchWithProfessionalFeatures(testDataGenerator.generateRealisticProps(5), { enableCLVTracking: true }),
                // Alert checking
                alertAgent.checkSteamAlerts(),
                // Feature retrieval
                featureStore.getFeatures('NFL', 'player_props'),
                // Database queries
                supabaseClient_1.supabaseClient.from('unified_picks').select('*').limit(3)
            ];
    ...
    async function generatePerformanceReport() {
        ...
        console.log(`⏱️  Avg GC duration: ${(gcMetrics.reduce((sum, gc) => sum + gc.duration, 0) / gcMetrics.length).toFixed(2)}ms`);
        console.log('=====================================');
    }
    async function cleanupSyndicateTestData() {
        ...
        try {
            ...
            const cleanupOperations = [
                supabaseClient_1.supabaseClient
                    .from('raw_props')
                    .delete()
                    .like('id', 'syndicate-test-%'),
                supabaseClient_1.supabaseClient
                    .from('unified_picks')
                    .delete()
                    .like('prop_id', 'syndicate-test-%'),
                supabaseClient_1.supabaseClient
                    .from('clv_tracking')
                    .delete()
                    .like('prop_id', 'syndicate-test-%')
 ...
 */ 
//# sourceMappingURL=high-volume-processing.test.js.map