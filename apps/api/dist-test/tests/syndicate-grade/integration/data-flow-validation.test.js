...
const BridgeWorker_1 = require("../../../workflows/BridgeWorker");
...
const data_flow_tracker_1 = require("../utils/data-flow-tracker");
const e2e_validator_1 = require("../utils/e2e-validator");
...
(0, globals_1.describe)('Syndicate-Grade Integration Tests', () => {
    let gradingAgent;
    let alertAgent;
    let feedAgent;
    let propProcessor;
    ...
    (0, globals_1.beforeAll)(async () => {
        ...
        testSessionId = (0, crypto_1.randomUUID)();
        testDataGenerator = new test_data_generator_1.TestDataGenerator({ testSessionId });
        dataFlowTracker = new data_flow_tracker_1.DataFlowTracker({ testSessionId });
        e2eValidator = new e2e_validator_1.EndToEndValidator({ testSessionId });
    ...
    (0, globals_1.describe)('Smart Form to Grading Pipeline', () => {
        (0, globals_1.test)('should process complete Smart Form submission to professional grading', async () => {
            ...
            await flowTracker.trackStage('bridge_worker_processing', async () => {
                ...
                (0, globals_1.expect)(processed).toBe(true);
                return processed;
            });
            ...
            await flowTracker.trackStage('temporal_workflow', async () => {
                ...
                (0, globals_1.expect)(workflowResult).toBeDefined();
                (0, globals_1.expect)(workflowResult.status).toBe('completed');
            ...
            await flowTracker.trackStage('professional_grading', async () => {
                ...
                (0, globals_1.expect)(gradingResult).toBeDefined();
                (0, globals_1.expect)(gradingResult.professionalFeaturesUsed).toBeGreaterThanOrEqual(INTEGRATION_THRESHOLDS.MIN_PROFESSIONAL_FEATURES);
            ...
            await flowTracker.trackStage('data_persistence', async () => {
                ...
                (0, globals_1.expect)(persistedData.unified_picks).toBeDefined();
                (0, globals_1.expect)(persistedData.clv_tracking).toBeDefined();
                (0, globals_1.expect)(persistedData.grading_history).toBeDefined();
                return persistedData;
            });
            const totalDuration = perf_hooks_1.performance.now() - testStartTime;
            ...
            (0, globals_1.expect)(totalDuration).toBeLessThan(INTEGRATION_THRESHOLDS.SMART_FORM_TO_GRADING);
            (0, globals_1.expect)(flowSummary.dataConsistency).toBeGreaterThan(INTEGRATION_THRESHOLDS.MIN_DATA_CONSISTENCY);
            (0, globals_1.expect)(gradingResult.accuracy).toBeGreaterThan(INTEGRATION_THRESHOLDS.MIN_PROCESSING_ACCURACY);
            (0, globals_1.expect)(flowSummary.errorRate).toBeLessThan(INTEGRATION_THRESHOLDS.MAX_ERROR_PROPAGATION);
        }, 60000); // 1 minute timeout
        (0, globals_1.test)('should handle concurrent Smart Form submissions without data corruption', async () => {
            ...
            const submissions = Array.from({ length: concurrentSubmissions }, (_, index) => ({
                submissionId: `concurrent-${index}-${(0, crypto_1.randomUUID)()}`,
            ...
            const flowTrackers = submissions.map(sub => dataFlowTracker.createFlowTracker(sub.submissionId));
            ...
            const successfulSubmissions = results.filter(r => r.status === 'fulfilled' && r.value.success).length;
            ...
            (0, globals_1.expect)(successRate).toBeGreaterThan(0.95); // 95% success rate
            (0, globals_1.expect)(duplicateCount).toBe(0); // No duplicates allowed
            (0, globals_1.expect)(missingCount).toBe(0); // No missing data allowed
            (0, globals_1.expect)(corruptedCount).toBe(0); // No corrupted data allowed
            (0, globals_1.expect)(throughput).toBeGreaterThan(5); // At least 5 submissions/sec
        }, 120000); // 2 minute timeout
    });
    (0, globals_1.describe)('Real-Time Data Ingestion and Processing', () => {
        (0, globals_1.test)('should process real-time market data with sub-second latency', async () => {
            ...
            const marketDataStream = setInterval(async () => {
                ...
                try {
                    ...
                    const totalLatency = perf_hooks_1.performance.now() - updateStartTime;
            ...
            (0, globals_1.expect)(avgLatency).toBeLessThan(INTEGRATION_THRESHOLDS.MAX_REAL_TIME_LATENCY);
            (0, globals_1.expect)(p95Latency).toBeLessThan(INTEGRATION_THRESHOLDS.MAX_REAL_TIME_LATENCY * 2);
            (0, globals_1.expect)(actualThroughput).toBeGreaterThan(INTEGRATION_THRESHOLDS.MIN_REAL_TIME_THROUGHPUT * 0.8); // 80% of target
            (0, globals_1.expect)(maxLatency).toBeLessThan(INTEGRATION_THRESHOLDS.MAX_REAL_TIME_LATENCY * 5);
        }, 60000); // 1 minute timeout
        (0, globals_1.test)('should maintain data consistency during high-frequency updates', async () => {
            ...
            (0, globals_1.expect)(consistencyRate).toBeGreaterThan(INTEGRATION_THRESHOLDS.MIN_DATA_CONSISTENCY);
            (0, globals_1.expect)(finalConsistency.consistent).toBe(true);
            (0, globals_1.expect)(maxInconsistencyDuration).toBeLessThan(1000); // Max 1 second inconsistency
            (0, globals_1.expect)(finalConsistency.dataLossRate).toBeLessThan(INTEGRATION_THRESHOLDS.MAX_DATA_LOSS_RATE);
        }, 90000); // 1.5 minute timeout
    });
    (0, globals_1.describe)('Professional Grading End-to-End Validation', () => {
        (0, globals_1.test)('should execute all 8 professional grading features accurately', async () => {
            ...
            await gradingTracker.trackStage('steam_detection', async () => {
                ...
                (0, globals_1.expect)(gradingResults.steamDetection).toBeDefined();
                (0, globals_1.expect)(gradingResults.steamDetection.steamScore).toBeGreaterThanOrEqual(0);
            ...
            await gradingTracker.trackStage('clv_tracking', async () => {
                ...
                (0, globals_1.expect)(gradingResults.clvTracking).toBeDefined();
                (0, globals_1.expect)(gradingResults.clvTracking.clvPercentage).toBeDefined();
            ...
            await gradingTracker.trackStage('timing_analysis', async () => {
                ...
                (0, globals_1.expect)(gradingResults.timingAnalysis).toBeDefined();
                (0, globals_1.expect)(gradingResults.timingAnalysis.timingScore).toBeGreaterThanOrEqual(0);
            ...
            await gradingTracker.trackStage('volume_analysis', async () => {
                ...
                (0, globals_1.expect)(gradingResults.volumeAnalysis).toBeDefined();
                (0, globals_1.expect)(gradingResults.volumeAnalysis.volumeScore).toBeGreaterThanOrEqual(0);
            ...
            await gradingTracker.trackStage('movement_tracking', async () => {
                ...
                (0, globals_1.expect)(gradingResults.movementTracking).toBeDefined();
                (0, globals_1.expect)(gradingResults.movementTracking.movementScore).toBeGreaterThanOrEqual(0);
            ...
            await gradingTracker.trackStage('bookmaker_analysis', async () => {
                ...
                (0, globals_1.expect)(gradingResults.bookmakerAnalysis).toBeDefined();
                (0, globals_1.expect)(gradingResults.bookmakerAnalysis.bookmakerScore).toBeGreaterThanOrEqual(0);
            ...
            await gradingTracker.trackStage('market_efficiency', async () => {
                ...
                (0, globals_1.expect)(gradingResults.marketEfficiency).toBeDefined();
                (0, globals_1.expect)(gradingResults.marketEfficiency.efficiencyScore).toBeGreaterThanOrEqual(0);
            ...
            await gradingTracker.trackStage('advanced_metrics', async () => {
                ...
                (0, globals_1.expect)(gradingResults.advancedMetrics).toBeDefined();
                (0, globals_1.expect)(gradingResults.advancedMetrics.compositeScore).toBeGreaterThanOrEqual(0);
            ...
            await gradingTracker.trackStage('composite_grading', async () => {
                ...
                (0, globals_1.expect)(gradingResults.compositeGrade).toBeDefined();
                (0, globals_1.expect)(gradingResults.compositeGrade.finalScore).toBeGreaterThanOrEqual(0);
                (0, globals_1.expect)(gradingResults.compositeGrade.finalScore).toBeLessThanOrEqual(100);
            ...
            const featuresExecuted = Object.keys(gradingResults).filter(key => gradingResults[key] && gradingResults[key].executed !== false).length;
            const avgFeatureAccuracy = Object.values(gradingResults)
                .filter((result) => result && result.accuracy)
                .reduce((sum, result) => sum + result.accuracy, 0) / featuresExecuted;
            const featureExecutionTimes = Object.values(gradingResults)
                .filter((result) => result && result.executionTime)
            ...
            (0, globals_1.expect)(featuresExecuted).toBe(INTEGRATION_THRESHOLDS.MIN_PROFESSIONAL_FEATURES);
            (0, globals_1.expect)(avgFeatureAccuracy).toBeGreaterThan(INTEGRATION_THRESHOLDS.MIN_PROCESSING_ACCURACY);
            (0, globals_1.expect)(totalGradingTime).toBeLessThan(5000); // <5 seconds total
            (0, globals_1.expect)(gradingResults.compositeGrade.confidence).toBeGreaterThan(0.80); // 80% confidence
            (0, globals_1.expect)(gradingSummary.dataConsistency).toBeGreaterThan(INTEGRATION_THRESHOLDS.MIN_DATA_CONSISTENCY);
        }, 60000); // 1 minute timeout
        (0, globals_1.test)('should handle grading failure recovery and fallback mechanisms', async () => {
            ...
            for (const [index, pick] of testPicks.entries()) {
                ...
                try {
                    ...
                    const gradingDuration = perf_hooks_1.performance.now() - gradingStart;
                    gradingResults.push({
                        pickId: pick.id,
                        success: true,
                    ...
                    if (gradingResult.recoveryEvents && gradingResult.recoveryEvents.length > 0) {
                        failureRecoveries.push(...gradingResult.recoveryEvents.map((event) => ({
                            pickId: pick.id,
                            ...event
                        })));
                    }
            ...
            (0, globals_1.expect)(successRate).toBeGreaterThan(INTEGRATION_THRESHOLDS.MIN_RECOVERY_SUCCESS);
            (0, globals_1.expect)(avgFeaturesUsed).toBeGreaterThan(6); // Should use at least 6/8 features on average
            (0, globals_1.expect)(recoverySuccessRate).toBeGreaterThan(0.90); // 90% recovery success
            (0, globals_1.expect)(avgRecoveryTime).toBeLessThan(3000); // <3 seconds recovery
            (0, globals_1.expect)(totalFallbacks).toBeGreaterThan(0); // Should use fallbacks when needed
        }, 120000); // 2 minute timeout
    });
    (0, globals_1.describe)('Monitoring and Observability Integration', () => {
        (0, globals_1.test)('should collect and process comprehensive metrics across all systems', async () => {
            ...
            const completeCategoriesCount = requiredMetricCategories.filter(cat => categoriesCollected.has(cat)).length;
            ...
            (0, globals_1.expect)(metricsCollectionRate).toBeGreaterThan(0.90); // 90% collection rate
            (0, globals_1.expect)(metricsCompleteness).toBe(1.0); // 100% category completeness
            (0, globals_1.expect)(totalMetricsCollected).toBeGreaterThan(20); // Reasonable amount of data
        }, 90000); // 1.5 minute timeout
    });
    ...
    async function initializeIntegrationServices() {
        ...
        bridgeWorker = new BridgeWorker_1.BridgeWorker({
    ...
    async function setupIntegrationTestEnvironment() {
        ...
        await supabaseClient_1.supabaseClient.from('users').upsert([
            {
    ...
    async function waitForTemporalWorkflow(submissionId, timeoutMs) {
    ...
    async function verifyProfessionalGrading(submissionId) {
        const { data } = await supabaseClient_1.supabaseClient
    ...
    async function verifyDataPersistence(submissionId) {
        const results = await Promise.all([
            supabaseClient_1.supabaseClient.from('unified_picks').select('*').eq('submission_id', submissionId),
            supabaseClient_1.supabaseClient.from('clv_tracking').select('*').eq('submission_id', submissionId),
            supabaseClient_1.supabaseClient.from('grading_history').select('*').eq('submission_id', submissionId)
        ]);
        return {
    ...
    async function checkDataIntegrity(submissionIds) {
    ...
    async function updatePropInDatabase(update) {
    ...
    async function updatePropInFeatureStore(update) {
    ...
    async function updatePropInCache(update) {
        // Simulate cache update
        await new Promise(resolve => setTimeout(resolve, 10));
    }
    async function checkDataConsistency(propId) {
        const [dbData, featureData, cacheData] = await Promise.all([
            supabaseClient_1.supabaseClient.from('raw_props').select('*').eq('id', propId).single(),
            featureStore.getPropFeatures(propId),
            getCachedPropData(propId)
        ]);
        const consistent = (dbData.data?.line === featureData?.line &&
    ...
    async function performComprehensiveConsistencyCheck(propId) {
    ...
    async function getCachedPropData(propId) {
        // Simulate cache lookup
        return { version: 1, line: 10.5, odds: -110 };
    }
    async function simulateServiceFailure(service, duration) {
    ...
    async function generateGradingActivity(duration) {
    ...
    async function generateAlertActivity(duration) {
    ...
    async function generateDataIngestionActivity(duration) {
    ...
    async function generateUserActivity(duration) {
 ...
 */ 
//# sourceMappingURL=data-flow-validation.test.js.map