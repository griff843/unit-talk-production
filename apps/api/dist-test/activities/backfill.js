...
exports.validateBackfillRequest = validateBackfillRequest;
exports.checkIdempotency = checkIdempotency;
exports.backfillPropsForHour = backfillPropsForHour;
exports.recordBackfillProgress = recordBackfillProgress;
exports.triggerProcessor = triggerProcessor;
exports.triggerPromoter = triggerPromoter;
const supabaseClient_1 = require("../services/supabaseClient");
const dataSourceRouter_1 = require("../agents/FeedAgent/dataSourceRouter");
// Note: Using console.log instead of Logger to avoid import issues
const crypto_1 = __importDefault(require("crypto"));
/**
 * Validate backfill request parameters
 */
async function validateBackfillRequest(request) {
    try {
        ...
        if (maxCalls && maxCalls < diffHours) {
            return { valid: false, error: `maxCalls (${maxCalls}) is less than required hours (${diffHours})` };
        }
        // Validate batchSize
 ...
 */
async function backfillPropsForHour(request) {
    ...
    try {
        ...
        for (let i = 0; i < props.length; i += 50) {
            ...
            const propsForDB = batch.map(prop => ({
                ...
                external_prop_id: prop.id || crypto_1.default.randomUUID(),
            ...
            const existingIds = await supabaseClient_1.supabaseClient
                .from('raw_props')
        ...
        console.log(`[Backfill] Completed hour ${date}: ${processedCount} new props, ${duplicateCount} duplicates`);
        return {
            processedCount,
            duplicateCount,
            totalCount: props.length,
 ...
 */
async function recordBackfillProgress(params) {
    try {
 ...
 */
async function triggerProcessor(params) {
    try {
 ...
 */
async function triggerPromoter(params) {
    try {
