...
exports.ingestOptimalProps = ingestOptimalProps;
exports.ingestSGOProps = ingestSGOProps;
exports.validateIngestionData = validateIngestionData;
exports.ingestOptimalGames = ingestOptimalGames;
const axios_1 = __importDefault(require("axios"));
const optimal_1 = require("../agents/FeedAgent/optimal");
const logger_1 = require("../utils/logger");
const supabase_1 = require("../utils/supabase");
const logger = (0, logger_1.makeLogger)('IngestionActivities');
 ...
 */
async function ingestOptimalProps(params) {
    try {
        ...
        const response = await axios_1.default.get(`https://api.optimal.com/props/${params.league}`, {
...
async function ingestSGOProps(params) {
    try {
...
async function validateIngestionData(params) {
    try {
 ...
 */
async function ingestOptimalGames(params) {
    try {
        ...
        const events = await (0, optimal_1.fetchEvents)();
        // Filter for requested leagues
        const filteredEvents = events.filter(event => params.leagues.map(l => l.toUpperCase()).includes(event.league?.toUpperCase()));
 ...
 */
async function ensureTeamExists(teamCode, teamDisplayName, sport) {
    if (!teamCode || !sport)
        return;
    try {
