...
exports.AdaptiveMLPipeline = void 0;
const enhanced_monitoring_1 = require("../monitoring/enhanced-monitoring");
const logger_1 = require("../utils/logger");
const cached_pipeline_1 = require("./cached-pipeline");
class AdaptiveMLPipeline extends cached_pipeline_1.CachedMLPipeline {
    constructor(config) {
        super({});
        this.logger = (0, logger_1.createLogger)('AdaptiveMLPipeline');
    ...
    trackPrediction(input, prediction) {
        const performance = {
            timestamp: Date.now(),
            input: input,
    ...
    async updateResult(predictionId, actualResult, profitLoss) {
    ...
    async evaluatePerformance() {
        ...
        const avgProfitLoss = recentPerformance.reduce((sum, p) => sum + p.profitLoss, 0) / recentPerformance.length;
    ...
    calculateConfidenceAccuracy(performance) {
    ...
    async identifyAdaptationTriggers() {
        const triggers = [];
        const recentPerformance = this.performanceHistory
            .filter(p => p.actualResult !== null)
    ...
    async triggerAdaptation(triggers) {
    ...
    async generateModelUpdate(triggers) {
        const recentPerformance = this.performanceHistory
            .filter(p => p.actualResult !== null)
    ...
    analyzePerformancePatterns(performance) {
        ...
        const patterns = {
            sportPerformance: new Map(),
            marketPerformance: new Map(),
            confidencePerformance: new Map(),
        ...
        performance.forEach(p => {
            ...
            const confidenceStats = patterns.confidencePerformance.get(confidenceBin) || { correct: 0, total: 0 };
    ...
    generateAdaptationStrategy(triggers, patterns) {
    ...
    generateNewFeatures(patterns) {
        // Generate new features based on performance patterns
        const newFeatures = [];
        // Time-based features
        patterns.timeOfDayPerformance.forEach((stats, timeBin) => {
            if (stats.total >= 5) {
                newFeatures.push({
    ...
    async updateFeatures(parameters) {
    ...
    async incrementalUpdate(parameters) {
...
exports.AdaptiveMLPipeline = AdaptiveMLPipeline;
