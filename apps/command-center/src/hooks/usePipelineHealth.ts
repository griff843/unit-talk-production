...
export function usePipelineHealth(options: UsePipelineHealthOptions = {}) {
  ...
  const fetchPipelineHealth = useCallback(async () => {
    try {
      ...
      if (supabase) {
        ...
        const processingLag = processedProps && processedProps.length > 0 
        ...
        const promotionLag = promotedProps && promotedProps.length > 0
        ...
        const promotionSuccessRate = (processedCount && processedCount > 0)
          ? ((promotedCount || 0) / processedCount) * 100 
          : 0
        ...
        const writerAuditScore = (totalUnifiedPicks && totalUnifiedPicks > 0)
  ...
  const logConflictEvent = useCallback(async (rawPropId: string, reason?: string) => {
    try {
  ...
  const logPromotionEvent = useCallback(async (
    rawPropId: string, 
    success: boolean, 
    reason?: string, 
  ...
  useEffect(() => {
    ...
    const conflictsSub = supabase
      ...
      .on('postgres_changes', 
        { event: 'INSERT', schema: 'public', table: 'conflict_events' },
        () => fetchPipelineHealth()
      )
      .subscribe()
