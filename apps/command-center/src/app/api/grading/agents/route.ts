...
export async function GET(request: NextRequest) {
  try {
    ...
    // Main grading agent
    const mainGradingHealth = agentHealth?.find(
      (h: any) =>
        h.agent &&
    ...
    const mainGradingMetrics = agentMetrics?.find(
      (m: any) =>
        m.agent_id &&
        typeof m.agent_id === 'string' &&
        m.agent_id.toLowerCase().includes('grading') &&
    ...
    const processed =
      pickStats?.filter(
        (p: any) =>
          ...
          ['approved', 'rejected', 'published'].includes(p.workflow_stage)
      ).length || 0;
    ...
    const pending =
      pickStats?.filter(
        (p: any) =>
          !p.workflow_stage ||
          (typeof p.workflow_stage === 'string' &&
            ['draft', 'pending_review'].includes(p.workflow_stage))
      ).length || 0;

    const avgConfidence =
      pickStats && pickStats.length > 0
        ? pickStats.reduce(
            (sum: number, p: any) => sum + (typeof p.confidence === 'number' ? p.confidence : 50),
            0
          ) / pickStats.length
        : 85;
    ...
    gradingAgents.push({
      ...
      status:
        mainGradingHealth?.status === 'healthy'
          ? 'active'
          : mainGradingHealth?.status === 'error'
            ? 'error'
            : 'inactive',
      processed,
      pending,
    ...
    const settlementHealth = agentHealth?.find(
      (h: any) =>
    ...
    if (settlementHealth) {
      gradingAgents.push({
        ...
        status:
          settlementHealth?.status === 'healthy'
            ? 'active'
            : settlementHealth?.status === 'error'
              ? 'error'
              : 'inactive',
        ...
        lastActivity:
          typeof settlementHealth?.created_at === 'string'
            ? settlementHealth.created_at
            : new Date().toISOString(),
        tier: 'S',
        configuration: {
          batchSize: 100,
