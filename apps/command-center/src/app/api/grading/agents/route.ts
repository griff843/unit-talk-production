...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!supabase) {
      console.warn('DB unavailable in env – returning mock grading agents')
    ...
    const mainGradingMetrics = agentMetrics?.find((m: any) =>
      m.agent && typeof m.agent === 'string' && m.agent.toLowerCase().includes('grading') &&
      !m.agent.toLowerCase().includes('settlement')
    )
