...
async function getBasicHealthCheck(): Promise<NextResponse> {
  ...
  try {
    ...
    const { data, error } = client ? await client.from('agent_health').select('count').limit(1).single() : { data: null, error: null as any }
  ...
  try {
    const client = getTypedSupabaseClient()
    const { data: agents, error } = client ? await client.from('agent_health').select('status') : { data: [], error: null as any }
...
export async function HEAD() {
  try {
    // Quick database ping
    const client = getTypedSupabaseClient()
    const { error } = client ? await client.from('agent_health').select('count').limit(1).single() : { error: null as any }
...
async function checkDatabaseHealth(): Promise<ComponentHealthStatus> {
  ...
  try {
    ...
    const client = getTypedSupabaseClient()
    const { data: connectionTest } = client ? await client
      .from('unified_picks')
      .select('count')
      .limit(1) : { data: null as any }
    ...
    const coreTableChecks = client ? await Promise.allSettled([
      client.from('unified_picks').select('count').limit(1),
      client.from('users').select('count').limit(1),
      client.from('raw_props').select('count').limit(1),
      client.from('events').select('count').limit(1),
      client.from('bridge_outbox').select('count').limit(1),
    ]) : [] as any;
...
async function checkEventsStreamHealth(): Promise<ComponentHealthStatus> {
  ...
  try {
    ...
    const recentEventSources = client ? await Promise.allSettled([
      client
        .from('events')
        .select('count')
...
async function checkSmartFormBridgeHealth(): Promise<ComponentHealthStatus> {
  ...
  try {
    ...
    let outboxStats: any[] = []
    if (client) {
      const res6 = await client
        .from('bridge_outbox')
        .select('status')
        .gte('created_at', new Date(Date.now() - 60 * 60 * 1000).toISOString())
      outboxStats = res6.data || []
    }
    ...
    let stuckEvents: any[] = []
    if (client) {
      const res7 = await client
        .from('bridge_outbox')
        .select('id')
        .eq('status', 'processing')
        .lt('updated_at', new Date(Date.now() - 30 * 60 * 1000).toISOString())
      stuckEvents = res7.data || []
    }
