...
import { dbOperations, User, getTypedSupabaseClient } from '@/lib/supabase';
  ...
  try {
    ...
    try {
      ...
      const { data, error } = client ? await client
        ...
        .select()
        .single() : { data: null, error: { message: 'no client' } as any }
  ...
  try {
    ...
    try {
      if (permanent) {
        ...
        const client = getTypedSupabaseClient()
        const { error } = client ? await client.from('users').delete().eq('id', userId) : { error: { message: 'no client' } as any }
