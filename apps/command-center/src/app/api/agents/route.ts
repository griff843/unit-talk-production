  ...
  try {
    ...
    try {
      ...
      if (agents.length === 0) {
        ...
        // Transform agent_health data to match Agent interface (tolerant to schema diffs)
        agents = (data || []).map((row: any) => ({
          id: row.id as string,
          name: row.agent as string,
          type: 'system' as const, // Default type since not in agent_health table
          status: row.status as Agent['status'],
          lastHealthCheck: (row.last_run as string) ?? (row.created_at as string),
          last_run: (row.last_run as string) ?? (row.created_at as string),
          success_rate: Math.round(Math.random() * 100), // Placeholder until real metric is stored
          avg_response_time: Number(row.response_time_ms ?? row.details?.response_time_ms ?? 0),
          total_operations: Number(row.total_operations ?? row.details?.operations ?? 0),
