...
async function handleGradingReplay(
  ...
  const { error: insertError } = await supabase.from('events').insert(replayEvents);
...
async function handleAlertReemission(
  criteria: ReplayCriteria,
  userId: string
): Promise<ReplayResult> {
  if (criteria.dryRun) {
  ...
  const { error: insertError } = await supabase.from('events').insert(alertEvents);
