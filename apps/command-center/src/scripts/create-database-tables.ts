...
async function createTables() {
  ...
  try {
    ...
    if (agentsError && !agentsError.message.includes('already exists')) {
      ...
      const { error: createAgentsError } = await supabase.from('agents').select('*').limit(1);
...
CREATE POLICY "Allow service role to manage agents" ON public.agents
  FOR ALL USING (auth.role() = 'service_role');
    ...
    const { error: securityError } = await supabase.from('security_events').select('*').limit(1);
...
CREATE POLICY "Allow authenticated users to read security events" ON public.security_events
  FOR SELECT USING (auth.role() = 'authenticated');
    ...
    for (const agent of sampleAgents) {
      const { error } = await supabase.from('agents').upsert(agent, { onConflict: 'name' });
    ...
    console.log(
      'Please create the tables manually in Supabase dashboard using the SQL provided above'
    );
  }
}
