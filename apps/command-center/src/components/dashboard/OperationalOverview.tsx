...
export function OperationalOverview() {
  ...
  const fetchOperationalData = async () => {
    try {
      ...
      const sportsBreakdown = sportsData?.reduce((acc, item) => {
      ...
      const tierDistribution = tierData?.reduce((acc, item) => {
      ...
      const recentGames = Array.from(
        recentGamesData?.reduce((acc, game) => {
      ...
      const topPerformers = Array.from(
        performersData?.reduce((acc, prop) => {
      ...
      ).sort((a: any, b: any) => b.picks_count - a.picks_count).slice(0, 5)
      ...
      setMetrics({
        ...
        pendingProps,
        gradingProgress,
        processingRate: Math.min(8000, Math.round((gradedProps || 0) * 60 / Math.max(1, 24 * 60))), // Optimized rate - props per minute, capped at theoretical max
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex items-center space-x-1 text-xs mt-2">
              <CheckCircle className="h-3 w-3 text-green-500" />
              <span className="text-green-500">{health.agents.healthy}/{health.agents.total} agents</span>
            </div>
            <p className="text-xs text-muted-foreground mt-1">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Proposition distribution across sports leagues
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Latest games with proposition data
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Professional grading tier breakdown
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {['S', 'A', 'B', 'C', 'D'].map((tier) => {
                ...
                const percentage = metrics.gradedProps > 0 ? (count / metrics.gradedProps) * 100 : 0
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Players with highest proposition volume
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {metrics.topPerformers.map((performer, index) => (
                <div key={`${performer.player}-${performer.sport}`} className="flex items-center justify-between p-3 rounded-lg border bg-muted/20">
