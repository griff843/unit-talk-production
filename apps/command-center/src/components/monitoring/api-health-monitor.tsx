        ...
        <CardContent>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className={`w-4 h-4 rounded-full animate-pulse ${
                    ...
                    summary.overallStatus === 'degraded' ? 'bg-yellow-500' : 'bg-red-500'
                  }`} />
                  <div>
                    <h3 className={`font-semibold ${
                      summary.overallStatus === 'healthy' ? 'text-green-800' : 
                      summary.overallStatus === 'degraded' ? 'text-yellow-800' : 'text-red-800'
                    }`}>
                      System Status: {summary.overallStatus.toUpperCase()}
                    </h3>
                    <p className={`text-sm ${
                      summary.overallStatus === 'healthy' ? 'text-green-600' : 
                      summary.overallStatus === 'degraded' ? 'text-yellow-600' : 'text-red-600'
                    }`}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {statuses.map((api) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full animate-pulse ${getStatusColor(api.status)}`} />
                  <div>
                    <div className="flex items-center space-x-2">
                ...
                <div className="text-right space-y-1">
                  <div className="flex items-center space-x-2">
                    ...
                    <Badge
                      variant="outline"
                      className={api.status === 'healthy' ? 'text-green-600' : 
      ...
      {alerts.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Recent critical issues requiring immediate attention
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {dataIngestion && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Real-time monitoring of data flow from all sources
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2">
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div className={`w-3 h-3 rounded-full ${
                      ...
                      source.status === 'stale' ? 'bg-yellow-500' :
                      'bg-red-500'
                    }`} />
                    <div>
                  ...
                  <div className="text-right">
                    <Badge variant="outline" className={
                      source.status === 'healthy' ? 'text-green-600' :
                      source.status === 'stale' ? 'text-yellow-600' :
                      'text-red-600'
                    }>
                      {source.status}
                    </Badge>
