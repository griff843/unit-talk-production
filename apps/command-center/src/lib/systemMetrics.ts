...
if (typeof window !== 'undefined') {
  throw new Error('SystemMetrics module can only be used on the server side. Import this module in API routes only.')
}
...
class SystemMetricsCollector {
  ...
  private async getNetworkMetrics() {
    ...
    this.networkCounters.packetsIn += Math.round(baseTraffic / 1000 * (0.8 + Math.random() * 0.4))
    this.networkCounters.packetsOut += Math.round(baseTraffic / 1000 * (0.6 + Math.random() * 0.4))
