  ...
  static async logAudit(event: Omit<AuditEvent, 'id' | 'created_at'>): Promise<void> {
    try {
      ...
      if (error) {
        ...
        // FALLBACK: Log to file system for compliance
        try {
          const fs = require('fs');
          ...
          const logEntry = JSON.stringify({
            ...auditEvent,
            fallback_reason: 'database_insert_failed',
            original_error: error.message
          }) + '\n';

          fs.appendFileSync(logFile, logEntry, 'utf8');
          console.warn('✅ Audit event logged to fallback file:', logFile);

        } catch (fallbackError) {
          console.error('💥 CRITICAL: Audit fallback also failed:', fallbackError);

          // Last resort: Emergency console logging
          console.error('🚨 EMERGENCY AUDIT LOG:', JSON.stringify(auditEvent));
        }
      }
    } catch (error) {
      console.error('Error logging audit event:', error);

      // FALLBACK: Emergency file logging
      try {
        const fs = require('fs');
        const path = require('path');
        const emergencyFile = path.join(process.cwd(), 'logs', 'emergency-audit.log');
        const logEntry = `EMERGENCY: ${JSON.stringify({ error: error instanceof Error ? error.message : 'Unknown error' })}\n`;

        fs.appendFileSync(emergencyFile, logEntry, 'utf8');
        console.warn('🚨 Emergency audit logging activated');

      } catch (emergencyError) {
        console.error('💥 CRITICAL: All audit logging failed:', emergencyError);
      }
    }
  }
