...
- ✅ Updated `SubmitTicketForm.tsx` to use API client instead of direct Supabase calls
- ✅ Updated `Step1Essentials.tsx`, `Step4GameSelection.tsx`, and `LegCard.tsx`
- ✅ All client-side components now use `/api/cappers`, `/api/games`, `/api/props` routes
...
- ✅ Added sport enum validation: `CREATE TYPE sport_t AS ENUM ('NFL', 'NBA', 'MLB', 'NHL', 'NCAAF')`
...
**Status**: COMPLETE  
**Implementation**:
- ✅ Dual-write pattern: `smart_tickets` (authoritative) + `unified_picks` (individual legs)
