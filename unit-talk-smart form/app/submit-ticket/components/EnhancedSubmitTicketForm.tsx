...
const ENHANCED_SPORT_CONFIGS: Record<Sport, SportConfig> = {
  NFL: {
    ...
    logo: '🏈',
    color: '#013369',
    betTypes: ['Spread', 'Moneyline', 'Total', 'Player Prop', 'Team Total'],
    ...
    dynamicFields: {
      'Player Prop': ['player', 'stat_type', 'line', 'over_under', 'game_context'],
      'Team Total': ['team', 'stat_type', 'line', 'over_under', 'half_context']
    },
    validation: {
      minOdds: -10000,
      maxOdds: 10000,
  ...
  NBA: {
    ...
    dynamicFields: {
      'Player Prop': ['player', 'stat_type', 'line', 'over_under', 'game_context'],
      'Team Total': ['team', 'stat_type', 'line', 'over_under', 'quarter_context']
    },
    validation: {
      minOdds: -5000,
      maxOdds: 5000,
  ...
  MLB: {
    ...
    logo: '⚾',
    color: '#041E42',
    betTypes: ['Moneyline', 'Spread', 'Total', 'Player Prop', 'Team Total'],
    playerProps: ['Hits', 'RBIs', 'Home Runs', 'Strikeouts', 'Walks', 'Stolen Bases'],
    teamProps: ['Total Runs', 'First 5 Innings', 'Hits', 'Errors'],
    dynamicFields: {
      'Player Prop': ['player', 'stat_type', 'line', 'over_under', 'inning_context'],
      'Team Total': ['team', 'stat_type', 'line', 'over_under', 'inning_context']
    },
    validation: {
      minOdds: -2000,
      maxOdds: 2000,
...
export function EnhancedSubmitTicketForm({ 
  ...
  const validateLegInRealTime = useCallback(async (legIndex: number) => {
    ...
    if (config?.validation?.minOdds && config?.validation?.maxOdds &&
        (odds < config.validation.minOdds || odds > config.validation.maxOdds)) {
      errors.push({
        field: 'odds',
    ...
    if (watchedSport === 'NFL' && leg.bet_type === 'Player Prop') {
          ...
          <AnimatePresence>
            {legs.map((leg, index) => (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className={`w-3 h-3 rounded-full ${
                      (leg as any).validation_status === 'valid' ? 'bg-green-500' :
                      (leg as any).validation_status === 'invalid' ? 'bg-red-500' :
                      'bg-yellow-500'
                    }`} />
                ...
                {userTier === 'vip_plus' && (leg as any).ai_analysis && (
                  <div className="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-lg p-4">
                    <h4 className="font-medium text-gray-900 dark:text-white mb-2">
                      🤖 AI Analysis
                    </h4>
                    <div className="text-sm text-gray-700 dark:text-gray-300 space-y-1">
                      <p><strong>Recommendation:</strong> {(leg as any).ai_analysis.recommendation}</p>
                      <p><strong>Key Factors:</strong> {(leg as any).ai_analysis.key_factors?.join(', ')}</p>
                      <p><strong>Confidence:</strong> {(leg as any).ai_analysis.confidence}%</p>
                    </div>
                  </div>
                )}
              </motion.div>
              ...
              <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                {searchResults.players.map((player: PlayerSearchResult) => (
                  ...
                  >
                    ...
                    {player.injury_status && (player.injury_status as any).status !== 'healthy' && (
                      <div className="ml-auto">
                        <span className="px-2 py-1 bg-red-100 text-red-800 text-xs rounded-full">
                          {(player.injury_status as any).status}
                        </span>
                      </div>
                    )}
                  </button>
