...
This guide helps you migrate from the legacy `SupabaseService` to the new
strictly typed `DatabaseService`.
...
```typescript
const isOnCooldown = await databaseService.isUserOnCooldown(
  userId,
  'create_pick'
);
...
// New: strongly typed
const tier: 'member' | 'vip' | 'vip_plus' | 'staff' | 'admin' | 'owner' =
  profile.tier;
```
...
The legacy `SupabaseService` still works and now uses the new `DatabaseService`
internally. However, it's marked as deprecated and should be migrated to the new
service for:
