...
- Bot permissions: `Manage Channels`, `Send Messages`, `Create Public Threads`,
  `Manage Threads`
...
Once the bot is running, use the `/faq-init` command in Discord to create all
FAQ threads:
...
1. **Commands are registered**: Type `/faq` in Discord and see the three
   commands
2. **Forum channel exists**: Confirm channel ID `1387837517298139267` is
   accessible
...
Edit the `FAQ_DATA` array in `src/commands/faq-init.ts` and run `/faq-init`
again.
...
Your FAQ system is now ready to use. Staff can manage FAQs with the slash
commands, and users can browse the comprehensive FAQ forum.
