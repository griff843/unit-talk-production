...
- Market-type level exposure caps: `ExposureCalculator` extended with
  `markets!inner(category)` join through `ticket_legs`; `byMarketType`
  aggregation added; `detectBreaches()` covers `dimension='market_type'`;
  `market_type_kelly_limit` config key (default 0.35); `MarketTypePanel` added
  to Command Center risk dashboard (3-col grid alongside Correlation +
  Drawdown); 5 new vitest tests; migration
  `20260314141057_market_type_exposure_config.sql`
...
**PHASE 3 is 100% complete.** All risk controls fully implemented and verified:
Kelly sizing, exposure caps (total + event + sport + market-type), correlation
detection, drawdown freeze. Risk state fully visible in Command Center (all 5
panels). Decision audit trail queryable via `/api/risk/decisions`.
...
| **Phase 3** — Risk Engine Dominance    | Done    | 100%       | All risk controls complete: Kelly, exposure (total/event/sport/market-type), correlation, drawdown, dashboard |
