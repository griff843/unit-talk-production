...
This document is the **single source of truth** for pick lifecycle management in the Unit Talk platform. All implementations MUST conform to this contract.
...
**Composite Key**: `(bet_slip_id, leg_index)` uniquely identifies a leg within a ticket.
...
// Settle
export function settleIfNotSettled(pickId: UUID, settlement: Settlement): SettleResult;
```
...
**Review Cycle**: On any schema change
