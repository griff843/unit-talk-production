...
export interface AgentConfigOptions {
  ...
  customMetrics?: Partial<MetricsConfig>;
  customRetry?: Partial<RetryConfig>;
  customHealth?: Partial<HealthConfig>;
}
...
export class AgentConfigFactory {
   ...
   */
  static createBaseConfig(options: AgentConfigOptions): BaseAgentConfig {
    const {
      name,
      version = '1.0.0',
      enabled = true,
   ...
   */
  static createBatchConfig(options: AgentConfigOptions): BaseAgentConfig {
    return this.createBaseConfig({
      ...options,
      customMetrics: {
   ...
   */
  static createRealTimeConfig(options: AgentConfigOptions): BaseAgentConfig {
    return this.createBaseConfig({
      ...options,
      customMetrics: {
   ...
   */
  static createDevConfig(options: AgentConfigOptions): BaseAgentConfig {
    return this.createBaseConfig({
      ...options,
      logLevel: 'debug',
      customMetrics: {
   ...
   */
  static createProductionConfig(options: AgentConfigOptions): BaseAgentConfig {
    return this.createBaseConfig({
      ...options,
      logLevel: 'info',
      customMetrics: {
...
export function createAgentConfig(
  name: string, 
  type: 'base' | 'high-frequency' | 'batch' | 'real-time' | 'dev' | 'production' = 'base',
  overrides: Partial<AgentConfigOptions> = {}
): BaseAgentConfig {
  const options: AgentConfigOptions = { name, ...overrides };
