...
export const data = new SlashCommandBuilder()
  ...
  .addSubcommand(subcommand =>
    subcommand
      .setName('status')
      .setDescription('View your current alert settings')
  )
  .addSubcommand(subcommand =>
    subcommand
      .setName('test')
      .setDescription('Send a test alert to verify settings')
  );
        ...
        .addFields(
          {
            ...
            value: '• Custom alert preferences\n• Capper-specific notifications\n• Sport filtering\n• Confidence level alerts\n• Real-time push notifications\n• SMS alerts (premium)',
            inline: false
          },
          {
            name: '⭐ Basic VIP Alerts',
            value: 'VIP members receive basic pick notifications. Upgrade to VIP+ for advanced customization!',
            inline: false
          }
        );
...
async function handleConfigure(interaction: ChatInputCommandInteraction, userTier: string) {
  ...
  const sportSelect = new StringSelectMenuBuilder()
    ...
    .addOptions(
      new StringSelectMenuOptionBuilder()
        .setLabel('NBA Basketball')
        .setValue('nba')
        .setEmoji('🏀'),
      new StringSelectMenuOptionBuilder()
        .setLabel('NFL Football')
        .setValue('nfl')
        .setEmoji('🏈'),
      new StringSelectMenuOptionBuilder()
        .setLabel('MLB Baseball')
        .setValue('mlb')
        .setEmoji('⚾'),
      new StringSelectMenuOptionBuilder()
        .setLabel('NHL Hockey')
  ...
  const actionRow2 = new ActionRowBuilder<StringSelectMenuBuilder>()
    .addComponents(confidenceSelect);
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      },
      {
        ...
        value: alertSettings.confidence === 'all' ? 'All Picks' : 
               alertSettings.confidence === 'medium' ? 'Medium+' : 'High Only',
        inline: true
      },
      {
      ...
      {
        name: '📱 Alert Methods',
        value: alertSettings.alertTypes.map(type => 
          type === 'discord_dm' ? 'Discord DM' :
          type === 'discord_mention' ? 'Channel Mentions' :
