...
export async function execute(interaction: ChatInputCommandInteraction) {
  try {
    ...
    if (!hasMinimumTier(member as any, 'vip_plus')) {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: '🤖 AI Features',
            value: '• Personalized betting analysis\n• Game prediction insights\n• Strategy recommendations\n• Risk assessment\n• Market trend analysis\n• Custom betting scenarios',
            inline: false
          },
          {
            name: '⭐ VIP Benefits',
            value: 'As a VIP member, you have access to expert picks and analysis. Upgrade to VIP+ for AI-powered insights!',
            inline: false
          }
        );
    ...
    setTimeout(async () => {
      ...
      logger.info(`AI analysis provided to ${interaction.user.tag} (${userTier}) - Sport: ${sport}`);
...
function generateMockAIResponse(question: string, sport: string, context: string) {
  ...
  const responses = {
    nba: {
      analysis: "Based on current team performance metrics, injury reports, and historical matchup data, I've identified several key factors that could influence this game's outcome.",
      insights: "• Home team has 68% win rate in similar scenarios\n• Key player matchups favor the underdog\n• Recent form suggests value in the over\n• Weather/venue factors are neutral",
      riskAssessment: "Medium Risk\n(3/5 stars)",
      recommendation: "Consider the spread bet with 2-unit stake",
      confidence: 72
    },
    nfl: {
      analysis: "NFL matchup analysis shows strong correlation between recent defensive performance and total points. Weather conditions and injury reports are critical factors.",
      insights: "• Defense ranks suggest under value\n• Quarterback matchup favors home team\n• Historical divisional trends support spread\n• Line movement indicates sharp money",
      riskAssessment: "Low-Medium Risk\n(2/5 stars)",
      recommendation: "Strong lean on under with 3-unit confidence",
      confidence: 78
    },
    mlb: {
      analysis: "Pitching matchup analysis combined with recent offensive trends and ballpark factors provides clear betting direction for this game.",
      insights: "• Starting pitcher ERA differential significant\n• Bullpen usage patterns favor away team\n• Wind conditions support over play\n• Umpire tendencies lean toward strikes",
      riskAssessment: "High Risk\n(4/5 stars)",
      recommendation: "Wait for better line or consider alternate markets",
      confidence: 65
    },
    general: {
      analysis: "General betting strategy analysis suggests focusing on bankroll management and value identification rather than chasing losses or betting on emotion.",
      insights: "• Maintain 1-3% unit sizing\n• Track all bets for analysis\n• Focus on sports you understand\n• Avoid parlays and exotic bets",
