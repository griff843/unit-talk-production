...
export class WelcomeService {
  ...
  /**
   * Handle new member welcome based on their tier
   */
  async handleNewMemberWelcome(member: GuildMember): Promise<void> {
    try {
      // Determine member tier based on roles
      const memberTier = this.getMemberTier(member);
      
      if (!memberTier) {
        logger.info(`No welcome tier determined for ${member.user.username}`);
        return;
      }

      logger.info(`Sending ${memberTier} welcome to ${member.user.username}`);

      switch (memberTier) {
        case 'free':
          await this.sendFreeWelcome(member);
          break;
        case 'vip':
          await this.sendVIPWelcome(member);
          break;
        case 'vip_plus':
          await this.sendVIPPlusWelcome(member);
          break;
        default:
          logger.info(`No specific welcome for tier: ${memberTier}`);
      }

    } catch (error) {
      logger.error('Error in handleNewMemberWelcome:', error);
    }
  }
   ...
   * Determine member tier based on roles
   */
  private getMemberTier(member: GuildMember): string | null {
    const roles = member.roles.cache;
    
    // Check for VIP+ first (highest tier)
    if (roles.some(role => role.name.toLowerCase().includes('vip+') || role.name.toLowerCase().includes('elite'))) {
      return 'vip_plus';
    }
    
    // Check for VIP
    if (roles.some(role => role.name.toLowerCase().includes('vip') && !role.name.toLowerCase().includes('+'))) {
      return 'vip';
    }
    
    // Check for Free
    if (roles.some(role => role.name.toLowerCase().includes('free'))) {
      return 'free';
    }
    
    // Default to free for testing (remove this when role checking is enabled)
    return 'free';
  }
  ...
  /**
   * Send welcome message with fallback to channel
   */
  private async sendWelcomeMessage(
    ...
    try {
      ...
      logger.info(`Welcome DM sent to ${member.user.username} (${member.id})`);
    } catch (dmError) {
      // If DM fails, send to welcome channel or general channel
      logger.warn(`Failed to send DM to ${member.user.username}, sending to channel instead`);
      
      const welcomeChannel = member.guild.channels.cache.find(
        channel => channel.name.includes('welcome') || channel.name.includes('general')
      ) as TextChannel;

      if (welcomeChannel && this.isTextChannel(welcomeChannel)) {
        ...
        logger.info(`Welcome message sent to channel ${welcomeChannel.name} for ${member.user.username}`);
      }
    }
  }
  ...
  public createVIPFAQEmbed(): EmbedBuilder {
    return new EmbedBuilder()
      ...
      .setDescription(
        ...
        `All picks are in Capper Corner (<#1288596108876054559>). Each expert has threads for official plays and for in-depth discussion/Q&A—just select your favorite and subscribe for instant access.\n\n` +
