...
export type PickUnion = FinalPick | DailyPick; // Renamed to avoid conflict with TypeScript's Pick utility type
...
export interface Database {
  public: {
    Tables: {
      final_picks: {
        Row: FinalPick;
        Insert: Partial<FinalPick> & Pick<FinalPick, 'capper_id' | 'player_id' | 'game_id' | 'stat_type' | 'line' | 'odds' | 'stake' | 'payout' | 'result' | 'actual_value' | 'tier' | 'ticket_type' | 'sport' | 'league' | 'confidence'>;
        Update: Partial<FinalPick>;
      };
      daily_picks: {
        Row: DailyPick;
...
// --- Helper Functions ---
export function validateFinalPick(data: unknown): FinalPick {
  return FinalPickSchema.parse(data);
}
