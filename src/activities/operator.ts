...
const logger = createLogger('OperatorActivities');
...
export async function logUSPError(params: {
  ...
  try {
    ...
    logger.error('Failed to log USP error:', error);
...
export async function monitorAPIQuota(params: {
  ...
  try {
    ...
    logger.error(`API quota monitoring failed for ${params.provider}:`, error);
    return {
      success: false,
      shouldFallback: true,
      percentage: 100
...
export async function detectLiveGames(params: {
  ...
  try {
    ...
    logger.error('Live game detection failed:', error);
    return {
      success: false,
      liveGames: [],
      errors: [String(error)]
...
export async function logWorkflowMetrics(params: {
  ...
  try {
    ...
    logger.error('Failed to log workflow metrics:', error);
    return { success: false };
  }
}
