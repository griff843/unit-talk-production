...
const logger = createLogger('E2ETestRunner');
...
export class E2ETestRunner {
  ...
  private async startTemporalInfrastructure(): Promise<void> {
    ...
    try {
      ...
      const workerPromise = this.worker.run();
  ...
  private async executeLeaguePipeline(league: string): Promise<{ recordsProcessed: number }> {
    ...
    const gradingResult = await this.executeGrading(league);
  ...
  private async executeFiltering(league: string, batchId: string): Promise<{ filteredProps: any[] }> {
    ...
    try {
      ...
      const filteredProps = rawProps?.filter(prop => 
  ...
  private async executeFinalizer(league: string, props: any[]): Promise<{ recordsProcessed: number; finalProps: any[] }> {
    ...
    try {
      ...
      // Insert into final_picks table (simulated)
...
${this.results.fallbackEvents.map(event => 
  `- **${event.timestamp.toISOString()}:** ${event.primaryProvider} → ${event.fallbackProvider} (${event.success ? 'Success' : 'Failed'})`
).join('\n')}
`;
  }
...
### ${error.stage} - ${error.timestamp.toISOString()}
- **Error:** ${error.error}
...
`;
  ...
  private printManualReviewInstructions(): void {
    ...
    console.log('   - final_picks: Verify S/A tier props are present');
