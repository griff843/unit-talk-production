...
export async function syndicateSchedulerWorkflow(): Promise<void> {
  ...
  while (!isEmergencyStopped) {
    try {
      ...
      const intervalMs = isLiveMode ? 120000 : 600000; // 2 min vs 10 min
      ...
      const maxCycleTime = isLiveMode ? 110000 : 300000; // 110s for live, 5min for off-peak
...
export async function leagueIngestionWorkflow(params: {
  ...
  try {
    // 1. PRIMARY INGESTION (Optimal API)
    let ingestionResult = await feedActivities.ingestOptimalProps({
      league,
  ...
  try {
    ...
    if (successfulGrading.length > 0) {
      ...
      if (successfulScoring.length > 0) {
        await gradingActivities.updateFinalPicks({
...
export async function discordAlertWorkflow(params: {
  ...
  try {
    ...
    const newPicks = await gradingActivities.getNewFinalPicks({ cycleCount });
