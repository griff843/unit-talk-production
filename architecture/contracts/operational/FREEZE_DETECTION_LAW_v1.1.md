...
# 4. CANONICAL FREEZE REASON BINDING

`freeze_reason_code` values MUST be selected from
`FREEZE_REASON_CODE_CANON_v1.0`.

This document MUST NOT redefine the enum.

Any trigger emitting a non-canonical code MUST emit
`HEALTH_SIGNAL_INTEGRITY_FAIL` instead.

## 4.1 BINDING REFERENCE

All freeze triggers in this document emit codes from the canonical enum defined
in: `architecture/contracts/operational/FREEZE_REASON_CODE_CANON_v1.0.md`

## 4.2 INTEGRITY CONSTRAINT

If a freeze trigger would emit a value not present in the canonical enum:

- System MUST substitute `HEALTH_SIGNAL_INTEGRITY_FAIL`
- System MUST log integrity violation
- Freeze MUST still activate
