...
# 12. AUDIT INTEGRITY INVARIANTS

The system must expose signals to verify audit log integrity:

- `hash_chain_valid` — Boolean indicating audit log hash chain is intact
- `seq_monotonicity_valid` — Boolean indicating sequence numbers are
  monotonically increasing
- `seq_gap_detected` — Boolean indicating a sequence gap exists
- `clock_sanity_valid` — Boolean indicating timestamps are within expected
  bounds
- `cross_env_contamination_detected` — Boolean indicating cross-environment
  audit entries

Violation Conditions:

- `hash_chain_valid == false`
- `seq_monotonicity_valid == false`
- `seq_gap_detected == true`
- `clock_sanity_valid == false`
- `cross_env_contamination_detected == true`

If any audit integrity signal indicates violation, system MUST enter FROZEN
state using `freeze_reason_code = AUDIT_LOG_INTEGRITY_FAIL`.

Audit integrity signals are mandatory for HEALTHY determination.

---

# 13. COMMAND CENTER PROJECTION INVARIANT
...
# 14. HEALTH STATE COMPUTATION MODEL
...
# 16. CANONICAL BINDING REFERENCES
