...
import { evaluateDrift } from './drift-sentinel.js';
...
function deriveRelevantDomains(request: SprintExecutionRequest): string[] {
  const domains: string[] = [];
  const text =
    `${request.summary} ${request.objective ?? ''} ${(request.touchedAreas ?? []).join(' ')}`.toLowerCase();

  // Map keywords to context domain triggers
