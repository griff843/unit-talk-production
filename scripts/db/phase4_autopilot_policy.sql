...
-- ============================================================================
-- This migration creates the decision ledger and supporting tables for
-- the Autopilot Policy Engine.
--
-- Tables:
...
-- ============================================================================
-- Append-only table for all autopilot decisions
-- Every policy evaluation is recorded here for audit trail
...
CREATE TABLE IF NOT EXISTS autopilot_decisions (
  ...
  -- Evidence snapshot (what was evaluated)
...
CREATE INDEX IF NOT EXISTS idx_autopilot_decisions_trace_id
  ON autopilot_decisions(trace_id);

CREATE INDEX IF NOT EXISTS idx_autopilot_decisions_action_type
  ON autopilot_decisions(action_type);

CREATE INDEX IF NOT EXISTS idx_autopilot_decisions_created_at
  ON autopilot_decisions(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_autopilot_decisions_evaluation_result
  ON autopilot_decisions(evaluation_result);

CREATE INDEX IF NOT EXISTS idx_autopilot_decisions_mode
  ON autopilot_decisions(mode);

-- Composite index for common queries
CREATE INDEX IF NOT EXISTS idx_autopilot_decisions_action_created
  ON autopilot_decisions(action_type, created_at DESC);
...
-- ============================================================================
-- Tracks active cooldowns to prevent repeated actions
...
-- Index for expired cooldown cleanup
CREATE INDEX IF NOT EXISTS idx_autopilot_cooldowns_expires_at
  ON autopilot_cooldowns(expires_at);

-- ============================================================================
-- 4. Views for Analysis
-- ============================================================================

-- View: Recent decisions summary
...
-- View: Decision ledger with policy violations
...
-- Function: Get active cooldowns for an entity
...
-- Function: Clean up expired cooldowns
CREATE OR REPLACE FUNCTION cleanup_expired_cooldowns()
RETURNS INTEGER AS $$
DECLARE
  deleted_count INTEGER;
BEGIN
  DELETE FROM autopilot_cooldowns
...
CREATE OR REPLACE FUNCTION get_autopilot_decision_stats(
...
BEGIN
  ...
  SELECT
    ...
    ROUND(
      COUNT(*) FILTER (WHERE d.evaluation_result = 'ALLOW')::NUMERIC /
      NULLIF(COUNT(*), 0) * 100,
      2
    ) AS allow_rate,
    ROUND(
      COUNT(*) FILTER (WHERE d.executed = true)::NUMERIC /
      NULLIF(COUNT(*) FILTER (WHERE d.evaluation_result = 'ALLOW'), 0) * 100,
      2
    ) AS execution_rate
...
-- Trigger: Prevent updates to decision ledger (append-only)
...
-- Trigger: Prevent deletes from decision ledger (immutable)
...
ON autopilot_decisions FOR ALL
TO service_role
USING (true)
WITH CHECK (true);

CREATE POLICY "Service role has full access to policy config"
ON autopilot_policy_config FOR ALL
TO service_role
USING (true)
WITH CHECK (true);
...
-- Policies for authenticated users (read-only on decisions)
...
ON autopilot_policy_config FOR SELECT
TO authenticated
USING (true);
