...
async function main() {
  ...
  if (!publishId) {
    console.error('❌ Usage: npx tsx scripts/verify-pick-publish.ts <publish_id>');
    process.exit(1);
  }

  console.log('🔍 Verifying pick_publish in SUPABASE CLOUD\n');
  ...
  console.log(`   Last Error: ${data.last_error || 'null'}`);
  console.log(`   Created At: ${data.created_at}`);
  console.log(`   Sent At: ${data.sent_at || 'null'}`);
  console.log();

  if (data.status === 'sent') {
    console.log('✅ PASS: Pick successfully sent to Discord');
    console.log(`   Discord Message: https://discord.com/channels/@me/${data.discord_channel_id}/${data.external_message_id}`);
    process.exit(0);
  } else if (data.status === 'failed') {
    console.error('❌ FAIL: Pick failed to send');
