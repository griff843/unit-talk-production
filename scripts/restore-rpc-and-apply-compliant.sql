-- Restore atomic_submit_ticket_v2 from 20260220200000_true_manual_entry_rpc.sql
-- Then apply compliant outbox migration

-- First, drop and recreate the enqueue function (idempotent)
...
-- Now restore the original atomic_submit_ticket_v2 WITHOUT the enqueue step
-- This is copied from 20260220200000_true_manual_entry_rpc.sql
...
BEGIN
  ...
  ELSE
    -- CATALOG MODE: Validate against canonical tables
    WITH leg_data AS (
      SELECT
        (row_number() OVER () - 1)::INTEGER AS leg_index,
        leg
...
COMMENT ON FUNCTION atomic_submit_ticket_v2 IS
  'Single-writer ticket submission RPC. Supports catalog/manual modes. Idempotent by bet_slip_id. Discord enqueue handled in app layer.';
