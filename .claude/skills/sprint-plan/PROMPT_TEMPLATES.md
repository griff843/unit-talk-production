...
## LLM Routing Section (Required in All Sprint Prompts — COS-006)

Every sprint prompt must include a Routing section generated by the LLM router
(`tools/claude-os/src/cli.ts route`). Insert immediately before the task list.

```
LLM Routing:
  Work type(s):    <classification output from router>
  Instance mode:   <SINGLE_INSTANCE | TWO_INSTANCES | THREE_INSTANCES>
  Lanes:
    Lane <N> — <Name>: <Model> (<claude_code_internal | external_advisory>)
    ...
  External prompts: <count> generated (see LLM_ROUTING_DECISION.md)
  Authority:       docs/02_architecture/claude_os_multi_llm_orchestration_blueprint.md
```

Example (Mode A, implementation sprint):
...
LLM Routing:
  ...
  Lanes:
    Lane 1 — Implementation: Claude Sonnet (claude_code_internal)
    Lane 3 — Verification:   Claude Sonnet (claude_code_internal)
    Lane 4 — Governance/Docs: Claude Sonnet (claude_code_internal)
  External prompts: 0 (Mode A — single-LLM execution)
  Authority:       docs/02_architecture/claude_os_multi_llm_orchestration_blueprint.md
```

Example (Mode B, architecture sprint):

```
LLM Routing:
  Work type(s):    architecture, implementation
  Instance mode:   TWO_INSTANCES
  Lanes:
    Lane 1 — Implementation: Claude Sonnet (claude_code_internal)
    Lane 3 — Verification:   Claude Sonnet (claude_code_internal)
    Lane 6 — Design/Architecture: Claude Opus + Gemini Advanced (external_advisory)
  External prompts: 1 generated (see LLM_ROUTING_DECISION.md)
  Authority:       docs/02_architecture/claude_os_multi_llm_orchestration_blueprint.md
```
