...
cleanly to Claude OS `sprint:close` for governed verification and inventory
generation. Bridges implementation completion → governed proof → status sync.

**Authority boundary**: This skill prepares and validates inputs. Claude OS
...
| Claude OS Sprint Type | Tier | Verification Requirements               |
| --------------------- | ---- | --------------------------------------- |
...
The report MUST be populated before Claude OS can validate the sprint. Fill in
...
echo "      proof_proof_inventory.txt are generated by Claude OS sprint:close."
echo "      Do NOT create these manually."
```

If any ❌ appears: **HALT** — do not proceed to Claude OS until resolved.

### Step 9: Generate Claude OS Envelope (for supervised-run)
...
**Invoke Claude OS supervised-run:**
...
**Expected outcome**: Claude OS runs verification, generates
`proof_proof_inventory.txt`, prints the compliance table, and exits 0.

If Claude OS exits non-zero: do NOT proceed. Read the compliance table output,
...
After Claude OS exits 0, the sprint is verified. Proceed to:

```bash
# Commit and tag (if not yet done by Claude OS)
git add -A
...
| Claude OS exits non-zero                         | HALT — read compliance table, fix missing artifacts |
...
- This skill captures evidence; Claude OS `sprint:close` generates the inventory
