-- =============================================================================
-- PHASE 3: FINAL SAAS SETUP (HANDLING EXISTING TABLES)
...
-- Step 5: Create new compatibility views (with different names to avoid conflicts)
-- =============================================================================

-- Enhanced cappers view (new name to avoid conflict)
CREATE OR REPLACE VIEW cappers_unified AS
SELECT 
  u.id,
  u.discord_id,
  u.username,
...
-- Enhanced final picks view (new name to avoid conflict)
CREATE OR REPLACE VIEW final_picks_unified AS
SELECT 
  up.id,
...
-- Daily picks view (new name to avoid conflict)
CREATE OR REPLACE VIEW daily_picks_unified AS
SELECT 
  up.id,
...
SELECT 
  ...
  'New compatibility views created' as compatibility,
  NOW() as completed_at;
...
SELECT 
  ...
  (SELECT COUNT(*) FROM cappers_unified) as cappers_in_new_view,
  (SELECT COUNT(*) FROM final_picks_unified) as final_picks_in_new_view,
  (SELECT COUNT(*) FROM daily_picks_unified) as daily_picks_in_new_view,
