...
-- Date: 2026-02-15
-- Purpose: Close two remaining security gaps:
--   1. Trigger now protects ALL settlement lifecycle fields
--   2. RPC context flag explicitly reset before every return
-- ============================================================================

-- ========================================================================
-- 1. PATCH: Trigger to protect ALL settlement lifecycle fields
-- ========================================================================
...
BEGIN
  ...
  -- This covers ALL fields that affect settlement semantics
  ...
  THEN
    RAISE EXCEPTION 'SETTLEMENT-GUARD: Direct modification of settlement fields is prohibited. Use manual_settle_pick() or correct_settlement() RPC. Protected fields: settlement_version, settlement_result, settled_at, settlement_hash, settlement_status, actual_outcome, settlement_frozen, freeze_enforced_at';
  END IF;
...
COMMENT ON FUNCTION prevent_direct_settlement_update() IS
  'SETTLEMENT-GUARD-SEAL-PATCH-010: Blocks direct UPDATE of ALL settlement lifecycle fields. Covers 8 fields total.';

-- ========================================================================
-- 2. PATCH: manual_settle_pick() with explicit context reset
-- ========================================================================
...
-- ========================================================================
-- 3. PATCH: correct_settlement() with explicit context reset
-- ========================================================================
...
BEGIN
  ...
  -- Get freeze config (sport-specific or default)
  ...
  IF v_freeze_config IS NOT NULL AND v_pick.settled_at IS NOT NULL THEN
    ...
    IF v_now > v_freeze_deadline THEN
      -- Settlement is past freeze window
      IF v_is_admin_override AND v_freeze_config.admin_override_allowed THEN
        -- Admin override allowed, continue but log it
        NULL; -- Will log below with elevated flag
      ELSE
        -- Freeze-lock active, reject correction
        -- First, mark as frozen if not already
...
COMMENT ON FUNCTION manual_settle_pick(UUID, TEXT, TIMESTAMPTZ, JSONB) IS
  'SETTLEMENT-GUARD-SEAL-PATCH-010: RPC for first-time settlement with explicit context reset on all exit paths.';

COMMENT ON FUNCTION correct_settlement(UUID, TEXT, TEXT, JSONB) IS
  'SETTLEMENT-GUARD-SEAL-PATCH-010: RPC for settlement corrections with explicit context reset on all exit paths.';
...
COMMENT ON SCHEMA public IS
  'SETTLEMENT-GUARD-SEAL-PATCH-010 applied: Complete trigger coverage (8 fields) + explicit context reset.';
